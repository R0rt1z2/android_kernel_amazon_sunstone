...
int optee_kreeconsole_init(void)
{
	...
	tee_ctx = tee_client_open_context(NULL, optee_dev_match,
						NULL, NULL);
	if (IS_ERR(tee_ctx)) {
		pr_warn("open_context failed err %ld", PTR_ERR(tee_ctx));
		ret = -1;
		goto out;
	}
	...
	if (rc || osarg.ret) {
		pr_warn("open_session failed err %d, ret=%d", rc, osarg.ret);
		ret = -1;
		goto close_context;
	}
	...
	if (arg.ret != 0) {
		pr_warn("%s(): ret %d, orig %d", __func__,
...
close_session:
	...
	if (rc != 0)
		pr_warn("close_session failed err %d", rc);
close_context:
	/* close context */
