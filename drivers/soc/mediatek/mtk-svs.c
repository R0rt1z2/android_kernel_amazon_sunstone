 ...
 * @volt_od: bank voltage over drive
 ...
 * @volt_bin_turn_pt: turn point informs which opp_volt calculated by volt bin/high bank.
 * @type: bank type to represent it is 2-line (high/low) bank or 1-line bank.
 * @volt_bin: voltage bin for GPU only
 *
...
static void svs_get_vops_v3(struct svs_platform *svsp)
{
	...
	for (i = opp_start; i < opp_stop; i++)
		svsb->volts[i] += svsb->volt_od;

	/* mt8188 only */
	if (svsb->sw_id == SVSB_GPU && svsb->type == SVSB_HIGH &&
	    svsb->opp_freqs[0] > svsb->freq_base) {
		if (svsb->volt_bin == 0x3 || svsb->volt_bin == 0x4)
			svsb->volts[0] = 0x40;  /* 0.8 V */
		else if (svsb->volt_bin == 0x5)
			svsb->volts[0] = 0x3c;  /* 0.775 V */
		else if (svsb->volt_bin == 0x6)
			svsb->volts[0] = 0x38;  /* 0.75 V */
		else
			svsb->volts[0] =  /* no volt bin, use opp_volts[0] */
				svs_opp_volt_to_bank_volt(svsb->opp_volts[0],
							  svsb->volt_step,
							  svsb->volt_base);

		dev_notice(svsb->dev, "volt bin = %u\n", svsb->volt_bin);

		/* find volt bin turn point */
		for (i = 0; i < svsb->opp_count; i++) {
			if (svsb->opp_freqs[i] <= svsb->freq_base) {
				svsb->volt_bin_turn_pt = i;
				break;
			}
		}

		/* override previous results */
		for (i = 1; i < svsb->volt_bin_turn_pt; i++)
			svsb->volts[i] =
				interpolate(svsb->freqs_pct[0],
					    svsb->freqs_pct[svsb->volt_bin_turn_pt],
					    svsb->volts[0],
					    svsb->volts[svsb->volt_bin_turn_pt],
					    svsb->freqs_pct[i]);
	}
}
...
static bool svs_mt8188_efuse_parsing(struct svs_platform *svsp)
{
	...
	if (svsp->fake_efuse) {
		dev_info(svsp->dev, "fake efuse\n");
		svsp->efuse[0] = 0x00000000;
		svsp->efuse[4] = 0x5def2c59;
		svsp->efuse[5] = 0x30162c08;
		svsp->efuse[14] = 0x1b031b03;
		svsp->efuse[15] = 0x1b031b03;
	}
	...
	for (idx = 0; idx < svsp->bank_num; idx++) {
		...
		case SVSB_GPU:
			...
			svsb->volt_bin = (svsp->efuse[19] >> 8) & GENMASK(2, 0);
			break;
		default:
			break;
		}
...
static int svs_status_debug_show(struct seq_file *m, void *v)
{
	...
	if (ret)
		seq_printf(m, "%s: no \"%s\" zone? volt_bin_turn_pt = %u turn_pt = %u\n",
			   svsb->name, svsb->tzone_name, svsb->volt_bin_turn_pt, svsb->turn_pt);
	else
		seq_printf(m, "%s: temperature = %d, volt_bin_turn_pt = %u turn_pt = %u\n",
			   svsb->name, tzone_temp, svsb->volt_bin_turn_pt, svsb->turn_pt);
...
static int svs_get_svs_mt8188_platform_data(struct svs_platform *svsp)
{
#if LVTS_READY
	struct device *dev;
#endif
	...
	svsp->name = "mt8188-svs";
	svsp->banks = svs_mt8188_banks;
	svsp->efuse_parsing = svs_mt8188_efuse_parsing;
	svsp->regs = svs_regs_v2;
	svsp->irqflags = IRQF_TRIGGER_HIGH;
	svsp->bank_num = ARRAY_SIZE(svs_mt8188_banks);
	svsp->fake_efuse = false;
	svsp->efuse_check = 5;  /* check if GPU low bank efuse available */
	...
	for (idx = 0; idx < svsp->bank_num; idx++) {
		...
		case SVSB_GPU:
			if (svsb->type == SVSB_LOW)
				...
				dev_err(svsp->dev, "unknown bank type = %u",
					svsb->type);
			break;
		default:
			WARN_ON(1);
...
static const struct of_device_id mtk_svs_of_match[] = {
	{
		...
		.data = &svs_get_svs_mt8188_platform_data,
	}, {
		/* Sentinel */
	},
};
...
static int svs_probe(struct platform_device *pdev)
{
	...
	svsp->dev = &pdev->dev;
	svs_get_svs_platform_data = of_device_get_match_data(svsp->dev);
