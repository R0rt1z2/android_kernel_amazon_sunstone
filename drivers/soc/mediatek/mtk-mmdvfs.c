...
int mtk_mmdvfs_dbg_level;
EXPORT_SYMBOL(mtk_mmdvfs_dbg_level);
module_param(mtk_mmdvfs_dbg_level, int, 0644);
#ifdef MMDVFS_DBG
#define mmdvfs_debug(fmt, args...)                              \
	do {                                                             \
		if (mtk_mmdvfs_dbg_level)           \
			pr_info("[MMDVFS] %s(),%d: " fmt "\n",\
				__func__, __LINE__, ##args);      \
	} while (0)
#else
#define mmdvfs_debug(fmt, args...)
#endif
#define mmdvfs_notice(fmt, args...)                              \
	pr_notice("[MMDVFS] %s(),%d: " fmt "\n",\
		__func__, __LINE__, ##args)
...
static bool enable_mmdvfs;

static BLOCKING_NOTIFIER_HEAD(mmdvfs_notifier_list);

void mtk_mmdvfs_enable(void)
{
	enable_mmdvfs = true;
}
EXPORT_SYMBOL_GPL(mtk_mmdvfs_enable);
...
static void set_all_muxes(struct mmdvfs_drv_data *drv_data, u32 opp_level)
{
	...
	for (i = 0; i < num_muxes; i++) {
		...
		if (err) {
			mmdvfs_notice("prepare mux(%s) fail:%d opp_level:%d\n",
				drv_data->muxes[i].mux_name, err, opp_level);
			continue;
		}
		err = clk_set_parent(mux, clk_src);
		if (err)
			mmdvfs_notice("set parent(%s) fail:%d opp_level:%d\n",
				drv_data->muxes[i].mux_name, err, opp_level);
...
static void set_all_hoppings(struct mmdvfs_drv_data *drv_data, u32 opp_level)
{
	...
	for (i = 0; i < num_hoppings; i++) {
		...
		if (err) {
			mmdvfs_notice("prepare hopping(%s) fail:%d opp_level:%d\n",
		...
		if (err)
			mmdvfs_notice("set %s rate(%u) fail:%d opp_level:%d\n",
				drv_data->hoppings[i].hopping_name,
	...
	struct mmdvfs_drv_data *drv_data, u32 voltage, bool vol_inc)
{
	...
	if (i == MAX_OPP_NUM) {
		mmdvfs_notice("voltage(%d) is not found\n", voltage);
		return;
	}
	...
	mmdvfs_debug("set clk to opp level:%d\n", opp_level);
}
				...
				unsigned long event, void *data)
{
	...
	if (event == REGULATOR_EVENT_PRE_VOLTAGE_CHANGE) {
		...
		mmdvfs_debug("regulator event=PRE_VOLTAGE_CHANGE old=%lu new=%lu\n",
		...
		mmdvfs_debug("regulator event=VOLTAGE_CHANGE voltage=%lu\n", uV);
		...
		mmdvfs_debug("regulator event=ABORT_VOLTAGE_CHANGE voltage=%lu\n",
			uV);
	}
	return 0;
}
...
static int force_clk_set(void *data, u64 val)
{
	...
	if (val == 0) {
		...
		if (ret)
			mmdvfs_notice("Failed to register notifier: %d\n", ret);
	...
	mmdvfs_notice("%s: val=%llu\n", __func__, val);
	return 0;
}

static int force_step_set(void *data, u64 val)
{
	struct mmdvfs_dbg_data *dbg_data = (struct mmdvfs_dbg_data *)data;
	struct mmdvfs_drv_data *drv_data = dbg_data->drv_data;

	if (val < dbg_data->max_index &&
		dbg_data->max_index - val - 1 < MAX_OPP_NUM)
		val = drv_data->voltages[dbg_data->max_index - val - 1];
	else
		val = drv_data->voltages[0];
...
DEFINE_SIMPLE_ATTRIBUTE(force_step_ops, NULL, force_step_set, "%llu\n");
...
static int mmdvfs_probe(struct platform_device *pdev)
{
	...
	if (!enable_mmdvfs) {
		mmdvfs_notice("mmdvfs is not enabled\n");
		return -EPROBE_DEFER;
	}
	...
	of_property_for_each_string(
		...
		if (num_mux >= MAX_MUX_NUM) {
			mmdvfs_notice("Too many items in support_mux\n");
			return -EINVAL;
		}
		...
		of_property_for_each_string(
			...
			if (num_clksrc >= MAX_OPP_NUM) {
				mmdvfs_notice("Too many items in %s\n", prop_name);
				return -EINVAL;
			}
	...
	of_property_for_each_string(dev->of_node, "mediatek,support_hopping",
		...
		if (num_hopping >= MAX_HOPPING_CLK_NUM) {
			mmdvfs_notice("Too many items in support_hopping\n");
			return -EINVAL;
		}
	...
	if (of_property_read_u32(dev->of_node,
		...
		mmdvfs_notice("Failed to get property: mediatek,action\n");
		return -EINVAL;
	}
	...
	dbg_data->max_index = index;
	dentry = debugfs_create_file("force_clk", 0200,
			mmdvfs_debugfs_dir, dbg_data, &force_clk_ops);
	if (IS_ERR(dentry))
		mmdvfs_notice("Failed to create debugfs force_clk: %ld\n",
			PTR_ERR(dentry));
	dentry = debugfs_create_file("force_step", 0200,
			mmdvfs_debugfs_dir, dbg_data, &force_step_ops);
	if (IS_ERR(dentry))
		mmdvfs_notice("Failed to create debugfs force_step: %ld\n",
			PTR_ERR(dentry));
#endif
...
#if 0
	//set default voltage as max
	mmdvfs_notice("%s: notifier_call done\n", __func__);
	ret = regulator_set_voltage(reg, drv_data->voltages[index-1], INT_MAX);
	if (ret) {
		pr_debug("%s Failed to set regulator voltage:%d\n",
			__func__, drv_data->voltages[index-1]);
	}
	mmdvfs_notice("%s: regulator_set_voltage to default: %d done!\n", __func__, drv_data->voltages[index-1]);
#endif
...
static int __init mtk_mmdvfs_init(void)
{
	...
	if (status) {
		mmdvfs_notice("Failed to register MMDVFS driver(%d)\n", status);
		return -ENODEV;
	}
	return 0;
}
