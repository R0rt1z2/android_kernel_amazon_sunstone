...
#endif
	void (*set_dram_boost_level)(struct mtk_dvfsrc *dvfsrc, u32 level);
};
...
static int dvfsrc_wait_for_idle(struct mtk_dvfsrc *dvfsrc)
{
	...
	return readx_poll_timeout(dvfsrc_is_idle, dvfsrc,
...
static void mt6873_set_boost_level(struct mtk_dvfsrc *dvfsrc, u32 level)
{
	spin_lock(&dvfsrc->req_lock);
	dvfsrc_rmw(dvfsrc, DVFSRC_SW_REQ5, level, 0x7, 12);
	spin_unlock(&dvfsrc->req_lock);
}
...
void mtk_dvfsrc_send_request(const struct device *dev, u32 cmd, u64 data)
{
	...
	case MTK_DVFSRC_CMD_FORCEOPP_REQUEST:
		...
		goto out;
#endif
	case MTK_DVFSRC_CMD_BOOST_REQUEST:
		if (dvfsrc->dvd->set_dram_boost_level)
			dvfsrc->dvd->set_dram_boost_level(dvfsrc, data);
		break;
	default:
...
#if DVFS_FOR_BOOT_SPEED
	mtk_dvfsrc_send_request(dvfsrc->dev,
				MTK_DVFSRC_CMD_BOOST_REQUEST,
				0x7);
	dev_info(dvfsrc->dev, "dvfsrc: boost ddr opp = %d\n",
		 dvfsrc->dvd->get_dram_level(dvfsrc));
#endif
	return 0;
...
#endif
	.set_dram_boost_level = mt6873_set_boost_level,
};
