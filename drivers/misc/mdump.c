...
static struct mdump_buffer *g_mdump_back_buf;
#endif
		...
		char *buf, loff_t off, size_t count)
{
	...
	if (expbase > (unsigned int)(uintptr_t)(mdcattr->paddr)) {
		vunmap((void *)mdcattr->vaddr);
		mdcattr->vaddr = remap_lowmem((phys_addr_t)expbase,
					(phys_addr_t)PHYMEM_MAP_CHUNK);
		if (mdcattr->vaddr == NULL) {
			pr_err("comp dump function can't map phyaddr: 0x%x\n",
				expbase);
			return 0;
		}
		mdcattr->paddr = (void *)(uintptr_t)expbase;
	}
	/* make sure not to read beyond the mapped memory region */
	if ((off - (expbase - COMPRESS_START_ADDRESS) + count) >
		PHYMEM_MAP_CHUNK)
		count = PHYMEM_MAP_CHUNK -
			(off - (expbase - COMPRESS_START_ADDRESS));

	if (count != 0) {
		memcpy(buf,
			mdcattr->vaddr + off-(expbase-COMPRESS_START_ADDRESS),
			count);
		memset(mdcattr->vaddr + off-(expbase-COMPRESS_START_ADDRESS),
			0, count);
		__dma_flush_area(
			mdcattr->vaddr + off-(expbase-COMPRESS_START_ADDRESS),
			count);
...
static void check_compress_dump(void)
{
	...
	/*
	 *  No action needed if mdump is not enabled,
	 *  or reboot reason is not kernel panic or
	 *  watchdog timeout.
	 */
	...
	s_mdump_compdump.vaddr = remap_lowmem(
					(phys_addr_t)COMPRESS_START_ADDRESS,
					(phys_addr_t)PHYMEM_MAP_CHUNK);
	if (s_mdump_compdump.vaddr == NULL) {
		pr_err("%s: Can not map physical address 0x%x\n", __func__,
			COMPRESS_START_ADDRESS);
		return;
	}

	/* check the header string */
	compresshdr = (struct compress_file_header *)s_mdump_compdump.vaddr;
	if (strncmp(compresshdr->header_signature, COMP_HEAD_SIGNATURE,
			COMP_SIGNATURE_SIZE)) {
		pr_info("%s: The comp header sign is not valid!\n", __func__);

		goto error_exit;
	}
	/* check number of compressed segments and total file size. */
	if (!compresshdr->num_of_segments ||
	(compresshdr->total_file_size <= sizeof(struct compress_file_header))) {
		pr_info("%s: No valid compressed conent!\n", __func__);
		goto error_exit;
	}
	...
	pr_err("%s: compression dump size is: %zu\n", __func__,
		s_mdump_compdump.binattr.size);
	if (sysfs_create_bin_file(&s_mdump_kobj,
				&s_mdump_compdump.binattr) == 0)
		return;
...
static int mdump_panic_notify(struct notifier_block *this,
...
#ifdef CONFIG_MDUMP_COMPRESS
		/* Since preloader and lk log in current mdump buffer will be overwrited in mdump mode
		 * so backup here, then we can check it in ramdump
		 */
		g_mdump_back_buf = kmalloc(sizeof(struct mdump_buffer), GFP_KERNEL);
		if (g_mdump_back_buf)
			memcpy(g_mdump_back_buf, s_mdump_buffer, sizeof(struct mdump_buffer));
#endif
	}
...
{
	pr_err("[memblock]%s: 0x%llx - 0x%llx (0x%llx)\n",
...
RESERVEDMEM_OF_DECLARE(reserve_memory_mdump, "amazon,mdump-reserve-memory",
			mdump_reserve_memory);
/*---------------------------------------------------------------------------*/
