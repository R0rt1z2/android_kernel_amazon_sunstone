	...
	struct cmdqRecStruct *handle, u32 index)
{
	...
	va = cmdq_pkt_get_va_by_offset(handle->pkt, offset);
	if (!va) {
		CMDQ_ERR("%s: get va by offset fail\n", __func__);
		return;
	}

	inst = *va;

	logic_va = cmdq_pkt_get_va_by_offset(handle->pkt,
	...
	u32 arg_a, u32 arg_b, u32 arg_a_type, u32 arg_b_type)
{
	...
	u32 arg_type = 0;
	u16 gpr_event = cmdq_util_get_platform_event_id(CMDQ_TOKEN_GPR_SET_4);
...
s32 cmdq_task_reset(struct cmdqRecStruct *handle)
{
	...
	handle->reorder = 0;
	handle->submit = 0;
	handle->transInstrBeg = 0;
	handle->transInstrEnd = 0;
	handle->trigger = 0;
	handle->beginWait = 0;
	handle->gotIRQ = 0;
	handle->wakedUp = 0;
...
static s32 cmdq_get_event_op_id(u16 event)
{
	s32 event_id = 0;
...
s32 cmdq_op_wait(struct cmdqRecStruct *handle, u16 event)
{
...
s32 cmdq_op_set_event(struct cmdqRecStruct *handle, u16 event)
{
	s32 arg_a = cmdq_get_event_op_id(event);
	...
	s32 new_arg_a, s32 new_arg_b, s32 new_arg_c)
{
	...
	if (!va) {
		CMDQ_ERR("get va by offset fail\n");
		return -EFAULT;
	}
...
s32 cmdq_op_wait_ex(struct cmdqRecStruct *handle,
	struct cmdq_command_buffer *cmd_buf, u16 event)
{
	s32 arg_a = cmdq_get_event_op_id(event);
	u32 arg_b;
	s32 err;
...
s32 cmdq_task_query_offset(struct cmdqRecStruct *handle, u32 startIndex,
	const enum cmdq_code opCode, u16 event)
{
	s32 offset = -1;
	u32 arg_a, arg_b;
	u64 inst;
...
s32 cmdq_resource_acquire_and_write(struct cmdqRecStruct *handle,
	u16 resourceEvent, u32 addr, u32 value, u32 mask)
{
	s32 status;
...
s32 cmdqRecWait(struct cmdqRecStruct *handle, u16 event)
{
