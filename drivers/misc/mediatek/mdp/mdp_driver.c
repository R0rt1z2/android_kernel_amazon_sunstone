	...
	unsigned long param)
{
	...
	case CMDQ_IOCTL_SIMULATE:
		...
		break;
#endif
	case CMDQ_IOCTL_CONVERT_SEC_FD:
		CMDQ_MSG("convert fd to Secure handle\n");
		status = mdp_ioctl_convert_sec_fd(pf, param);
		break;
	case CMDQ_IOCTL_CHECK_FD_VALID:
		CMDQ_LOG("check fd valid\n");
		status = mdp_ioctl_check_valid_fd(pf, param);
		break;
	default:
...
static int cmdq_probe(struct platform_device *pDevice)
{
	...
	CMDQ_MSG("[MDP] MDP driver probe begin\n");
...
static int __init cmdq_init(void)
{
	...
	CMDQ_MSG("%s MDP driver init begin\n", __func__);
