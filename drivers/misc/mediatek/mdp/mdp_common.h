...
typedef void (*CmdqMdpDumpMERGE) (const unsigned long base, const char *label);
...
typedef u32 (*MdpGetLarbID) (u16 engine);

typedef void (*MdpHandlePrepare)(struct cmdqRecStruct *handle);

typedef void (*MdpHandleUnprepare)(struct cmdqRecStruct *handle);
...
struct cmdqMDPFuncStruct {
	...
	MdpGetLarbID getLarbID;

	MdpHandlePrepare handlePrepare;
	MdpHandleUnprepare handleUnprepare;
};
