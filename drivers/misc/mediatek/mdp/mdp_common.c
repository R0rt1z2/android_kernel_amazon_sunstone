...
#define LEGACY_DRE30_HIST_SRAM_START	1536
...
static void cmdq_mdp_common_clock_disable(u64 engine_flag)
{
	...
	if (smi_ref < 0) {
		CMDQ_ERR("[CLOCK] MDP SMI clock check error %d\n", smi_ref);
		return;
	}
...
bool cmdq_mdp_acquire_resource(u16 res_event,
	u64 *engine_flag_out)
{
	struct ResourceUnitStruct *res = NULL;
	bool result = false;
...
void cmdq_mdp_release_resource(u16 res_event,
	u64 *engine_flag_out)
{
	struct ResourceUnitStruct *res = NULL;
...
void cmdq_mdp_set_resource_callback(u16 res_event,
...
static void cmdq_mdp_lock_thread(struct cmdqRecStruct *handle)
{
	...
	if (thread < 0 ||
		thread >= CMDQ_MAX_THREAD_COUNT) {
		CMDQ_ERR("invalid thread:%d\n", thread);
		return;
	}
...
void cmdq_mdp_unlock_thread(struct cmdqRecStruct *handle)
{
	...
	if (thread < 0 ||
		thread >= CMDQ_MAX_THREAD_COUNT) {
		CMDQ_ERR("invalid thread %d for unlock\n", thread);
		return;
	}
...
static void cmdq_mdp_handle_prepare(struct cmdqRecStruct *handle)
{
	...
	if (mdp_funcs.handlePrepare)
		mdp_funcs.handlePrepare(handle);
}
...
static void cmdq_mdp_handle_unprepare(struct cmdqRecStruct *handle)
{
	...
	if (mdp_funcs.handleUnprepare)
		mdp_funcs.handleUnprepare(handle);
...
static s32 cmdq_mdp_find_free_thread(struct cmdqRecStruct *handle)
{
	...
	if (thread == CMDQ_INVALID_THREAD) {
		thread = cmdq_core_acquire_thread(handle->scenario, handle->secData.is_secure);
	...
	struct cmdqRecStruct *handle)
{
	...
	if (!desc->userDebugStr || !desc->userDebugStrLen ||
		(desc->userDebugStrLen > CMDQ_MAX_DBG_STR_LEN))
		return;
	...
	struct cmdqRecStruct *handle)
{
	...
	if (!cl) {
		CMDQ_ERR("fail to get mbox client:%d\n", handle->thread);
		return -EINVAL;
	}
	...
	u32 sec_handle, u32 index, u32 instr_index)
{
	...
	addr[index].baseHandle = sec_handle;
...
void cmdq_mdp_init(struct platform_device *pdev)
{
	...
	CMDQ_MSG("dre hist sram start:%u\n", dre30_hist_sram_start);
...
void cmdq_mdp_dump_merge_virtual(const unsigned long base, const char *label)
{
	u32 value[6] = { 0 };
...
static void mdp_request_voltage(unsigned long frequency, bool is_mdp)
{
	...
	if (!is_mdp && cmdq_mdp_get_func()->mdpIsCaminSupport()) {
		CMDQ_ERR("isp is not support\n");
		return;
	}
	...
	CMDQ_LOG_PMQOS("%s frequency %ld vol:%d\n", __func__, frequency, low_volt);
...
static void mdp_update_voltage(u32 thread_id, u64 freq, bool is_mdp)
{
	...
	CMDQ_LOG_PMQOS("%s thread %u freq %llu\n",
	...
	struct cmdqRecStruct **handle_list, u32 size)
{
	...
	CMDQ_LOG_PMQOS(
		"[MDP]mdp %d pixel, mdp %d byte, isp %d pixel, isp %d byte, submit %06ld ns, end %06ld ns duration:%d ns\n",
	...
	struct cmdqRecStruct **handle_list, u32 size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (cmdq_mdp_get_func()->mdpIsIspCamin(curTask))
	...
	u16 engine, phys_addr_t base, dma_addr_t pa, u32 param)
{
	...
	u16 gpr_event = cmdq_util_get_platform_event_id(CMDQ_TOKEN_GPR_SET_1);
	...
	if (handle->secData.is_secure) {
		#ifdef CMDQ_SECURE_PATH_SUPPORT
		u64 sec_eng_flag = cmdq_mdp_get_func()->mdpGetSecEngine(1LL << engine);

		if (sec_eng_flag == 0) {
			CMDQ_ERR("invalid engine:%d\n", engine);
			return;
		}
		rb->engine = ffs(sec_eng_flag);
		#else
		CMDQ_ERR("cmdq svp not enable\n");
		return;
		#endif
	} else
		rb->engine = engine;
	...
	*condi_inst = (u32)CMDQ_REG_SHIFT_ADDR(cmdq_pkt_get_curr_buf_pa(pkt));
	...
	cmdq_pkt_set_event(pkt, gpr_event);
}
...
u32 mdp_get_larb_id_virtual(u16 engine)
{
	return 0;
}
...
void cmdq_mdp_virtual_function_setting(void)
{
	...
	pFunc->mdpDumpMerge = cmdq_mdp_dump_merge_virtual;
	...
	pFunc->getLarbID = mdp_get_larb_id_virtual;
...
void cmdq_mdp_dump_rdma(const unsigned long base, const char *label)
{
	...
	if (gCmdqRdmaPrebuiltSupport) {
		...
		cmdq_util_prebuilt_dump(0,
			cmdq_util_get_platform_event_id(CMDQ_TOKEN_PREBUILT_MDP_WAIT));
	}
...
u32 cmdq_mdp_get_larb_id(u16 engine)
{
	return cmdq_mdp_get_func()->getLarbID(engine);
}
