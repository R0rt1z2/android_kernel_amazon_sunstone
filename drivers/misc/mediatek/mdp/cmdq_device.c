	...
	uint32_t *pa)
{
	...
	CMDQ_MSG("DEV: VA ref(%s):0x%lx pa:%#x\n", ref_name, VA, *pa);
	return VA;
}
...
phys_addr_t cmdq_dev_get_reference_PA(const char *ref_name, int index)
{
	...
	do {
		...
		start_pa = res.start;
		CMDQ_MSG("DEV: PA ref(%s): start:%pa\n",
			ref_name, &start_pa);
	} while (0);
...
void cmdq_dev_init_event_table(struct device_node *node)
{
	...
	for (id = 0; id < CMDQ_TOKEN_MAX; id++)
		cmdq_core_set_event_table(id, cmdq_util_get_platform_event_id(id));
}
...
void cmdq_dev_init(struct platform_device *pDevice)
{
	...
	gCmdqDev.dma_mask_result = dma_set_mask_and_coherent(
		&pDevice->dev, DMA_BIT_MASK(dma_mask_bit));
	CMDQ_MSG("set dma mask bit:%u result:%d\n",
