...
struct device *cmdq_mdp_get_device(u32 engine)
{
	struct device *larb_dev = NULL;
	u32 larb_id = 0;

	larb_id = cmdq_mdp_get_larb_id(engine);
	if (larb_id)
		larb_dev = mdp_larb_get_dev(larb_id);
	else
		larb_dev = mdpsys_con_ctx.dev;

	if (!larb_dev)
		CMDQ_ERR("%s fail to get larb dev larb_id:%d engine:%d\n",
			__func__,
			larb_id, engine);
				...
				struct mdp_job_mapping *mapping_job)
{
	...
	if (i == mapping_job->handle_count) {
		...
		larb_dev = cmdq_mdp_get_device(meta->engine);
		if (larb_dev == NULL)
			return 0;

		if (!mdp_ion_get_dma_buf(larb_dev, meta->fd, &buf,
			&attach, &sgt))
			return 0;
		...
		if (ion_addr) {
			...
			mapping_job->engines[i] = meta->engine;
			...
			CMDQ_ERR("%s fail to get iova for fd:%d\n",
				__func__, meta->fd);
			mdp_ion_free_dma_buf(larb_dev, buf, attach, sgt);
			return 0;
		}
	} else {
		ion_addr = mapping_job->mvas[i];
		sgt = mapping_job->sgts[i];
	}

	/* normal buffer: mva = base_mva + offset
	 * secure buffer: secure handle = base_mva. no need to add fd_offset.
	 */
	if (sg_page(sgt->sgl) != 0)	// normal buffer
				...
				struct cmdqRecStruct *handle)
{
	...
	if (!(user_job->userDebugStrLen > CMDQ_MAX_DBG_STR_LEN ||
		user_job->userDebugStrLen <= 0 ||
		!user_job->userDebugStr)) {
		long len;

		handle->user_debug_str = kzalloc(user_job->userDebugStrLen + 1, GFP_KERNEL);
		if (!handle->user_debug_str) {
			CMDQ_ERR("allocate user debug memory failed, size:%d\n",
				user_job->userDebugStrLen);
			return -EINVAL;
		}

		len = strncpy_from_user(handle->user_debug_str,
			(const char *)(unsigned long)user_job->userDebugStr,
			user_job->userDebugStrLen);
		if (len < 0) {
...
s32 mdp_ioctl_async_exec(struct file *pf, unsigned long param)
{
	...
	handle->transInstrBeg = sched_clock();
	status = translate_user_job(&user_job, mapping_job, handle, &cmd_buf);
	handle->transInstrEnd = sched_clock();
	handle->instrCount = user_job.meta_count;
...
void mdp_check_pending_task(struct mdp_job_mapping *mapping_job)
{
	...
	for (i = 0; i < mapping_job->handle_count; i++)
		mdp_ion_free_dma_buf(
			cmdq_mdp_get_device(mapping_job->engines[i]),
			mapping_job->dma_bufs[i],
...
s32 mdp_ioctl_async_wait(unsigned long param)
{
	...
	mdp_debug_printf_record(
		"mdp job[%lld] handle[%p] total:%d ms instrCnt %d ",
		mapping_job->id,
		mapping_job->job,
		cmdq_get_time_duration_ms(handle->submit, sched_clock()),
		handle->instrCount);
	mdp_debug_printf_record(
		"submit %d transInstrBeg %d transInstrEnd %d trigger %d gotIRQ\n",
		cmdq_get_time_duration_ms(handle->submit,
			handle->transInstrBeg),
		cmdq_get_time_duration_ms(handle->transInstrBeg,
			handle->transInstrEnd),
		cmdq_get_time_duration_ms(handle->transInstrEnd,
			handle->trigger),
		cmdq_get_time_duration_ms(handle->trigger,
			handle->gotIRQ));
...
s32 mdp_ioctl_free_readback_slots(void *fp, unsigned long param)
{
	...
	if (rb_slot[free_slot_index].ref_cnt) {
		CMDQ_ERR("%s slot id[%d] is in use, using slot count : %d\n", __func__,
			free_slot_index,
			rb_slot[free_slot_index].ref_cnt);
		mutex_unlock(&rb_slot_list_mutex);
		return -EINVAL;
	}
	alloc_slot[free_slot_group] &= ~(1LL << free_slot);
...
int cmdq_mdp_check_fd_valid(int fd)
{
	...
	larb_dev = cmdq_mdp_get_device(2); /* larb 4 device */

	if (!larb_dev) {
		CMDQ_ERR("%s fail to get larb dev\n", __func__);
		status = -1;
		goto exit;
	}
	...
	if (IS_ERR(dma_buffer)) {
		CMDQ_ERR("MDP secure handle[%d] get dma_buffer failed[%ld]!",
			fd, PTR_ERR(dma_buffer));
		status = -2;
		goto exit;
	}

	dma_attach = dma_buf_attach(dma_buffer, larb_dev);
	if (IS_ERR(dma_attach)) {
		CMDQ_ERR("fd buf attach fail %ld", PTR_ERR(dma_attach));
		status = -3;
		goto put;
	}

	table = dma_buf_map_attachment(dma_attach, 0);
	if (IS_ERR(table)) {
		CMDQ_ERR("fd buf map fail %ld", PTR_ERR(table));
		status = -4;
		goto detach;
	}
...
u32 cmdq_mdp_convert_fd_to_sec_handle(int sec_fd)
{
	...
	dma_buffer = dma_buf_get(sec_fd);
	if (IS_ERR(dma_buffer)) {
		CMDQ_ERR("MDP secure handle[%d] get dma_buffer failed[%ld]!",
			sec_fd, PTR_ERR(dma_buffer));
		goto exit;
	}
	...
	if (sg_page(table->sgl) == 0) {
		/* sec buffer */
		dma_addr = sg_dma_address(table->sgl);
	} else {
		/* non sec buffer */
		CMDQ_ERR("try to get normal buffer dma add\n");
	}
...
{
	struct mdp_convert_sec_fd sec_info = {0};

	if (copy_from_user(&sec_info,
			(void *)param,
			sizeof(struct mdp_convert_sec_fd))) {
		CMDQ_ERR("%s copy sec fd from user fail\n", __func__);
		return -EFAULT;
	}

	sec_info.sec_handle = cmdq_mdp_convert_fd_to_sec_handle(
		sec_info.sec_fd);
	if (sec_info.sec_handle == 0) {
		CMDQ_ERR("convert sec fd:%d fail\n", sec_info.sec_fd);
		return -EFAULT;
	}
	...
	if (copy_to_user((void *)param,
			&sec_info,
			sizeof(struct mdp_convert_sec_fd))) {
		CMDQ_ERR("%s copy sec handle to user fail\n", __func__);
		return -EFAULT;
	}
...
s32 mdp_ioctl_check_valid_fd(void *fp, unsigned long param)
{
	...
	return cmdq_mdp_check_fd_valid(sec_info.sec_fd);
}
