	...
	const u32 op, const u32 arg_a, const u32 arg_b)
{
	...
	case CMDQ_CODE_WFE:
		if (arg_b == 0x80008001) {
			reqLen = snprintf(textBuf, bufLen,
				"Wait And Clear Event:%d\n", arg_a);
		} else if (arg_b == 0x80000000) {
			reqLen = snprintf(textBuf, bufLen, "Clear Event:%d\n", arg_a);
		} else if (arg_b == 0x80010000) {
			reqLen = snprintf(textBuf, bufLen, "Set Event:%d\n", arg_a);
		} else if (arg_b == 0x00008001) {
			reqLen = snprintf(textBuf, bufLen,
				"Wait No Clear Event:%d\n", arg_a);
		} else {
			reqLen = snprintf(textBuf, bufLen,
				"SYNC:%d, upd:%d op:%d val:%d wait:%d wop:%d val:%d\n",
				arg_a,
				(arg_b >> 31) & 0x1,
				(arg_b >> 28) & 0x7,
				(arg_b >> 16) & 0xFFF,
...
int cmdq_core_print_status_seq(struct seq_file *m, void *v)
{
	...
	list_for_each_entry(handle, &cmdq_ctx.handle_active, list_entry) {
		...
		if (handle->thread < 0 ||
			handle->thread >= ARRAY_SIZE(cmdq_clients))
			continue;
...
void cmdq_core_set_event_table(u16 event, const s32 value)
{
	if (event < CMDQ_TOKEN_MAX)
		cmdq_dts_data.eventTable[event] = value;
}

s32 cmdq_core_get_event_value(u16 event)
{
	if (event >= CMDQ_TOKEN_MAX)
		return -EINVAL;
...
void cmdqCoreClearEvent(u16 event)
{
...
void cmdqCoreSetEvent(u16 event)
{
	s32 eventValue = cmdq_core_get_event_value(event);
...
u32 cmdqCoreGetEvent(u16 event)
{
	s32 eventValue = cmdq_core_get_event_value(event);
	...
	u32 *insts, u32 size, u32 **pc_va)
{
	...
	if (handle->thread < 0 ||
		handle->thread >= CMDQ_MAX_THREAD_COUNT) {
	...
	u32 *insts, int thread, u32 lineNum)
{
	...
	if (op == CMDQ_CODE_WFE) {
		...
		CMDQ_ERR("CMDQ_SYNC_TOKEN_VAL(idx:%u) is %d\n",
			eventID, cmdqCoreGetEvent(eventID));
	}
}
	...
	u32 *irq_flag_out, u32 *inst_a_out, u32 *inst_b_out, bool *aee_out)
{
	...
	if (((nginfo.inst[1] & 0xFF000000) >> 24) == CMDQ_CODE_WFE) {
		...
		if (event >= CMDQ_TOKEN_RESOURCE_WROT0)
	...
	const struct cmdqRecStruct *handle, enum TASK_STATE_ENUM state)
{
	...
	case CMDQ_CODE_WFE:
		CMDQ_ERR(
			"%s in CMDQ IRQ:0x%02x, INST:(0x%08x, 0x%08x), OP:WAIT EVENT:0x%x\n",
			module, irq_flag, inst_a, inst_b, (inst_a & (~0xFF000000)));
		break;
	default:
		CMDQ_ERR(
...
void cmdq_core_dump_thread_usage(void)
{
	u32 idx;
...
s32 cmdq_core_acquire_thread(enum CMDQ_SCENARIO_ENUM scenario, bool secure)
{
...
#ifdef CMDQ_SECURE_PATH_SUPPORT
	if (secure) {
		idx = cmdq_ctx.thread[CMDQ_THREAD_SEC_MDP_0].acquire >
			cmdq_ctx.thread[CMDQ_THREAD_SEC_MDP_1].acquire ?
			CMDQ_THREAD_SEC_MDP_1 : CMDQ_THREAD_SEC_MDP_0;
	} else
#endif
	{
		for (idx = CMDQ_DYNAMIC_THREAD_ID_START;
...
void cmdq_core_release_thread(s32 scenario, s32 thread)
{
	if (thread < 0 ||
		thread >= CMDQ_MAX_THREAD_COUNT) {
		CMDQ_ERR("release invalid thread by scenario:%d thread:%d\n",
			scenario,
			thread);
		return;
	}
...
s32 cmdq_core_suspend_hw_thread(s32 thread)
{
	...
	if (thread < 0 ||
		thread >= ARRAY_SIZE(cmdq_clients)) {
	...
	CmdqAsyncFlushCB cb, u64 user_data)
{
	...
	CMDQ_MSG("%s handle:0x%p pkt:0x%p size:%zu thread:%d sec:%d\n",
		__func__, handle, handle->pkt, handle->pkt->cmd_buf_size,
		handle->thread, handle->secData.is_secure);
...
void cmdq_core_initialize(void)
{
	...
	CMDQ_MSG("available thread pool:%s max:%u\n",
...
{
	CMDQ_MSG("CMDQ driver late init\n");
...
u32 cmdq_get_time_duration_ms(CMDQ_TIME start, CMDQ_TIME end)
{
	return div_u64(end - start, 1000000);
}
