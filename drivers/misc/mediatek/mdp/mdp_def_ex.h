...
struct hyfbcHeaderStruct {
	bool enable;
	bool is10b;
	uint32_t width_in_mb;
	uint32_t height_in_mb;
	uint32_t w_stride_in_mb;
	uint32_t h_stride_in_mb;
	uint32_t y_byte_per_mb;
	uint32_t c_byte_per_mb;
	uint32_t y_pa_base;
	uint32_t c_pa_base;
};
struct mdp_submit {
	...
	struct hyfbcHeaderStruct hyfbcHeader;
...
#define CMDQ_IOCTL_CONVERT_SEC_FD _IOW(CMDQ_IOCTL_MAGIC_NUMBER, 26, \
	struct mdp_convert_sec_fd)

#define CMDQ_IOCTL_CHECK_FD_VALID _IOW(CMDQ_IOCTL_MAGIC_NUMBER, 27, \
	struct mdp_convert_sec_fd)
