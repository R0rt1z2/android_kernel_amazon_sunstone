		...
		int idx)
{
	...
	if (strlen(dcxo->xo_bufs[idx].xo_name) >= sizeof(buf))
		return -EINVAL;
	strncpy(buf, dcxo->xo_bufs[idx].xo_name,
		strlen(dcxo->xo_bufs[idx].xo_name));
	buf[strlen(dcxo->xo_bufs[idx].xo_name) + 1] = '\0';
	p = buf;
	strsep(&p, "_");
	if (!(*p))
		return 0;
	...
	strncpy(xo_en_prop, CLKBUF_XO_PROP, strlen(CLKBUF_XO_PROP) + 1);
	strncat(xo_en_prop, p, strlen(p) + 1);
