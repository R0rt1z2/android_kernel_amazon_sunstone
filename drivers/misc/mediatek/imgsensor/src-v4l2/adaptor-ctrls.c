				...
				unsigned int framelength)
{
	...
	if (unlikely(p_ctx == NULL)) {
		ret = 1;
		pr_info("%s: ERROR: p_ctx is NULL (cmd_id:%u, fl:%u), return:%d\n",
			__func__,
						  ...
						  struct mtk_hdr_ae *ae_ctrl)
{
	...
	dev_dbg(ctx->dev, "exposure[LLLE->SSSE] %d %d %d %d %d ana_gain[LLLE->SSSE] %d %d %d %d %d\n",
	...
	case V4L2_CID_SEAMLESS_SCENARIOS:
		{
			...
			ret = copy_from_user(&ctx->seamless_scenarios, info->target_scenario_ids,
...
int adaptor_init_ctrls(struct adaptor_ctx *ctx)
{
	...
	dev_info(ctx->dev, "----control init 06: %d\n", ctrl_hdlr->error);
...
//    dev_info(ctx->dev, "----min:%d, max:%d, step:%d, def:%d\n", min, max, step, def);
...
//====================================
	...
	if (ctrl_hdlr->error) {
		ret = ctrl_hdlr->error;
		dev_info(ctx->dev, "control init failed 1: %d\n", ret);
	}
	...
	if (ctrl_hdlr->error) {
		ret = ctrl_hdlr->error;
		dev_info(ctx->dev, "control init failed 2: %d\n", ret);
	}
	...
	if (ctrl_hdlr->error) {
		ret = ctrl_hdlr->error;
		dev_info(ctx->dev, "control init failed 3: %d\n", ret);
		goto error;
	}
