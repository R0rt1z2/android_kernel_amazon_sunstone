 ...
 * Filename:
 * ---------
 *	 sc800rearsuntxdmipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "sc800rearsuntxdmipiraw_Sensor.h"
#include "sc800_rear_suntxd_ana_gain_table.h"
...
#define PFX "SC800_rear_suntxd_camera_sensor"
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	write_cmos_sensor_8(ctx, 0x3650, 0x51);/*mipitest*/
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(ctx, 0x3107) << 8)
				      | read_cmos_sensor_8(ctx, 0x3108)) + 2;
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(sc800_rear_suntxd_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)sc800_rear_suntxd_ana_gain_table,
			sizeof(sc800_rear_suntxd_ana_gain_table));
		}
		break;
...
const struct subdrv_entry sc800_rear_suntxd_mipi_raw_entry = {
	.name = "sc800_rear_suntxd_mipi_raw",
	.id = SC800_REAR_SUNTXD_SENSOR_ID,
	.pw_seq = pw_seq,
