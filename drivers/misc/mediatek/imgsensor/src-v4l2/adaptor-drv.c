...
struct mutex dpd_ctl_lock;
static int dpd_user[DPD_SESSION_MAX];

void  dpdSingleCameraReset(struct adaptor_ctx *ctx)
{
	int i;
	int sensorid = 0;

	sensorid = ctx->sensor_id;
	if ((sensorid == DUALSENID_1ST) || (sensorid == DUALSENID_2ND))
		;
	else
		return;

	mutex_lock(&dpd_ctl_lock);
	for (i = DPD_OPEN_CLOSE; i < DPD_SESSION_MAX; i++) {
		dpd_user[i] = 0;
		dev_info(ctx->dev, "----reset dpd_user[%d] = %d\n", i, dpd_user[i]);
	}
	mutex_unlock(&dpd_ctl_lock);
}


int dpdSingleCameraCtrl(struct adaptor_ctx *ctx, enum DPDSESSION session, int op, int sensorid)
{
	int res = DPD_CTRL_ACTIVE;

	dev_info(ctx->dev, "----[DPTZ]sensor_id:0x%x, session= %d, op= %d, userCnt= %d",
			sensorid, session, op, dpd_user[session]);
	...
	switch (op) {
	case DPD_CTR_GET:
		res = (dpd_user[session] == 0) ? DPD_CTRL_ACTIVE : DPD_CTRL_PASS;
		dpd_user[session] += 1;
		break;
	case DPD_CTR_RELEASE:
		res = (dpd_user[session] == 1) ? DPD_CTRL_ACTIVE : DPD_CTRL_PASS;
		if (dpd_user[session] > 0)
			dpd_user[session] -= 1;
		break;
	}
	mutex_unlock(&dpd_ctl_lock);
  ...
  //
	{
		...
		for (i = DPD_OPEN_CLOSE; i < DPD_SESSION_MAX; i++)
			dev_info(ctx->dev, "---- [DPTZ] final dpd_user[%d] = %d\n",
					i, dpd_user[i]);
	}
	return res;
}
...
static int search_sensor(struct adaptor_ctx *ctx)
{
	...
	for (i = 0; i < subdrvs_cnt; i++) {
		...
		if (!ret) {
			dev_info(ctx->dev, "[DPTZ] sensor %s(%d),id %d found\n",
				ctx->subdrv->name, ctx->subdrv->id, sensor_id);
#if MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
...
#if MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	if (!dpdSingleCameraCtrl(ctx, DPD_OPEN_CLOSE, DPD_CTR_GET, ctx->sensor_id)) {
		dev_info(ctx->dev, "[DPTZ] %s sensor skip open(%d)\n", __func__, ctx->sensor_id);
		return 0;
	}
	dev_info(ctx->dev, "[DPTZ] %s sensor(%d)\n", __func__, ctx->sensor_id);
#endif
...
#if MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	if (!dpdSingleCameraCtrl(ctx, DPD_OPEN_CLOSE, DPD_CTR_RELEASE, ctx->sensor_id)) {
		dev_info(ctx->dev, "[DPTZ] %s sensor(%d) skip close\n", __func__, ctx->sensor_id);
		return 0;
	}
	dev_info(ctx->dev, "[DPTZ] %s sensor(%d)\n", __func__, ctx->sensor_id);
#endif
				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	if (mode) {
		...
		dev_info(ctx->dev, "update_pad_format done\n");
		...
		if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
			...
			*framefmt = fmt->format;
...
#if MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
			if (!dpdSingleCameraCtrl(ctx, DPD_SET_PAD_FORMAT,
				DPD_CTR_GET, ctx->sensor_id)) {
				dev_info(ctx->dev, "[DPTZ] %s sensor(%d) skip open\n",
					__func__, ctx->sensor_id);
				ctx->is_sensor_inited = 1;
			}
#endif
			...
			dev_info(ctx->dev, "is_sensor_inited = %u\n", ctx->is_sensor_inited);
		...
		dev_info(ctx->dev, "failed in v4l2_find_nearest_size\n");
	}
...
#if MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	if (!dpdSingleCameraCtrl(ctx, DPD_SUSPEND_RESUME, DPD_CTR_GET, ctx->sensor_id)) {
		dev_info(ctx->dev, "[DPTZ] %s sensor(%d) skip resumed\n", __func__, ctx->sensor_id);
		return 0;
	}
	dev_info(ctx->dev, "[DPTZ] %s sensor(%d)\n", __func__, ctx->sensor_id);
#endif
...
static int imgsensor_runtime_suspend(struct device *dev)
{
...
#if MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	if (!dpdSingleCameraCtrl(ctx, DPD_SUSPEND_RESUME, DPD_CTR_RELEASE, ctx->sensor_id)) {
		dev_info(ctx->dev, "[DPTZ] %s sensor(%d) skip suspend\n", __func__, ctx->sensor_id);
		return 0;
	}
	dev_info(ctx->dev, "[DPTZ] %s sensor(%d)\n", __func__, ctx->sensor_id);
#endif
	...
	ctx->is_sensor_scenario_inited = 0;
	dev_info(ctx->dev, "%s, is_sensor_inited = %d\n",
		__func__, ctx->is_sensor_inited);
...
#if MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	if (!dpdSingleCameraCtrl(ctx, DPD_STREAM_ON_OFF, DPD_CTR_GET, ctx->sensor_id)) {
		dev_info(ctx->dev, "[DPTZ] %s sensor(%d) skip streaming on\n",
		__func__, ctx->sensor_id);
		//ctx->is_sensor_inited = 1;
		return 0;
	}
	dev_info(ctx->dev, "[DPTZ] %s sensor(%d) start streaming\n", __func__, ctx->sensor_id);
#endif
	dev_info(ctx->dev, "----is_sensor_inited(%d)\n", ctx->is_sensor_inited);
...
#if MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	if (!dpdSingleCameraCtrl(ctx, DPD_STREAM_ON_OFF, DPD_CTR_RELEASE, ctx->sensor_id)) {
		dev_info(ctx->dev, "[DPTZ] %s sensor(%d) skip streaming off\n",
		__func__, ctx->sensor_id);
		return 0;
	}
	dev_info(ctx->dev, "[DPTZ] %s sensor(%d) stop streaming\n", __func__, ctx->sensor_id);
#endif
...
static int __init adaptor_drv_init(void)
{
	...
	mutex_init(&dpd_ctl_lock);
	for (i = 0; i < DPD_SESSION_MAX; i++)
		dpd_user[i] = 0;
#endif
...
#if MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	mutex_destroy(&dpd_ctl_lock);
#endif
}
#ifdef NEED_LATE_INITCALL
late_initcall(adaptor_drv_init);
#else
module_init(adaptor_drv_init);
#endif
