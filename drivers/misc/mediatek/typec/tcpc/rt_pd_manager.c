...
struct rt_pd_manager_data {
	struct device *dev;
	struct gpio_desc *discharge_en_gpio;
#if IS_ENABLED(CONFIG_MTK_CHARGER)
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_EXT_DISCHARGE:
		...
		if (rpmd->support_discharge_gpio) {
			if (noti->en_state.en) {
				gpiod_set_value(rpmd->discharge_en_gpio,1);
			} else {
				gpiod_set_value(rpmd->discharge_en_gpio,0);
			}
		}
#if IS_ENABLED(CONFIG_MTK_CHARGER)
...
static int get_discharge_gpio_info(struct rt_pd_manager_data *rpmd)
{
	int ret = 0;

	rpmd->discharge_en_gpio = devm_gpiod_get(rpmd->dev, "discharge-en",
							GPIOD_OUT_LOW);

	if (IS_ERR(rpmd->discharge_en_gpio)) {
		ret = PTR_ERR(rpmd->discharge_en_gpio);
		dev_err(rpmd->dev,"get discharge-en-gpio fail: %d\n",ret);
		return ret;
	}

	rpmd->support_discharge_gpio = true;
	dev_info(rpmd->dev, "[%s] end\n", __func__);
