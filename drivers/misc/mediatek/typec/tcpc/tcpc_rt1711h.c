...
static int rt1711_read_device(void *client, u32 reg, int len, void *dst)
{
	...
	while (1) {
		...
		t2 = local_clock();
		RT1711_DBG("%s del = %lluus, reg = %02X, len = %d\n",
...
static int rt1711_init_alert(struct tcpc_device *tcpc)
{
	...
	ret = request_threaded_irq(chip->irq, NULL, rt1711_intr_handler,
		IRQF_ONESHOT, name, chip);
	if (ret < 0) {
		pr_notice("Error: failed to request irq%d, ret = %d\n",
			chip->irq, ret);
		goto init_alert_err;
	}
...
static int rt_parse_dt(struct rt1711_chip *chip, struct device *dev)
{
	...
	if (!np) {
		pr_notice("%s find node usb_type_c fail\n", __func__);
		return -ENODEV;
	}
	dev->of_node = np;
	return 0;
				...
				const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		pr_notice("rt1711 init alert fail\n");
		goto err_irq_init;
	}
	...
	if (client) {
		...
		if (chip) {
			...
			if (typec_is_drp_toggling()) {
				...
				/* mask cc alert */
				rt1711_i2c_update_bits(tcpc, TCPC_V10_REG_ALERT_MASK,
						       0x00, TCPC_V10_REG_ALERT_CC_STATUS);
				/* debug only */
				ret = rt1711_i2c_read8(tcpc, TCPC_V10_REG_ALERT_MASK);
				dev_info(dev, "%s: reg0x12 = 0x%02x\n", __func__, ret);
				/* switch cc to Rd, Rd */
				tcpci_set_cc(tcpc, TYPEC_CC_RD);
				mdelay(1);
				/* clear cc alert event */
				rt1711_i2c_write8(tcpc, TCPC_V10_REG_ALERT,
