...
static int husb311_block_read(struct i2c_client *i2c, u8 reg, int len,
			void *dst)
{
	struct husb311_chip *chip = i2c_get_clientdata(i2c);
	int ret = 0;
...
static int husb311_block_write(struct i2c_client *i2c, u8 reg, int len,
			const void *src)
{
	struct husb311_chip *chip = i2c_get_clientdata(i2c);
	int ret = 0;
...
static int32_t husb311_write_word(struct i2c_client *client, uint8_t reg_addr,
			uint16_t data)
{
	int ret;
...
static int32_t husb311_read_word(struct i2c_client *client, uint8_t reg_addr,
			uint16_t *data)
{
	int ret;
...
static inline int husb311_i2c_write8(struct tcpc_device *tcpc, u8 reg,
				const u8 data)
{
...
static inline int husb311_i2c_write16(struct tcpc_device *tcpc, u8 reg,
				const u16 data)
{
	struct husb311_chip *chip = tcpc_get_dev_data(tcpc);
...
static inline int husb311_i2c_read16(struct tcpc_device *tcpc, u8 reg)
{
	struct husb311_chip *chip = tcpc_get_dev_data(tcpc);
	u16 data;
	int ret;
...
static inline int husb311_i2c_update_bits(struct tcpc_device *tcpc, u8 reg,
					u8 val, u8 mask)
{
	int ret;
	u8 data;
	...
	down(&chip->io_lock);
	ret = husb311_reg_read(chip->client, reg);
	if (ret < 0) {
		up(&chip->io_lock);
		return ret;
	}
	...
	ret = husb311_reg_write(chip->client, reg, data);
	up(&chip->io_lock);
	return ret;
}
...
static int husb311_regmap_init(struct husb311_chip *chip)
{
	...
	props->rt_regmap_mode =
		RT_MULTI_BYTE | RT_IO_PASS_THROUGH | RT_DBG_SPECIAL;
	...
	chip->m_dev = rt_regmap_device_register(props, &husb311_regmap_fops,
...
static int husb311_init_alert_mask(struct tcpc_device *tcpc)
{
	...
	mask |= TCPC_V10_REG_ALERT_TX_SUCCESS |
		TCPC_V10_REG_ALERT_TX_DISCARDED | TCPC_V10_REG_ALERT_TX_FAILED |
		TCPC_V10_REG_ALERT_RX_HARD_RST | TCPC_V10_REG_ALERT_RX_STATUS |
...
static int husb311_init_power_status_mask(struct tcpc_device *tcpc)
{
	...
	return husb311_i2c_write8(tcpc, TCPC_V10_REG_POWER_STATUS_MASK, mask);
}
...
static int husb311_init_fault_mask(struct tcpc_device *tcpc)
{
	...
	return husb311_i2c_write8(tcpc, TCPC_V10_REG_FAULT_STATUS_MASK, mask);
}
...
{
	struct husb311_chip *chip =
		container_of(work, struct husb311_chip, poll_work.work);
...
static int husb311_set_clock_gating(struct tcpc_device *tcpc, bool en)
{
	...
	if (!en) {
		clk2 |= HUSB311_REG_CLK_BCLK2_EN | HUSB311_REG_CLK_BCLK_EN;
		clk3 |= HUSB311_REG_CLK_CK_24M_EN | HUSB311_REG_CLK_PCLK_EN;
	}

	if (en) {
		for (i = 0; i < 2; i++)
			ret = husb311_alert_status_clear(
				tcpc, TCPC_REG_ALERT_RX_ALL_MASK);
	}
#endif /* CONFIG_TCPC_CLOCK_GATING */
	return ret;
}

static inline int husb311_init_cc_params(struct tcpc_device *tcpc,
					 uint8_t cc_res)
{
	int rv = 0;
...
static int husb311_tcpc_init(struct tcpc_device *tcpc, bool sw_reset)
{
	...
	if (chip->chip_id != HUSB311_DID) {
		husb311_i2c_write8(tcpc, HUSB311_REG_I2CRST_CTRL,
	...
	husb311_i2c_write16(tcpc, HUSB311_REG_DRP_DUTY_CTRL,
			TCPC_NORMAL_RP_DUTY);
...
static int husb311_get_power_status(struct tcpc_device *tcpc,
				uint16_t *pwr_status)
{
	int ret;
...
static int husb311_set_polarity(struct tcpc_device *tcpc, int polarity)
{
	...
	data = husb311_init_cc_params(tcpc, tcpc->typec_remote_cc[polarity]);
	if (data)
		return data;
...
static int husb311_set_low_power_mode(struct tcpc_device *tcpc, bool en,
					int pull)
{
	struct husb311_chip *chip = tcpc_get_dev_data(tcpc);
	int ret = 0;
...
int husb311_set_watchdog(struct tcpc_device *tcpc, bool en)
{
	...
	return husb311_i2c_write8(tcpc, HUSB311_REG_WATCHDOG_CTRL, data);
}
#endif /* CONFIG_TCPC_WATCHDOG_EN */
...
static int husb311_tcpc_deinit(struct tcpc_device *tcpc)
{
	...
	if (chip->chip_id == HUSB311_DID) {
		...
		husb311_write_word(chip->client, TCPC_V10_REG_ALERT_MASK,
					0x0); //0x12 0x13
		husb311_i2c_write8(tcpc, TCPC_V10_REG_POWER_STATUS_MASK,
					0x0); //0x14
	...
	husb311_i2c_write8(tcpc, HUSB311_REG_INTRST_CTRL,
...
#if IS_ENABLED(CONFIG_USB_POWER_DELIVERY)
static int husb311_set_msg_header(struct tcpc_device *tcpc, uint8_t power_role,
				uint8_t data_role)
{
	uint8_t msg_hdr = TCPC_V10_REG_MSG_HDR_INFO_SET(data_role, power_role);

	return husb311_i2c_write8(tcpc, TCPC_V10_REG_MSG_HDR_INFO, msg_hdr);
}
				...
				enum tcpm_transmit_type *frame_type)
{
	...
	rv = husb311_block_read(chip->client, TCPC_V10_REG_RX_BYTE_CNT, 4, buf);
	cnt = buf[0];
	type = buf[1];
...
static int husb311_set_bist_carrier_mode(struct tcpc_device *tcpc,
					uint8_t pattern)
{
			...
			const uint32_t *data)
{
	...
	if (type < TCPC_TX_HARD_RESET) {
		...
		rv = husb311_block_write(chip->client, TCPC_V10_REG_TX_BYTE_CNT,
...
static int rt_parse_dt(struct husb311_chip *chip, struct device *dev)
{
	...
	pr_info("%s\n", __func__);
	np = of_find_node_by_name(NULL, "husb311_type_c_port0");
	if (!np) {
		pr_info("%s find node usb_type_c fail\n", __func__);
		return -ENODEV;
	}
	dev->of_node = np;
	...
	chip->irq_gpio = ret;
#else
	ret = of_property_read_u32(np, "husb311pd,intr_gpio_num",
				&chip->irq_gpio);
	if (ret < 0)
...
static int husb311_tcpcdev_init(struct husb311_chip *chip, struct device *dev)
{
	...
	chip->tcpc->tcpc_flags =
		TCPC_FLAGS_LPM_WAKEUP_WATCHDOG | TCPC_FLAGS_VCONN_SAFE5V_ONLY;
...
static int husb311_i2c_suspend(struct device *dev)
{
	...
	if (client) {
		...
		if (chip) {
			down(&chip->suspend_lock);
			tcpc = chip->tcpc;
			dev_info(dev, "%s: cc1 = %d\n", __func__, typec_get_cc1());
			if (typec_is_drp_toggling()) {
				dev_info(dev, "%s: drp is toggling\n", __func__);
				/* mask cc alert */
				husb311_i2c_update_bits(tcpc, TCPC_V10_REG_ALERT_MASK,
							0x00, TCPC_V10_REG_ALERT_CC_STATUS);
				/* debug only */
				ret = husb311_i2c_read8(tcpc, TCPC_V10_REG_ALERT_MASK);
				dev_info(dev, "%s: reg0x12 = 0x%02x\n", __func__, ret);
				/* switch cc to Rd, Rd */
				tcpci_set_cc(tcpc, TYPEC_CC_RD);
				mdelay(1);
				/* clear cc alert event */
				husb311_i2c_write8(tcpc, TCPC_V10_REG_ALERT,
...
static int husb311_i2c_resume(struct device *dev)
{
	...
	if (client) {
		...
		if (chip) {
			...
			ret = tcpci_get_alert_status(tcpc, &alert);
			cc_alert = !!(alert & TCPC_V10_REG_ALERT_CC_STATUS);
			dev_info(dev, "%s: (cc1, cc2, alert, unattached) = (%d, %d, %d, %d)\n",
				__func__, tcpc->typec_remote_cc[0],
				tcpc->typec_remote_cc[1], cc_alert,
				tcpci_typec_is_unattached(tcpc));
			if (typec_is_cc_open() && tcpc->typec_role >= TYPEC_ROLE_DRP &&
				tcpci_typec_is_unattached(tcpc) && !cc_alert &&
				tcpc->typec_state != typec_disabled) {
				/* mask cc alert */
				husb311_i2c_update_bits(tcpc, TCPC_V10_REG_ALERT_MASK,
					0x00, TCPC_V10_REG_ALERT_CC_STATUS);
...
#endif /* #if IS_ENABLED(CONFIG_PM_RUNTIME) */

static const struct dev_pm_ops husb311_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(husb311_i2c_suspend, husb311_i2c_resume)
#if IS_ENABLED(CONFIG_PM_RUNTIME)
	SET_RUNTIME_PM_OPS(husb311_pm_suspend_runtime,
			husb311_pm_resume_runtime, NULL)
#endif /* #if IS_ENABLED(CONFIG_PM_RUNTIME) */
};
