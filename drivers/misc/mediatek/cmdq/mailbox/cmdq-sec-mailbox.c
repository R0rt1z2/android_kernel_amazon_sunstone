...
static inline void cmdq_mmp_init(struct cmdq_sec *cmdq)
{
	...
	if (len < 0)
		cmdq_log("failed to snprintf(name with cmdq_hwid:%d)", cmdq->hwid);
...
{
	s32 usage = atomic_read(&cmdq->usage), err = clk_enable(cmdq->clock_d);

	if (err) {
		cmdq_err("clk_enable gce-d failed:%d usage:%d", err, usage);
		return err;
	}

	err = clk_enable(cmdq->clock);
	if (err) {
		cmdq_err("clk_enable gce failed:%d usage:%d", err, usage);
		return err;
	}
...
s32 cmdq_sec_insert_backup_cookie(struct cmdq_pkt *pkt)
{
	...
	u16 sec_eof_event = cmdq_util_get_platform_event_id(CMDQ_TOKEN_SECURE_THR_EOF);
	s32 err;
	...
	if (err)
		return err;
	return cmdq_pkt_set_event(pkt, sec_eof_event);
}
...
static s32 cmdq_sec_irq_notify_start(struct cmdq_sec *cmdq)
{
	...
	cmdq_pkt_wfe(cmdq->clt_pkt, sec_eof_event);
	cmdq_pkt_finalize_loop(cmdq->clt_pkt);
	cmdq_clear_event(cmdq->clt->chan, sec_eof_event);
	...
	struct cmdq_sec_task *task, u32 thrd_idx)
{
	...
	if (iwc_msg == NULL)
		return -EINVAL;
#if IS_ENABLED(CONFIG_MTK_CAM_SECURITY_SUPPORT)
	if (iwc_msg_ex1 == NULL || iwc_msg_ex2 == NULL)
		return -EINVAL;
#endif
	...
	if (sizeof(iwc_msg->command.pVABase)
			< task->pkt->cmd_buf_size + 4 * CMDQ_INST_SIZE) {
		cmdq_err("task:%p size:%zu > iwc_msg buffer size %zu",
			task,
			task->pkt->cmd_buf_size,
			sizeof(iwc_msg->command.pVABase));
		return -EFAULT;
	}
	...
	if (data->addrMetadataCount) {
		...
		/* CMDQ_SEC_PRIMARY_MEMOUT is frame trigger mode */
		if (task->scenario == CMDQ_SEC_USER_MDP ||
		    task->scenario == CMDQ_SEC_PRIMARY_MEMOUT)
			iwc_msg->command.metadata.secMode =
				CMDQ_IWC_MDP_USER_MODE;
#endif
	}
	...
	struct cmdq_sec *cmdq, bool mtee)
{
	...
	case CMD_CMDQ_TL_PATH_RES_ALLOCATE:
		...
		iwc_msg->pathResource.shareMemoryPA =
			cmdq->shared_mem->pa;
	...
	u64 **inst, const char **dispatch)
{
	...
	*inst = (u64 *)cancel->errInstr;
	...
	case -CMDQ_ERR_ADDR_CONVERT_ALLOC_MVA_N2S:
		switch (thread_id) {
		case CMDQ_THREAD_SEC_PRIMARY_MEMOUT:
		case CMDQ_THREAD_SEC_PRIMARY_DISP:
		case CMDQ_THREAD_SEC_SUB_DISP:
			*dispatch = "DISP";
			break;
		case CMDQ_THREAD_SEC_MDP_0:
		case CMDQ_THREAD_SEC_MDP_1:
			*dispatch = "MDP";
			break;
...
static int cmdq_sec_probe(struct platform_device *pdev)
{
	...
	cmdq->clock_d = devm_clk_get(&pdev->dev, "gce-d");
	if (IS_ERR(cmdq->clock_d)) {
		cmdq_err("gce-d devm_clk_get failed:%ld", PTR_ERR(cmdq->clock_d));
		return PTR_ERR(cmdq->clock_d);
	}
	...
	if (!cmdq->context) {
		context = kzalloc(sizeof(*cmdq->context),
			GFP_ATOMIC);
		if (!context)
			return -CMDQ_ERR_NULL_SEC_CTX_HANDLE;
		...
		err = cmdq_sec_session_init(cmdq->context);
		if (err) {
			cmdq_err("cmdq_sec_session_init failed:%d", err);
			return -CMDQ_ERR_SEC_CTX_SETUP;
		}
	}
...
static int cmdq_sec_remove(struct platform_device *pdev)
{
	...
	if (!cmdq) {
		cmdq_err("release iwc_msg failed, cmdq is NULL");
		return -CMDQ_ERR_NULL_SEC_CTX_HANDLE;
	}
	context = cmdq->context;
#ifdef CMDQ_GP_SUPPORT
	if (context->iwc_msg)
		TEEC_ReleaseSharedMemory(&context->tee.shared_mem[CMDQ_IWC_MSG]);
#if IS_ENABLED(CONFIG_MTK_CAM_SECURITY_SUPPORT)
	if (!context->iwc_ex1)
		TEEC_ReleaseSharedMemory(&context->tee.shared_mem[CMDQ_IWC_MSG1]);
	if (!context->iwc_ex2)
		TEEC_ReleaseSharedMemory(&context->tee.shared_mem[CMDQ_IWC_MSG2]);
#endif
#endif
