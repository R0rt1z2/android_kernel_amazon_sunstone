...
struct test_node {
	...
	void __iomem	*va;
	phys_addr_t	pa;
	struct clk_bulk_data	clks[CMDQ_GCE_CLK_NUM_MAX];
};
...
static void cmdq_test_mbox_err_dump(struct cmdq_test *test, const bool sec)
{
	...
	if (clk_bulk_prepare_enable(CMDQ_GCE_CLK_NUM_MAX, test->gce.clks))
		return;
	...
	clk_bulk_disable_unprepare(CMDQ_GCE_CLK_NUM_MAX, test->gce.clks);
...
static void cmdq_test_mbox_gpr_sleep(struct cmdq_test *test, const bool sleep)
{
	...
	u64			cpu_time;
	const u16		event =
		cmdq_util_get_platform_event_id(CMDQ_EVENT_GPR_TIMER) + CMDQ_GPR_DEBUG_TIMER;
	...
	const bool aee)
{
	...
	unsigned long	va = (unsigned long)(secure ?
		...
		CMDQ_THR_SPR3(test->gce.va, thread->idx));
	unsigned long	pa = (secure ?
		CMDQ_GPR_R32(test->gce.pa, CMDQ_GPR_DEBUG_DUMMY) :
		CMDQ_THR_SPR3(test->gce.pa, thread->idx));
...
static void cmdq_test_mbox_handshake_event(struct cmdq_test *test)
{
	...
	u16 event = cmdq_util_get_platform_event_id(CMDQ_EVENT_HANDSHAKE);
	int ret;
	...
	cmdq_pkt_wfe(pkt_wait, event);
	cmdq_pkt_handshake_event(pkt_wait, event + 1);

	pkt_shake = cmdq_pkt_create(clt2);
	cmdq_pkt_handshake_event(pkt_shake, event);
	cmdq_pkt_wfe(pkt_shake, event + 1);
...
{
	struct cmdq_pkt *pkt;
	struct cmdq_util_subsys *subsys_list = NULL;
	u32 count, *va, i;
	dma_addr_t pa;
	u8 swap_reg = CMDQ_THR_SPR_IDX1;
	u32 pat_init = 0xdeaddead, pat_src = 0xbeefbeef;

	count = cmdq_util_test_get_subsys_list(&subsys_list);
	if (count <= 0 || subsys_list == NULL) {
	...
	for (i = 0; i < count; i++) {
		u32 reg = subsys_list[i].reg + subsys_list[i].test_offset;

		/* just test the subsys which has test_offset */
		if (subsys_list[i].test_offset == 0)
			continue;

		*va = pat_init;
		pkt = cmdq_pkt_create(test->clt);
		cmdq_pkt_write_value_addr(pkt, reg, pat_src, ~0);
		cmdq_pkt_mem_move(pkt, NULL, reg, pa, swap_reg);
		cmdq_pkt_flush(pkt);

		/* it will fail when HW module clk doesn't enable or not 32bits writable */
		cmdq_msg(
			"%s access reg addr:%#x val:%#x (should be:%#x), %s!",
			tag, reg, *va, pat_src,
			(*va != pat_src) ? "fail" : "success");
...
static void cmdq_test_devapc_vio(struct cmdq_test *test)
{
	...
	/* should change to the HW reg you want to check */
	u32 reg = 0x14000000;

	cmdq_msg("%s", __func__);

	pkt = cmdq_pkt_create(test->clt);
	cmdq_pkt_read(pkt, NULL, reg, CMDQ_THR_SPR_IDX3);
	ret = cmdq_pkt_flush(pkt);
	cmdq_pkt_destroy(pkt);
...
cmdq_test_trigger(struct cmdq_test *test, const s32 sec, const s32 id)
{
	...
	case 18:
		cmdq_test_mbox_prebuilt_instr(test,
			cmdq_util_get_platform_event_id(CMDQ_PREBUILT_MDP),
			cmdq_util_get_platform_event_id(CMDQ_TOKEN_PREBUILT_MDP_WAIT));
		cmdq_test_mbox_prebuilt_instr(test,
			cmdq_util_get_platform_event_id(CMDQ_PREBUILT_MML),
			cmdq_util_get_platform_event_id(CMDQ_TOKEN_PREBUILT_MML_WAIT));
		break;
	case 19:
...
static int cmdq_test_probe(struct platform_device *pdev)
{
	...
	s32					ret;
	int					i;
	static const char * const clk_names[] = { "gce", "gce-d", "gce-timer" };
	...
	for (i = 0; i < CMDQ_GCE_CLK_NUM_MAX; i++) {
		test->gce.clks[i].id = clk_names[i];
		test->gce.clks[i].clk = devm_clk_get(&np_pdev->dev, clk_names[i]);
		if (IS_ERR(test->gce.clks[i].clk)) {
			cmdq_err("devm_clk_get gce clk:%s failed:%ld",
				clk_names[i], PTR_ERR(test->gce.clks[i].clk));
			test->gce.clks[i].clk = NULL;
		}
	}
