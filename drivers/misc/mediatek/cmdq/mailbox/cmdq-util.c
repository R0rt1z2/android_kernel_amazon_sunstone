...
struct cmdq_util_helper_fp util_helper_fp = {
...
void cmdq_util_set_fp(struct cmdq_util_platform_fp *cust_cmdq_platform)
{
	...
	util_helper_fp.hw_name = cmdq_platform->util_hw_name;
	util_helper_fp.get_platform_event_id = cmdq_platform->util_get_event_id;
	util_helper_fp.event_module_dispatch = cmdq_platform->event_module_dispatch;
	util_helper_fp.thread_module_dispatch = cmdq_platform->thread_module_dispatch;
...
u32 cmdq_util_test_get_subsys_list(struct cmdq_util_subsys **subsys_out)
{
	...
	return cmdq_platform->test_get_subsys_list(subsys_out);
}
...
{
	if (!cmdq_platform) {
		cmdq_msg("%s cmdq_platform is not ready ", __func__);
		return -EPROBE_DEFER;
	}

	if (!cmdq_platform->get_backup_event_list) {
		cmdq_err("%s get_backup_event_list is NULL ", __func__);
		return -EINVAL;
	}
	return cmdq_platform->get_backup_event_list(gce_pa, list_out);
}
EXPORT_SYMBOL(cmdq_util_get_backup_event_list);

u16 cmdq_util_get_platform_event_id(u16 event_id)
{
	if (!cmdq_platform->util_get_event_id) {
		cmdq_err("%s util_get_event_id is NULL ", __func__);
		return -EINVAL;
	}
	return cmdq_platform->util_get_event_id(event_id);
}
EXPORT_SYMBOL(cmdq_util_get_platform_event_id);
...
{
	struct arm_smccc_res res;
	const u16 mod = (event - cmdq_util_get_platform_event_id(CMDQ_TOKEN_PREBUILT_MDP_WAIT)) /
...
int cmdq_util_init(void)
{
	...
	cmdq_helper_set_fp(&util_helper_fp);
