...
int cmdq_pkt_realloc_cmd_buffer(struct cmdq_pkt *pkt, size_t size)
{
	...
	while (pkt->buf_size < size) {
		status = cmdq_pkt_add_cmd_buffer(pkt);
		if (status)
			return (int)status;
	}
...
s32 cmdq_pkt_sleep(struct cmdq_pkt *pkt, u32 tick, u16 reg_gpr)
{
	...
	const u16 event = cmdq_util_get_platform_event_id(CMDQ_EVENT_GPR_TIMER) + reg_gpr;
	struct cmdq_client *cl = (struct cmdq_client *)pkt->cl;
...
{
	u16 handshake_event = cmdq_util_get_platform_event_id(CMDQ_EVENT_HANDSHAKE);
	u16 shake_bit = 1 << (event - handshake_event);

	return cmdq_pkt_assign_command(pkt, handshake_event, shake_bit);
}
...
#define txt_len 128
	...
	void *base;
	u32 gprid, val, len;
	u16 gpr_timer_event = cmdq_util_get_platform_event_id(CMDQ_EVENT_GPR_TIMER);
	...
	if (inst->arg_a >= gpr_timer_event &&
		inst->arg_a <= gpr_timer_event + CMDQ_GPR_R15) {
		base = cmdq_mbox_get_base(chan);
		gprid = inst->arg_a - gpr_timer_event;
	...
	} else if (inst->arg_a >= cmdq_util_get_platform_event_id(CMDQ_TOKEN_PREBUILT_MDP_WAIT) &&
		inst->arg_a <= cmdq_util_get_platform_event_id(CMDQ_TOKEN_DISP_VA_END)) {
		const u16 mod =
			(inst->arg_a -
			...
			(cmdq_util_get_platform_event_id(CMDQ_TOKEN_PREBUILT_MML_WAIT) -
			 cmdq_util_get_platform_event_id(CMDQ_TOKEN_PREBUILT_MDP_WAIT));
		const u16 event =
			cmdq_util_get_platform_event_id(CMDQ_TOKEN_PREBUILT_MDP_WAIT) + mod *
			(cmdq_util_get_platform_event_id(CMDQ_TOKEN_PREBUILT_MML_WAIT) -
			 cmdq_util_get_platform_event_id(CMDQ_TOKEN_PREBUILT_MDP_WAIT));
...
void cmdq_pkt_err_dump_cb(struct cmdq_cb_data data)
{
	...
	if (inst && inst->op == CMDQ_CODE_WFE) {
		...
		if (inst->arg_a >= cmdq_util_get_platform_event_id(CMDQ_TOKEN_PREBUILT_MDP_WAIT) &&
			inst->arg_a <= cmdq_util_get_platform_event_id(CMDQ_TOKEN_DISP_VA_END))
