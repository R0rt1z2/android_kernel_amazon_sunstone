	...
	const u64 base, const u32 offset, const u32 size, const u32 port)
{
	...
	if (!sec_data->addrMetadatas) {
		...
		meta = kzalloc((sizeof(*meta) * max), GFP_KERNEL);
		if (!meta)
			return -ENOMEM;
	...
	if (idx < max) {
		...
		cmdq_err("meta + idx:%d make out of bounds write max:%d",
			idx, max);
		return -ENOMEM;
	}
	return 0;
	...
	u32 count, void *meta_array)
{
	...
	if (!count || (count >= CMDQ_IWC_MAX_ADDR_LIST_LENGTH)) {
		cmdq_err("invalid secure metadata count:%d", count);
		return -EINVAL;
	}
	...
	if ((void *)data->addrMetadatas)
		kfree((void *)data->addrMetadatas);
