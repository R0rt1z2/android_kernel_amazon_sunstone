...
void cmdq_event_backup(struct cmdq *cmdq)
{
	int i;
	u32 array_size = 0;
	unsigned long flags;
	...
	array_size = cmdq->cmdq_backup_event_list->size;

	for (i = 0; i < array_size; i++) {
		if (cmdq->cmdq_backup_event_list->backup_event[i].event_id > CMDQ_EVENT_MAX)
			continue;

		writel((0x3FF & cmdq->cmdq_backup_event_list->backup_event[i].event_id),
				cmdq->base + CMDQ_SYNC_TOKEN_ID);
		cmdq->cmdq_backup_event_list->backup_event[i].value =
			readl(cmdq->base + CMDQ_SYNC_TOKEN_VAL);

		cmdq_log("[backup event] hwid:%d, event_id:%d, value:%d",
			cmdq->hwid, cmdq->cmdq_backup_event_list->backup_event[i].event_id,
			cmdq->cmdq_backup_event_list->backup_event[i].value);
	}
...
void cmdq_event_restore(struct cmdq *cmdq)
{
	int i;
	u32 array_size = 0;
	unsigned long flags;
	...
	for (i = 0; i < array_size; i++) {
		...
		if (cmdq->cmdq_backup_event_list->backup_event[i].value == 1) {
			writel(((1L<<16) | cmdq->cmdq_backup_event_list->backup_event[i].event_id),
					cmdq->base + CMDQ_SYNC_TOKEN_UPD);
		} else if (cmdq->cmdq_backup_event_list->backup_event[i].value == 0) {
			writel(cmdq->cmdq_backup_event_list->backup_event[i].event_id,
					cmdq->base + CMDQ_SYNC_TOKEN_UPD);
		}

		cmdq_log("[restore event] hwid:%d, event_id:%d, value:%d",
			cmdq->hwid, cmdq->cmdq_backup_event_list->backup_event[i].event_id,
			cmdq->cmdq_backup_event_list->backup_event[i].value);
	}
...
{
	s32 usage, err = 0, err_clk, err_clk_d, err_timer;
	unsigned long flags;
	...
	if (!usage)
		cmdq_log("%s: pa:%pa hwid:%hu count:%d PM OFF",
	...
	err_clk_d = clk_enable(cmdq->clock_d);
	if (err_clk_d < 0) {
		err = err_clk_d;
		cmdq_err("enable gce-clk-d fail:%d", err_clk_d);
	}
	err_clk = clk_enable(cmdq->clock);
	if (err_clk < 0) {
		err = err_clk;
		cmdq_err("enable gce-clk fail:%d", err_clk);
	}
	err_timer = clk_enable(cmdq->clock_timer);
	if (err_timer < 0) {
		err = err_timer;
		cmdq_err("enable gce-timer-clk fail:%d", err_timer);
	}
	if (usage <= 0 || err < 0) {
		cmdq_err("ref count error after inc:%d err:%d suspend:%s",
			usage, err, cmdq->suspended ? "true" : "false");
		if (err_clk_d >= 0)
			clk_disable(cmdq->clock_d);
		if (err_clk >= 0)
		...
		cmdq_init(cmdq);
		/* resote backup event after cmdq_init */
		cmdq_event_restore(cmdq);
	}
...
static void cmdq_clk_disable(struct cmdq *cmdq)
{
	...
	if (usage == -1)
		...
		/* backup event before gce clk shutdown */
		cmdq_event_backup(cmdq);
...
static int cmdq_probe(struct platform_device *pdev)
{
	...
	/* init backup event list for the sw token of each platform */
	err = cmdq_util_get_backup_event_list(cmdq->base_pa, &cmdq->cmdq_backup_event_list);
	if (err < 0)
		return err;
	...
	cmdq->clock_d = devm_clk_get(dev, "gce-d");
	if (IS_ERR(cmdq->clock_d)) {
		cmdq_err("failed to get gce-d clk");
		cmdq->clock_d = NULL;
	}
	...
	WARN_ON(clk_prepare(cmdq->clock) < 0);
	WARN_ON(clk_prepare(cmdq->clock_d) < 0);
	WARN_ON(clk_prepare(cmdq->clock_timer) < 0);
	...
	if (!of_parse_phandle_with_args(dev->of_node, "mboxes",
		...
		cmdq_msg("cmdq device node no mboxes setting\n");
...
static const struct gce_plat gce_plat_v6 = {
	.thread_nr = 32, .shift = 3, .mminfra = 0};
...
static const struct of_device_id cmdq_of_ids[] = {
	...
	{.compatible = "mediatek,mt8188-gce", .data = (void *)&gce_plat_v6},
	{.compatible = "mediatek,mt8188-gce-d", .data = (void *)&gce_plat_v6},
	{}
};
...
void cmdq_set_event(void *chan, u16 event_id)
{
	...
	// make sure event can be backup when clk off, force set event in backup list
	for (i = 0; i < array_size; i++) {
		if (cmdq->cmdq_backup_event_list->backup_event[i].event_id == event_id) {
			cmdq->cmdq_backup_event_list->backup_event[i].value = 1;
			break;
		}
	}
