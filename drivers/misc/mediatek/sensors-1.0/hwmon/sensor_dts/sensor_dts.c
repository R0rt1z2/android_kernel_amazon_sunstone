...
int check_direction_by_gpio(int *direction, struct device_node *node)
{
	int ret = 0;
	unsigned int gpio_pin = 0;
	int gpio_state = 0;
	int is_gpio_direction = 0;
	u32 is_direction_change = 0;

	if (node) {
		ret = of_property_read_u32(node, "is_direction_change", &is_direction_change);
		if (ret == 0)
			is_gpio_direction = is_direction_change;

		if (is_gpio_direction == 1) {
			gpio_pin = of_get_named_gpio_flags(node, "gpio_pin", 0, NULL);

			ret = gpio_is_valid(gpio_pin);
			if (!ret) {
				pr_info("Unable to read gpio_pin[%d]\n", gpio_pin);
				return -1;
			}
			gpio_direction_input(gpio_pin);
			gpio_state = gpio_get_value(gpio_pin);
			if (gpio_state)
				*direction = HVT_U2604_DIRECTION;
			else
				*direction = HVT_U2600_DIRECTION;
		}
	} else {
		pr_info("node is null pointer\n");
		return -1;
	}

	pr_debug("gpio_state:[%d]  direction: [%d]\n", gpio_state, *direction);
	return 0;
}
EXPORT_SYMBOL(check_direction_by_gpio);
...
EXPORT_SYMBOL(get_accel_dts_func);
...
EXPORT_SYMBOL(get_alsps_dts_func);
