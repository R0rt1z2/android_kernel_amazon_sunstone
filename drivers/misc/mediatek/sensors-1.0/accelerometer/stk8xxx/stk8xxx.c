...
static ssize_t orientation_show(struct device_driver *ptDevDrv, char *pbBuf)
{
	ssize_t _tLength = 0;
	struct stk8xxx_data *stk = gStk;
	struct acc_hw *_ptAccelHw = &stk->hw;

	STK_ACC_LOG("[%s] default direction: %d\n", __func__, _ptAccelHw->direction);
					...
					const char *pbBuf, size_t tCount)
{
	int _nDirection = 0;
	int ret = 0;
	struct stk8xxx_data *_pt_i2c_obj = gStk;
	...
	if (ret != 0) {
		if (hwmsen_get_convert(_nDirection, &_pt_i2c_obj->cvt))
			STK_ACC_LOG("ERR: fail to set direction\n");
	}

	STK_ACC_LOG("[%s] set direction: %d\n", __func__, _nDirection);
