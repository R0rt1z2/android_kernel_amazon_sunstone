...
#if IS_ENABLED(CONFIG_CUSTOM_KERNEL_GYROSCOPE)
	if (gyro_probe())
		pr_info("failed to register gyro driver\n");
#endif

#if IS_ENABLED(CONFIG_CUSTOM_KERNEL_MAGNETOMETER)
	if (mag_probe())
		pr_info("failed to register mag driver\n");
#endif

#if IS_ENABLED(CONFIG_CUSTOM_KERNEL_ALSPS)
	if (alsps_probe())
		pr_info("failed to register alsps driver\n");
#endif

#if IS_ENABLED(CONFIG_CUSTOM_KERNEL_BAROMETER)
	if (baro_probe())
		pr_info("failed to register baro driver\n");
#endif

#if IS_ENABLED(CONFIG_CUSTOM_KERNEL_STEP_COUNTER)
	if (step_c_probe())
		pr_info("failed to register step_c driver\n");
#endif

#if IS_ENABLED(CONFIG_CUSTOM_KERNEL_SITUATION)
	if (situation_probe()) {
		pr_info("failed to register situ driver\n");
		return -ENODEV;
	}
#endif
