...
static void als_work_func(struct work_struct *work)
{
	...
	if (cxt->als_data.get_data == NULL) {
		pr_info("alsps driver not register data path\n");
		return;
	}

	time.tv_sec = time.tv_nsec = 0;
	ktime_get_coarse_ts64(&time);
	nt = time.tv_sec * 1000000000LL + time.tv_nsec;
	/* add wake lock to make sure data can be read before system suspend */
	err = cxt->als_data.get_data(&value, &status);
	if (err) {
		pr_info("get alsps data fails!!\n");
		goto als_loop;
	} else {
...
static struct alsps_context *alsps_context_alloc_object(void)
{
	...
	pr_debug("%s start\n", __func__);
	if (!obj) {
		pr_info("Alloc alsps object error!\n");
		return NULL;
	}
...
static int als_enable_and_batch(void)
{
	...
	if (cxt->als_power == 1 && cxt->als_enable == 0) {
		...
		if (err) {
			pr_info("als turn off als_power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->als_power == 0 && cxt->als_enable == 1) {
		...
		if (err) {
			pr_info("als turn on als_power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->als_power == 1 && cxt->als_delay_ns >= 0) {
		...
		if (err) {
			pr_info("als set batch(ODR) err %d\n", err);
			return -1;
		}
				...
				size_t count)
{
	...
	if (handle == ID_LIGHT) {
		if (en) {
			...
			pr_info("alsps_store_active error !!\n");
			err = -1;
			goto err_out;
		}
#if IS_ENABLED(CONFIG_NANOHUB) && IS_ENABLED(CONFIG_MTK_ALSPSHUB)
		if (cxt->als_enable) {
			...
			if (err) {
				pr_info("als turn on err = %d\n", err);
				goto err_out;
			}
		} else {
			err = cxt->als_ctl.enable_nodata(cxt->als_enable);
			if (err) {
				pr_info("als turn off err = %d\n", err);
				goto err_out;
			}
		}
#else
		...
		if (cxt->rgbw_enable) {
			...
			if (err) {
				pr_info("rgbw turn on err = %d\n", err);
				goto err_out;
			}
		} else {
			err = cxt->als_ctl.rgbw_enable(cxt->rgbw_enable);
			if (err) {
				pr_info("rgbw turn off err = %d\n", err);
				goto err_out;
			}
		}
#endif
			       ...
			       size_t count)
{
	...
	if (handle == ID_LIGHT) {
		if (cxt->als_ctl.flush != NULL)
			...
			pr_info("DON'T SUPPORT ALS COMMON VERSION FLUSH\n");
		if (err < 0)
			pr_info("als enable flush err %d\n", err);
	} else if (handle == ID_RGBW) {
		if (cxt->als_ctl.rgbw_flush != NULL)
			err = cxt->als_ctl.rgbw_flush();
		else
			pr_info("DON'T SUPPORT RGB COMMON VERSION FLUSH\n");
		if (err < 0)
			pr_info("rgbw enable flush err %d\n", err);
	}
	mutex_unlock(&alsps_context_obj->alsps_op_mutex);
	if (err)
		return err;
 ...
 */
		...
		if (err) {
			pr_info("ps turn off ps_power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->ps_power == 0 && cxt->ps_enable == 1) {
		...
		if (err) {
			pr_info("ps turn on ps_power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->ps_power == 1 && cxt->ps_delay_ns >= 0) {
		...
		if (err) {
			pr_info("ps set batch(ODR) err %d\n", err);
			return -1;
		}
...
static int als_misc_init(struct alsps_context *cxt)
{
	...
	if (err)
		pr_info("unable to register alsps misc device!!\n");
...
int als_register_control_path(struct als_control_path *ctl)
{
	...
	err = sysfs_create_group(&alsps_context_obj->als_mdev.this_device->kobj,
				 &als_attribute_group);
	if (err < 0) {
		pr_info("unable to create alsps attribute file\n");
		return -3;
	}
...
int alsps_probe(void)
{
	...
	if (err) {
		pr_info("alsps real driver init fail\n");
