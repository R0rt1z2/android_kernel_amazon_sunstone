...
static int ltr578_als_read(struct i2c_client *client, u16 *data, bool is_calibration)
{
	...
	if (ratio <= 112) {
		...
		cal_factor = ltr578_obj->cal_factor[0];
	} else if (ratio <= 142) {
		/* D6500, LED4500, LED6500 */
		cal_factor = ltr578_obj->cal_factor[1];
	} else if (ratio <= 300) {
		/* LED2700 */
		cal_factor = ltr578_obj->cal_factor[2];
	} else {
		/* H2700 */
		cal_factor = ltr578_obj->cal_factor[3];
	}
...
static void get_ltr_dts_func(struct device_node *node)
{
	if (als_cal != 0) {
		...
		ltr578_obj->als_cal_high_reading = DEF_HIGH_LUX_THRESHOLD * ENLARGE_TEN_TIMES;
		ltr578_obj->als_cal_low_reading = DEF_LOW_LUX_THRESHOLD * ENLARGE_TEN_TIMES;
	}
				...
				const struct i2c_device_id *id)
{
	...
	err = of_property_read_u32_array(client->dev.of_node, "cal_factoy", obj->cal_factor, RATIO_RANGE);
	if (err) {
		APS_ERR("get cal_factoy info from dts failed\n");
		return -EFAULT;
	}
	ltr578_obj = obj;
