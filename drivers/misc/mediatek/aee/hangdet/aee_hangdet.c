				...
				unsigned int original_kicker)
{
	...
	if (aee_dump_timer_t && ((sched_clock() - aee_dump_timer_t)
		...
		if (!aee_dump_timer_c) {
			aee_dump_timer_c = 1;
			snprintf(msg_buf, WK_MAX_MSG_SIZE, "wdtk-et %s %d cpu=%d o_k=%u\n",
		...
		snprintf(msg_buf, WK_MAX_MSG_SIZE,
			"all wdtk was already stopped cpu=%d o_k=%u\n",
			cpu, original_kicker);
	...
	snprintf(msg_buf, WK_MAX_MSG_SIZE,
	 "[wdk-c] cpu=%d o_k=%u lbit=0x%x cbit=0x%x,%x,%d,%d,%lld,%x,%ld,%ld,%ld,%ld,[%lld,%ld] %d\n",
	 cpu,
	 original_kicker,
	 local_bit,
	 get_check_bit(),
	 ...
	 lasthpg_act,
	 lasthpg_t,
	 atomic_read(&plug_mask),
	 lastsuspend_t / 1000000,
	 lastsuspend_syst / 1000000,
	 lastresume_t / 1000000,
	 lastresume_syst / 1000000,
	 wk_tsk_kick_time[cpu],
	 curInterval,
	 r_counter);
	...
	if (dump_timeout) {
		...
		if (dump_timeout == 2) {
			pm_system_wakeup();
			pr_info("[wdk] pm_system_wakeup,dump_timeout = %d\n", dump_timeout);
		}
		else {
			spin_lock_bh(&lock);
			if (g_hang_detected && !timer_pending(&aee_dump_timer)) {
				pm_system_wakeup();
...
static int start_kicker(void)
{
	...
	g_kicker_init = 1;
	pr_info("[wdk] WDT start kicker done CPU_NR=%d online cpu NR%u\n",
...
static int __init hangdet_init(void)
{
	...
	for_each_matching_node(np_toprgu, toprgu_of_match) {
		pr_err("%s: compatible node found: %s\n",
	...
	if (systimer_base) {
		...
		aee_rr_rec_wdk_systimer_cnt(cnt);
#endif
		pr_info("%s systimer_cnt %llu\n", __func__, cnt);

		cnt = sched_clock();
#if IS_ENABLED(CONFIG_MTK_AEE_IPANIC)
		aee_rr_rec_wdk_ktime(cnt);
#endif
		pr_info("%s set wdk_ktime %llu\n", __func__, cnt);
	}
