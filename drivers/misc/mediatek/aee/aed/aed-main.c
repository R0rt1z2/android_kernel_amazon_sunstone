...
static int aed_proc_done(void)
{
	...
	remove_proc_entry("last_atfmsg", NULL);
	remove_proc_entry("last_debug_info", NULL);
...
#define KEDUMP_BUFFER_RESERVED_COMPATIBLE "mediatek,kedump_buffer"
#define KEDUMP_BUFFER_SIG 0x504D55444B4C  //"LKDUMP"
...
enum {
	IPANIC_LAST_SSPM_LOG = 0,
	IPANIC_SSPM_DATA,
	IPANIC_LAST_ATF_LOG ,
	IPANIC_LAST_SPM_DATA,
	IPANIC_LAST_PC,
	IPANIC_PLAT_DEBUG_NUM,
};

static const char *ipanic_names[IPANIC_PLAT_DEBUG_NUM] = {
	"SYS_SSPM_LAST_LOG",
	"SYS_SSPM_DATA",
	"TFA_DEBUG_BUF",
	"SYS_LAST_SPM_DATA",
...
struct kedump_buffer_info {
	phys_addr_t kedump_buffer_paddr;
	u32 kedump_buffer_size;
	void __iomem *kedump_buffer_vaddr;
};
static struct kedump_buffer_info info;
static int ipanic_header_idx;
...
static int lookup_atf_log(unsigned char *atf_buffer, struct atf_log *data)
{
	...
	if (debug_buf_table_p->hdr_size != sizeof(struct debug_buf_table_header)) {
		pr_info("TFA buffer data error\n");
		return -1;
	}

	debug_buf_hdr_offset = debug_buf_table_p->hdr_size +(debug_buf_table_p->debug_buf_entry_size *
	...
	data->buffer = (void *)((char *)debug_buf_hdr_p + sizeof(struct debug_buf_hdr));
	data->size = strlen(data->buffer);
	if (data->size > debug_buf_hdr_p->debug_buf_size) {
		pr_info("TFA buffer data size too large\n");
		data->size = 0;
	}
...
static u32 kedump_get_data_info(struct kedump_reserved_buffer *buffer, int index, struct ipanic_data *data)
{
	...
	if (header->valid != 1) {
		pr_notice("ipanic_header(%d) invalid: 0x%x\n", index, header->valid);
		return 0;
	}
	...
	if (strncmp(header->name, "TFA_DEBUG_BUF", len) == 0) {
		...
		if (lookup_atf_log(data->buffer, &atf_log_data) == 0) {
			...
			data->size = atf_log_data.size;
		} else {
			pr_warn("lookup_atf_log failed\n");
		}
	}
	ipanic_header_idx++;
	return data->size;
...
static int proc_last_atfmsg_show(struct seq_file *m, void *v)
{
	int i;
	struct ipanic_data data;
	struct kedump_reserved_buffer *buffer = (struct kedump_reserved_buffer *)info.kedump_buffer_vaddr;
	ipanic_header_idx = 0;

	if (buffer->sig != KEDUMP_BUFFER_SIG) {
		seq_printf(m, "kedump_buffer: sig(0x%llx) mismatch, the buffer may be corrupted!\n", buffer->sig);
		return -EINVAL;
	}

	for (i = 0; i < IPANIC_PLAT_DEBUG_NUM && ipanic_names[i] != NULL; i++) {
		memset(&data, 0, sizeof(data));
		if (kedump_get_data_info(buffer, i, &data) > 0) {
			seq_printf(m, "\n********%s********\n", ipanic_names[i]);
			if((u64)data.buffer + data.size <= (u64)buffer + info.kedump_buffer_size){
				seq_write(m, data.buffer, data.size);
			} else {
				pr_warn("i = %d, seq_write error\n",i);
			}
		}
	}
...
static int proc_last_atfmsg_file_open(struct inode *inode,
		struct file *file)
{
	return single_open(file, proc_last_atfmsg_show, inode->i_private);
}

static const struct proc_ops proc_last_atfmsg_file_ops = {
	.proc_open = proc_last_atfmsg_file_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
...
static int kedump_buffer_show(struct seq_file *m, void *v)
{
	...
	seq_write(m, buffer, info.kedump_buffer_size);
...
static const struct proc_ops proc_kedump_buffer_file_ops = {
	.proc_open = kedump_buffer_file_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
{
	struct device_node *kedump_buffer_node;
	struct reserved_mem *rmem;

	kedump_buffer_node = of_find_compatible_node(NULL, NULL,
			KEDUMP_BUFFER_RESERVED_COMPATIBLE);
	if (!kedump_buffer_node) {
		pr_notice("compatible:%s not found\n", KEDUMP_BUFFER_RESERVED_COMPATIBLE);
		return -EINVAL;
	}

	rmem = of_reserved_mem_lookup(kedump_buffer_node);
	if (!rmem) {
		pr_notice("%s reserved mem not found\n", KEDUMP_BUFFER_RESERVED_COMPATIBLE);
		return -EINVAL;
	}

	pr_info("[kedump-buffer-reserved] phys:0x%llx - 0x%llx (0x%llx)\n",
			(u64)rmem->base,
			(u64)rmem->base + (unsigned long long)rmem->size,
			(u64)rmem->size);
	info.kedump_buffer_paddr = rmem->base;
	info.kedump_buffer_size = rmem->size;
	info.kedump_buffer_vaddr = remap_lowmem(info.kedump_buffer_paddr, info.kedump_buffer_size);
	if (IS_ERR(info.kedump_buffer_vaddr)) {
		pr_notice("Fail to remap kedump-buffer vaddr: 0x%llx\n", PTR_ERR(info.kedump_buffer_vaddr));
		return -ENOMEM;
	}
	pr_info("kedump-buffer vaddr: 0x%llx\n", (u64)info.kedump_buffer_vaddr);
...
static int __init aed_init(void)
{
	...
	struct proc_dir_entry *last_debug_info_dir, *last_atfmsg_dir;

	err = lookup_reserved_memory();
	if (err) {
		pr_notice("Fail to look up kedump-buffer reserved memory:%d\n", err);
	} else {
		last_debug_info_dir = proc_create("last_debug_info", 0440, NULL, &proc_kedump_buffer_file_ops);
		if (!last_debug_info_dir)
			pr_notice("%s: Can't create last_debug_info proc entry\n", __func__);

		last_atfmsg_dir = proc_create("last_atfmsg", 0440, NULL, &proc_last_atfmsg_file_ops);
		if (!last_atfmsg_dir)
			pr_notice("%s: Can't create last_atfmsg proc entry\n", __func__);
	}
