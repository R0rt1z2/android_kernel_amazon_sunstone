...
extern KREE_SESSION_HANDLE sys_session;
#endif
...
/**
 * secure (chunk) memory type
 *
 * enum kree_mem_type: define the enum types for secure memory.
 *
...
/**
 * secure chunk memory extended allocation
 *
 * Similar with KREE_AllocSecuremem() but the type is an additional parameters.
 *
 * @param session    The session handle.
 * @param cm_handle    [out] A pointer to secure chunk memory handle.
 * @param type    Type of secure chunk memory, it enumerates in kree_mem_type.
 * @param alignment    Memory alignment in bytes.
 * @param size    The size of the buffer to be allocated in bytes.
...
int KREE_AllocSecurechunkmemEx(KREE_SESSION_HANDLE session,
	KREE_SECURECM_HANDLE *cm_handle, enum kree_mem_type type,
	uint32_t alignment, uint32_t size);

/**
 * Secure chunk memory extended allocation with tag
 *
 * Same as KREE_AllocSecuremem() but with one additional tag for debugging.
 *
 * @param session    The session handle.
...
/**
 * Zeroed secure chunk memory extended allocation with tag
 *
 * Same as KREE_AllocSecurememWithTag() but the context is initilaized as zero.
 *
 * @param session    The session handle.
