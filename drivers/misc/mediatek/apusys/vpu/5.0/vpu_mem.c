	...
	uint32_t given_addr)
{
	...
	if (given_addr) {
		...
		if (!is_au_align(ab, start)) {
			pr_info("%s: %s: size: 0x%x, given addr: 0x%x, start 0x%lx is un-aligned to AU 0x%x\n",
	...
	if (offset >= ab->nbits) {
		pr_info("%s: %s: Out of memory: size: 0x%x, given addr: 0x%x, offset: %lu, nbits: %lu\n",
...
void apu_bmap_free(struct apu_bmap *ab, uint32_t addr, unsigned int size)
{
	...
	if (!is_au_align(ab, offset)) {
		pr_info("%s: %s: addr 0x%x, offset 0x%lx is un-aligned to AU 0x%x\n",
	...
	offset = offset / ab->au;
	if (offset >= ab->nbits) {
		pr_info("%s: %s: addr 0x%x, offset-bit 0x%lx is out of limit %lu\n",
	...
	unsigned int nents, size_t len, dma_addr_t given_iova)
{
	...
	if (domain == NULL) {
		dev_info(&pdev->dev, "%s: iommu_map_sg fail because domain is NULL\n", __func__);
		goto err;
	}
