...
//#define vpu_drv_debug(format, args...)  dprintf(INFO, MODULE_NAME format, ##args)
#define LOG_CRI(format, args...)  pr_info(MODULE_NAME format, ##args)
//#define LOG_INF(format, args...)  pr_info(MODULE_NAME format, ##args);
...
static int set_property(struct vpu_device *vpu_device, struct prop_params *prop)
{
	...
	vpu_drv_debug("algo: addr: %08xh, size: %08xh, bin: %08xh\n",
		vpu_drv->iova_algo.addr, vpu_drv->iova_algo.size, vpu_drv->iova_algo.bin);

	i = vpu_device->id;
	vpu_iram_data[i][2] = prop->alg_offset + prop->total_alg_sz + (i * IRAM_SEG_SZ);
	...
	vpu_drv_debug("iram: addr: %08xh, size: %08xh, bin: %08xh\n",
		vpu_device->iova_iram.addr, vpu_device->iova_iram.size, vpu_device->iova_iram.bin);
...
static unsigned long long get_addr(struct addr_param *param)
{
	...
	if (is_dsp_addr(addr, CORE0, MAIN_PROG)) {
		...
		*iram_num = (param->iram_num + 1);
		...
		LOG_CRI("invalid segment addr 0x%llx\n", addr);
		return 0;
	}
...
static int install_main(struct install_params *install)
{
	...
	for (j = 0; j < part_hdr->segment_num; j++) {
		...
		if (param.iram_segment) {
			vpu_drv_debug("seg[%2d] [0x%05x:0x%05x] -> [0x%x/0x%llx]: iram[0x%x:%d]\n",
				j, offset, read_size, seg->pAddr, dst,
				seg->vpu_core, param.iram_num);
		} else {
			vpu_drv_debug("seg[%2d] [0x%05x:0x%05x] -> [0x%x/0x%llx]\n",
				j, offset, read_size, seg->pAddr, dst);
		}
...
static int install_preload(struct install_params *install)
{
	...
	for (j = 0; j < part_hdr->pre_num; j++) {
		...
		if (param.iram_segment) {
			vpu_drv_debug("pre[%2d] [0x%05x:0x%06x] -> [0x%x/0x%llx]: iram[0x%x:%d]\n",
				 j, offset, read_size, pre->pAddr, dst,
				 pre->vpu_core, param.iram_num);
		} else {
			if (pre->pAddr == 0xFFFFFFFF) {
				param.iram_hdr = dst;
				vpu_drv_debug("pre[%2d] [0x%05x:0x%06x] -> [0x%x/0x%llx]: iram[0x%x] head\n",
					 j, offset, pre->file_sz, pre->pAddr,
					 dst, pre->vpu_core);
			} else {
				param.iram_hdr = 0;
				vpu_drv_debug("pre[%2d] [0x%05x:0x%06x] -> [0x%x/0x%llx]\n",
					j, offset, read_size, pre->pAddr, dst);
			}
		}
...
static int install_algo(struct install_params *install)
{
	...
	for (j = 0; j < part_hdr->alg_num; j++) {
		...
		vpu_drv_debug("alg[%2d] [0x%06x:0x%06x] -> [0x%llx] => 0x%x\n",
...
void *read_img(struct vpu_device *vpu_device)
{
	...
	struct img_info *info = kzalloc(total_num * sizeof(struct img_info), GFP_KERNEL);
	//unsigned int img_buf = (unsigned int)SCRATCH_ADDR +
	//			SCRATCH_SIZE - DEF_RSRV_SIZE;
	//start from tail to avoid polluting kernel zimage
	//void *part = (void *)img_buf;

	if (!info) {
		LOG_CRI("kzalloc fail\n");
		return NULL;
	}
	...
	for (i = 0; i < total_num; i++) {
		...
		len = request_firmware_direct(&(vpu_drv->fw[i]), g_vpu_image_name[i],
			vpu_device->dev);
		if (len < 0) {
		...
		vpu_drv_debug("total preload mem size: 0x%x\n", head->mem_size);
		hdr_total += head->hdr_size;
		pre_cnt += head->pre_num;
		//part += max;

		// cam_vpu1.img carries only main program
		// & preload and does not carry normal libraries
		if (i)
...
int vpu_lk(struct vpu_device *vpu_device, struct platform_device *pdev)
{
	int i, ret = 0, num;
	int *overlap = NULL;
	u32 *pIramNum;
	...
	if (vpu_device->id == 0) {
		...
		if (!np) {
			dev_info(vpu_device->dev, "No %s specified\n", "memory-region");
			goto exit;
			}
		...
		vpu_drv->bin_va = memremap(r.start, resource_size(&r), MEMREMAP_WB);
		...
		dev_info(vpu_device->dev,
			...
			(u64)vpu_drv->bin_va, vpu_drv->bin_pa, vpu_drv->bin_size);
	}
	...
	vpu_drv_debug("extra_size: 0x%llx\n", extra_size);

	mblock_size = (DSP_BOOT_SZ + IRAM_SEG_SZ) * (u64)img->dsp_num +
	...
	LOG_CRI("aligned_size is 0x%llx.\n", aligned_size);
	...
	alg_offset = (u64)img->dsp_num * DSP_BOOT_SZ;
	...
	vpu_drv_debug("lk ver: %x\n", VERSION);
	vpu_drv_debug("mblk:[0x%llx:0x%llx] | head:0x%llx | alg: 0x%llx | pre: 0x%llx\n",
	...
	*pIramNum = 0;
	...
	for (i = 0 ; i < num ; i++) {
		...
		vpu_drv_debug("%s| v:%x str:%s sz:0x%x/0x%x s:%d a:%d v:%d\n",
		...
		vpu_drv_debug("copy %s header(0x%X) to 0x%llx\n",
