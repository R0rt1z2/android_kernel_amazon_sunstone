...
void fix_dvfs_debug(void)
{
	...
	for (i = 0; i < APUSYS_BUCK_DOMAIN_NUM; i++) {
		...
		if ((buck < VPU_BUCK) || (buck >= APUSYS_BUCK_NUM))
			continue;
...
void apusys_power_debugfs_init(void)
{
	...
	apusys_power_dir = debugfs_lookup("apusys", NULL);
	if (apusys_power_dir == NULL) {
...
{
	if (is_debugfs_dir_exists == false)
		debugfs_remove(apusys_power_dir);
	else
		dput(apusys_power_dir);
}
