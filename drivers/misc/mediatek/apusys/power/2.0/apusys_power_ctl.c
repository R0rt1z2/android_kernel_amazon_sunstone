...
int32_t apusys_thermal_en_throttle_cb(unsigned int user,
...
int32_t apusys_thermal_dis_throttle_cb(unsigned int user)
{
	switch (user) {
#if IS_ENABLED(CONFIG_MTK_APUSYS_VPU)
	case VPU0:
...
uint8_t apusys_boost_value_to_opp(enum DVFS_USER user, uint8_t boost_value)
{
	...
	uint32_t max_freq = 0, min_freq = 0, freq = 0;
	unsigned int buck_domain = (unsigned int)apusys_user_to_buck_domain[user];

	if (buck_domain >= APUSYS_BUCK_DOMAIN_NUM)
		return APUSYS_DEFAULT_OPP;

	if (boost_value >= 100) {
		opp = 0;
	} else {
		max_freq = apusys_opps.opps[0][buck_domain].freq;
		min_freq = apusys_opps.opps[opp][buck_domain].freq;
		freq = min_freq + (boost_value * (max_freq - min_freq) / 100);
...
enum DVFS_FREQ apusys_opp_to_freq(unsigned int user, uint8_t opp)
{
	enum DVFS_FREQ freq = DVFS_FREQ_NOT_SUPPORT;
	unsigned int buck_domain;

	if (user >= APUSYS_DVFS_USER_NUM)
		return freq;

	buck_domain = (unsigned int)apusys_user_to_buck_domain[user];
...
int8_t apusys_get_opp(unsigned int user)
{
	unsigned int buck_domain;
...
int8_t apusys_get_ceiling_opp(unsigned int user)
{
	uint8_t used_opp;
	unsigned int buck_domain;
...
void apusys_pwr_constraint_check(void)
{
	...
	for (i = 0; i < APUSYS_DVFS_CONSTRAINT_NUM; i++) {
		...
		if ((buck0 < VPU_BUCK) || (buck0 >= APUSYS_BUCK_NUM) ||
			(buck1 < VPU_BUCK) || (buck1 >= APUSYS_BUCK_NUM))
			continue;
...
void apusys_dvfs_info(void)
{
	...
	for (domain = 0; domain < APUSYS_BUCK_DOMAIN_NUM; domain++) {
		...
		if (apusys_opps.cur_opp_index[domain] == APUSYS_INVALID_OPP) {
			cur_opp_index[domain] = APUSYS_DEFAULT_OPP;
			c_opp_index = APUSYS_DEFAULT_OPP;
		}
		...
		if (apusys_opps.next_opp_index[domain] == APUSYS_INVALID_OPP) {
			next_opp_index[domain] = APUSYS_DEFAULT_OPP;
			n_opp_index = APUSYS_DEFAULT_OPP;
		}
...
bool apusys_check_opp_change(void)
{
	...
	for (user = 0; user < APUSYS_DVFS_USER_NUM; user++) {
		...
		if (apusys_opps.user_opp_index[user] !=
			apusys_opps.driver_opp_index[user]) {
			PWR_LOG_INF("%s DVFS since opp change, user[%d]: %d -> %d\n",
					__func__, user,
					apusys_opps.driver_opp_index[user],
					apusys_opps.user_opp_index[user]);
			return true;
		...
		if (apusys_opps.is_power_on[user] != prev_is_power_on[user]) {
			PWR_LOG_INF("%s DVFS since power change, user[%d]: %d -> %d\n",
					__func__, user,
					prev_is_power_on[user],
					apusys_opps.is_power_on[user]);
			prev_is_power_on[user] = apusys_opps.is_power_on[user];
			return true;
		}

		if (apusys_opps.thermal_opp[user] != prev_thermal_opp[user]) {
			PWR_LOG_INF("%s DVFS since thermal event, user[%d]: %d -> %d\n",
					__func__, user,
					prev_thermal_opp[user],
					apusys_opps.thermal_opp[user]);
...
#ifdef APUSYS_POWER_OFF_KEEP_OPP
static void _apusys_power_on_set_default_opp(enum DVFS_USER user)
{
	unsigned int buck_domain;
	...
	if (user >= VPU0 && user < APUSYS_DVFS_USER_NUM) {
		buck_domain = (unsigned int)apusys_user_to_buck_domain[user];
		if (buck_domain >= V_VPU0 && buck_domain < APUSYS_BUCK_DOMAIN_NUM) {
...
static void _apusys_power_off_set_default_opp(enum DVFS_USER user)
{
	unsigned int buck_domain;
...
#else
static void _apusys_power_on_set_default_opp(unsigned int user)
{
	unsigned int buck_domain;
...
static void _apusys_power_off_set_default_opp(unsigned int user)
{
	unsigned int buck_domain;
...
int apusys_power_on(enum DVFS_USER user)
{
	...
	if (ret == 0) {
		_apusys_power_on_set_default_opp(user);
...
int apusys_power_off(enum DVFS_USER user)
{
	...
	if (ret == 0) {
		...
		_apusys_power_off_set_default_opp(user);
...
enum DVFS_FREQ apusys_get_dvfs_freq(unsigned int domain)
{
	uint8_t cur_opp_index = APUSYS_DEFAULT_OPP;

	if (domain >= APUSYS_BUCK_DOMAIN_NUM)
		return DVFS_FREQ_NOT_SUPPORT;

	cur_opp_index = apusys_opps.cur_opp_index[domain];
	if (cur_opp_index == APUSYS_INVALID_OPP)
		cur_opp_index = APUSYS_DEFAULT_OPP;

	return apusys_opps.opps[cur_opp_index][domain].freq;
}

enum DVFS_VOLTAGE apusys_get_dvfs_volt(unsigned int domain)
{
	uint8_t cur_opp_index = APUSYS_DEFAULT_OPP;

	if (domain >= APUSYS_BUCK_DOMAIN_NUM)
		return DVFS_VOLT_NOT_SUPPORT;

	cur_opp_index = apusys_opps.cur_opp_index[domain];
	if (cur_opp_index == APUSYS_INVALID_OPP)
		cur_opp_index = APUSYS_DEFAULT_OPP;

	return apusys_opps.opps[cur_opp_index][domain].voltage;
}
