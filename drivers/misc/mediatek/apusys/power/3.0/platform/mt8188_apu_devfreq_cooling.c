				 ...
				 enum dvfs_device_id dvfs_dev_id)
{
	...
	for (i = 0; i < table_size; i++) {
		if (of_property_read_u32_index(node, "power_table", i,
					   &apu_power_table->power[i]))
			dev_info(dev, "[%s][%d] read power_table[%d] failed!\n",
					__func__, __LINE__, i);
	}
...
static unsigned long mt8188_apu_static_power(struct devfreq *devfreq,
					     unsigned long voltage_mv)
{
	return 0;
}

static unsigned long mt8188_apu_dynamic_power(struct devfreq *devfreq,
					      ...
					      unsigned long voltage_mv)
{
	...
	if (opp >= opp_tbl->tbl_size)
		opp = opp_tbl->tbl_size - 1;
