...
#define SPM_VMM_ISO_REG_OFST 0x3EC
#define SPM_IMG_EXT_BUCK_ISO_BIT 11
#define SPM_CAM_EXT_BUCK_ISO_BIT 12
...
static void vmm_buck_isolation_off(void __iomem *base)
{
	...
	reg_buck_iso_val &= ~(1UL << SPM_IMG_EXT_BUCK_ISO_BIT);
	writel_relaxed(reg_buck_iso_val, reg_buck_iso_addr);

	reg_buck_iso_val = readl_relaxed(reg_buck_iso_addr);
	reg_buck_iso_val &= ~(1UL << SPM_CAM_EXT_BUCK_ISO_BIT);
	writel_relaxed(reg_buck_iso_val, reg_buck_iso_addr);
...
static int vmm_spm_probe(struct platform_device *pdev)
{
	...
	reg = devm_regulator_get(dev, "buck-vmm");
	if (IS_ERR(reg)) {
		ISP_LOGE("devm_regulator_get buck-vmm fail");
		return PTR_ERR(reg);
	}
