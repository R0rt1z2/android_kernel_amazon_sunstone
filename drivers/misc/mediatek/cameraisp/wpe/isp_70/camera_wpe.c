...
/* EP stage to disable these configs */
// #define CONFIG_MTK_MMDVFS NOT_DEFINED
// #define CONFIG_INTERCONNECT_MTK_MMQOS_MT8139 NOT_DEFINED
/* EP stage to disable these configs */
...
#endif
/* for MMQOS */
#if IS_ENABLED(CONFIG_INTERCONNECT_MTK_MMQOS_MT8188)
...
#define WPESYS_TOP_REG              (0x14E00000)
...
static inline unsigned long WPE_MMDVFS_Get_Lowest_Frequency(void)
{
	...
	if (!IS_ERR(opp)) {
		// speeds[i] = freq;  available freq is stored in speeds[i]
		dev_pm_opp_put(opp);
		return freq;
	} else
		return 0;
}
static inline unsigned long WPE_MMDVFS_Get_Highest_Frequency_Core(unsigned long freq)
{
	struct dev_pm_opp *opp;
	...
	if (!IS_ERR(opp)) {
		freq++;
		dev_pm_opp_put(opp);
		return WPE_MMDVFS_Get_Highest_Frequency_Core(freq);
	} else
		return freq;
}

static inline unsigned long WPE_MMDVFS_Get_Highest_Frequency(void)
{
	return WPE_MMDVFS_Get_Highest_Frequency_Core(0);
}
...
static inline int WPE_MMDVFS_Set_Frequency(unsigned long freq)
{
	...
	if (ret)
		LOG_ERR("Set voltage(%d) and frequency(%lu) fail for WPESYS", volt, freq);
...
static inline int WPE_Prepare_Enable_ccf_clock(unsigned int devno)
{
	...
	WPE_dev = &(WPE_devs[devno]);
	pm_runtime_get_sync(WPE_dev->dev);
...
#if IS_ENABLED(CONFIG_MTK_MMDVFS)
	if (devno == 0) {
		ret = WPE_MMDVFS_Set_Frequency(WPE_MMDVFS_Get_Highest_Frequency()); // only wpe_a
		if (ret) {
			LOG_ERR("cannot Set_Frequency (WPE %d, CLOCK %lu) clock",
				devno, WPE_MMDVFS_Get_Highest_Frequency());
			return ret;
		}
	}
#endif
...
static inline void WPE_Disable_Unprepare_ccf_clock(unsigned int devno)
{
...
#if IS_ENABLED(CONFIG_MTK_MMDVFS)
	if (devno == 0) {
		if (WPE_MMDVFS_Set_Frequency(WPE_MMDVFS_Get_Lowest_Frequency())) // only wpe_a
			LOG_ERR("cannot Set_Frequency (WPE %d, CLOCK %lu) clock",
				devno, WPE_MMDVFS_Get_Lowest_Frequency());
	}
#endif
#if IS_ENABLED(CONFIG_INTERCONNECT_MTK_MMQOS_MT8188)
	...
	pm_runtime_put_sync(WPE_dev->dev);
...
static signed int WPE_open(struct inode *pInode, struct file *pFile)
{
	...
	wpesys_reg = ioremap(WPESYS_TOP_REG, 0x30);
...
static inline signed int WPE_RegCharDev(void)
{
	...
	if (Ret < 0) {
		LOG_ERR("Attach file operation failed, %d", Ret);
		goto EXIT;
	}
	/*  */
EXIT:
...
static signed int WPE_probe(struct platform_device *pDev)
{
	...
	/* Device iova in 8G~12G */
	Ret = dma_set_mask_and_coherent(WPE_dev->dev, DMA_BIT_MASK(34));
	LOG_INF("set dma mask bit:34 result:%d\n", Ret);
	...
	if (nr_WPE_devs == 1) {
		...
		for (dev_i = 0 ; dev_i < WPE_DEV_NUM ; dev_i++) {
			char wpe_name[strlen(WPE_DEV_NAME) + 5] /*= {0}*/;

			sprintf(wpe_name, WPE_DEV_NAME);
			strncat(wpe_name, (dev_i == 0)?"-a":"-b", 3);
