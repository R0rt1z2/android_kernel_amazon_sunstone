...
#include <mt8188_spm_reg.h>
#include <mt8188_pwr_ctrl.h>
#include <lpm_timer.h>
#include <mtk_lpm_sysfs.h>
#include "mt8188_lpm_compat.h"

#define MT8188_LOG_DEFAULT_MS		5000
...
static char *mt8188_spm_cond_cg_str[PLAT_SPM_COND_MAX] = {
	[PLAT_SPM_COND_MTCMOS1] = "MTCMOS1",
	[PLAT_SPM_COND_MTCMOS2] = "MTCMOS2",
	...
	[PLAT_SPM_COND_CG_PERI_SW_0] = "PERI_SW_0",
	[PLAT_SPM_COND_CG_VPPSYS0_SW_CG_0] = "VPPSYS0_SW_CG_0",
	[PLAT_SPM_COND_CG_VPPSYS0_SW_CG_1] = "VPPSYS0_SW_CG_1",
	[PLAT_SPM_COND_CG_VPPSYS0_SW_CG_2] = "VPPSYS0_SW_CG_2",
	[PLAT_SPM_COND_CG_VPPSYS1_SW_CG_0] = "VPPSYS1_SW_CG_0",
	[PLAT_SPM_COND_CG_VPPSYS1_SW_CG_1] = "VPPSYS1_SW_CG_1",
	[PLAT_SPM_COND_CG_VDOSYS0_SW_CG_0] = "VDOSYS0_SW_CG_0",
	[PLAT_SPM_COND_CG_VDOSYS0_SW_CG_1] = "VDOSYS0_SW_CG_1",
	[PLAT_SPM_COND_CG_VDOSYS1_SW_CG_0] = "VDOSYS1_SW_CG_0",
	[PLAT_SPM_COND_CG_VDOSYS1_SW_CG_1] = "VDOSYS1_SW_CG_1",
	[PLAT_SPM_COND_CG_VDOSYS1_SW_CG_2] = "VDOSYS1_SW_CG_2",
	[PLAT_SPM_COND_CG_I2C_SW_CG] = "I2C_SW_CG",
};

const char *mt8188_wakesrc_str[32] = {
	[0] = "R12_PCM_TIMER",
	[1] = "R12_TWAM_IRQ_B",
	[2] = "R12_KP_IRQ_B",
	[3] = "R12_APWDT_EVENT_B",
...
static struct lpm_spm_wake_status mt8188_wakesrc;

static struct lpm_log_helper mt8188_log_help = {
	.wakesrc = &mt8188_wakesrc,
	.cur = 0,
	.prev = 0,
};
...
{
	struct lpm_log_helper *help = &mt8188_log_help;
...
static void mt8188_suspend_show_detailed_wakeup_reason(
	...
	for (i = 1 ; i < PLAT_SPM_COND_MAX ; i++) {
		...
		if (blkcg != 0)
			pr_info("suspend warning: CG: %6s = 0x%08lx\n"
				, mt8188_spm_cond_cg_str[i], blkcg);
	}
}

static void mt8188_suspend_spm_rsc_req_check(
	struct lpm_spm_wake_status *wakesta)
{
...
static int lpm_show_message(int type, const char *prefix, void *data)
{
	static const struct {
		...
		{ WAKE_MISC_DVFSRC_IRQ, WR_DVFSRC, "DVFSRC" },
		{ WAKE_MISC_TWAM_IRQ_B, WR_TWAM, "TWAM" },
		{ WAKE_MISC_PMSR_IRQ_B_SET0, WR_PMSR, "PMSR" },
		{ WAKE_MISC_PMSR_IRQ_B_SET1, WR_PMSR, "PMSR" },
		{ WAKE_MISC_SPM_ACK_CHK_WAKEUP_0, WR_SPM_ACK_CHK, "SPM_ACK_CHK" },
		{ WAKE_MISC_SPM_ACK_CHK_WAKEUP_1, WR_SPM_ACK_CHK, "SPM_ACK_CHK" },
		{ WAKE_MISC_SPM_ACK_CHK_WAKEUP_2, WR_SPM_ACK_CHK, "SPM_ACK_CHK" },
		{ WAKE_MISC_SPM_ACK_CHK_WAKEUP_3, WR_SPM_ACK_CHK, "SPM_ACK_CHK" },
		{ WAKE_MISC_SPM_ACK_CHK_WAKEUP_ALL, WR_SPM_ACK_CHK, "SPM_ACK_CHK" },
	};
	struct lpm_spm_wake_status *wakesrc = mt8188_log_help.wakesrc;
	int i;
	...
	if (wakesrc->is_abort != 0) {
		...
		for (i = 0; i < 32; i++) {
			if (wakesrc->r12 & BIT(i)) {
				ws_log_len += scnprintf(buf + ws_log_len,
						sizeof(buf) - ws_log_len,
						"%s ", mt8188_wakesrc_str[i]);
				wr = WR_WAKE_SRC;
			}
		}

		if (wakesrc->r12 & R12_PCM_TIMER &&
			wakesrc->wake_misc & WAKE_MISC_PCM_TIMER_EVENT) {
			wr = WR_PCM_TIMER;
		}

		if (wakesrc->r12 & R12_TWAM_IRQ_B) {
			for (i = 0; i < ARRAY_SIZE(wm); i++) {
				if ((wakesrc->wake_misc & wm[i].wm) == 0)
					continue;
	...
	if (type == LPM_ISSUER_SUSPEND) {
		pr_err("[name:spm&][SPM] %s", log_buf);
		mt8188_suspend_show_detailed_wakeup_reason(wakesrc);
		mt8188_suspend_spm_rsc_req_check(wakesrc);
...
static struct lpm_dbg_plat_ops mt8188_dbg_ops = {
...
int mt8188_dbg_ops_register(void)
{
	int ret;

	ret = lpm_dbg_plat_ops_register(&mt8188_dbg_ops);
