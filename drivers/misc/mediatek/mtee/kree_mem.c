...
static inline int _allocFunc(uint32_t cmd, KREE_SESSION_HANDLE session,
				uint32_t *mem_handle, uint32_t type, uint32_t alignment,
				uint32_t size, const char *dbg, const char *tag)
{
	...
	case TZCMD_MEM_SECURECM_ZALLOC_WITH_TAG:
		if (tag == NULL)
			...
			param.size = strlen(tag)+1;

		tagbuff = kzalloc(param.size, GFP_KERNEL);
		if (tagbuff == NULL)
			return TZ_RESULT_ERROR_OUT_OF_MEMORY;
		param.buffer = tagbuff;
		ret = KREE_RegisterSharedmem(session, &shh, &param);
		if (ret != TZ_RESULT_SUCCESS) {
			kfree(tagbuff);
			return ret;
		}
		if (tag != NULL)
			memcpy(tagbuff, tag, param.size);
		p[2].memref.handle = shh;
		p[2].memref.size = param.size;
		p[2].memref.offset = 0;
		...
		ret = KREE_TeeServiceCall(session, cmd,
					TZ_ParamTypes4(TZPT_VALUE_INPUT,
							...
							TZPT_VALUE_OUTPUT),
					p);
		KREE_UnregisterSharedmem(session, shh);
		kfree(tagbuff);
		handle = p[3].value.a;
		break;
	default:
				...
				uint32_t alignment, uint32_t size)
{
	...
	ret =
	    _allocFunc(TZCMD_MEM_SECUREMEM_ALLOC, session, mem_handle,
			KREE_MEM_SEC_MEM, alignment, size, __func__, NULL);
				...
				const char *tag)
{
	...
	ret =
	    _allocFunc(TZCMD_MEM_SECUREMEM_ALLOC_WITH_TAG, session, mem_handle,
			KREE_MEM_SEC_MEM, alignment, size, __func__, tag);
					...
					uint32_t size)
{
	...
	ret =
	    _allocFunc(TZCMD_MEM_SECURECM_ALLOC, session, cm_handle,
			KREE_MEM_SEC_CM_TZ, alignment, size, __func__, NULL);

	return ret;
}
EXPORT_SYMBOL(KREE_AllocSecurechunkmem);
					...
					uint32_t size, const char *tag)
{
	...
	ret =
	    _allocFunc(TZCMD_MEM_SECURECM_ALLOC_WITH_TAG, session, cm_handle,
			KREE_MEM_SEC_CM_TZ, alignment, size, __func__, tag);

	return ret;
}
EXPORT_SYMBOL(KREE_AllocSecurechunkmemWithTag);
...
EXPORT_SYMBOL(KREE_ZallocSecurechunkmemWithTag);

int KREE_AllocSecurechunkmemEx(KREE_SESSION_HANDLE session,
					KREE_SECUREMEM_HANDLE *cm_handle,
					...
					uint32_t size)
{
	...
	ret =
	    _allocFunc(TZCMD_MEM_SECURECM_ALLOC, session, cm_handle,
			type, alignment, size, __func__, NULL);

	return ret;
}
EXPORT_SYMBOL(KREE_AllocSecurechunkmemEx);

int KREE_AllocSecurechunkmemWithTagEx(KREE_SESSION_HANDLE session,
					KREE_SECUREMEM_HANDLE *cm_handle,
					enum kree_mem_type type,
					uint32_t alignment,
					uint32_t size, const char *tag)
{
	...
	ret =
	    _allocFunc(TZCMD_MEM_SECURECM_ALLOC_WITH_TAG, session, cm_handle,
			type, alignment, size, __func__, tag);

	return ret;
}
EXPORT_SYMBOL(KREE_AllocSecurechunkmemWithTagEx);

int KREE_ZallocSecurechunkmemWithTagEx(KREE_SESSION_HANDLE session,
					KREE_SECUREMEM_HANDLE *cm_handle,
					enum kree_mem_type type,
					uint32_t alignment,
					uint32_t size, const char *tag)
...
EXPORT_SYMBOL(KREE_ZallocSecurechunkmemWithTagEx);
...
EXPORT_SYMBOL(KREE_ReferenceSecurechunkmem);
...
EXPORT_SYMBOL(KREE_UnreferenceSecurechunkmem);
...
EXPORT_SYMBOL(KREE_ReadSecurechunkmem);
...
EXPORT_SYMBOL(KREE_WriteSecurechunkmem);
...
EXPORT_SYMBOL(KREE_GetSecurechunkReleaseSize);
...
EXPORT_SYMBOL(KREE_GetTEETotalSize);
