...
#if IS_ENABLED(CONFIG_MTK_SLBC) || IS_ENABLED(CONFIG_MTK_SLBC_MMSRAM)
...
extern int set_mmsram_data_if(struct mmsram_data *d);
extern void clr_mmsram_data_if(struct mmsram_data *d);
extern int mmsram_power_on_if(struct mmsram_data *d);
extern int mmsram_power_off_if(struct mmsram_data *d);
#endif
