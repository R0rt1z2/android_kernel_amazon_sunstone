...
static int sbuft_det_MV_table[] = {100, 200, 300, 400, 500, 600,
	700, 800};

/* Resistance table for moisture detection */
static int mos_det_R_table[] = {17, 36, 56, 80, 107, 137, 172, 213,
	...
	if (pre_event != event) {
		...
		*/
		...
		minerva_metrics_log(g_fusb_buf, METRICS_BUFF_SIZE_FUSB251,
				"%s:%s:100:%s,%s,%s,%s,ld_current_state=%d;IN,ld_previous_state=%d;IN,"
		...
		memcpy(&fusb251->event_ts, &now_ts, sizeof(struct timespec64));
	} else {
		if (event == TYPE_DRY)
			pr_info("%s: dry\n", __func__);
...
static bool fusb251_check_vbus_valid(struct fusb251 *fusb251)
{
	...
	if (!fusb251->chg_psy) {
		fusb251->chg_psy = power_supply_get_by_name("mtk-master-charger");
		if (!fusb251->chg_psy) {
	...
	ret = power_supply_get_property(fusb251->chg_psy,
...
static int fusb251_limit_charging_current(struct fusb251 *fusb251, bool en)
{
	...
	if (ret < 0)
		pr_err("%s: set input current limit failed, ret = %d\n", __func__, ret);
...
static void fusb251_routine_work(struct work_struct *work)
{
	...
	if (tcpm_inquire_typec_attach_state(fusb251->tcpc) == TYPEC_ATTACHED_AUDIO) {
	...
	if (state == TYPE_DRY) {
		...
		fusb251_config_interface(fusb251, FUSB251_THRESHOLD1,
			 ...
			 fusb251_mos_det_R_convert_reg(fusb251->threshold_sbu[THRESHOLD_DRY2WET]));
		...
		pr_info("%s Line%d: mos_status_sbuft[%d %d]\n", __func__, __LINE__,
			 fusb251->mos_status_sbu1_ft, fusb251->mos_status_sbu2_ft);
		...
		fusb251_config_interface(fusb251, FUSB251_THRESHOLD1,
			 SBU_MOS_R_DET_MASK, SBU_MOS_R_DET_SHIFT,
			 fusb251_mos_det_R_convert_reg(fusb251->threshold_sbu[THRESHOLD_WET2DRY]));
		...
		pr_info("%s Line%d: mos_status_sbu[%d %d]\n", __func__, __LINE__,
			 fusb251->mos_status_sbu1, fusb251->mos_status_sbu2);
...
static void fusb251_parse_dt(struct fusb251 *fusb251)
{
	...
	if (ret < 0) {
		...
		pr_err("%s: not define threshold_sbu, use THRESHOLD_SBU_DRY2WET / THRESHOLD_SBU_WET2DRY\n",
			 __func__);
	} else {
	...
	if (ret < 0) {
		...
		pr_err("%s: not define work_interval, use SBUFT_DET_DELAY / SBU_DET_DELAY\n",
			 __func__);
	} else {
...
static int fusb251_init(struct fusb251 *fusb251)
{
	...
	/* reset moisture */
	if (fusb251_write_reg(fusb251, FUSB251_RESET, RESET_MOSSTURE) < 0) {
		pr_err("%s: reset moisture failed\n", __func__);
		return -1;
	}
	/* mask all the interrupt */
	if (fusb251_write_reg(fusb251, FUSB251_INT_MASK, MSK_I_MASK_ALL) < 0) {
...
static void fusb251_vbus_detect(struct work_struct *work)
{
	int state;
...
static int fusb251_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int ret = 0;
	...
	g_fusb251 = fusb251;
	ktime_now =  ktime_get_boottime();
	fusb251->event_ts = ktime_to_timespec64(ktime_now);
	wakeup_source_add(&fusb251->wake_lock);
