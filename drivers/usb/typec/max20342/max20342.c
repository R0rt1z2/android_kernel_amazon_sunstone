...
static void max20342_report_event(struct max20342 *max20342, int event)
{
	...
	if (pre_event != event) {
		...
		minerva_metrics_log(g_max_buf_max20342, METRICS_BUFF_SIZE_MAX20342,
				"%s:%s:100:%s,%s,%s,%s,ld_current_state=%d;IN,ld_previous_state=%d;IN,ld_duration_sec=%d;IN:us-east-1",
		...
		memcpy(&max20342->event_ts, &now_ts, sizeof(struct timespec64));
	} else {
		if (event == TYPE_DRY)
			pr_info("[%s] dry\n", __func__);
...
static bool max20342_check_vbus_valid(struct max20342 *max20342)
{
	...
	if (!max20342->chg_psy) {
		max20342->chg_psy = power_supply_get_by_name("mtk-master-charger");
		if (!max20342->chg_psy) {
	...
	ret = power_supply_get_property(max20342->chg_psy,
...
static void max20342_start_detection(struct max20342 *max20342)
{
	...
	if (max20342->mos_is_valid) {
		...
		pinctrl_select_state(max20342->mos_pinctrl, max20342->mos_ctrl2_high);
	}
	...
	if (max20342->mos_is_valid) {
		...
		pinctrl_select_state(max20342->mos_pinctrl, max20342->mos_ctrl2_low);
	}
}
...
static void max20342_routine_work(struct work_struct *work)
{
	...
	if (tcpm_inquire_typec_attach_state(max20342->tcpc) == TYPEC_ATTACHED_AUDIO) {
...
static bool max20342_pinctrl_init(struct max20342 *max20342)
{
	...
	max20342->mos_ctrl2_init = pinctrl_lookup_state(pinctrl, "mosctrl2_init");
	if (IS_ERR(max20342->mos_ctrl2_init)) {
		pr_err("%s: can't find mos_ctrl2_init\n", __func__);
		goto out;
	}

	max20342->mos_ctrl2_low = pinctrl_lookup_state(pinctrl, "mosctrl2_low");
	if (IS_ERR(max20342->mos_ctrl2_low)) {
		pr_err("%s: can't find mos_ctrl2_low\n", __func__);
		goto out;
	}

	max20342->mos_ctrl2_high = pinctrl_lookup_state(pinctrl, "mosctrl2_high");
	if (IS_ERR(max20342->mos_ctrl2_high)) {
		pr_err("%s: can't find mos_ctrl2_high\n", __func__);
		goto out;
	}

	pinctrl_select_state(max20342->mos_pinctrl, max20342->mos_ctrl1_init);
	pinctrl_select_state(max20342->mos_pinctrl, max20342->mos_ctrl2_init);
	 ...
	 const struct i2c_device_id *id)
{
	...
	g_max20342 = max20342;
	ktime_now =  ktime_get_boottime();
	max20342->event_ts = ktime_to_timespec64(ktime_now);
	wakeup_source_add(&max20342->wake_lock);
...
late_initcall(max20342_i2c_init);
