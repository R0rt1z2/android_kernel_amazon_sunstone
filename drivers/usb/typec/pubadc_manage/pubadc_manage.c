...
static unsigned char pubadc_get_id(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(public_adc.gpio_pubadc_id)) {
		pr_err("%s: no valid gpio-id\n", __func__);
		return 0;
	}

	ret = gpio_request(public_adc.gpio_pubadc_id, "pubadc_id");
	if (ret) {
		pr_err("[%s] request pubadc gpio-id failed ret = %d\n",
			 __func__, ret);
		return 0;
	}
	gpio_direction_input(public_adc.gpio_pubadc_id);
	liquid_id = gpio_get_value(public_adc.gpio_pubadc_id);
	gpio_free(public_adc.gpio_pubadc_id);

	return (unsigned char)liquid_id;
}
