...
static int pd_sc_set_charger(struct chg_alg_device *alg)
{
	...
	if (pd->is_hv_charger_state &&
		pd->input_current1 > pd->hv_input_current_limit &&
		pd->hv_input_current_limit != -1)
		pd->input_current1 = pd->hv_input_current_limit;
...
{
	struct mtk_pd *pd = dev_get_drvdata(&alg->dev);
	int vbus = 0, cur, idx, ret, ret_value = ALG_RUNNING;

	ret = __mtk_pdc_get_setting(alg, &vbus, &cur, &idx);

	if (vbus > PD_DEFAULT_CHARGING_VOLTAGE)
		pd->is_hv_charger_state = true;
	else
		pd->is_hv_charger_state = false;
				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "pd_hv_input_current_limit", &val) >= 0)
		pd->hv_input_current_limit = val;
	else
		pd->hv_input_current_limit = -1;
