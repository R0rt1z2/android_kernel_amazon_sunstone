	...
	int _prd, int _zcv_mv, int _thr_mv)
{
	...
	if (gauge->thr_reg < 0) {
		bm_err("[%s]nag_c_dltv_thr mv=%d ,thr_reg=%d,thr_reg greater than 0\n",
			__func__, _thr_mv, gauge->thr_reg);
		gauge->thr_reg = 256;
	}
...
static void mt6359_charger_auto_on(struct device *dev, bool bEn)
{
	struct mtk_gauge *gauge;
	...
	/* enable envtem */
	if (bEn)
		regmap_update_bits(gauge->regmap, 0xa8c, 0x1, 0);
	else
		regmap_update_bits(gauge->regmap, 0xa8c, 0x1, 1);

	/* enable envtem write */
	regmap_update_bits(gauge->regmap, 0xa8c, 0x2, 0x2);
}
...
static void mt6359_gauge_shutdown(struct platform_device *pdev)
{
	...
	union power_supply_propval dcap, online;
	int ret, i;
	unsigned int val;
	...
	chg_psy = devm_power_supply_get_by_phandle(&pdev->dev, "charger");
	if (IS_ERR_OR_NULL(chg_psy)) {
		dev_err(&pdev->dev, "%s Couldn't get chg_psy\n", __func__);
	} else {
		ret = power_supply_get_property(chg_psy,
		...
		if (!ret && online.intval) {
			...
			for (i = 0; i < 7; i++) {
				regmap_read(gauge->regmap, MT6359P_RGS_CHRDET_ADDR, &val);
				val &= MT6359P_RGS_CHRDET_MASK;
				val >>= MT6359P_RGS_CHRDET_SHIFT;
				if (0 == val)
					break;
				msleep(100);
			}
			/* second, check vbus re-on in 450ms */
			for (i = 0; i < 45; i++) {
				regmap_read(gauge->regmap, MT6359P_RGS_CHRDET_ADDR, &val);
				val &= MT6359P_RGS_CHRDET_MASK;
				val >>= MT6359P_RGS_CHRDET_SHIFT;
		...
		if (!ret && dcap.intval) {
			mt6359_charger_auto_on(&pdev->dev, false);
			/* disable charging */
			charger_dev_enable(get_charger_by_name("primary_chg"), false);
			dev_info(&pdev->dev, "[%s] disable charger\n", __func__);
		}
	}
...
static int mt6359_gauge_probe(struct platform_device *pdev)
{
	...
	struct iio_channel *bat_id_iio_channel;
	...
	if (of_property_read_u32(np, "battery_id_method", &val)
		>= 0 && val == FROM_ADC) {
		bat_id_iio_channel = devm_iio_channel_get(&pdev->dev,
			"pmic_battery_id");
		if (IS_ERR(bat_id_iio_channel)) {
			bm_err("mt6359 requests probe deferral, batt_id ADC channel get fail\n");
			return -EPROBE_DEFER;
		}
	}
	...
	gauge->chan_bat_temp = chan_bat_temp;
	...
	gauge->gm->g_bat_id_iio_channel = bat_id_iio_channel;
