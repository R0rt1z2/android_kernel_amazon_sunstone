	...
	int cmd, char *rcv, char *ret)
{
	...
	case FG_DAEMON_CMD_SEND_VERSION_CONTROL:
		...
		break;
	case FG_DAEMON_CMD_SEND_CUSTOM_LOG_DATA:
		{
			char *ptr;
			...
			ptr = (char *)&gm->fg_log_data;
			memcpy(&ptr[prcv->idx],
				prcv->input,
				prcv->size);
			bm_debug("FG_DATA_TYPE_TABLE type:%d size:%d %d idx:%d\n",
				FG_DAEMON_CMD_SEND_CUSTOM_LOG_DATA,
				prcv->total_size,
				prcv->size,
				prcv->idx);

			bat_metrics_aging(gm->fg_log_data.gm30_aging_factor,
					gm->fg_log_data.gm30_bat_cycle,
					gm->fg_log_data.gm30_qmax_t_0ma_tb1);
		}
		break;
...
static ssize_t fake_rtc_soc_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
...
static ssize_t fake_rtc_soc_store(
	struct device *dev, struct device_attribute *attr,
	const char *buf, size_t size)
{
	int cur, ret;
	...
	if (!ret && (cur > 0 && cur < 100)) {
		gm->fake_rtc_soc = cur;
		gauge_set_property(GAUGE_PROP_RTC_UI_SOC, gm->fake_rtc_soc);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (gm->battery_id >= BATTERY_ID_NUM_MAX)
		return scnprintf(buf, PAGE_SIZE, "Invalid\n");

	name = gm->bat_name[gm->battery_id];
	if (name)
...
static DEVICE_ATTR_RW(fake_rtc_soc);
...
static int mtk_battery_setup_files(struct platform_device *pdev)
{
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_cell_vendor_name);
	if (ret)
		goto _out;
...
static ssize_t gm30_soc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_soc);
}

static ssize_t gm30_fg_c_soc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_fg_c_soc);
}

static ssize_t gm30_fg_v_soc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_fg_v_soc);
}

static ssize_t gm30_ui_soc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_ui_soc);
}

static ssize_t gm30_vc_diff_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_vc_diff);
}

static ssize_t gm30_vc_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_vc_mode);
}

static ssize_t gm30_init_vbat_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_init_vbat);
}

static ssize_t gm30_t_d0_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_t_d0);
}

static ssize_t gm30_T_table_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_T_table);
}

static ssize_t gm30_T_table_c_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_T_table_c);
}

static ssize_t gm30_fg_c_d0_soc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_fg_c_d0_soc);
}

static ssize_t gm30_fg_v_d0_soc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_fg_v_d0_soc);
}

static ssize_t gm30_quse_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_quse);
}

static ssize_t gm30_quse_wo_lf_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_quse_wo_lf);
}

static ssize_t gm30_quse_tb1_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_quse_tb1);
}

static ssize_t gm30_quse_wo_lf_tb1_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_quse_wo_lf_tb1);
}

static ssize_t gm30_aging_factor_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_aging_factor);
}

static ssize_t gm30_bat_cycle_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
...
static ssize_t gm30_cv_soc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_cv_soc);
}

static ssize_t gm30_car_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_car);
}

static ssize_t gm30_car_v_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_car_v);
}

static ssize_t gm30_qmax_t_0ma_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_qmax_t_0ma);
}

static ssize_t gm30_qmax_t_0ma_tb1_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
...
static ssize_t gm30_init_path_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_init_path);
}

static ssize_t gm30_vboot_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", gm->fg_log_data.gm30_vboot);
}

static ssize_t gm30_vboot_c_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;
...
static int mtk_battery_setup_gm30_node(struct platform_device *pdev)
{
	/* runtime data */
	device_create_file(&(pdev->dev), &dev_attr_gm30_ui_soc);
	device_create_file(&(pdev->dev), &dev_attr_gm30_soc);
	device_create_file(&(pdev->dev), &dev_attr_gm30_fg_c_soc);
	device_create_file(&(pdev->dev), &dev_attr_gm30_fg_v_soc);
	device_create_file(&(pdev->dev), &dev_attr_gm30_vc_diff);
	device_create_file(&(pdev->dev), &dev_attr_gm30_vc_mode);
	device_create_file(&(pdev->dev), &dev_attr_gm30_car);
	device_create_file(&(pdev->dev), &dev_attr_gm30_car_v);
	device_create_file(&(pdev->dev), &dev_attr_gm30_aging_factor);
	device_create_file(&(pdev->dev), &dev_attr_gm30_bat_cycle);

	/* initial data */
	device_create_file(&(pdev->dev), &dev_attr_gm30_init_path);
	device_create_file(&(pdev->dev), &dev_attr_gm30_init_vbat);
	device_create_file(&(pdev->dev), &dev_attr_gm30_fg_c_d0_soc);
	device_create_file(&(pdev->dev), &dev_attr_gm30_fg_v_d0_soc);
	device_create_file(&(pdev->dev), &dev_attr_gm30_qmax_t_0ma);
	device_create_file(&(pdev->dev), &dev_attr_gm30_qmax_t_0ma_tb1);

	/* keep them in case it's useful in future */
	device_create_file(&(pdev->dev), &dev_attr_gm30_t_d0);
	device_create_file(&(pdev->dev), &dev_attr_gm30_cv_soc);
	device_create_file(&(pdev->dev), &dev_attr_gm30_vboot);
	device_create_file(&(pdev->dev), &dev_attr_gm30_vboot_c);
	device_create_file(&(pdev->dev), &dev_attr_gm30_quse);
	device_create_file(&(pdev->dev), &dev_attr_gm30_quse_wo_lf);
	device_create_file(&(pdev->dev), &dev_attr_gm30_quse_tb1);
	device_create_file(&(pdev->dev), &dev_attr_gm30_quse_wo_lf_tb1);
	device_create_file(&(pdev->dev), &dev_attr_gm30_T_table);
	device_create_file(&(pdev->dev), &dev_attr_gm30_T_table_c);
	...
	struct fgd_nl_msg_t *ret_msg)
{
	...
	case FG_DAEMON_CMD_SET_RTC_UI_SOC:
	{
		...
		if (gm->fake_rtc_soc > 0 && gm->fake_rtc_soc < 100) {
			pr_notice("skip FG_DAEMON_CMD_SET_RTC_UI_SOC\n");
			break;
		}
...
//timotodo
	...
	case FG_DAEMON_CMD_SEND_CUSTOM_LOG_DATA:
	{
		fg_daemon_send_data(gm, FG_DAEMON_CMD_SEND_CUSTOM_LOG_DATA,
			&msg->fgd_data[0], &ret_msg->fgd_data[0]);
	}
	break;
#ifdef CONFIG_MTK_USE_AGING_ZCV
...
int mtk_battery_daemon_init(struct platform_device *pdev)
{
	...
	mtk_battery_setup_gm30_node(pdev);
	return 0;
}
