...
int charger_dev_enable_dcap(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
			chg_dev->ops->enable_dcap)
		return chg_dev->ops->enable_dcap(chg_dev, en);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_enable_dcap);

int set_shutdown_enable_dcap(void)
{
	struct charger_device *chg1_dev = NULL;

	chg1_dev = get_charger_by_name("primary_chg");
	if (chg1_dev != NULL) {
		return charger_dev_enable_dcap(chg1_dev, true);
	} else {
		pr_err("%s can't find primary_chg!\n", __func__);
		return -ENODEV;
	}
}
EXPORT_SYMBOL(set_shutdown_enable_dcap);
