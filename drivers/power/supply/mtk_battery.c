...
bool get_battery_id_status(void)
{
	...
	gm = get_mtk_battery();
	if (gm == NULL || IS_ERR_OR_NULL(gm->g_bat_id_iio_channel)) {
		bm_err("%s: get bat_id info failed\n", __func__);
		return false;
	}
	...
	gm->battery_id_volt = id_volt;
	if ((id_volt < gm->battery_id_volt_min) || (id_volt > gm->battery_id_volt_max)) {
		bm_err("[%s] battery ID disconnect(%d) id_volt_max:%d id_volt_min:%d\n",
			__func__, id_volt, gm->battery_id_volt_max, gm->battery_id_volt_min);
		return false;
	}
	...
	struct mtk_battery *gm)
{
	...
	if (id_volt < gm->battery_id_volt_min ||
		id_volt > gm->battery_id_volt_max) {
		bm_err("%s: read id_volt=%d error\n",
			__func__, id_volt);
		return false;
	}
	bm_err("%s: read id_volt=%d\n", __func__, id_volt);
	...
	if (id == gm->total_battery_number) {
		bm_err("%s: Battery id get error,use default battery id 0\n",
			__func__);
		return false;
	}
...
static bool fg_custom_bat_id_from_dts(const struct device_node *np,
	struct mtk_battery *gm)
{
	return false;
}

static bool fg_custom_bat_id_init(const struct device_node *np,
	struct mtk_battery *gm)
{
	unsigned int val;
	unsigned int idx = 0;
	...
	if (!of_property_read_u32(np, "battery_id_method", &val)) {
		gm->battery_id_method = (int)val;
		bm_err("%s: read battery_id_method=%u, gm->battery_id_method=%d\n",
			__func__, val, gm->battery_id_method);
	} else {
		gm->battery_id_method = 0;
		bm_err("%s: read battery_id_method fail,use default\n",__func__);
	}

	if (!of_property_read_u32(np, "battery_id_volt_max", &val)) {
		gm->battery_id_volt_max = (int)val;
		bm_err("%s: read battery_id_volt_max=%d, gm->battery_id_volt_max=%d\n",
			__func__, val, gm->battery_id_volt_max);
	} else {
		bm_err("%s: read id_volt_max fail\n",__func__);
		return false;
	}

	if (!of_property_read_u32(np, "battery_id_volt_min", &val)) {
		gm->battery_id_volt_min = (int)val;
		bm_err("%s: read battery_id_volt_min=%d, gm->battery_id_volt_min=%d\n",
			__func__, val, gm->battery_id_volt_min);
	} else {
	...
	while (!of_property_read_u32_index(np,
		...
		gm->gauge_battery_id_voltage[idx++] = val;
		bm_err("gauge_battery_id_voltage[%d]= %d\n", idx, val);
	}
	gm->total_battery_number = idx;

	return true;
}

static void fg_get_battery_id(struct platform_device *dev,
	struct mtk_battery *gm)
{
	struct device_node *np = dev->dev.of_node;
	bool ret = 0;

	ret = fg_custom_bat_id_init(np, gm);
	if (!ret) {
		gm->battery_id = 0;
		bm_err("%s:fail choose dts battery_id=0 as default\n",__func__);
		return;
	}

	switch (gm->battery_id_method) {
	case FROM_DTB:
		ret = fg_custom_bat_id_from_dts(np, gm);
		if (!ret) {
			gm->battery_id = 0;
			bm_err("%s:choose dts battery_id=0 as default\n",__func__);
		}
		break;
	case FROM_ADC:
		ret = fg_custom_bat_id_from_adc(dev, gm);
		if (!ret) {
			gm->battery_id = 0;
			bm_err("%s:choose adc battery_id=0 as default\n",__func__);
		}
		break;
	case FROM_IDME:
		ret = fg_custom_bat_id_from_idme();
		if (!ret) {
			gm->battery_id = 0;
			bm_err("%s:choose idme battery_id=0 as default\n",__func__);
		}
		break;
	default:
		gm->battery_id = 0;
		bm_err("%s:choose battery_id=0 as default\n",__func__);
		break;
	}
}
...
void fg_update_difference_full_cv(struct mtk_battery *gm, unsigned long val)
{
...
static enum power_supply_property battery_props[] = {
	...
	POWER_SUPPLY_PROP_DIFF_FULL_CV,
};
	...
	enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_DIFF_FULL_CV:
		return 1;
	default:
		return 0;
	}
	...
	const union power_supply_propval *val)
{
	...
	if (IS_ERR_OR_NULL(gm->chg_psy)) {
		gm->chg_psy = power_supply_get_by_name("mtk-master-charger");
		bm_err("%s retry to get chg_psy\n", __func__);
	}

	if (!IS_ERR_OR_NULL(gm->chg_psy)) {
		info = (struct mtk_charger *)power_supply_get_drvdata(gm->chg_psy);
	} else {
		bm_err("%s failed to get chg_psy\n", __func__);
		return -ENOENT;
	}
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		if (info) {
			info->chg_data[0].force_charging_current = val->intval;
			pr_notice("%s:force_charging_current:%d\n",
				__func__, val->intval);
			charger_dev_notify(info->chg1_dev, CHARGER_DEV_NOTIFY_UPDATE);
		}
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		if (info) {
			info->chg_data[0].force_input_current_limit = val->intval;
			pr_notice("%s:force_input_current_limit:%d\n",
				__func__, val->intval);
			charger_dev_notify(info->chg1_dev, CHARGER_DEV_NOTIFY_UPDATE);
		}
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		...
		break;
	case POWER_SUPPLY_PROP_DIFF_FULL_CV:
		fg_update_difference_full_cv(gm, val->intval);
		break;
	default:
		return -EINVAL;
	}
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		if (info)
			val->intval = info->chg_data[0].force_input_current_limit;
		else
			val->intval = -1;
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		if (info)
			val->intval = info->chg_data[0].force_charging_current;
		else
			val->intval = -1;
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		if (info)
			val->intval = info->data.battery_cv;
		else
			val->intval = -1;
		break;
	case POWER_SUPPLY_PROP_DIFF_FULL_CV:
		if (info) {
			if (info->top_off_mode_enable)
				val->intval = info->top_off_difference_full_cv;
			else
				val->intval = info->normal_difference_full_cv;
		} else {
			val->intval = -1;
		}
		break;
...
static int is_bat_eoc_protect(struct power_supply *psy)
{
	struct mtk_battery *gm;
	struct mtk_charger *info = NULL;
	bool enable_eoc_protect;
	...
	if (!info->enable_bat_eoc_protect) {
		bm_err("%s disable bat_eoc_protect\n", __func__);
		return 0;
	}

	enable_eoc_protect = info->bat_eoc_protect || info->dpm_disable_charging;

	return enable_eoc_protect;
}
...
static void mtk_battery_external_power_changed(struct power_supply *psy)
{
	...
	if (IS_ERR_OR_NULL(chg_psy)) {
		...
		if (!online.intval)
			...
			if (is_bat_eoc_protect(psy)) {
				bm_err("Force to report POWER_SUPPLY_STATUS_FULL\n");
				bs_data->bat_status = POWER_SUPPLY_STATUS_FULL;
			} else {
				info = (struct mtk_charger *)power_supply_get_drvdata(gm->chg_psy);
				if (IS_ERR_OR_NULL(info)) {
					bm_err("%s fail to get info\n", __func__);
					return;
				}

				charger_dev_is_charging_done(info->chg1_dev, &chg_done);
				if (chg_done && gm->ui_soc == 100)
					bs_data->bat_status = POWER_SUPPLY_STATUS_FULL;
...
void battery_service_data_init(struct mtk_battery *gm)
{
	...
	bs_data->psd.property_is_writeable =
		battery_psy_property_is_writeable;
...
void fg_custom_init_from_header(struct mtk_battery *gm)
{
	...
	gm->min_battery_temp = DISABLE_LOW_BATTERY_TEMP;
	...
	struct mtk_battery *gm)
{
	...
	fg_read_dts_val(np, "NAFG_RESISTANCE", &(fg_cust_data->nafg_resistance),
		1);
	fg_read_dts_val(np, "MIN_BATTERY_TEMP", &(gm->min_battery_temp), 1);
	...
	for (table_index = 0; table_index < MAX_TABLE; table_index++) {
		...
		if (num_data <= 0) {
			bm_err("%s can not find Q_MAX_T in dts", __func__);
			break;
		}
	...
	for (table_index = 0; table_index < MAX_TABLE; table_index++) {
		...
		num_data = of_property_count_u32_elems(np, node_buffer);
		if (num_data <= 0) {
			bm_err("%s can not find Q_MAX_H_CURRENT_T in dts", __func__);
			break;
		}
		if (num_data > TOTAL_BATTERY_NUMBER)
			num_data = TOTAL_BATTERY_NUMBER;
...
static ssize_t battery_id_volt_show(struct device *dev,
	 struct device_attribute *attr, char *buf)
{
	struct mtk_battery *gm;

	get_battery_id_status();
	gm = get_mtk_battery();
	if (gm != NULL)
...
static DEVICE_ATTR_RO(battery_id_volt);
	 ...
	 struct device_attribute *attr, char *buf)
{
	...
	gm = get_mtk_battery();
	if (gm != NULL)
		return scnprintf(buf, PAGE_SIZE, "%d\n", gm->gauge->hw_status.sw_ocv);

	return scnprintf(buf, PAGE_SIZE, "%d\n", -1);
}
static DEVICE_ATTR_RO(sw_ocv);

static void mtk_battery_charger_logger_node(struct platform_device *dev)
{
	device_create_file(&(dev->dev), &dev_attr_battery_id_volt);
...
void fg_drv_update_hw_status(struct mtk_battery *gm)
{
	...
	if (gm->cur_bat_temp <= gm->min_battery_temp)
		set_shutdown_cond(gm, LOW_BATTERY_TEMP);
...
int battery_report_uevent(struct mtk_battery *gm)
{
	...
	if (gm->chg_psy) {
		info = (struct mtk_charger *)power_supply_get_drvdata(gm->chg_psy);
		pdev = info->pdev;
		power_supply_get_property(gm->chg_psy,
	...
	bm_err("%s: old_ui_soc=%d new_ui_soc=%d old_status=%d new_status=%d\n",
			__func__, old_ui_soc, gm->fg_cust_data.ui_old_soc,
			old_status, gm->bs_data.bat_status);
	if (abs(gm->fg_cust_data.ui_old_soc - old_ui_soc) < REPORT_RESOLUTION
		&& old_status == gm->bs_data.bat_status)
		return 0;

	old_ui_soc = gm->fg_cust_data.ui_old_soc;
	old_status = gm->bs_data.bat_status;
	vbat = gauge_get_int_property(GAUGE_PROP_BATTERY_VOLTAGE);
	ibat = gauge_get_int_property(GAUGE_PROP_BATTERY_CURRENT);
	snprintf(capacity_str, ENV_SIZE, "BATTERY_LEVEL=%d",
		gm->bs_data.bat_capacity);
	snprintf(status_str, ENV_SIZE, "BATTERY_STATUS=%d",
		gm->bs_data.bat_status);
	snprintf(iusb_lim_str, ENV_SIZE, "BATTERY_IUSB_LIM=%d",
		info->chg_data[0].input_current_limit/1000);
	snprintf(ibat_lim_str, ENV_SIZE, "BATTERY_IBAT_LIM=%d",
		info->chg_data[0].charging_current_limit/1000);
	snprintf(ui_soc_str, ENV_SIZE, "BATTERY_UI_SOC=%d", gm->fg_cust_data.ui_old_soc);
	snprintf(temperature_str, ENV_SIZE, "BATTERY_TEMP=%d", gm->cur_bat_temp);
	...
	snprintf(c_soc_str, ENV_SIZE, "BATTERY_C_SOC=%d", gm->fg_log_data.gm30_fg_c_soc);
	snprintf(v_soc_str, ENV_SIZE, "BATTERY_V_SOC=%d", gm->fg_log_data.gm30_fg_v_soc);
	snprintf(vc_diff_str, ENV_SIZE, "BATTERY_VC_DIFF=%d", gm->fg_log_data.gm30_vc_diff);
	snprintf(vc_mode_str, ENV_SIZE, "BATTERY_VC_MODE=%d", gm->fg_log_data.gm30_vc_mode);
	snprintf(quse_str, ENV_SIZE, "BATTERY_QUSE=%d",
		gm->fg_log_data.gm30_quse_wo_lf_tb1/10);
...
int set_shutdown_cond(struct mtk_battery *gm, int shutdown_cond)
{
	...
	case LOW_BATTERY_TEMP:
		...
		pr_err("%s: battery low temperature:%d, "
			"trip temp:%d", __func__, gm->cur_bat_temp,
			gm->min_battery_temp);
	...
	while (1) {
		...
		if (sdd->overheat == true) {
			...
			bm_err("%s battery overheat~ power off, ret = %d\n",
				__func__, ret);
			set_shutdown_enable_dcap();
			pr_err("%s: battery overheat~ current_temp:%d, "
			       "trip temp:%d", __func__, gm->cur_bat_temp,
				BATTERY_SHUTDOWN_TEMPERATURE);
...
int battery_init(struct platform_device *pdev)
{
	...
	mtk_battery_charger_logger_node(pdev);
