...
static const struct mt_class mt_classes[] = {
	...
	{ .name = MT_CLS_NSMU, .quirks = MT_QUIRK_NOT_SEEN_MEANS_UP },
	{ .name = MT_CLS_SERIAL, .quirks = MT_QUIRK_ALWAYS_VALID },
	{ .name = MT_CLS_CONFIDENCE, .quirks = MT_QUIRK_VALID_IS_CONFIDENCE },
	{ .name = MT_CLS_CONFIDENCE_CONTACT_ID,
	  .quirks = MT_QUIRK_VALID_IS_CONFIDENCE | MT_QUIRK_SLOT_IS_CONTACTID },
	{ .name = MT_CLS_CONFIDENCE_MINUS_ONE,
	  .quirks = MT_QUIRK_VALID_IS_CONFIDENCE |
		    MT_QUIRK_SLOT_IS_CONTACTID_MINUS_ONE },
	{ .name = MT_CLS_DUAL_INRANGE_CONTACTID,
	  .quirks = MT_QUIRK_VALID_IS_INRANGE | MT_QUIRK_SLOT_IS_CONTACTID,
	  .maxcontacts = 2 },
	{ .name = MT_CLS_DUAL_INRANGE_CONTACTNUMBER,
	  .quirks = MT_QUIRK_VALID_IS_INRANGE | MT_QUIRK_SLOT_IS_CONTACTNUMBER,
	  .maxcontacts = 2 },
	{ .name = MT_CLS_INRANGE_CONTACTNUMBER,
	  .quirks =
		  MT_QUIRK_VALID_IS_INRANGE | MT_QUIRK_SLOT_IS_CONTACTNUMBER },
	{ .name = MT_CLS_WIN_8,
	  .quirks = MT_QUIRK_ALWAYS_VALID | MT_QUIRK_IGNORE_DUPLICATES |
		    MT_QUIRK_HOVERING | MT_QUIRK_CONTACT_CNT_ACCURATE |
		    MT_QUIRK_STICKY_FINGERS | MT_QUIRK_WIN8_PTP_BUTTONS,
	...
	},
	{
		.name = MT_CLS_EGALAX,
		.quirks =
			MT_QUIRK_SLOT_IS_CONTACTID | MT_QUIRK_VALID_IS_INRANGE,
		.sn_move = 4096,
		.sn_pressure = 32,
	},
	{
	...
	{ .name = MT_CLS_GENERALTOUCH_PWT_TENFINGERS,
	  .quirks = MT_QUIRK_NOT_SEEN_MEANS_UP | MT_QUIRK_SLOT_IS_CONTACTID },

	{
		.name = MT_CLS_FLATFROG,
		.quirks = MT_QUIRK_NOT_SEEN_MEANS_UP | MT_QUIRK_NO_AREA,
		.sn_move = 2048,
		.maxcontacts = 40,
	},
	{ .name = MT_CLS_LG,
	  .quirks = MT_QUIRK_ALWAYS_VALID | MT_QUIRK_FIX_CONST_CONTACT_ID |
		    MT_QUIRK_IGNORE_DUPLICATES | MT_QUIRK_HOVERING |
	...
	{ .name = MT_CLS_GOOGLE,
	  .quirks = MT_QUIRK_ALWAYS_VALID | MT_QUIRK_CONTACT_CNT_ACCURATE |
		    MT_QUIRK_SLOT_IS_CONTACTID | MT_QUIRK_HOVERING },
	{
...
static ssize_t mt_show_quirks(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
...
static ssize_t mt_set_quirks(struct device *dev, struct device_attribute *attr,
...
static struct attribute *sysfs_attrs[] = { &dev_attr_quirks.attr, NULL };
...
static u8 check_raw_data(u8 *buf, u8 len)
{
	int i = 0;
	u8 check_sum = 0;
...
static void mt_feature_mapping(struct hid_device *hdev, struct hid_field *field,
						      ...
						      struct hid_report *report)
{
	...
	for (r = 0; r < report->maxfield; r++) {
		...
		if (field->logical == HID_DG_FINGER ||
		    td->hdev->group != HID_GROUP_MULTITOUCH_WIN_8) {
...
static void mt_store_field(struct hid_device *hdev,
			   struct mt_application *application, __s32 *value,
			   size_t offset)
{
	struct mt_usages *usage;
	__s32 **target;
	...
	if (*target != DEFAULT_TRUE && *target != DEFAULT_FALSE &&
	    *target != DEFAULT_ZERO) {
		if (usage->contactid == DEFAULT_ZERO ||
		    usage->x == DEFAULT_ZERO || usage->y == DEFAULT_ZERO) {
			hid_dbg(hdev, "ignoring duplicate usage on incomplete");
			return;
		}
		usage = mt_allocate_usage(hdev, application);
		if (!usage)
...
#define MT_STORE_FIELD(__name)                                                 \
	mt_store_field(hdev, app, &field->value[usage->usage_index],           \
		       offsetof(struct mt_usages, __name))

static int mt_touch_input_mapping(struct hid_device *hdev, struct hid_input *hi,
				  struct hid_field *field,
				  struct hid_usage *usage, unsigned long **bit,
				  int *max, struct mt_application *app)
{
	struct mt_device *td = hid_get_drvdata(hdev);
	...
	case HID_UP_DIGITIZER:
		...
		case HID_DG_INRANGE:
			if (app->quirks & MT_QUIRK_HOVERING) {
				input_set_abs_params(hi->input, ABS_MT_DISTANCE,
						     0, 1, 0, 0);
			}
...
static int mt_compute_slot(struct mt_device *td, struct mt_application *app,
			   struct mt_usages *slot, struct input_dev *input)
{
...
static int mt_process_slot(struct mt_device *td, struct input_dev *input,
			   struct mt_application *app, struct mt_usages *slot)
{
	...
	if ((quirks & MT_QUIRK_IGNORE_DUPLICATES) && mt) {
		...
		if (input_mt_is_active(i_slot) && input_mt_is_used(mt, i_slot))
			return -EAGAIN;
	}
	...
	if (app->application == HID_GD_SYSTEM_MULTIAXIS)
		...
		tool = MT_TOOL_PALM;
		if (!active && mt && input_mt_is_active(&mt->slots[slotnum])) {
			/*
	...
	if (active) {
		...
		if (slot->a != DEFAULT_ZERO) {
			...
			azimuth = *slot->a;
			max_azimuth =
				input_abs_get_max(input, ABS_MT_ORIENTATION);
			    ...
			    struct mt_report_data *rdata)
{
	...
	if (contact_count >= 0) {
		 ...
		 */
		if ((app->quirks & MT_QUIRK_WIN8_PTP_BUTTONS) &&
		    app->num_received == 0 && app->prev_scantime != scantime)
	...
	for (r = 0; r < report->maxfield; r++) {
		...
		for (n = 0; n < count; n++)
			mt_process_mt_event(hid, app, field, &field->usage[n],
				     ...
				     struct mt_application *app)
{
	...
	/* check for clickpads */
	if ((app->mt_flags & INPUT_MT_POINTER) && (app->buttons_count == 1))
		td->is_buttonpad = true;

	if (td->is_buttonpad)
		__set_bit(INPUT_PROP_BUTTONPAD, input->propbit);

	app->pending_palm_slots =
		devm_kcalloc(&hi->input->dev, BITS_TO_LONGS(td->maxcontacts),
			    ...
			    unsigned long **bit, int *max)
{
	...
	if (field->application == HID_VD_ASUS_CUSTOM_MEDIA_KEYS &&
		...
		case 0x10:
			mt_map_key_clear(KEY_BRIGHTNESSDOWN);
			break;
		case 0x20:
			mt_map_key_clear(KEY_BRIGHTNESSUP);
			break;
		case 0x35:
			mt_map_key_clear(KEY_DISPLAY_OFF);
			break;
		case 0x6b:
			mt_map_key_clear(KEY_F21);
			break;
		case 0x6c:
			mt_map_key_clear(KEY_SLEEP);
			break;
		default:
			return -1;
		}
	...
	list_for_each_entry (rep, &rep_enum->report_list, list) {
		...
		for (i = 0; i < rep->maxfield; i++) {
			...
			for (j = 0; j < rep->field[i]->maxusage; j++) {
				...
				if (mt_need_to_apply_feature(
						hdev, rep->field[i], usage, latency,
...
void mt_release_contacts(struct hid_device *hid)
{
	...
	struct mt_application *application = NULL;
	struct mt_device *td = hid_get_drvdata(hid);
...
static struct hid_input *ilitek_input_allocate(struct hid_device *hid)
{
	...
	ILI_INFO("Allocate ilitek hid input.\n");

	input_set_drvdata(input_dev, hid);

	input_dev->name = HID_TOUCH_INPUT_DEVICE;
	...
	__set_bit(KEY_GESTURE_POWER, input_dev->keybit);
...
static int mt_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	ilitek_hid_plat_dev_init(client);

	/* ILITEK HID input register create */
	ilits->input_hid = ilitek_input_allocate(hdev);

	if (input_register_device(ilits->input_hid->input) < 0) {
...
static int mt_raw_event(struct hid_device *hdev, struct hid_report *report,
			u8 *raw_data, int size)
{
	...
	check_sum = check_raw_data(raw_data, size - 1);

	if(raw_data[size - 1] != check_sum) {
		ILI_ERR("ILITEK check sum error (%d) !!\n ",check_sum);
		ili_hid_ic_cascade_get_pc_latch(DO_I2C_RECOVER);
		return -EINVAL;
	}

	ili_hid_report(raw_data, size, field);
...
static const struct hid_device_id mt_devices[] = {
	{
		HID_I2C_DEVICE(HID_VID_ILITEK_SCREEN, HID_PID1_ILITEK_SCREEN),
	},
	{
		HID_I2C_DEVICE(HID_VID_ILITEK_SCREEN, HID_PID2_ILITEK_SCREEN),
	},
	{}
};
...
static struct hid_driver ilitek_mt_driver = {
	.name = "hid-ilitek",
	.id_table = mt_devices,
	.probe = mt_probe,
	.remove = mt_remove,
...
module_hid_driver(ilitek_mt_driver);

MODULE_DESCRIPTION("ilitek HID multitouch panels");
MODULE_LICENSE("GPL");
