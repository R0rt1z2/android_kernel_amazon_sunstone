...
int ili_hid_fw_dump_iram_data(u32 start, u32 end, bool save, bool mcu)
{
	...
	ret = -1;
	debug_en = DEBUG_ALL;
	ili_hid_dump_data(fw_buf, 8, len, 0, "IRAM");
	debug_en = tmp;
#else
...
#endif
out:
	ili_hid_ice_mode_ctrl(DISABLE, mcu);
...
static int ilitek_tddi_flash_poll_busy(int timer)
{
	...
	do {
		if (ili_hid_ice_mode_write(FLASH_BASED_ADDR, 0x0, 1) < 0)
			ILI_ERR("Pull cs low failed\n");

		if (ili_hid_ice_mode_write(FLASH1_ADDR, 0x66aa55, 3) < 0)
			ILI_ERR("Write key failed\n");

		if (ili_hid_ice_mode_write(FLASH2_ADDR, cmd, 1) < 0)
			ILI_ERR("Write 0x5 cmd failed\n");

		if (ili_hid_ice_mode_write(FLASH2_ADDR, 0xFF, 1) < 0)
			ILI_ERR("Write dummy failed\n");

		mdelay(1);

		if (ili_hid_ice_mode_read(FLASH4_ADDR, &temp, sizeof(u8)) < 0)
...
void ili_hid_flash_clear_dma(void)
{
	if (ili_hid_ice_mode_bit_mask_write(FLASH0_ADDR, FLASH0_reg_preclk_sel,
					    (2 << 16)) < 0)
		ILI_ERR("Write %lu at %x failed\n", FLASH0_reg_preclk_sel,
			FLASH0_ADDR);

	if (ili_hid_ice_mode_bit_mask_write(FLASH4_ADDR,
					    FLASH4_reg_flash_dma_trigger_en,
					    (0 << 24)) < 0)
		ILI_ERR("Write %lu at %x failed\n",
			FLASH4_reg_flash_dma_trigger_en, FLASH4_ADDR);

	if (ili_hid_ice_mode_bit_mask_write(FLASH0_ADDR, FLASH0_reg_rx_dual,
					    (0 << 24)) < 0)
		ILI_ERR("Write %lu at %x failed\n", FLASH0_reg_rx_dual,
			FLASH0_ADDR);

	if (ili_hid_ice_mode_write(FLASH3_reg_rcv_cnt, 0x00, 1) < 0)
...
static int ilitek_tddi_flash_read_int_flag(void)
{
	...
	do {
		if (ili_hid_ice_mode_read(INTR1_ADDR & BIT(25), &data,
					  sizeof(u32)) < 0)
...
void ili_hid_flash_dma_write(u32 start, u32 end, u32 len)
{
	if (ili_hid_ice_mode_bit_mask_write(FLASH0_ADDR, FLASH0_reg_preclk_sel,
					    1 << 16) < 0)
		ILI_ERR("Write %lu at %x failed\n", FLASH0_reg_preclk_sel,
	...
	if (ili_hid_ice_mode_write(FLASH0_reg_flash_csb, 0x00, 1) < 0)
		ILI_ERR("Pull cs low failed\n");

	if (ili_hid_ice_mode_write(FLASH1_reg_flash_key1, 0x66aa55, 3) < 0)
		ILI_ERR("Write key failed\n");

	if (ili_hid_ice_mode_write(FLASH2_reg_tx_data, 0x0b, 1) < 0)
	...
	if (ili_hid_ice_mode_bit_mask_write(
		    INTR1_ADDR, INTR1_reg_flash_int_flag, (1 << 25)) < 0)
		ILI_ERR("Write %lu at %x failed\n", INTR1_reg_flash_int_flag,
			INTR1_ADDR);

	if (ili_hid_ice_mode_write(FLASH2_reg_tx_data, (start & 0xFF0000) >> 16, 1) < 0)
	...
	if (ili_hid_ice_mode_write(FLASH2_reg_tx_data, (start & 0x00FF00) >> 8,
				   1) < 0)
		ILI_ERR("Write %x at %x failed\n", (start & 0x00FF00) >> 8,
			FLASH2_reg_tx_data);
	...
	if (ili_hid_ice_mode_write(FLASH2_reg_tx_data, (start & 0x0000FF), 1) < 0)
		ILI_ERR("Write %x at %x failed\n", (start & 0x0000FF),
			FLASH2_reg_tx_data);
	...
	if (ili_hid_ice_mode_bit_mask_write(FLASH0_ADDR, FLASH0_reg_rx_dual, 0 << 24) < 0)
		ILI_ERR("Write %lu at %x failed\n", FLASH0_reg_rx_dual,
			FLASH0_ADDR);

	if (ili_hid_ice_mode_write(FLASH2_reg_tx_data, 0x00, 1) < 0)
		ILI_ERR("Write dummy failed\n");
	...
	if (ili_hid_ice_mode_write(FLASH3_reg_rcv_cnt, len, 4) < 0)
...
static void ilitek_tddi_flash_write_enable(void)
{
	...
	if (ili_hid_ice_mode_write(FLASH2_ADDR, 0x6, 1) < 0)
...
int ili_hid_fw_read_hw_crc(u32 start, u32 end, u32 *flash_crc)
{
	int retry = 100;
	u32 busy = 0;
	u32 write_len = end;
	...
	if (ili_hid_ice_mode_write(FLASH2_ADDR, 0x3b, 1) < 0)
		ILI_ERR("Write 0x3b failed\n");

	if (ili_hid_ice_mode_write(FLASH2_ADDR, (start & 0xFF0000) >> 16, 1) < 0)
		ILI_ERR("Write address failed\n");

	if (ili_hid_ice_mode_write(FLASH2_ADDR, (start & 0x00FF00) >> 8, 1) < 0)
		ILI_ERR("Write address failed\n");

	if (ili_hid_ice_mode_write(FLASH2_ADDR, (start & 0x0000FF), 1) < 0)
		ILI_ERR("Write address failed\n");

	if (ili_hid_ice_mode_write(0x041003, 0x01, 1) < 0)
		ILI_ERR("Write enable Dio_Rx_dual failed\n");

	if (ili_hid_ice_mode_write(FLASH2_ADDR, 0xFF, 1) < 0)
		ILI_ERR("Write dummy failed\n");

	if (ili_hid_ice_mode_write(0x04100C, write_len, 3) < 0)
		ILI_ERR("Write Set Receive count failed\n");

	if (ili_hid_ice_mode_write(0x048007, 0x02, 1) < 0)
		ILI_ERR("Write clearing int flag failed\n");

	if (ili_hid_ice_mode_write(0x041016, 0x00, 1) < 0)
		ILI_ERR("Write 0x0 at 0x041016 failed\n");

	if (ili_hid_ice_mode_write(0x041016, 0x01, 1) < 0)
		ILI_ERR("Write Checksum_En failed\n");

	if (ili_hid_ice_mode_write(FLASH4_ADDR, 0xFF, 1) < 0)
		ILI_ERR("Write start to receive failed\n");

	do {
		if (ili_hid_ice_mode_read(0x048007, &busy, sizeof(u8)) < 0)
	...
	if (ili_hid_ice_mode_write(0x041003, 0x0, 1) < 0)
		ILI_ERR("Write disable dio_Rx_dual failed\n");

	if (ili_hid_ice_mode_read(0x04101C, flash_crc, sizeof(u32)) < 0) {
...
static int ilitek_tddi_fw_read_flash_data(u32 start, u32 end, u8 *data, int len)
{
	...
	if (ili_hid_ice_mode_write(FLASH2_ADDR, 0x03, 1) < 0)
	...
	for (i = start, j = 0; i <= end; i++, j++) {
		...
		if (ili_hid_ice_mode_read(FLASH4_ADDR, &tmp, sizeof(u8)) < 0)
...
int ili_hid_fw_dump_flash_data(u32 start, u32 end, bool user, bool mcu)
{
...
#endif
	ret = ili_hid_ice_mode_ctrl(ENABLE, mcu);
	if (ret < 0)
		goto out;
	...
	debug_en = DEBUG_ALL;
	ili_hid_dump_data(buf, 8, len, 0, "flash");
	debug_en = tmp;
#else
	pos = 0;
#if LINUX_VERSION_CODE <= KERNEL_VERSION(4, 14, 0)
...
void ilitek_hid_tddi_flash_protect(bool enable, bool mcu)
{
	...
	if (ili_hid_ice_mode_write(FLASH_BASED_ADDR, 0x0, 1) < 0) //CS Low
		ILI_ERR("Write cs low failed\n");
	if (ili_hid_ice_mode_write(FLASH1_ADDR, 0x66aa55, 3) < 0)
		ILI_ERR("Write key failed\n");
	if (ili_hid_ice_mode_write(FLASH2_ADDR, 0x01, 1) < 0)
		ILI_ERR("Write Start_Write failed\n");

	if (enable) {
		if (ili_hid_ice_mode_write(FLASH2_ADDR, 0x5A, 1) < 0)
			ILI_ERR("Write Protect Low byte failed\n");
	} else {
		if (ili_hid_ice_mode_write(FLASH2_ADDR, 0x00, 1) < 0)
			ILI_ERR("Write Un-Protect Low byte failed\n");
	}

	switch (ilits->flash_mid) {
	case 0xEF: //W25 Flash
		if (ilits->flash_devid == 0x6012 ||
		    ilits->flash_devid == 0x6011) {
			if (enable) {
				if (ili_hid_ice_mode_write(FLASH2_ADDR, 0x7E, 1) < 0)
					ILI_ERR("Write Protect High byte failed\n");
			} else {
				if (ili_hid_ice_mode_write(FLASH2_ADDR, 0x0, 1) < 0)
					ILI_ERR("Write Un-Protect High byte failed\n");
			}
		}
		break;
	case 0xC8: //GD25 Flash
		if (ilits->flash_devid == 0x6013) {
			if (enable) {
				// do nothing
			} else {
				if (ili_hid_ice_mode_write(FLASH2_ADDR, 0x0, 1) < 0)
...
static int ilitek_fw_check_ddi_chunk(u8 *pfw)
{
	...
	if (ili_hid_ic_get_fw_ver() < 0) {
		ILI_INFO(
			"Failed to get fw ver, reading flash by dma instead\n");
		goto dma;
	}
	...
	cmd[6] = ili_hid_calc_packet_checksum(cmd, sizeof(cmd) - 1);
	ILI_INFO("Read customer reserved block: start addr = 0x%X, len = 0x%X ",
		 start, DDI_RSV_BK_SIZE);
	if (ilits->wrapper(cmd, sizeof(cmd), fw_flash_tmp, sizeof(fw_flash_tmp),
			   OFF, ON) < 0) {
		ILI_ERR("Failed to read customer reserved block data from fw\n");
		goto dma;
	}
	ipio_memcpy(&pfw[start], fw_flash_tmp + 6, DDI_RSV_BK_SIZE,
		    MAX_HEX_FILE_SIZE - start);
	...
	cmd[5] = (start >> 16) & 0xFF; //addr H byte
	cmd[6] = ili_hid_calc_packet_checksum(cmd, sizeof(cmd) - 1);
	ILI_INFO("Read mp data block: start addr = 0x%X, len = 0x%X ", start,
		 DDI_RSV_BK_SIZE);
	if (ilits->wrapper(cmd, sizeof(cmd), fw_flash_tmp, sizeof(fw_flash_tmp),
			   OFF, ON) < 0) {
...
static int ili_hid_fw_version_check(void)
{
	if (ilits->force_fw_update) {
		if (tfd.new_fw_cb != ilits->chip->fw_ver) {
			ILI_ERR("FW version not matched\n");
			return NEED_UPDATE;
		}
	} else {
#if (BOOT_FW_UPDATE_MODE == BOOT_FW_VER_UPGRADE)
		if (tfd.new_fw_cb > ilits->chip->fw_ver ||
		    tfd.new_mp_fw_cb > ilits->chip->fw_mp_ver) {
		...
		if (tfd.new_fw_cb != ilits->chip->fw_ver ||
		    tfd.new_mp_fw_cb != ilits->chip->fw_mp_ver) {
#endif
			ILI_INFO("fw version is not the latest, do FW Upgrade.\n");
			return  NEED_UPDATE;
		} else if (ilits->chip->fw_ver == 0x0 || ilits->chip->fw_mp_ver == 0x0) {
			ILI_ERR("flash FW damage, do FW Upgrade.\n");
			return NEED_UPDATE;
		} else {
...
static int ilitek_tddi_flash_fw_crc_check(u8 *pfw)
{
	int i = 0, crc_byte_len = 4;
	u8 flash_crc[4] = { 0 };
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		if (ilitek_tddi_fw_read_flash_data(end_addr - crc_byte_len + 1,
						   end_addr, flash_crc,
						   sizeof(flash_crc)) < 0) {
		...
		flash_crc_cb = flash_crc[0] << 24 | flash_crc[1] << 16 |
			       flash_crc[2] << 8 | flash_crc[3];

		if (ili_hid_fw_read_hw_crc(start_addr,
					   end_addr - start_addr -
						   crc_byte_len + 1,
					   &hw_crc) < 0) {
...
static int ilitek_tddi_fw_flash_program(u8 *pfw)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		ILI_INFO(
			"Block[%d]: Programing from (0x%x) to (0x%x), tfd.end_addr = 0x%x\n",
		...
		for (addr = fbi[i].start; addr < fbi[i].end; addr += page) {
			...
			if (ili_hid_ice_mode_write(FLASH2_ADDR, 0x2, 1) < 0)
				ILI_ERR("Write 0x2 failed\n");

			recv_addr = ((addr & 0xFF0000) >> 16) |
				    (addr & 0x00FF00) |
				    ((addr & 0x0000FF) << 16);
			if (ili_hid_ice_mode_write(FLASH2_ADDR, recv_addr, 3) < 0)
				ILI_ERR("Write address failed\n");
			...
			if (ilitek_tddi_flash_poll_busy(TIMEOUT_PROGRAM) < 0)
				return UPDATE_FAIL;
			if (ilits->fw_update_stat !=
			    ((j * 100) / tfd.end_addr)) {
				ilits->fw_update_stat =
					(j * 100) / tfd.end_addr;
				ILI_DBG("Program flahse data .... %d%c",
					ilits->fw_update_stat, '%');
			}
		}
	}
...
static int ilitek_tddi_fw_flash_erase(bool mcu)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		ILI_INFO("Block[%d]: Erasing from (0x%x) to (0x%x) \n", i,
			 fbi[i].start, fbi[i].end);

		for (addr = fbi[i].start; addr <= fbi[i].end;
		     addr += ilits->flash_sector) {
			ilitek_tddi_flash_write_enable();
			...
			if (addr == fbi[AP].start) {
				...
				if (ili_hid_ice_mode_write(FLASH2_ADDR, 0xD8, 1) < 0)
					ILI_ERR("Write 0xB at %x failed\n",
						FLASH2_ADDR);
				bk_erase = true;
			} else {
				/* sector erase with 4k bytes. */
				if (ili_hid_ice_mode_write(FLASH2_ADDR, 0x20, 1) < 0)
			...
			if (addr == fbi[AP].start)
				...
				ret = ilitek_tddi_flash_poll_busy(
					TIMEOUT_SECTOR);
...
{
	int ret = UPDATE_PASS;
	int ic_fw_valid = UPDATE_PASS;
	int fw_ver_need_updated = NO_NEED_UPDATE;
	bool hex_info = ilits->info_from_hex;

	if (ilits->cascade_info_block.nNum != 0) {
		if (ilits->reset == TP_HW_RST_ONLY) {
			ili_hid_wdt_reset_status(true, false);
	...
	ILI_INFO("New MP FW ver = 0x%x, Current MP FW ver = 0x%x\n",
		 tfd.new_mp_fw_cb, ilits->chip->fw_mp_ver);

	if (ilits->cascade_info_block.nNum != 0) {
		ili_hid_cascade_sync_ctrl(false);
		ret = ili_hid_ice_mode_ctrl_by_mode(ENABLE, mcu, BOTH);
	} else {
		ret = ili_hid_ice_mode_ctrl(ENABLE, mcu);
	}
	...
	ic_fw_valid = ilitek_tddi_flash_fw_crc_check(pfw);

	fw_ver_need_updated = ili_hid_fw_version_check();

	if ((ic_fw_valid == UPDATE_PASS) &&
	    (fw_ver_need_updated == NO_NEED_UPDATE)) {
		if (ilits->cascade_info_block.nNum != 0) {
			if (ili_hid_ice_mode_ctrl_by_mode(DISABLE, OFF, BOTH) < 0) {
				if (ilits->reset == TP_HW_RST_ONLY) {
					ili_hid_wdt_reset_status(false, true);
					if (ili_hid_reset_ctrl(ilits->reset) < 0) {
						ILI_ERR("TP reset failed during flash progam\n");
						return -EFW_REST;
					}
...
static void ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	ipio_memcpy(ilits->fw_mp_ver, pfw + fw_mp_ver_addr,
		    sizeof(ilits->fw_mp_ver), sizeof(ilits->fw_mp_ver));
	...
	tfd.new_fw_cb = (ilits->fw_info[48] << 24) |
			(ilits->fw_info[49] << 16) | (ilits->fw_info[50] << 8) |
			ilits->fw_info[51];
	...
	ilits->PenType = (ilits->chip->core_ver >= CORE_VER_1700) ?
	...
	ILI_INFO(
		"report_info_block : nReportByPixel = %d, nIsHostDownload = %d, nIsSPIICE = %d, nIsSPICLIENT = %d, nIsI2C = %d\n",
		ilits->rib.nReportByPixel, ilits->rib.nIsHostDownload,
		ilits->rib.nIsSPIICE, ilits->rib.nIsSPICLIENT,
		ilits->rib.nIsI2C);

	ILI_INFO(
		"report_info_block : nReserved00 = %d, nReportResolutionMode = %d, nCustomerType = %d, nReserved02 = %x,  nReserved03 = %x\n",
		ilits->rib.nReserved00, ilits->rib.nReportResolutionMode,
		ilits->rib.nCustomerType, ilits->rib.nReserved02,
	...
	ipio_memcpy(&ilits->pen_info_block, (pfw + fw_info_addr),
		    sizeof(ilits->pen_info_block),
		    sizeof(ilits->pen_info_block));

	ILI_INFO(
		"pen_info_block : nPxRaw = %d, nPyRaw = %d, nPxVa = %d,nPyVa = %d, nReserved0 = %d, nReserved01 = %d, nReserved2 = %d, nReserved03 = %d\n",
		ilits->pen_info_block.nPxRaw, ilits->pen_info_block.nPyRaw,
		ilits->pen_info_block.nPxVa, ilits->pen_info_block.nPyVa,
		ilits->pen_info_block.nReserved00,
		ilits->pen_info_block.nReserved01,
		ilits->pen_info_block.nReserved02,
	...
	ILI_INFO("Parsing cascade info block start addr = 0x%x\n",
		 fw_info_addr);
	ipio_memcpy(&ilits->cascade_info_block, (pfw + fw_info_addr),
		    sizeof(ilits->cascade_info_block),
		    sizeof(ilits->cascade_info_block));

	ILI_INFO(
		"cascade_info_block : nDisable = %d, nNum = %d, nReserved0 = %d, nReserved1 = %d, nReserved2 = %d, nReserved03 = %d\n",
...
static int ilitek_tddi_fw_ili_convert(u8 *pfw)
{
	...
	if (size < ILI_FILE_HEADER ||
	    size > (MAX_HEX_FILE_SIZE + ILI_FILE_HEADER)) {
		ILI_ERR("size of ILI file is invalid\n");
		return -EINVAL;
	}

	/* Check if it's old version of ILI format. */
	if (CTPM_FW[22] == 0xFF && CTPM_FW[23] == 0xFF && CTPM_FW[24] == 0xFF &&
	    CTPM_FW[25] == 0xFF) {
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		if (((blk_map >> i) & 0x01) == 0x01) {
			...
			ILI_INFO(
				"Block[%d]: start_addr = %x, end = %x, fix_mem_start = 0x%x\n",
...
static int ilitek_tddi_fw_hex_convert(u8 *phex, int size, u8 *pfw)
{
	...
	u32 offset;
	char signstr[HEX_SIGN_KEY_LEN_MAX] = { 0 };
	...
	for (; i < size;) {
		...
		if (type == 0x04) {
			...
			fbi[num].fix_mem_start = INT_MAX;
			fbi[num].len = fbi[num].end - fbi[num].start + 1;
			ILI_INFO("Block[%d]: start_addr = %x, end = %x", num,
				 fbi[num].start, fbi[num].end);
		...
		} else if (type == BLOCK_TAG_SIGN) {
			/* checking hex file signature */
			for (j = 0, k = 0; (j < (len * 2) &&
			     k < (HEX_SIGN_KEY_LEN_MAX - 1));
			     j += 2, k++)
				signstr[k] = HexToDec(&phex[i + 9 + j], 2);
			signstr[k] = '\0';
			ILI_DBG("hex sign = [%s]\n", signstr);
			if (strcmp(signstr, HEX_SIGN_KEY) == 0) {
				ILI_INFO("hex file sign key is correct\n");
				ilits->sign_verify_pass_flag = true;
			} else {
				ILI_ERR("hex sign key is error(%s)\n", signstr);
				ilits->sign_verify_pass_flag = false;
			}
		}
...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
	...
	case REQUEST_FIRMWARE:
		if (request_firmware(&fw, ilits->md_fw_rq_path, ilits->dev) <
		    0) {
			ILI_ERR("Request firmware failed, try again\n");
			if (request_firmware(&fw, ilits->md_fw_rq_path,
					     ilits->dev) < 0) {
		...
		ipio_memcpy((u8 *)ilits->tp_fw.data, fw->data,
			    fsize * sizeof(*fw->data), fsize);
	...
	if (ilitek_tddi_fw_hex_convert((u8 *)ilits->tp_fw.data,
				       ilits->tp_fw.size, pfw) < 0) {
...
#if (BOOT_FW_UPDATE_MODE == BOOT_FW_VER_DOWNGRADE)
		if ((tfd.new_fw_cb & 0x0000FFFF) <= SIGN_HEX_FW_VER ||
		    (tfd.new_mp_fw_cb & 0x0000FFFF) >= SIGN_HEX_FW_MP_VER) {
#else
		if ((tfd.new_fw_cb & 0x0000FFFF) >= SIGN_HEX_FW_VER ||
		    (tfd.new_mp_fw_cb & 0x0000FFFF) >= SIGN_HEX_FW_MP_VER) {
#endif
			if (ilits->sign_verify_pass_flag == false) {
				ILI_ERR("checked hex file sign fail, abort it.\n");
				ipio_vfree((void **)&pfw);
				ret = -EFW_HEXSIGH;
				goto fail;
			} else {
				ILI_INFO("checked hex file sign PASS\n");
			}
		} else {
			ILI_INFO("no need check hex file sign\n");
		}

		if (ilits->chip->core_ver >= CORE_VER_1470 &&
		    ilits->rib.nIsHostDownload == 1) {
	...
	if (!ilits->eng_flow) {
		...
		if (ilits->cascade_info_block.nNum != 0) {
			if (ilits->reset == TP_HW_RST_ONLY) {
				...
				ili_hid_cascade_reset_ctrl(ilits->reset, true);
			}
		} else {
			ili_hid_reset_ctrl(ilits->reset);
		}
	...
	if (ret != UPDATE_PASS) {
		...
		if (ilits->cascade_info_block.nNum != 0) {
			if (ilits->reset == TP_HW_RST_ONLY) {
				...
				if (ili_hid_cascade_reset_ctrl(ilits->reset,
							       true) < 0)
					ILI_ERR("TP reset failed after erase flash\n");
			}
		} else {
...
fail:
	if (ilits->cascade_info_block.nNum != 0) {
		if (ilits->reset == TP_HW_RST_ONLY) {
			ili_hid_wdt_reset_status(true, true);
			if (ili_hid_reset_ctrl(ilits->reset) < 0)
				ILI_ERR("TP reset for fail case\n");
		} else {
			if (ili_hid_cascade_reset_ctrl(ilits->reset, true) < 0)
				ILI_ERR("TP reset for fail case\n");
		}
		...
		if (ili_hid_reset_ctrl(ilits->reset) < 0)
			ILI_ERR("TP reset failed for fail case\n");
	}
...
void ili_hid_fw_read_flash_info(bool mcu)
{
	int i = 0;
	u8 buf[3] = { 0 };
	u8 cmd = 0x9F;
	...
	if (ili_hid_ice_mode_write(FLASH2_ADDR, cmd, 1) < 0) //Read JEDEC ID
	...
	if (ili_hid_ice_mode_write(FLASH_BASED_ADDR, 0x1, 1) < 0) //CS High
		ILI_ERR("Write cs high failed\n");
	...
	for (i = 0; i < ARRAY_SIZE(flash_t); i++) {
		if (flash_mid == flash_t[i].mid &&
		    flash_id == flash_t[i].dev_id) {
