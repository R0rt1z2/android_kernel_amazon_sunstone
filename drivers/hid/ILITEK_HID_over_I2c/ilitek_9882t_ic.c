...
static int ili_check_polling_bist(char *pstrArea, u16 SramRsultStatus)
{
	...
	ili_hid_ice_mode_write(0x46000, 0, 4);

	ili_hid_ice_mode_read(0x04600C, &bist_done, 1);
	ili_hid_ice_mode_read(0x04600D, &bist_fail, 1);
	ili_hid_ice_mode_read(0x046010, &bist_fail_bus, sizeof(bist_fail_bus));

	/* Check Clear MBist Status */
	if (check_failbus) {
		if (((bist_done & BIT(0)) != 0x00) ||
		    ((bist_fail & BIT(0)) != 0x01) ||
		    (bist_fail_bus != 0xFFFFFFFF)) {
	...
	ili_hid_ice_mode_write(0x046000, 0x00000001, 4);
	ili_hid_ice_mode_write(0x046000, 0x00000101, 4);
	...
	ili_hid_ice_mode_read(0x04600D, &bist_fail, 1);
	ili_hid_ice_mode_read(0x046010, &inbist_failbus,
			      sizeof(inbist_failbus));

	if (ilits->chip->id == ILI9883_CHIP) {
		ili_hid_ice_mode_read(0x046040, &Bist_Sram_Fail, 2);

		/* S6 S3 */
		if (pstrArea != NULL) {
			if (strcmp(pstrArea, "S3.S6") == 0) {
				if ((bist_fail & BIT(0)) != 0x00) {
					ILI_ERR("Check Bist Fail Flag Fail\n");
					ILI_ERR("bist_fail = 0x%X\n", bist_fail);
					ret = -1;
					goto out;
				}
			}
		} else {
			ILI_ERR("pstrArea is NULL\n");
		}
		...
		if (((bist_fail & BIT(0)) != 0x00) ||
		    (inbist_failbus != 0x00000000)) {
			ILI_ERR("Check Bist Fail Flag Fail\n");
			ILI_DBG("bist_fail = %d, inbist_failbus = %d\n",
				bist_fail, inbist_failbus);
			ret = -1;
			goto out;
		}
	}
...
#define SRAM_PARTIAL_AREA_TEST_LEN 3
static int ili_sram_by_partial_area_test(void)
{
	int ret = 0;
	u32 MinAddr = 0, MaxAddr = 0x1FFF, SetValue;
	u8 nSramNumIdx;
	char *strSramPartialArea[3] = { "S1.S5.S8.S10", "S2.S4.S7.S9",
					"S3.S6" };
	u32 SramPartialAreaTest_en[SRAM_PARTIAL_AREA_TEST_LEN] = { 0xA0201,
								   0x50102, 0x404 };
	u16 SramResult[SRAM_PARTIAL_AREA_TEST_LEN] = {
		BIT(0) | BIT(4) | BIT(7) | BIT(9),
		BIT(1) | BIT(3) | BIT(6) | BIT(8), BIT(2) | BIT(5)
	};

	for (nSramNumIdx = 0; nSramNumIdx < SRAM_PARTIAL_AREA_TEST_LEN;
	     nSramNumIdx++) {
		ILI_DBG("Sram Partial Area : %s\n",
			strSramPartialArea[nSramNumIdx]);

		ili_hid_ice_mode_write(0x4603C,
				       SramPartialAreaTest_en[nSramNumIdx], 4);
		ILI_DBG("Write Sram Partial Area En : 0x%X\n",
			SramPartialAreaTest_en[nSramNumIdx]);

		/* Strat Sram Test */
		ili_hid_ice_mode_write(0x40004, 0x00000001, 4);
		ili_hid_ice_mode_write(0x40010, 0x00009878, 4);
		ili_hid_ice_mode_write(0x40088, 0x00006B8A, 4);

		/* Set MinAddr */
		ili_hid_ice_mode_write(0x46038, MinAddr, 2);

		/* March_C */
		SetValue = ((MaxAddr << 16) & 0xFFFF0000) | 0xAA55;

		ili_hid_ice_mode_write(0x46000, 0x00010000, 4);
		ili_hid_ice_mode_write(0x46004, SetValue, 4);
		ili_hid_ice_mode_write(0x46014, 0xD4609800, 4);
		ili_hid_ice_mode_write(0x46020, 0xC460C480, 4);
		ili_hid_ice_mode_write(0x46018, 0xB440D480, 4);
		ili_hid_ice_mode_write(0x4601C, 0x00009080, 4);

		if (ili_check_polling_bist(strSramPartialArea[nSramNumIdx],
					   SramResult[nSramNumIdx]) < 0) {
			ILI_ERR("Check March_C Bist Fail during partial area test\n");
			ret = -1;
			goto out;
		}
		...
		ili_hid_ice_mode_write(0x46000, 0x00010000, 4);
		ili_hid_ice_mode_write(0x46004, SetValue, 4);
		ili_hid_ice_mode_write(0x46014, 0xA4409800, 4);
		ili_hid_ice_mode_write(0x46018, 0xB480F590, 4);
		ili_hid_ice_mode_write(0x4601C, 0x9000F560, 4);
		ili_hid_ice_mode_write(0x46020, 0x9280C460, 4);
		ili_hid_ice_mode_write(0x46024, 0x0000, 2);
		...
		ili_hid_ice_mode_write(0x46000, 0x00010000, 4);
		ili_hid_ice_mode_write(0x46004, SetValue, 4);
		ili_hid_ice_mode_write(0x46014, 0xFA309880, 4);
		ili_hid_ice_mode_write(0x46018, 0xB4800000, 4);

		if (ili_check_polling_bist(strSramPartialArea[nSramNumIdx],
					   SramResult[nSramNumIdx]) < 0) {
			ILI_ERR("Check March_WOM1 Bist Fail during partial area test\n");
			ret = -1;
			goto out;
		}

		/* March_WOM2 */
		SetValue = ((MaxAddr << 16) & 0xFFFF0000) | 0xCC33;
		...
		if (ili_check_polling_bist(strSramPartialArea[nSramNumIdx],
					   SramResult[nSramNumIdx]) < 0) {
			ILI_ERR("Check March_WOM2 Bist Fail during partial area test\n");
			ret = -1;
			goto out;
		}

		/* March_WOM3 */
		SetValue = ((MaxAddr << 16) & 0xFFFF0000) | 0xF00F;
		...
		if (ili_check_polling_bist(strSramPartialArea[nSramNumIdx],
					   SramResult[nSramNumIdx]) < 0) {
			ILI_ERR("Check March_WOM3 Bist Fail during partial area test\n");
			ret = -1;
			goto out;
		}
		...
		ili_hid_ice_mode_write(0x46000, 0x00010000, 4);
		ili_hid_ice_mode_write(0x46004, SetValue, 4);
		ili_hid_ice_mode_write(0x46014, 0xFA349884, 4);
		ili_hid_ice_mode_write(0x46018, 0xB4800000, 4);

		if (ili_check_polling_bist(strSramPartialArea[nSramNumIdx],
					   SramResult[nSramNumIdx]) < 0) {
			ILI_ERR("Check March_WOM4 Bist Fail during partial area test\n");
			ret = -1;
			goto out;
		}
	}
...
int ili_hid_tddi_ic_sram_test(void)
{
	int retry = 10, ret = 0;
	u32 bist_done = 0, bist_fail = 0, bist_fail_bus = 0, inbist_failbus = 0;
	u32 bist_client_done = 0, bist_client_fail = 0, bist_client_fail_bus = 0,
	    client_inbist_failbus = 0;
	bool check_failbus = true;

	atomic_set(&ilits->mp_stat, START);

	ili_hid_ice_mode_write_by_mode(0x040004, 0x01, 1, BOTH);
	ili_hid_ice_mode_write_by_mode(0x040010, 0x9878, 2, BOTH);
	ili_hid_ice_mode_write_by_mode(0x047003, 0x00, 1, BOTH);
	ili_hid_ice_mode_write_by_mode(0x04001C, 0x01, 1, BOTH);
	ili_hid_ice_mode_write_by_mode(0x071001, 0x02, 1, BOTH);
	ili_hid_ice_mode_write_by_mode(0x071005, 0x3C, 1, BOTH);
	ili_hid_ice_mode_write_by_mode(0x040088, 0x6B8A, 2, BOTH);
	ili_hid_ice_mode_write_by_mode(0x046003, 0x00, 1, BOTH);
	ili_hid_ice_mode_write_by_mode(0x046000, 0x010001, 3, BOTH);
	ili_hid_ice_mode_write_by_mode(0x046002, 0x00, 1, BOTH);

	switch (ilits->chip->id) {
	case ILI9881_CHIP:
		if (ilits->chip->type == 0x1D) {
			/* 9881T */
			ili_hid_ice_mode_write_by_mode(0x046004, 0x27FFFF00, 4, BOTH);
		} else {
			ili_hid_ice_mode_write_by_mode(0x046004, 0x1FFFFF00, 4, BOTH);
		}
		break;
	case ILI9882_CHIP:
		ili_hid_ice_mode_write_by_mode(0x046004, 0x1FFFFF00, 4, BOTH);
	...
	case ILI7807_CHIP:
		if (ilits->chip->type == 0x1F) {
			/* 7807V */
			ili_hid_ice_mode_write_by_mode(0x046004, 0x0FFFFF00, 4, BOTH);
		} else {
			ili_hid_ice_mode_write_by_mode(0x046004, 0x1FFFFF00, 4, BOTH);
		}
		break;
	...
	ili_hid_ice_mode_write_by_mode(0x046014, 0xA4409800, 4, BOTH);
	ili_hid_ice_mode_write_by_mode(0x046018, 0xB480F590, 4, BOTH);
	ili_hid_ice_mode_write_by_mode(0x04601C, 0x9000F560, 4, BOTH);
	ili_hid_ice_mode_write_by_mode(0x046020, 0x9280C460, 4, BOTH);
	ili_hid_ice_mode_write_by_mode(0x046024, 0x0000, 2, BOTH);

	ili_hid_ice_mode_write_by_mode(0x046000, 0x000000, 4, BOTH);

	if (ilits->cascade_info_block.nNum != 0) {
#if (TDDI_INTERFACE == BUS_I2C)
		ilits->i2c_addr_change = MASTER;
		ili_hid_ice_mode_read(0x04600C, &bist_done, sizeof(bist_done));
		ili_hid_ice_mode_read(0x04600D, &bist_fail, sizeof(bist_fail));
		ili_hid_ice_mode_read(0x046010, &bist_fail_bus,
				      sizeof(bist_fail_bus));

		ILI_ERR("bist_done = 0x%X, bist_fail = 0x%X, bist_fail_bus = 0x%X\n",
			bist_done, bist_fail, bist_fail_bus);

		ilits->i2c_addr_change = CLIENT;
		ili_hid_ice_mode_read(0x04600C, &bist_client_done,
				      sizeof(bist_client_done));
		ili_hid_ice_mode_read(0x04600D, &bist_client_fail,
				      sizeof(bist_client_fail));
		ili_hid_ice_mode_read(0x046010, &bist_client_fail_bus,
				      sizeof(bist_client_fail_bus));
		ILI_ERR("bist_client_done = 0x%X, bist_client_fail = 0x%X, bist_client_fail_bus = 0x%X\n",
			bist_client_done, bist_client_fail, bist_client_fail_bus);

		if (check_failbus) {
			if (((bist_done & BIT(0)) != 0x00) ||
			    ((bist_fail & BIT(0)) != 0x01) ||
			    (bist_fail_bus != 0xFFFFFFFF) ||
			    ((bist_client_done & BIT(0)) != 0x00) ||
			    ((bist_client_fail & BIT(0)) != 0x01) ||
			    (bist_client_fail_bus != 0xFFFFFFFF)) {
				ILI_DBG("SRAM TEST FAIL\n");
				ret = -1;
				goto out;
			}
		...
		ili_hid_ice_mode_write_by_mode(0x046000, 0x00000101, 4, BOTH);
		ili_hid_ice_mode_write_by_mode(0x046000, 0x00000001, 4, BOTH);

		while (retry > 0) {
			mdelay(10);
			ilits->i2c_addr_change = MASTER;
			ili_hid_ice_mode_read(0x04600C, &bist_done,
					      sizeof(bist_done));

			ilits->i2c_addr_change = CLIENT;
			ili_hid_ice_mode_read(0x04600C, &bist_client_done,
					      sizeof(bist_client_done));
			ILI_ERR("bist_done = 0x%X, bist_client_done = 0x%X\n",
				bist_done, bist_client_done);

			if (((bist_done & BIT(0)) == 0x01) &&
			    ((bist_client_done & BIT(0)) == 0x01)) {
				break;
			}
			retry--;
		}
		...
		ili_hid_ice_mode_read(0x046010, &inbist_failbus,
				      sizeof(inbist_failbus));
		ILI_ERR("bist_fail = 0x%X, inbist_failbus = 0x%X\n", bist_fail,
			inbist_failbus);

		ilits->i2c_addr_change = CLIENT;
		ili_hid_ice_mode_read(0x04600D, &bist_client_fail,
				      sizeof(bist_client_fail));
		ili_hid_ice_mode_read(0x046010, &client_inbist_failbus,
				      sizeof(client_inbist_failbus));
		ILI_ERR("bist_client_fail = 0x%X, client_inbist_failbus = 0x%X\n",
			bist_client_fail, client_inbist_failbus);

		if (((bist_fail & BIT(0)) != 0x00) ||
		    (inbist_failbus != 0x00000000) ||
		    (((bist_client_fail & BIT(24)) >> 24) != 0x00) ||
		    (client_inbist_failbus != 0x00000000)) {
			ILI_DBG("SRAM TEST FAIL\n");
			ret = -1;
			goto out;
		}
		...
		ili_hid_ice_mode_write(READ_BACK_LOCK_REG, 0x01, 1);
		ili_hid_ice_mode_read(0x04600C, &bist_done, sizeof(bist_done));
		ili_hid_ice_mode_read(CLIENT_READ_BACK_WORD_REG,
				      &bist_client_done,
				      sizeof(bist_client_done));
		...
		if (check_failbus) {
			if (((bist_done & BIT(0)) != 0x00) ||
			    ((bist_fail & BIT(0)) != 0x01) ||
			    (bist_fail_bus != 0xFFFFFFFF) ||
			    (((bist_client_done & BIT(24)) >> 24) != 0x00) ||
			    (((bist_client_fail & BIT(24)) >> 24) != 0x01) ||
			    (bist_client_fail_bus != 0xFFFFFFFF)) {
				ILI_DBG("SRAM TEST FAIL\n");
				ret = -1;
				goto out;
		...
		while (retry > 0) {
			...
			if (((bist_done & BIT(0)) == 0x01) &&
			    (((bist_client_done & BIT(24)) >> 24) == 0x01)) {
				break;
			}
			retry--;
		}
		...
		ili_check_polling_bist(NULL, false);
	}
...
out:
	...
	if (ilits->cascade_info_block.nNum != 0) {
		ili_hid_core_spi_setup(SPI_CLK);
	}
#endif
	atomic_set(&ilits->mp_stat, END);

	return ret;
}

#ifdef ROI
int ili_hid_config_knuckle_roi_ctrl(cmd_types cmd)
{
	int ret = 0;
	switch (cmd) {
	case CMD_DISABLE:
		ILI_DBG("knuckle roi disbale\n");
		ret = ili_hid_ic_func_ctrl("knuckle", cmd);
		break;
	case CMD_ENABLE:
...
int ili_hid_config_get_knuckle_roi_status(void)
{
	int ret = 0;
	u8 roi_status = 0;

	ret = ili_hid_config_knuckle_roi_ctrl(CMD_STATUS);
	if (ret) {
...
int ili_hid_ice_mode_bit_mask_write(u32 addr, u32 mask, u32 value)
{
	int ret = 0;
	u32 data = 0;

	if (ili_hid_ice_mode_read(addr, &data, sizeof(u32)) < 0) {
	...
	ret = ili_hid_ice_mode_write(addr, data, sizeof(u32));
	if (ret < 0)
...
int ili_hid_ice_mode_write(u32 addr, u32 data, int len)
{
	int ret = 0, i;
	u8 txbuf[64] = { 0 };
...
int ili_hid_ice_mode_read(u32 addr, u32 *data, int len)
{
	int ret = 0;
	u8 rxbuf[4] = { 0 };
	u8 txbuf[4] = { 0 };
...
int ili_hid_ice_mode_ctrl(bool enable, bool mcu)
{
	int ret = 0;
	...
	if (enable) {
		...
		if (ilits->cascade_ctrl_mode == BOTH) {
			if (ilits->i2c_addr_change == CLIENT)
		...
		if (ilits->ice_flash_cs_ctrl && !mcu) {
			if (ili_hid_ice_mode_write(FLASH_BASED_ADDR, 0x1, 1) < 0)
				ILI_ERR("Write cs high failed\n"); /* CS high */
			if (ili_hid_ice_mode_write(FLASH0_dual_mode, 0x0, 1) < 0)
...
#if (TDDI_INTERFACE == BUS_I2C && ENABLE_CASCADE)
			if (ilits->cascade_ctrl_mode == BOTH) {
				if (ilits->i2c_addr_change == MASTER)
...
void ili_hid_cascade_sync_ctrl(bool mode)
{
	bool stop = false;
	u8 backup_addr = MASTER;
	u8 cmd[2] = { 0 };
	...
	if (mode == stop) {
		...
		cmd[0] = 0x10;
		cmd[1] = 0x11;
		if (atomic_read(&ilits->ice_stat) == ENABLE) {
			if (ilits->wrapper(cmd, sizeof(cmd), NULL, 0, OFF, OFF) < 0) {
			ILI_ERR("Failed to start Sync Cascade\n");
			} else {
				ILI_INFO("Start Sync Cascade\n");
			}
		}
	}
...
int ili_hid_ice_mode_ctrl_by_mode(bool enable, bool mcu, int mode)
{
	int ret = 0;
#if (TDDI_INTERFACE == BUS_I2C && ENABLE_CASCADE)
	...
	case BOTH:
		ilits->i2c_addr_change = CLIENT;
		if (ili_hid_ice_mode_ctrl(enable, mcu) < 0) {
			ILI_ERR("Failed to write Client ice mode\n");
			ret = -1;
		} else {
			ILI_DBG("Client %s ice mode\n",
				enable ? "Enable" : "Disable");
		}
		if (mode == CLIENT) {
			ilits->i2c_addr_change = MASTER;
...
int ili_hid_ice_mode_write_by_mode(u32 addr, u32 data, int len, int mode)
{
	int ret = 1;
#if (TDDI_INTERFACE == BUS_I2C && ENABLE_CASCADE)
	switch (mode) {
	...
	case BOTH:
		ilits->i2c_addr_change = CLIENT;
		if (ili_hid_ice_mode_write(addr, data, len) < 0) {
			ILI_ERR("Failed to write Client, addr: 0x%X\n", addr);
			ret = -1;
		}
		if (mode == CLIENT) {
			ilits->i2c_addr_change = MASTER;
...
int ili_hid_ice_mode_read_by_mode(u32 addr, u32 *data, int len, int mode)
{
	int ret = 1;
#if (TDDI_INTERFACE == BUS_I2C && ENABLE_CASCADE)
	switch (mode) {
	...
	case BOTH:
		ilits->i2c_addr_change = CLIENT;
		if (ili_hid_ice_mode_read(addr, data, len) < 0) {
...
{
	if (ili_hid_ice_mode_write(MSPI_REG, 0x01, 1) < 0)
		ILI_ERR("Write MSPI_REG failed\n");

	if (ili_hid_ice_mode_write(MSPI_TRIG_BY_DMA_ADDR, 0x101, 2) < 0)
		ILI_ERR("Write MSPI_TRIG_BY_DMA_ADDR failed\n");

	if (ili_hid_ice_mode_write(MSPI_CLOCK_ADDR, 0x02, 1) < 0)
		ILI_ERR("Write MSPI_CLOCK_ADDR failed\n");

	if (ili_hid_ice_mode_write(MSPI_CS_ADDR, 0x01, 1) < 0)
		ILI_ERR("Write MSPI_CS_ADDR failed\n");
}

static void ili_dma_write_setting(void)
{
	if (ili_hid_ice_mode_write(DMA_ONE_CLEAR_ADDR, 0x02, 1) < 0)
		ILI_ERR("Write DMA_ONE_CLEAR_ADDR failed\n");

	if (ili_hid_ice_mode_write(SRC_ONE_ADDR, 0x308D0, 3) < 0)
		ILI_ERR("Write SRC_ONE_ADDR failed\n");

	if (ili_hid_ice_mode_write(SRC_ONE_SET_ADDR, 0x82000004, sizeof(u32)) < 0)
		ILI_ERR("Write SRC_ONE_SET_ADDR failed\n");

	if (ili_hid_ice_mode_write(SRC_TWO_EN_ADDR, 0x00, 1) < 0)
		ILI_ERR("Write SRC_TWO_EN_ADDR failed\n");

	if (ili_hid_ice_mode_write(DEST_ADDR, 0x6205C, 3) < 0)
		ILI_ERR("Write DEST_ADDR failed\n");

	if (ili_hid_ice_mode_write(DEST_SET_ADDR, 0x82000000, sizeof(u32)) < 0)
		ILI_ERR("Write DEST_SET_ADDR failed\n");

	if (ili_hid_ice_mode_write(TRIG_SEL_MSPI_ADDR, 0x0D, 1) < 0)
		ILI_ERR("Write TRIG_SEL_MSPI_ADDR failed\n");

	if (ili_hid_ice_mode_write(DMA_ONE_GROUP_ADDR, 0x00, 1) < 0)
...
static void ili_dma_read_setting(void)
{
	...
	if (ili_hid_ice_mode_write(SRC_ONE_ADDR, 0x62060, 3) < 0)
		ILI_ERR("Write SRC_ONE_ADDR failed\n");

	if (ili_hid_ice_mode_write(SRC_ONE_SET_ADDR, 0x80000000, sizeof(u32)) < 0)
		ILI_ERR("Write SRC_ONE_SET_ADDR failed\n");

	if (ili_hid_ice_mode_write(SRC_TWO_EN_ADDR, 0x00, 1) < 0)
		ILI_ERR("Write SRC_TWO_EN_ADDR failed\n");

	if (ili_hid_ice_mode_write(DEST_ADDR, 0x308D0, 3) < 0)
		ILI_ERR("Write DEST_ADDR failed\n");

	if (ili_hid_ice_mode_write(DEST_SET_ADDR, 0x80000001, sizeof(u32)) < 0)
		ILI_ERR("Write DEST_SET_ADDR failed\n");

	if (ili_hid_ice_mode_write(TRIG_SEL_MSPI_ADDR, 0x0E, 1) < 0)
		ILI_ERR("Write DEST_SET_ADDR failed\n");
...
static void ili_trigger_mspi(void)
{
	...
	if (ili_hid_ice_mode_write(MSPI_CS_ADDR, 0x00, 1) < 0)
		ILI_ERR("Write MSPI_CS_ADDR failed\n");

	if (ili_hid_ice_mode_write(MSPI_REG, 0x0F, 1) < 0)
		ILI_ERR("Write MSPI_REG failed\n");

	if (ili_hid_ice_mode_write(MSPI_TRIGGER_ADDR, 0x01, 1) < 0)
		ILI_ERR("Write WRITE_BUF_ADDR failed\n");

	//Polling Done Flag
	while (BusyCnt > 0) {
		if (ili_hid_ice_mode_read(MSPI_DONE_FLAG_ADDR, &BusyData, 1) < 0)
			ILI_ERR("Read MSPI_DONE_FLAG_ADDR failed\n");

		if ((BusyData & BIT(0)) == 0x01) {
			ILI_DBG("MSPI Done : 0x%X\n", BusyData);
			break;
		}
	...
	if (ili_hid_ice_mode_write(MSPI_DONE_FLAG_ADDR, 0x01, 1) < 0)
		ILI_ERR("Read MSPI_DONE_FLAG_ADDR failed\n");
...
void ili_hid_mspi_set_client_to_qual_mode(void)
{
	u32 read_data, backup_mspi_init_en;

	ili_mspi_init();

	if (ili_hid_ice_mode_read(MSPI_INIT_EN_ADDR, &backup_mspi_init_en, 1) < 0)
		ILI_ERR("Read MSPI_INIT_EN_ADDR failed\n");

	if (ili_hid_ice_mode_write(MSPI_INIT_EN_ADDR,
				   backup_mspi_init_en & 0xFE, 1) < 0)
		ILI_ERR("Write MSPI_INIT_EN_ADDR failed\n");

	ili_dma_write_setting();

	//Set Client to Qual Mode
	if (ili_hid_ice_mode_read(TRANSFER_CNT_ADDR, &read_data, sizeof(u32)) < 0)
		ILI_ERR("Read TRANSFER_CNT_ADDR failed\n");

	read_data = (read_data & 0xFFFC0000) + 0x18;

	if (ili_hid_ice_mode_write(TRANSFER_CNT_ADDR, read_data, sizeof(u32)) < 0)
		ILI_ERR("Write TRANSFER_CNT_ADDR failed\n");

	if (ili_hid_ice_mode_read(WRITE_BUF_SIZE_ADDR, &read_data, 2) < 0)
		ILI_ERR("Read WRITE_BUF_SIZE_ADDR failed\n");

	read_data = (read_data & 0xF800) + 0x18;

	if (ili_hid_ice_mode_write(WRITE_BUF_SIZE_ADDR, read_data, 2) < 0)
		ILI_ERR("Write WRITE_BUF_SIZE_ADDR failed\n");

	if (ili_hid_ice_mode_read(READ_BUF_SIZE_ADDR, &read_data, 2) < 0)
		ILI_ERR("Read READ_BUF_SIZE_ADDR failed\n");

	read_data = (read_data & 0xF800) + 0x00;

	if (ili_hid_ice_mode_write(READ_BUF_SIZE_ADDR, read_data, 2) < 0)
		ILI_ERR("Write READ_BUF_SIZE_ADDR failed\n");

	if (ili_hid_ice_mode_write(WRITE_BUF_ADDR, 0x40000040, sizeof(u32)) < 0)
		ILI_ERR("Write WRITE_BUF_ADDR failed\n");

	if (ili_hid_ice_mode_write(0x308D0, 0x4044000, sizeof(u32)) < 0)
		ILI_ERR("Write failed\n");

	if (ili_hid_ice_mode_write(0x308D4, 0x4000, sizeof(u32)) < 0)
		ILI_ERR("Write failed\n");

	if (ili_hid_ice_mode_write(0x308D8, 0x4000, sizeof(u32)) < 0)
		ILI_ERR("Write failed\n");

	if (ili_hid_ice_mode_write(0x308DC, 0x40040000, sizeof(u32)) < 0)
		ILI_ERR("Write failed\n");

	if (ili_hid_ice_mode_write(0x308E0, 0x40000000, sizeof(u32)) < 0)
		ILI_ERR("Write failed\n");

	ili_trigger_mspi();

	//Recovery Mspi int en
	if (ili_hid_ice_mode_write(MSPI_INIT_EN_ADDR, backup_mspi_init_en, 1) < 0)
		ILI_ERR("Write MSPI_INIT_EN_ADDR failed\n");

	ILI_INFO("Backup MSPI Init EN : 0x%X\n", backup_mspi_init_en);
}

int ili_hid_mspi_write_client_register(u32 addr, u32 data, int len)
{
	int ret = 0;
	...
	if (ili_hid_ice_mode_read(TRANSFER_CNT_ADDR, &write_data, sizeof(u32)) < 0)
		ILI_ERR("Read TRANSFER_CNT_ADDR failed\n");

	write_data = (write_data & 0xFFFC0000) + 5 + len;

	if (ili_hid_ice_mode_write(TRANSFER_CNT_ADDR, write_data, sizeof(u32)) < 0)
		ILI_ERR("Write TRANSFER_CNT_ADDR failed\n");

	if (ili_hid_ice_mode_read(WRITE_BUF_SIZE_ADDR, &write_data, 2) < 0)
		ILI_ERR("Read WRITE_BUF_SIZE_ADDR failed\n");

	write_data = (write_data & 0xF800) + 5 + len;

	if (ili_hid_ice_mode_write(WRITE_BUF_SIZE_ADDR, write_data, 2) < 0)
		ILI_ERR("Write WRITE_BUF_SIZE_ADDR failed\n");

	if (ili_hid_ice_mode_read(READ_BUF_SIZE_ADDR, &write_data, 2) < 0)
		ILI_ERR("Read READ_BUF_SIZE_ADDR failed\n");

	write_data = write_data & 0xF800;

	if (ili_hid_ice_mode_write(READ_BUF_SIZE_ADDR, write_data, 2) < 0)
		ILI_ERR("Write READ_BUF_SIZE_ADDR failed\n");

	if (addr == CONTROL_CLIENT_EXIT_ICE_ADDR) {
		write_data = ((addr & 0xFF00) << 16) + ((addr & 0xFF) << 16) +
			     (0x1B << 8) + 0x82;
	} else {
		write_data = ((addr & 0xFF00) << 16) + ((addr & 0xFF) << 16) +
			     (0x25 << 8) + 0x82;
	...
	if (ili_hid_ice_mode_write(WRITE_BUF_ADDR, write_data, sizeof(u32)) < 0)
		ILI_ERR("Write WRITE_BUF_ADDR failed\n");

	write_data = ((data & 0xFF0000) << 8) + ((data & 0xFF00) << 8) +
		     ((data & 0xFF) << 8) + ((addr & 0xFF0000) >> 16);

	if (ili_hid_ice_mode_write(0x308D0, write_data, sizeof(u32)) < 0) {
		ILI_ERR("Write failed\n");
		ret = -1;
	}
	...
	if (ili_hid_ice_mode_write(0x308D4, write_data, sizeof(u32)) < 0) {
		ILI_ERR("Write failed\n");
		ret = -1;
	}
...
void ili_hid_mspi_read_client_register(u32 addr, u32 *data, int len)
{
	u32 read_data, backup_mspi_init_en;

	ili_hid_mspi_write_client_register(addr, 0, 0);
	...
	if (ili_hid_ice_mode_write(WRITE_BUF_ADDR, 0x83, 1) < 0)
		ILI_ERR("Write WRITE_BUF_ADDR failed\n");
	...
	if (ili_hid_ice_mode_read(0x308D0, data, len) < 0)
		ILI_ERR("Read 0x308D0 failed\n");
}

#if (TDDI_INTERFACE == BUS_SPI)
void ili_hid_set_bypass_mode(void)
{
	ili_hid_cascade_sync_ctrl(false);

	ili_hid_ice_mode_ctrl_by_mode(ENABLE, OFF, MASTER);

	/* set mspi 0 */
	ili_hid_ice_mode_write(MSPI_REG, 0x00, 1);

	/* set s_to_q bit[1] = 1 */
	ili_hid_ice_mode_write(SINGLE_TO_QUAL_REG, 0x02, 1);

	/* set client ic to ice mode */
	atomic_set(&ilits->ice_stat, DISABLE);
	ili_hid_ice_mode_ctrl_by_mode(ENABLE, OFF, CLIENT);

	ILI_INFO("Set ByPass Mode\n");
}
#endif

void ili_hid_wdt_reset_status(bool enter_ice, bool exit_ice)
{
#if (TDDI_INTERFACE == BUS_I2C)
	if (enter_ice) {
		if (atomic_read(&ilits->ice_stat)) {
...
#endif
	if (ili_hid_ice_mode_write_by_mode(WDT_ADDR, 0x01, 1, BOTH) > 0)
		ILI_INFO("Clear Master & Client WDT Status\n");
	if (exit_ice) {
		if (!atomic_read(&ilits->ice_stat)) {
			ILI_INFO("ice mode already disabled\n");
		} else {
...
void ili_hid_ic_edge_palm_para_init(void)
{
	...
	ilits->edge_palm_para[30] = ili_hid_calc_packet_checksum(
		ilits->edge_palm_para, P5_X_EDGE_PALM_PARA_LENGTH - 1);

	for (i = 0; i < P5_X_EDGE_PALM_PARA_LENGTH; i++) {
		ILI_DBG("edge_palm_para[%d] = 0x%2x\n", i,
			ilits->edge_palm_para[i]);
	}
#endif
}
void ili_hid_ic_send_edge_palm_para(void)
{
#if ENABLE_EDGE_PALM_PARA
	int ret = 0;
	...
	ret = ilits->wrapper(ilits->edge_palm_para, P5_X_EDGE_PALM_PARA_LENGTH,
			     NULL, 0, OFF, OFF);
	if (ret < 0)
...
int ili_hid_ic_func_ctrl_export(const char *name, int ctrl)
{
	int i = 0, ret;
	...
	ret = ilits->wrapper(func_ctrl[i].cmd, func_ctrl[i].len, NULL, 0, OFF,
			     OFF);
	if (ret < 0)
...
EXPORT_SYMBOL(ili_hid_ic_func_ctrl_export);

int ili_hid_ic_cascade_get_pc_latch(int stat)
{
	...
	if (!ice) {
		if (stat == DO_I2C_RECOVER)
			ret = ili_hid_ice_mode_ctrl_by_mode(ENABLE, OFF, BOTH);
		else
			ret = ili_hid_ice_mode_ctrl_by_mode(ENABLE, ON, BOTH);

		if (ret < 0)
			ILI_ERR("Enable ice mode failed while reading pc counter\n");
	}

	ILI_INFO("ILITEK check pc latch.\n");
	ili_hid_ice_mode_read_by_mode(latch_addr, &latch, sizeof(u32), CLIENT);
	if (ilits->client_pc_latch != latch) {
		ILI_ERR("[Client] has been trigger WDT reset,PC Latch: [Previous] = 0x%08x, [Current] = 0x%08x.\n",
			ilits->client_pc_latch, latch);
		ilits->client_pc_latch = latch;
	}

	ili_hid_ice_mode_read_by_mode(latch_addr, &latch, sizeof(u32), MASTER);
	if (ilits->master_pc_latch != latch) {
		ILI_ERR("[Master] has been trigger WDT reset,PC Latch: [Previous] = 0x%08x, [Current] = 0x%08x.\n",
			ilits->master_pc_latch, latch);
		ilits->master_pc_latch = latch;
	}

	if (!ice) {
		if (ili_hid_ice_mode_ctrl_by_mode(DISABLE, ON, BOTH) < 0)
...
int ili_hid_ic_func_ctrl(const char *name, int ctrl)
{
	int i = 0, ret;
...
void ili_hid_ic_func_ctrl_reset(void)
{
	int i = 0;

	ili_hid_ic_send_edge_palm_para();

	for (i = 0; i < FUNC_CTRL_NUM; i++) {
		if (func_ctrl[i].rec_state == ENABLE) {
			ILI_DBG("reset func ctrl %s, record status = %d, cmd = %d\n",
				func_ctrl[i].name, func_ctrl[i].rec_state,
				func_ctrl[i].rec_cmd);
			if (ili_hid_ic_func_ctrl(func_ctrl[i].name,
						 func_ctrl[i].rec_cmd) < 0)
...
int ili_hid_ic_code_reset(bool mcu)
{
	int ret;
	bool ice = atomic_read(&ilits->ice_stat);
	...
	ret = ili_hid_ice_mode_write(0x40040, 0xAE, 1);
	if (ret < 0)
	...
	if (!ice) {
		if (ilits->cascade_info_block.nNum != 0) {
			...
			if (ili_hid_ice_mode_ctrl(DISABLE, mcu) < 0)
...
void ili_hid_get_dma1_config(struct ilitek_dma_config *dma)
{
	/* dma1 src1 address */
	if (ili_hid_ice_mode_read(0x072104, &dma->src_addr, 4) < 0)
		ILI_ERR("read dma1 src1 address failed\n");
	/* dma1 src1 format */
	if (ili_hid_ice_mode_read(0x072108, &dma->src_fmt, 4) < 0)
		ILI_ERR("read dma1 src1 format failed\n");
	/* dma1 dest address */
	if (ili_hid_ice_mode_read(0x072114, &dma->dest_addr, 4) < 0)
		ILI_ERR("read dma1 src1 format failed\n");
	/* dma1 dest format */
	if (ili_hid_ice_mode_read(0x072118, &dma->dest_fmt, 4) < 0)
		ILI_ERR("read dma1 dest format failed\n");
	/* Block size */
	if (ili_hid_ice_mode_read(0x07211C, &dma->block_size, 4) < 0)
	...
	ILI_DBG("dma.src_addr=0x%x, dma.src_fmt=0x%x, dma.dest_addr=0x%x, dma.dest_fmt=0x%x, dma.block_size=0x%x\n",
		dma->src_addr, dma->src_fmt, dma->dest_addr, dma->dest_fmt,
		dma->block_size);
}

void ili_hid_set_dma1_config(struct ilitek_dma_config *dma)
{
	ILI_DBG("dma.src_addr=0x%x, dma.src_fmt=0x%x, dma.dest_addr=0x%x, dma.dest_fmt=0x%x, dma.block_size=0x%x\n",
		dma->src_addr, dma->src_fmt, dma->dest_addr, dma->dest_fmt,
		dma->block_size);
	/* dma1 src1 address */
	if (ili_hid_ice_mode_write(0x072104, dma->src_addr, 4) < 0)
		ILI_ERR("Write dma1 src1 address failed\n");
	/* dma1 src1 format */
	if (ili_hid_ice_mode_write(0x072108, dma->src_fmt, 4) < 0)
		ILI_ERR("Write dma1 src1 format failed\n");
	/* dma1 dest address */
	if (ili_hid_ice_mode_write(0x072114, dma->dest_addr, 4) < 0)
		ILI_ERR("Write dma1 src1 format failed\n");
	/* dma1 dest format */
	if (ili_hid_ice_mode_write(0x072118, dma->dest_fmt, 4) < 0)
		ILI_ERR("Write dma1 dest format failed\n");
	/* Block size*/
	if (ili_hid_ice_mode_write(0x07211C, dma->block_size, 4) < 0)
		ILI_ERR("Write block size (%d) failed\n", dma->block_size);
	/* Disable CRC calc settings */
	if (ili_hid_ice_mode_write(0x041014, 0x0, 4) < 0)
		ILI_ERR("Write dma CRC calc settings failed\n");
	/* Dma1 stop */
	if (ili_hid_ice_mode_write(0x072100, 0x02040000, 4) < 0)
		ILI_ERR("Write dma1 stop failed\n");
	/* clr int */
	if (ili_hid_ice_mode_write(0x048006, 0x2, 1) < 0)
		ILI_ERR("Write clr int failed\n");
	/* Dma1 start */
	if (ili_hid_ice_mode_write(0x072100, 0x01040000, 4) < 0)
		ILI_ERR("Write dma1 start failed\n");
}

int ili_hid_ic_whole_reset(bool mcu, bool withflash)
{
	int ret = 0;
	bool ice = atomic_read(&ilits->ice_stat);
	...
	if (!withflash)
		ili_hid_ice_mode_write(
			ilits->chip->reset_addr,
			TDDI_WHOLE_CHIP_RST_WITHOUT_FLASH_PRE_KEY, sizeof(u32));

	ILI_INFO("ic whole reset key = 0x%x, edge_delay = %d, withflash = %d\n",
		 ilits->chip->reset_key, ilits->rst_edge_delay, withflash);

	ret = ili_hid_ice_mode_write(ilits->chip->reset_addr,
				     ilits->chip->reset_key, sizeof(u32));
	if (ret < 0) {
...
static void ilitek_tddi_ic_wr_pack(int packet)
{
	...
	while (retry--) {
		if (ili_hid_ice_mode_read(0x73010, &reg_data, sizeof(u8)) < 0)
	...
	if (ili_hid_ice_mode_write(0x73000, packet, 4) < 0)
...
static u32 ilitek_tddi_ic_rd_pack(int packet)
{
	...
	while (retry--) {
		if (ili_hid_ice_mode_read(0x4800A, &reg_data, sizeof(u8)) < 0)
	...
	if (ili_hid_ice_mode_write(0x4800A, 0x02, 1) < 0)
		ILI_ERR("Write 0x2 at 0x4800A\n");

	if (ili_hid_ice_mode_read(0x73016, &reg_data, sizeof(u8)) < 0)
		ILI_ERR("Read 0x73016 error\n");

	return reg_data;
}

void ili_hid_ic_set_ddi_reg_onepage(u8 page, u8 reg, u8 data, bool mcu)
{
...
void ili_hid_ic_get_ddi_reg_onepage(u8 page, u8 reg, u8 *data, bool mcu)
{
	u32 setpage = 0x1FFFFF00 | page;
...
void ili_hid_ic_get_pc_counter(int stat)
{
	bool ice = atomic_read(&ilits->ice_stat);
	...
	if (ilits->cascade_info_block.nNum != 0) {
		...
		ili_hid_cascade_rw_tp_reg(mcu_state, read, pc_addr, 0, 0, &pc);
		ili_hid_cascade_rw_tp_reg(mcu_state, read, latch_addr, 0, 0,
					  &latch);

		ilits->fw_pc = pc;
		ilits->fw_latch = latch;

		if (ilits->client_wr_ctrl) {
			ILI_ERR("Read Client counter (addr: 0x%x) = 0x%x, latch (addr: 0x%x) = 0x%x\n",
				pc_addr, ilits->fw_pc, latch_addr,
				ilits->fw_latch);
		} else {
		...
		if (ilits->client_wr_ctrl) {
			ilits->i2c_addr_change = CLIENT;
			ILI_DBG("Change to Client Address\n");
		} else {
			ilits->i2c_addr_change = MASTER;
		...
		if (!ice) {
			if (stat == DO_I2C_RECOVER)
				ret = ili_hid_ice_mode_ctrl_by_mode(
					ENABLE, OFF, ilits->client_wr_ctrl);
			else
				ret = ili_hid_ice_mode_ctrl_by_mode(
					ENABLE, ON, ilits->client_wr_ctrl);

			if (ret < 0)
				ILI_ERR("Enable ice mode failed while reading pc counter\n");
		}

		ili_hid_ice_mode_read_by_mode(pc_addr, &pc, sizeof(u32),
					      ilits->client_wr_ctrl);
		ili_hid_ice_mode_read_by_mode(latch_addr, &latch, sizeof(u32),
					      ilits->client_wr_ctrl);
		...
		if (!ice) {
			if (ili_hid_ice_mode_ctrl_by_mode(
				    DISABLE, ON, ilits->client_wr_ctrl) < 0)
				ILI_ERR("Disable ice mode failed while reading pc counter\n");
		}
		...
		if (!ice) {
			if (stat == DO_SPI_RECOVER || stat == DO_I2C_RECOVER)
				ret = ili_hid_ice_mode_ctrl(ENABLE, OFF);
			else
				ret = ili_hid_ice_mode_ctrl(ENABLE, ON);

			if (ret < 0)
				ILI_ERR("Enable ice mode failed while reading pc counter\n");
		}

		if (ili_hid_ice_mode_read(pc_addr, &pc, sizeof(u32)) < 0)
			ILI_ERR("Read pc conter error\n");

		if (ili_hid_ice_mode_read(latch_addr, &latch, sizeof(u32)) < 0)
		...
		if (!ice) {
			if (ili_hid_ice_mode_ctrl(DISABLE, ON) < 0)
				ILI_ERR("Disable ice mode failed while reading pc counter\n");
		}
	}
}

int ili_hid_ic_int_trigger_ctrl(bool pulse)
{
	/* It's supported by fw, and the level will be kept at high until data was already prepared. */
	if (ili_hid_ic_func_ctrl("int_trigger", pulse) < 0) {
...
int ili_hid_ic_check_int_level(bool level)
{
	int timer = 3000;
...
int ili_hid_ic_check_int_pulse(bool pulse)
{
...
int ili_hid_ic_check_int_flag(bool pulse)
{
	u8 retry = 100;

	do {
		if (ilits->hid_report_irq) {
	...
	ILI_ERR("Error! INT pulse no detected. Timeout = 2000 ms\n");
	atomic_set(&ilits->cmd_int_check, DISABLE);
	return -1;
}

int ili_hid_ic_check_busy(int count, int delay, bool irq)
{
	u8 cmd[2] = { 0 };
	u8 busy = 0, rby = 0;
	...
	ili_hid_ic_get_pc_counter(0);

	if (ilits->cascade_info_block.nNum != 0) {
		ilits->client_wr_ctrl = 1;
		ili_hid_ic_get_pc_counter(0);
		ilits->client_wr_ctrl = 0;
	}
	return -1;
}

int ili_hid_ic_get_project_id(u8 *pdata, int size)
{
	int i;
	u32 tmp;
	bool ice = atomic_read(&ilits->ice_stat);
	...
	if (ili_hid_ice_mode_write(0x041000, 0x0, 1) < 0)
		ILI_ERR("Pull cs low failed\n");
	if (ili_hid_ice_mode_write(0x041004, 0x66aa55, 3) < 0)
		ILI_ERR("Write key failed\n");

	if (ili_hid_ice_mode_write(0x041008, 0x03, 1) < 0)
		ILI_ERR("Write 0x03 at 0x041008\n");

	if (ili_hid_ice_mode_write(0x041008, (RSV_BK_ST_ADDR & 0xFF0000) >> 16, 1) < 0)
		ILI_ERR("Write address failed\n");
	if (ili_hid_ice_mode_write(0x041008, (RSV_BK_ST_ADDR & 0x00FF00) >> 8, 1) < 0)
		ILI_ERR("Write address failed\n");
	if (ili_hid_ice_mode_write(0x041008, (RSV_BK_ST_ADDR & 0x0000FF), 1) < 0)
		ILI_ERR("Write address failed\n");

	for (i = 0; i < size; i++) {
		if (ili_hid_ice_mode_write(0x041008, 0xFF, 1) < 0)
			ILI_ERR("Write dummy failed\n");
		if (ili_hid_ice_mode_read(0x41010, &tmp, sizeof(u8)) < 0)
			ILI_ERR("Read project id error\n");
		pdata[i] = tmp;
		ILI_INFO("project_id[%d] = 0x%x\n", i, pdata[i]);
	}

	ili_hid_flash_clear_dma();

	if (ili_hid_ice_mode_write(0x041000, 0x1, 1) < 0)
		ILI_ERR("Pull cs high\n");

	if (!ice)
		if (ili_hid_ice_mode_ctrl(DISABLE, OFF) < 0)
			ILI_ERR("Disable ice mode failed while reading project id\n");

	return 0;
}

int ili_hid_ic_get_core_ver(void)
{
	int i = 0, ret = 0;
	u8 cmd[2] = { 0 }, buf[10] = { 0 };
	...
	do {
		if (i == 0) {
			...
			ilits->protocol->core_ver_len =
				P5_X_CORE_VER_THREE_LENGTH;
		}

		if (ilits->wrapper(cmd, sizeof(cmd), NULL, 0, OFF, OFF) < 0)
			ILI_ERR("Write core ver cmd failed\n");

		if (ilits->wrapper(&cmd[1], sizeof(u8), buf,
				   ilits->protocol->core_ver_len, ON, OFF) < 0)
...
void ili_hid_fw_uart_ctrl(u8 ctrl)
{
	u8 cmd[4] = { 0 };
...
int ili_hid_ic_get_fw_ver(void)
{
	int ret = 0;
	u8 cmd[2] = { 0 };
	u8 buf[10] = { 0 };
	...
	if (ilits->wrapper(&cmd[1], sizeof(u8), buf,
			   ilits->protocol->fw_ver_len, ON, OFF) < 0) {
...
int ili_hid_ic_get_panel_info(void)
{
	int ret = 0;
	...
	if (ilits->info_from_hex && (ilits->chip->core_ver >= CORE_VER_1410)) {
		...
		if (ilits->chip->core_ver >= CORE_VER_1700) {
			if (ilits->PenType == POSITION_PEN_TYPE_ON) {
				...
				ili_hid_set_tp_data_len(DATA_FORMAT_DEMO, false,
							NULL);
				ILI_INFO(
					"Panel support Pen Mode, Packet Len = %d, Touch Number = %d\n",
	...
	if (buf[0] != cmd) {
		...
		ilits->trans_xy =
			(ilits->chip->core_ver >= CORE_VER_1430) ? buf[5] : OFF;
	}
	...
	if (ilits->chip->core_ver >= CORE_VER_1470) {
		...
		if (buf[0] != cmd) {
			ILI_INFO(
				"Invalid report format info, use default report format resolution\n");
			...
			if (ilits->chip->core_ver >= CORE_VER_1700) {
				...
				if (ilits->PenType == POSITION_PEN_TYPE_ON) {
					ilits->touch_num =
						MAX_TOUCH_NUM + MAX_PEN_NUM;
					ili_hid_set_tp_data_len(
						DATA_FORMAT_DEMO, false, NULL);
					ILI_INFO(
						"Panel support Pen Mode, Packet Len = %d, Touch Number = %d\n",
						ilits->tp_data_len,
						ilits->touch_num);
	...
	ILI_INFO(
		"cmd = 0x%X, buf[0] = 0x%X, buf[1] = 0x%X, buf[2] = 0x%X, buf[3] = 0x%X, buf[4] = 0x%X\n",
	...
	if (buf[0] != cmd) {
		ILI_INFO(
			"Invalid Pen info, use default report format resolution\n");
	...
	ILI_INFO(
		"cmd = 0x%X, buf[0] = 0x%X, buf[1] = 0x%X, buf[2] = 0x%X, buf[3] = 0x%X\n",
...
out:
	...
	if (ilits->chip->core_ver >= CORE_VER_1700) {
		ILI_INFO("Pen Type = %X\n", ilits->PenType);
		ILI_INFO(
			"Pen Info Data, nPxRaw = %d, nPyRaw = %d, nPxVa = %d, nPyVa = %d\n",
...
int ili_hid_ic_get_tp_info(void)
{
	int ret = 0;
	u8 cmd[2] = { 0 };
	u8 buf[20] = { 0 };
	...
	ret = ilits->wrapper(&cmd[1], sizeof(u8), buf,
			     ilits->protocol->tp_info_len, ON, OFF);
	if (ret < 0) {
...
int ili_hid_ic_get_protocl_ver(void)
{
	int ret = 0;
	u8 cmd[2] = { 0 };
	u8 buf[10] = { 0 };
	...
	if (ilits->wrapper(&cmd[1], sizeof(u8), buf,
			   ilits->protocol->pro_ver_len, ON, OFF) < 0) {
...
out:
	...
	ILI_INFO("Protocol version = %d.%d.%d\n", ilits->protocol->ver >> 16,
		 (ilits->protocol->ver >> 8) & 0xFF,
		 ilits->protocol->ver & 0xFF);
	return ret;
}

int ili_hid_ic_get_info(void)
{
	int ret = 0;
	u8 ddi_data = 0;
	...
	if (ili_hid_ice_mode_read(ilits->chip->pid_addr, &ilits->chip->pid,
				  sizeof(u32)) < 0)
		ILI_ERR("Read chip pid error\n");

	if (ili_hid_ice_mode_read(ilits->chip->otp_addr, &ilits->chip->otp_id,
				  sizeof(u32)) < 0)
		ILI_ERR("Read otp id error\n");
	if (ili_hid_ice_mode_read(ilits->chip->ana_addr, &ilits->chip->ana_id,
				  sizeof(u32)) < 0)
	...
	if (ilits->chip->id == ILI9882_CHIP) {
		ili_hid_ic_get_ddi_reg_onepage(0x6, 0xF3, &ddi_data, OFF);
		if (ddi_data == 0x30)
...
void ili_hid_check_enable_cascade(void)
{
	u32 enable_cascade;

	ili_hid_ice_mode_ctrl(ENABLE, ON);

	ili_hid_ice_mode_read(ENABLE_CASCADE_ADDR, &enable_cascade,
			      sizeof(u32));
	ilits->enable_cascade = enable_cascade >> 2;
	ILI_INFO(
		"Read Data = 0x%x, Enable Casecade = 0x%x, Check Client exist.\n",
		enable_cascade, ilits->enable_cascade);

	ili_hid_ice_mode_ctrl(DISABLE, ON);
}

int ili_hid_cascade_ic_get_info(bool enter_ice, bool exit_ice)
{
	int ret = 0;
	...
	if (ili_hid_ice_mode_read_by_mode(ilits->chip->pid_addr,
					  &ilits->chip->client_pid, sizeof(u32),
					  CLIENT) < 0)
		ILI_ERR("Read Client chip pid error\n");
	...
	ili_hid_mspi_read_client_register(TDDI_PID_ADDR, &ilits->chip->client_pid,
					 sizeof(u32));

	/* set mspi 0 */
	ili_hid_ice_mode_write_by_mode(MSPI_REG, 0x00, 1, MASTER);
	...
	if (ilits->chip->pid != ilits->chip->client_pid) {
...
int ili_hid_ic_dummy_check(void)
{
	int ret = 0;
	...
	for (i = 0; i < 3; i++) {
		if (ili_hid_ice_mode_write(WDT9_DUMMY2, wdata, sizeof(u32)) < 0)
			ILI_ERR("Write dummy error\n");

		if (ili_hid_ice_mode_read(WDT9_DUMMY2, &rdata, sizeof(u32)) < 0)
...
int ili_hid_ic_report_rate_set(u8 mode)
{
	int ret = 0;
	u8 cmd[4] = { 0 };
...
int ili_hid_ic_report_rate_get(void)
{
	int ret = 0;
...
int ili_hid_ic_report_rate_register_set(void)
{
	int ret = 0;
	...
	ILI_INFO("current_report_rate_mode: %X when reset\n",
		 ilits->current_report_rate_mode);
	ret = ili_hid_ice_mode_write(
		0x4005E, (0x5A00 | (ilits->current_report_rate_mode & 0xFF)),
		2);
	if (ret < 0)
