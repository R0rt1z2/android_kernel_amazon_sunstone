...
#ifndef __ILITEK_9882T_H
#define __ILITEK_9882T_H
...
#define SPI_TX_BUF_SIZE (6 * K) //4096
#define SPI_RX_BUF_SIZE (6 * K) //4096
...
#define BOOT_FW_UPDATE_MODE BOOT_FW_VER_DOWNGRADE
...
#define SPI_DMA_TRANSFER_SPLIT ENABLE //DISABLE
...
#define CSV_LCM_ON_PATH "/dfs/output_data"
#define CSV_LCM_OFF_PATH "/dfs/output_data"
...
#define HID_TOUCH_INPUT_DEVICE "ILITEK_HID_TOUCH"
#define TOUCH_INPUT_DEVICE "ILITEK_TOUCH"
#define PEN_INPUT_DEVICE "ILITEK_PEN"
#define HEX_SIGN_KEY "ilitekTP"
#define HEX_SIGN_KEY_LEN_MAX 128

#define SINGLE_TOUCH 0
#define MULTI_TOUCH 1
#define SPI_CLK_TABLE_NUM 3
...
enum TP_FW_OPEN_METHOD { REQUEST_FIRMWARE = 0, FILP_OPEN };
...
enum TP_RECORD_DATA { DISABLE_RECORD = 0, ENABLE_RECORD, DATA_RECORD };
...
#define INTR33_reserved_0                                                      \
	(BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5) | BIT(6) | BIT(7))
#define INTR33_reg_wdt_alarm_int_en BIT(8)
#define INTR33_reserved_1                                                      \
	(BIT(9) | BIT(10) | BIT(11) | BIT(12) | BIT(13) | BIT(14) | BIT(15))
...
#define FLASH4_reserved_2                                                      \
	(BIT(18) | BIT(19) | BIT(20) | BIT(21) | BIT(22) | BIT(23))
#define FLASH4_reg_flash_dma_trigger_en                                        \
...
#define CLIENT_READ_BACK_WORD_REG 0x62030
...
/* Pen data format */
#define DATA_FORMAT_SWITCH_PEN_DEBUG_TYPE_CMD 0xFA
#define DATA_FORMAT_DEBUG_PEN_CMD 0x06
#define DATA_FORMAT_DEBUG_HAND_CMD 0x07
#define DATA_FORMAT_DEBUG_VA_DATA_CMD 0x03
#define DATA_FORMAT_DEBUG_RAW_DATA_CMD 0x08
...
/* HID over I2c */
#define I2C_HID_RID_IDX 0
#define I2C_HID_GESTURE_LEN_IDX 2
#define I2C_HID_PACKET_ID_IDX 3
#define I2C_HID_GESTURE_TYPE_IDX 4
#define I2C_HID_RID 0x03
#define I2C_HID_GESTURE_LEN 0x02
#define I2C_HID_CHECK_MP_INT_IDX 1
#define I2C_HID_FINGER_RID 	     0x01
#define I2C_HID_PEN_RID 	     0x08
#define I2C_HID_TIME_STAMP_H_IDX      70
#define I2C_HID_TIME_STAMP_L_IDX      69

#define SIGN_HEX_FW_VER 0x7A00
#define SIGN_HEX_FW_MP_VER 0x0B00
...
#define POSITION_CUSTOMER_TYPE_OFF_3BITS                                       \
	0x07 /*core ver 1700, CustomerType 3 bits*/
...
#define ILITEK_HID_PEN_SERIAL_3 23
#define ILITEK_HID_PEN_SERIAL_4 24

/* stylus GID4 */
#define ILITEK_HID_PEN_VENDOR_USAGE_1_L 65
...
/*  add_for_charger_end  */
	...
	bool sign_verify_pass_flag;

	/* Cascade */
	struct cascade_info_block cascade_info_block;
	bool mspi_en;
	bool client_wr_ctrl;
	bool ice_by_pass_ctrl;
	bool enable_cascade;
	bool mspi_done;
	u8 i2c_addr_change;
...
extern struct hid_device *ilitek_hdev;
...
extern int ilitek_hid_plat_dev_init(struct i2c_client *client);
extern void ilitek_hid_plat_dev_exit(void);
extern int ilitek_hid_plat_probe(void);

/* Prototypes for tddi firmware/flash functions */
extern void ili_hid_flash_dma_write(u32 start, u32 end, u32 len);
extern void ili_hid_flash_clear_dma(void);
extern void ili_hid_fw_read_flash_info(bool mcu);
extern void ilitek_hid_tddi_flash_protect(bool enable, bool mcu);
extern int ili_hid_fw_read_hw_crc(u32 start, u32 end, u32 *flash_crc);
extern int ili_hid_fw_read_flash(u32 start, u32 end, u8 *data, int len);
extern int ili_hid_fw_dump_iram_data(u32 start, u32 end, bool save, bool mcu);
extern int ili_hid_fw_dump_flash_data(u32 start, u32 end, bool user, bool mcu);
extern int ili_hid_fw_upgrade(int op);

/* Prototypes for tddi mp test */
extern int ili_hid_mp_test_main(char *apk, bool lcm_on);

/* Prototypes for tddi core functions */
extern int ili_hid_touch_esd_gesture_flash(void);
extern int ili_hid_touch_esd_gesture_iram(void);
extern void ili_hid_set_gesture_symbol(void);
extern int ili_hid_move_gesture_code_flash(int mode);
extern int ili_hid_move_gesture_code_iram(int mode);
extern int ili_hid_move_mp_code_flash(void);
extern int ili_hid_move_mp_code_iram(void);
extern void ili_hid_touch_press(u16 x, u16 y, u16 pressure, u16 id);
extern void ili_hid_touch_release(u16 x, u16 y, u16 id);
extern void ili_hid_touch_release_all_point(void);
extern void ili_hid_report_ap_mode(u8 *buf, int len);
extern void ili_hid_pen_demo_mode_report_point(u8 *buf, int len);
extern void ili_hid_report_pen_ap_mode(u8 *buf, int len);
extern void ili_hid_report_pen_debug_mode(u8 *buf, int len);
extern void ili_hid_report_debug_mode(u8 *buf, int rlen);
extern void ili_hid_pen_debug_mode_report_point(u8 *buf, int len, u8 offset);
extern void ili_hid_report_debug_lite_mode(u8 *buf, int rlen);
extern void ili_hid_report_gesture_mode(u8 *buf, int rlen);
extern void ili_hid_report_proximity_mode(u8 *buf, int len);
extern void ili_hid_report_i2cuart_mode(u8 *buf, int rlen);
extern void ili_hid_ic_set_ddi_reg_onepage(u8 page, u8 reg, u8 data, bool mcu);
extern void ili_hid_ic_get_ddi_reg_onepage(u8 page, u8 reg, u8 *data, bool mcu);
extern int ili_hid_ic_whole_reset(bool mcu, bool withflash);
extern int ili_hid_ic_code_reset(bool mcu);
extern int ili_hid_ic_func_ctrl(const char *name, int ctrl);
extern int ili_hid_ic_func_ctrl_export(const char *name, int ctrl);
extern void ili_hid_ic_func_ctrl_reset(void);
extern void ili_hid_ic_edge_palm_para_init(void);
extern void ili_hid_ic_send_edge_palm_para(void);
extern int ili_hid_ic_int_trigger_ctrl(bool pulse);
extern void ili_hid_ic_get_pc_counter(int stat);
extern int ili_hid_ic_check_int_level(bool level);
extern int ili_hid_ic_check_int_pulse(bool pulse);
extern int ili_hid_ic_check_int_flag(bool pulse);
extern int ili_hid_ic_check_busy(int count, int delay, bool irq);
extern int ili_hid_ic_get_project_id(u8 *pdata, int size);
extern int ili_hid_ic_get_panel_info(void);
extern int ili_hid_ic_get_tp_info(void);
extern int ili_hid_ic_get_core_ver(void);
extern int ili_hid_ic_get_protocl_ver(void);
extern int ili_hid_ic_get_fw_ver(void);
extern int ili_hid_ic_get_info(void);
extern void ili_hid_set_bypass_mode(void);
extern int ili_hid_cascade_ic_get_info(bool enter_ice, bool exit_ice);
extern void ili_hid_mspi_set_client_to_qual_mode(void);
extern int ili_hid_mspi_write_client_register(u32 addr, u32 data, int len);
extern void ili_hid_mspi_read_client_register(u32 addr, u32 *data, int len);
extern void ili_hid_check_enable_cascade(void);
extern void ili_hid_wdt_reset_status(bool enter_ice, bool exit_ice);
extern int ili_hid_cascade_reset_ctrl(int reset_mode, bool enter_ice);
extern int ili_hid_cascade_rw_tp_reg(u32 mcu, u32 type, u32 addr, u32 wdata,
				     u32 wlen, u32 *rdata);
extern int ili_hid_ic_dummy_check(void);
extern int ili_hid_ic_report_rate_set(u8 mode);
extern int ili_hid_ic_report_rate_get(void);
extern int ili_hid_ic_report_rate_register_set(void);
extern int ili_hid_ice_mode_bit_mask_write(u32 addr, u32 mask, u32 value);
extern int ili_hid_ice_mode_write(u32 addr, u32 data, int len);
extern void ili_hid_cascade_sync_ctrl(bool mode);
extern int ili_hid_ice_mode_ctrl_by_mode(bool enable, bool mcu, int mode);
extern int ili_hid_ice_mode_write_by_mode(u32 addr, u32 data, int len,
					  int mode);
extern int ili_hid_ice_mode_read_by_mode(u32 addr, u32 *data, int len,
					 int mode);
extern int ili_hid_ice_mode_read(u32 addr, u32 *data, int len);
extern int ili_hid_ice_mode_ctrl(bool enable, bool mcu);
extern void ili_hid_ic_init(void);
extern void ili_hid_fw_uart_ctrl(u8 ctrl);
extern int ili_hid_tddi_ic_sram_test(void);
#ifdef ROI
extern int ili_hid_read_knuckle_roi_data(void);
extern int ili_hid_config_get_knuckle_roi_status(void);
extern int ili_hid_config_knuckle_roi_ctrl(cmd_types cmd);
#endif
/* Prototypes for tddi events */
#if RESUME_BY_DDI
extern void ili_hid_resume_by_ddi(void);
#endif
extern int ili_hid_proximity_far(int mode);
extern int ili_hid_proximity_near(int mode);
extern int ili_hid_switch_tp_mode(u8 data);
extern int ili_hid_set_tp_data_len(int format, bool send, u8 *data);
extern int ili_hid_set_pen_data_len(u8 header, u8 ctrl, u8 type);
extern int ili_hid_fw_upgrade_handler(void *data);
extern int ili_hid_wq_esd_i2c_check(void);
extern int ili_hid_wq_esd_spi_check(void);
extern int ili_hid_gesture_recovery(void);
extern void ili_hid_spi_recovery(void);
extern void ili_hid_wq_ctrl(int type, int ctrl);
extern int ili_hid_mp_test_handler(char *apk, bool lcm_on);
extern int ili_hid_sleep_handler(int mode);
extern int ili_hid_reset_ctrl(int mode);
extern int ili_hid_tddi_init(void);
extern int ili_hid_dev_init(struct ilitek_hwif_info *hwif,
			    struct i2c_client *client);
extern void ili_hid_dev_remove(bool flag);

/* Prototypes for i2c/spi interface */
extern int ili_hid_core_spi_setup(int num);
extern int ili_hid_interface_dev_init(struct ilitek_hwif_info *hwif,
				      struct i2c_client *client);

extern void ili_hid_interface_dev_exit(struct ilitek_ts_hid_data *ts,
				       bool flag);
extern int ili_hid_spi_write_then_read_split(struct spi_device *spi,
					     const void *txbuf, unsigned n_tx,
					     void *rxbuf, unsigned n_rx);
extern int ili_hid_spi_write_then_read_direct(struct spi_device *spi,
					      const void *txbuf, unsigned n_tx,
					      void *rxbuf, unsigned n_rx);

/* Prototypes for platform level */
extern void ili_hid_plat_regulator_power_on(bool status);
extern void ili_hid_input_register(void);
extern void ili_hid_input_pen_register(void);
extern void ili_hid_irq_disable(void);
extern void ili_hid_irq_enable(void);
extern void ili_hid_tp_reset(void);
extern void ili_hid_irq_unregister(void);
extern int ili_hid_irq_register(int type);
extern void ilitek_hid_plat_charger_init(void);

/* Prototypes for miscs */
extern void ili_hid_node_init(void);
extern void ili_hid_dump_data(void *data, int type, int len, int row_len,
			      const char *name);
extern u8 ili_hid_calc_packet_checksum(u8 *packet, int len);
extern void ili_hid_netlink_reply_msg(void *raw, int size);
extern int ili_hid_katoi(char *str);
extern int ili_str2hex(char *str);

extern int ili_hid_mp_read_write(u8 *wdata, u32 wlen, u8 *rdata, u32 rlen);
/* Prototypes for additional functionalities */
extern void ili_hid_gesture_fail_reason(bool enable);
extern int ili_hid_get_tp_recore_ctrl(int data);
extern int ili_hid_get_tp_recore_data(bool mcu);
extern void ili_hid_demo_debug_info_mode(u8 *buf, size_t rlen);
extern void ili_hid_demo_debug_info_id0(u8 *buf, size_t len);
extern void ili_hid_get_dma1_config(struct ilitek_dma_config *dma);
extern void ili_hid_set_dma1_config(struct ilitek_dma_config *dma);
extern int file_hid_write(struct file_buffer *file, bool new_open);
extern int ili_hid_ic_cascade_get_pc_latch(int stat);
/* HID over I2C */
extern void ili_hid_report(u8 *raw_data, int size, struct hid_field *field);
