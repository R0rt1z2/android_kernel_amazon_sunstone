...
/*openx mode*/
#define OPEN_DAC_KEY 0
#define OPEN_CAP1_DAC_KEY  4
#define OPENX_CAP1_DAC_KEY 6
...
enum mp_tipring_type { TIPRING_XY = 0, TIPRING_X, TIPRING_Y };
...
static struct open_test_para {
	...
	int cbk_step;
	int cint;
	/*Parameters for calculating openx values*/
	int invbk;
	int vadc_range;
} open_para;
...
static struct mp_test_items tItems[MP_TEST_ITEM] = {
	{ .desp = "baseline data(bg)",
	  .catalog = MUTUAL_TEST,
	  .cmd = CMD_MUTUAL_BG,
	  .lcm = ON },
	{ .desp = "untouch signal data(bg-raw-4096) - mutual",
	  .catalog = MUTUAL_TEST,
	  .cmd = CMD_MUTUAL_SIGNAL,
	  .lcm = ON },
	{ .desp = "manual bk data(mutual)",
	  .catalog = MUTUAL_TEST,
	  .cmd = CMD_MUTUAL_BK_DAC,
	  .lcm = ON },
	{ .desp = "calibration data(dac) - self",
	  .catalog = SELF_TEST,
	  .cmd = CMD_SELF_DAC,
	  .lcm = ON },
	{ .desp = "baselin data(bg,self_tx,self_r)",
	  .catalog = SELF_TEST,
	  .cmd = CMD_SELF_BG,
	  .lcm = ON },
	{ .desp = "untouch signal data(bg-raw-4096) - self",
	  .catalog = SELF_TEST,
	  .cmd = CMD_SELF_SIGNAL,
	  .lcm = ON },
	{ .desp = "raw data(no bk) - self",
	  .catalog = SELF_TEST,
	  .cmd = CMD_SELF_NO_BK,
	  .lcm = ON },
	{ .desp = "raw data(have bk) - self",
	  .catalog = SELF_TEST,
	  .cmd = CMD_SELF_HAVE_BK,
	  .lcm = ON },
	{ .desp = "manual bk dac data(self_tx,self_rx)",
	  .catalog = SELF_TEST,
	  .cmd = CMD_SELF_BK_DAC,
	  .lcm = ON },
	{ .desp = "calibration data(dac/icon)",
	  .catalog = KEY_TEST,
	  .cmd = CMD_KEY_DAC,
	  .lcm = ON },
	{ .desp = "key baseline data",
	  .catalog = KEY_TEST,
	  .cmd = CMD_KEY_BG,
	  .lcm = ON },
	{ .desp = "key raw data",
	...
	{ .desp = "key raw bk dac",
	  ...
	  .cmd = CMD_KEY_HAVE_BK,
	  .lcm = ON },
	{ .desp = "key raw open test",
	  .catalog = KEY_TEST,
	  .cmd = CMD_KEY_OPEN,
	  .lcm = ON },
	{ .desp = "key raw short test",
	  .catalog = KEY_TEST,
	  .cmd = CMD_KEY_SHORT,
	  .lcm = ON },
	{ .desp = "st calibration data(dac)",
	  .catalog = ST_TEST,
	  .cmd = CMD_ST_DAC,
	  .lcm = ON },
	{ .desp = "st baseline data(bg)",
	  .catalog = ST_TEST,
	  .cmd = CMD_ST_BG,
	  .lcm = ON },
	{ .desp = "st raw data(no bk)",
	  .catalog = ST_TEST,
	  .cmd = CMD_ST_NO_BK,
	  .lcm = ON },
	{ .desp = "st raw(have bk)",
	...
	{ .desp = "rx open",
	  ...
	  .cmd = CMD_RX_OPEN,
	  .lcm = ON },
	{ .desp = "untouch cm data",
	  .catalog = MUTUAL_TEST,
	  .cmd = CMD_CM_DATA,
	  .lcm = ON },
	{ .desp = "untouch cs data",
	  .catalog = MUTUAL_TEST,
	  .cmd = CMD_CS_DATA,
	  .lcm = ON },
	{ .desp = "tx/rx delta",
	  ...
	  .cmd = CMD_TX_RX_DELTA,
	  .lcm = ON },
	{ .desp = "untouch peak to peak",
	  .catalog = UNTOUCH_P2P,
	  .cmd = CMD_MUTUAL_SIGNAL,
	  .lcm = ON },
	{ .desp = "pixel raw (no bk)",
	  .catalog = PIXEL,
	  .cmd = CMD_MUTUAL_NO_BK,
	  .lcm = ON },
	{ .desp = "pixel raw (have bk)",
	  .catalog = PIXEL,
	  .cmd = CMD_MUTUAL_HAVE_BK,
	  .lcm = ON },
	{ .desp = "noise peak to peak(cut panel)",
	  .catalog = PEAK_TO_PEAK_TEST,
	  .lcm = ON },
	{ .desp = "open test(integration)",
	  .catalog = OPEN_TEST,
	  .cmd = CMD_RX_SHORT,
	  .lcm = ON },
	{ .desp = "open test(cap)",
	...
	{ .desp = "pin test ( int and rst )",
	  .catalog = PIN_TEST,
	  .cmd = CMD_PIN_TEST,
	  .lcm = ON },
	{ .desp = "noise peak to peak(with panel)",
	  .catalog = PEAK_TO_PEAK_TEST,
	  .lcm = ON },
	{ .desp = "noise peak to peak(ic only)",
	  .catalog = PEAK_TO_PEAK_TEST,
	  .cmd = CMD_PEAK_TO_PEAK,
	  .lcm = ON },
	...
	{ .desp = "raw data(no bk)",
	  ...
	  .cmd = CMD_MUTUAL_NO_BK,
	  .lcm = ON },
	{ .desp = "raw data(have bk)",
	  .catalog = MUTUAL_TEST,
	  .cmd = CMD_MUTUAL_HAVE_BK,
	  .lcm = ON },
	{ .desp = "calibration data(dac)",
	  .catalog = MUTUAL_TEST,
	  .cmd = CMD_MUTUAL_DAC,
	  .lcm = ON },
	{ .desp = "short test -ili9881",
	  .catalog = SHORT_TEST,
	  .cmd = CMD_RX_SHORT,
	  .lcm = ON },
	{ .desp = "short test",
	...
	{ .desp = "touch deltac",
	  .catalog = MUTUAL_TEST,
	  .lcm = ON },
	{ .desp = "pen tipring raw test",
	  .catalog = TIPRING_RAW,
	  .lcm = ON },
	{ .desp = "pen tipring p2p test",
	  .catalog = TIPRING_P2P,
	  .lcm = ON },
	{ .desp = "open test_c (openx enable)",
	  .catalog = OPEN_TEST,
	  .lcm = ON },
	/* LCM OFF TEST */
	{ .desp = "raw data(have bk) (lcm off)",
	  .catalog = MUTUAL_TEST,
	  .lcm = OFF },
	{ .desp = "raw data(no bk) (lcm off)",
	  .catalog = MUTUAL_TEST,
	  .lcm = OFF },
	{ .desp = "noise peak to peak(with panel) (lcm off)",
	  .catalog = PEAK_TO_PEAK_TEST,
	  .lcm = OFF },
	{ .desp = "noise peak to peak(ic only) (lcm off)",
	  .catalog = PEAK_TO_PEAK_TEST,
	  .lcm = OFF },
	{ .desp = "raw data_td (lcm off)",
	  .catalog = MUTUAL_TEST,
	  .lcm = OFF },
	{ .desp = "peak to peak_td (lcm off)",
	  .catalog = PEAK_TO_PEAK_TEST,
	  .lcm = OFF },
	{ .desp = "pen tipring raw test (lcm off)",
	  .catalog = TIPRING_RAW,
	  .lcm = OFF },
	{ .desp = "pen tipring p2p test (lcm off)",
	  .catalog = TIPRING_P2P,
	  .lcm = OFF },
};
...
static s32 *openx_cap_dac, *openx_cap_raw;	/*save openx test result*/
...
{
	ili_hid_dump_data(max_ptr, 32, core_mp.frame_len, 0,
			  "Dump Benchmark Max : ");
	ili_hid_dump_data(min_ptr, 32, core_mp.frame_len, 0,
			  "Dump Benchmark Min : ");
}

static void dump_node_type_buffer(s32 *node_ptr, u8 *name)
{
	ili_hid_dump_data(node_ptr, 32, core_mp.frame_len, 0,
			  "Dump NodeType : ");
}
...
static int parser_get_ini_key_value(char *section, char *key, char *value)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		...
		if (ipio_strcmp(key, ini_info[i].key_name) == 0) {
			ipio_memcpy(value, ini_info[i].key_value,
				    ini_info[i].key_val_len,
				    PARSER_MAX_KEY_VALUE_LEN);
			ILI_DBG("(key: %s, value:%s) => (ini key: %s, val: %s)\n",
				key, value, ini_info[i].key_name,
...
static void parser_ini_nodetype(s32 *type_ptr, char *desp, int frame_len)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		if (!strstr(ini_info[i].section_name, desp) ||
		    ipio_strcmp(ini_info[i].key_name, NODE_TYPE_KEY_NAME) !=
			    0) {
			continue;
		}
		...
		for (j = 0, index1 = 0; j <= ini_info[i].key_val_len; j++) {
			if (ini_info[i].key_value[j] == ';' ||
			    j == ini_info[i].key_val_len) {
				if (record != '.') {
					memset(str, 0, sizeof(str));
					ipio_memcpy(
						str,
						&ini_info[i].key_value[index1],
						(j - index1), sizeof(str));
					temp = ili_hid_katoi(str);
...
static void parser_ini_benchmark(s32 *max_ptr, s32 *min_ptr, int8_t type,
				 char *desp, int frame_len, char *bchmrk_name)
{
	int i = 0, j = 0, index1 = 0, temp, count = 0;
	char str[512] = { 0 }, record = ',';
	s32 data[4];
	...
	snprintf(benchmark_str, sizeof(benchmark_str), "%s%s%s", desp, "_",
		 bchmrk_name);
...
static void parser_tip_ring_ini_benchmark(s32 *max_ptr, s32 *min_ptr,
					  int8_t type, char *desp,
					  int frame_len)
{
	char benchmark_str[128] = { 0 };

	snprintf(benchmark_str, sizeof(benchmark_str), "%s%d",
		 BENCHMARK_KEY_NAME, 1);
	parser_ini_benchmark(max_ptr, min_ptr, type, desp,
			     core_mp.nPxRaw_tipring_len, benchmark_str);

	memset(benchmark_str, 0x0, sizeof(benchmark_str));
	snprintf(benchmark_str, sizeof(benchmark_str), "%s%d",
		 BENCHMARK_KEY_NAME, 2);

	parser_ini_benchmark(max_ptr + core_mp.nPxRaw_tipring_len,
			     min_ptr + core_mp.nPxRaw_tipring_len, type, desp,
			     core_mp.nPyRaw_tipring_len, benchmark_str);

	ili_hid_dump_data(max_ptr, 32, core_mp.tipring_len, 0,
			  "Dump Tip Ring Benchmark Max");
	ili_hid_dump_data(min_ptr, 32, core_mp.tipring_len, 0,
			  "Dump Tip Ring Benchmark Min");
}
...
{
	int i, n = 0, ret = 0, banchmark_flag = 0, empty_section,
	       nodetype_flag = 0;
	...
	tmpSectionName =
		kzalloc((PARSER_MAX_CFG_BUF + 1) * sizeof(char), GFP_KERNEL);
	...
	memset(seq_item, 0,
	       MP_TEST_ITEM * PARSER_MAX_KEY_NAME_LEN * sizeof(char));
	...
	while (true) {
		...
		n = parser_get_ini_phy_line(data + offset, ini_buf,
					    PARSER_MAX_CFG_BUF);
		if (n < 0) {
			ILI_ERR("End of Line\n");
			goto out;
		...
		/* Get section names */
		if (n > 2 && ((ini_buf[0] == M_CFG_SSL &&
			       ini_buf[n - 1] != M_CFG_SSR))) {
			...
			if (ini_buf[0] == M_CFG_SSL) {
				...
				if (ini_info[g_ini_items].section_len >
				...
				ini_buf[n - 1] = 0x00;
				strncpy((char *)tmpSectionName, ini_buf + 1,
					(PARSER_MAX_CFG_BUF + 1) *
						sizeof(char));
				banchmark_flag = 0;
				nodetype_flag = 0;
				strncpy(seq_item[scount], tmpSectionName,
					PARSER_MAX_KEY_NAME_LEN);
				scount++;
		...
		if (isEqualSign == 0) {
			...
			if (strstr(ini_info[g_ini_items].section_name,
				   BENCHMARK_KEY_NAME) != NULL) {
				isEqualSign = -1;
				ini_info[g_ini_items].key_name_len = strlen(
					ini_info[g_ini_items].section_name);
				strncpy(ini_info[g_ini_items].key_name,
					ini_info[g_ini_items].section_name,
					(PARSER_MAX_KEY_NAME_LEN *
					 sizeof(char)));
				ini_info[g_ini_items].key_val_len = n;
			} else if (strstr(ini_info[g_ini_items].section_name,
					  NODE_TYPE_KEY_NAME) != NULL) {
				isEqualSign = -1;
				ini_info[g_ini_items].key_name_len =
					strlen(NODE_TYPE_KEY_NAME);
				strncpy(ini_info[g_ini_items].key_name,
					NODE_TYPE_KEY_NAME,
					(PARSER_MAX_KEY_NAME_LEN *
					 sizeof(char)));
			...
			if (ini_info[g_ini_items].key_name_len >
			    PARSER_MAX_KEY_NAME_LEN) {
		...
		if (ini_info[g_ini_items].key_val_len >
		    PARSER_MAX_KEY_VALUE_LEN) {
...
static int precommd(u8 *cmd, int cmdlen, u8 *precmd, int precmdlen, u8 *rdata,
		    u32 rlen, bool spi_irq, bool i2c_irq)
{
	...
	if (ilits->wrapper(precmd, precmdlen, rdata, rlen, spi_irq, i2c_irq) <
	    0) {
		ILI_ERR("Failed to get cdc data\n");
		return -EMP_GET_CDC;
	}
	...
	for (y = 0; y < core_mp.ych_len; y++) {
		for (x = 0; x < core_mp.xch_len; x++) {
			...
			if (y == 0 && x == 0) {
				tmp[0] = Mathabs(
					centre -
					p_comb[(shift + 1) + x]); /* down */
				tmp[1] = Mathabs(
					centre -
					p_comb[shift + (x + 1)]); /* right */
				...
				tmp[1] = Mathabs(
					centre -
					p_comb[shift + (x - 1)]); /* left */
			} else if (y == (core_mp.ych_len - 1) &&
				   x == (core_mp.xch_len - 1)) {
				tmp[0] = Mathabs(
					centre -
					p_comb[(shift - 1) + x]); /* up */
	...
	for (y = 0; y < core_mp.ych_len; y++) {
		for (x = 0; x < core_mp.xch_len; x++) {
			...
			tItems[index].buf[shift + x] =
				tItems[index].max_buf[shift + x] -
				tItems[index].min_buf[shift + x];
		}
	}
}
	...
	for (y = 0; y < core_mp.ych_len; y++) {
		for (x = 0; x < core_mp.xch_len; x++) {
			...
			if (y != (core_mp.ych_len - 1))
				core_mp.tx_delta_buf[shift + x] =
					Mathabs(p_comb[shift + x] -
						p_comb[(shift + 1) + x]);

			/* Rx Delta */
			if (x != (core_mp.xch_len - 1))
				core_mp.rx_delta_buf[shift + x] =
					Mathabs(p_comb[shift + x] -
						p_comb[shift + (x + 1)]);
		}
	}
...
static char *get_date_time_str(void)
{
	...
	snprintf(time_data_buf, sizeof(time_data_buf),
		 "%04d%02d%02d-%02d%02d%02d", (rtc_now_time.tm_year + 1900),
	...
	snprintf(time_data_buf, sizeof(time_data_buf),
		 "%04d%02d%02d-%02d%02d%02d", (int)(tm.tm_year + 1900),
				...
				int file_size)
{
	...
	tmp_line++;
	tmp_len +=
		snprintf(csv + tmp_len, (file_size - tmp_len), "Test Item:\n");
	tmp_line++;
...
static void mp_print_csv_tail(char *csv, int *csv_len, int file_size)
{
	...
	for (seq = 0; seq < ri.count; seq++) {
		...
		if (tItems[i].item_result == MP_DATA_PASS)
			tmp_len +=
				snprintf(csv + tmp_len, (file_size - tmp_len),
					 "	  {%s}	   ,OK\n",
					 tItems[i].desp);
		else
			tmp_len +=
				 ...
				 int file_size)
{
	...
	char str[128] = { 0 };
	char *open_sp_cmd[] = { "open dac", "open raw1", "open raw2",
				"open raw3" };
	...
	if (ipio_strcmp(name, "open test(integration)_sp") == 0) {
		...
		if (core_mp.td_retry &&
		    (ipio_strcmp(name, "peak to peak_td (lcm off)") == 0)) {
			name = "peak to peak_td (lcm off)_2";
			parser_get_int_data("pv5_4 command", name, str,
					    sizeof(str));
			tmp_len +=
				snprintf(csv + tmp_len, (file_size - tmp_len),
					 "CDC command 2 = ,%s\n", str);
				       ...
				       int file_zise)
{
	...
	for (x = 0; x < core_mp.xch_len; x++) {
		if (x == 0) {
			DUMP("\n %s ", desp);
			tmp_len +=
				snprintf(csv + tmp_len, (file_zise - tmp_len),
					 "\n	   %s ,", desp);
		}
		DUMP("  X_%d	,", (x + 1));
...
static void mp_compare_tip_ring_cdc_show_result(int index, s32 *tmp, char *csv,
						int *csv_len, int type,
						s32 *max_ts, s32 *min_ts,
						const char *desp, int file_zise,
						int trtype)
{
	int x, y, tmp_len = *csv_len;
			  ...
			  int Charge_AA, int Charge_Border, int Charge_Notch)
{
	...
	for (k = 0; k < 8; k++) {
		...
		/*out of range */
		if ((tempX < 0) || (tempX >= core_mp.xch_len) || (tempY < 0) ||
		    (tempY >= core_mp.ych_len))
			continue;

		if ((inNodeType[ADDR(tempX, tempY)] == NO_COMPARE) ||
		    ((inNodeType[ADDR(tempX, tempY)] & Round_Corner) ==
		     Round_Corner) ||
		    ((inNodeType[ADDR(tempX, tempY)] & Skip_Micro) ==
		     Skip_Micro) ||
		    charge_rate[ADDR(tempX, tempY)] == 0)
			continue;

		if ((charge_rate[ADDR(tempX, tempY)] -
		     charge_rate[ADDR(x, y)]) > OpenThreadhold)
			return OpenThreadhold;
	}
	return ret;
}

static int full_open_rate_compare(s32 *full_open, s32 *cbk, int x, int y,
				  s32 inNodeType, int full_open_rate)
{
	int ret = true;

	if ((inNodeType == NO_COMPARE) ||
	    ((inNodeType & Round_Corner) == Round_Corner))
		return true;
...
static s32 open_sp_formula(int dac, int raw, int tvch, int tvcl)
{
	...
	if (id == ILI9881_CHIP) {
		ret = ((dac * 10000 * 161 / 100) -
		       (16384 / 2 - raw) * 20000 * 7 / 16384 * 36 / 10) /
		      (tvch - tvcl) / 2;
	} else {
		ret = ((dac * 10000 * 131 / 100) -
		       (16384 / 2 - raw) * 20000 * 7 / 16384 * 36 / 10) /
		      (tvch - tvcl) / 2;
	}
	return ret;
...
static s32 open_c_formula(int inCap1DAC, int inCap1Raw, int accuracy)
{
	...
	int inVadc_range = open_para.vadc_range; /* from mp.ini */
	...
	inPartRaw = ((inCap1Raw - inFoutRange_half) * inVadc_range * inCint *
		     10 / inFoutRange);

	if (accuracy) {
		inCap1Value = ((inPartDac + inPartRaw) * 10) / inVdrv /
			      inMagnification / inGain;
	} else {
		inCap1Value = (inPartDac + inPartRaw) / inVdrv /
			      inMagnification / inGain;
	}
...
static void allnode_open_cdc_result(int index, int *buf, int *dac, int *raw)
{
	...
	if (ipio_strcmp(desp, "open test(integration)_sp") == 0) {
		for (i = 0; i < core_mp.frame_len; i++)
			...
			ipio_strcmp(desp, "open test_c (openx enable)") == 0) {
		if (parser_get_int_data(desp, "accuracy", str, sizeof(str)) > 0)
			accuracy = ili_hid_katoi(str);
...
static int codeToOhm(s32 *ohm, s32 *Code, u16 *v_tdf, u16 *h_tdf)
{
	...
	for (j = 0; j < core_mp.frame_len; j++) {
		if (Code[j] == 0) {
			...
			temp = ((inTVCH - inTVCL) * inVariation *
				(inTDF1 - inTDF2) * (1 << 12) /
				(9 * Code[j] * inCint)) *
			       1000;
...
static int short_test(int index, int frame_index)
{
	...
	ILI_INFO(
		"TVCH = %d, TVCL = %d, Variation = %d, V_TDF1 = %d, V_TDF2 = %d, H_TDF1 = %d, H_TDF2 = %d, Cint = %d, Rinternal = %d\n",
		short_para.tvch, short_para.tvcl, short_para.variation,
		tItems[index].v_tdf_1, tItems[index].v_tdf_2,
		tItems[index].h_tdf_1, tItems[index].h_tdf_2, short_para.cint,
		short_para.rinternal);

	if (core_mp.protocol_ver >= PROTOCOL_VER_540) {
		/* Calculate code to ohm and save to tItems[index].buf */
		ili_hid_dump_data(frame_buf, 10, core_mp.frame_len,
				  core_mp.xch_len, "Short Raw 1");
		codeToOhm(&tItems[index].buf[frame_index * core_mp.frame_len],
			  frame_buf, v_tdf, h_tdf);
		ili_hid_dump_data(
			&tItems[index].buf[frame_index * core_mp.frame_len], 10,
...
static int allnode_key_cdc_data(int index)
{
	...
	ili_hid_dump_data(ori, 8, len, 0, "Key CDC original");
	...
	ili_hid_dump_data(key_buf, 32, core_mp.frame_len, core_mp.xch_len,
...
static int mp_cdc_get_pv5_4_command(u8 *cmd, int len, int index)
{
	...
	if (core_mp.td_retry &&
	    (ipio_strcmp(key, "peak to peak_td (lcm off)") == 0))
...
static int mp_cdc_init_cmd_common(u8 *cmd, int len, int index)
{
	...
	if (tItems[index].catalog == PEAK_TO_PEAK_TEST) {
		...
		if (ipio_strcmp(tItems[index].desp,
...
static int allnode_open_cdc_data(int mode, int *buf, int index)
{
	...
	if (tItems[index].pre_cmd_en) {
		...
		ili_hid_dump_data(precmd, 16, precmd_len, 0, "PreCommand");
	...
	if (parser_get_int_data("pv5_4 command", key[mode], str, sizeof(str)) <
	    0) {
	...
	ili_hid_dump_data(cmd, 8, sizeof(cmd), 0, "Open SP command");
	...
	if (!ilits->eng_flow) {
		if (tItems[index].pre_cmd_en) {
			if (precommd(cmd, core_mp.cdc_len, precmd, precmd_len,
				     ori, len, ON, ON) < 0)
				goto out;
		} else {
			if (ilits->wrapper(cmd, core_mp.cdc_len, ori, len, ON,
					   ON) < 0) {
				ILI_ERR("Failed to get cdc data\n");
				ret = -EMP_GET_CDC;
				goto out;
		...
		msleep(200);
		if (ilits->wrapper(cmd, core_mp.cdc_len, ori, len, OFF, OFF) <
		    0) {
			ILI_ERR("Failed to get cdc data\n");
		}
	}
#else
	if (tItems[index].pre_cmd_en) {
		if (precommd(cmd, core_mp.cdc_len, precmd, precmd_len, ori, len,
			     ON, ON) < 0)
			goto out;
	} else {
	...
	ili_hid_dump_data(ori, 8, len, 0, "Open SP CDC original");

	/* Convert original data to the physical one in each node */
	for (i = 0; i < core_mp.frame_len; i++) {
		if ((mode == OPEN_DAC_KEY) || (mode == OPEN_CAP1_DAC_KEY) ||
		    (mode == OPENX_CAP1_DAC_KEY)) {
			/* DAC - P */
			if (((ori[(2 * i) + 1] & 0x80) >> 7) == 1) {
				/* Negative */
				inDACp = 0 - (int)(ori[(2 * i) + 1] & 0x7F);
	...
	ili_hid_dump_data(buf, 10, core_mp.frame_len, core_mp.xch_len,
...
static int allnode_peak_to_peak_cdc_data(int index)
{
	...
	if (frm_buf == NULL) {
		...
		for (i = 0; i < tItems[index].bch_mrk_frm_num; i++) {
			frm_buf[i] = (s32 *)kzalloc(
				core_mp.frame_len * sizeof(s32), GFP_KERNEL);
			if (ERR_ALLOC_MEM(frm_buf)) {
	...
	ili_hid_dump_data(cmd, 8, core_mp.cdc_len, 0, "Mutual CDC command");
	...
	for (k = 0; k < rd_frame_num; k++) {
		...
		if (!ilits->eng_flow) {
			if (tItems[index].pre_cmd_en) {
				if (precommd(cmd, cmd_len, precmd, precmd_len,
					     ori, len, ON, ON) < 0) {
					goto out;
				}
			} else {
				if (ilits->wrapper(cmd, cmd_len, ori, len, ON,
						   ON) < 0) {
					ILI_ERR("Failed to get cdc data\n");
					ret = -EMP_GET_CDC;
					goto out;
			...
			msleep(200);
			if (ilits->wrapper(cmd, cmd_len, ori, len, OFF, OFF) <
			    0) {
				ILI_ERR("Failed to get cdc data\n");
			}
		}
#else
		if (tItems[index].pre_cmd_en) {
			if (precommd(cmd, cmd_len, precmd, precmd_len, ori, len,
				     ON, ON) < 0) {
				goto out;
			}
		} else {
			if (ilits->wrapper(cmd, cmd_len, ori, len, ON, ON) <
			    0) {
				ILI_ERR("Failed to get cdc data\n");
				ret = -EMP_GET_CDC;
				goto out;
		...
		ili_hid_dump_data(ori, 8, len, 0, "Mutual CDC original");
		...
		for (i = 0; i < (len / 2 - 1); i++) {
			...
			if (tItems[index].bch_mrk_multi && rd_frame_num == k + 1)
				frm_buf[k + 1][i] =
					frm_buf[k - 1][i] - frm_buf[k][i];
		}
	}

	if (tItems[index].bch_mrk_multi) {
		ili_hid_dump_data(frm_buf[0], 32, (len / 2 - 1),
				  core_mp.xch_len,
				  "Mutual CDC combined[0]/frame1");
		ili_hid_dump_data(frm_buf[1], 32, (len / 2 - 1),
				  core_mp.xch_len,
				  "Mutual CDC combined[1]/frame2");
		ili_hid_dump_data(frm_buf[2], 32, (len / 2 - 1),
				  core_mp.xch_len,
...
static int allnode_mutual_cdc_data(int index)
{
	...
	if (tItems[index].pre_cmd_en) {
		...
		if (ilits->wrapper(cmd, core_mp.cdc_len, ori, len, ON, ON) <
		    0) {
			ILI_ERR("Failed to get cdc data\n");
			ret = -EMP_GET_CDC;
			goto out;
	...
	/* Convert original data to the physical one in each node */
	for (i = 0; i < cdc_len; i++) {
		if (ipio_strcmp(tItems[index].desp, "calibration data(dac)") ==
		    0) {
			/* DAC - P */
			if (((ori[(2 * i) + 1] & 0x80) >> 7) == 1) {
				/* Negative */
			...
			if (ipio_strcmp(tItems[index].desp,
					"raw data(no bk)") == 0 ||
			    ipio_strcmp(tItems[index].desp,
					"raw data(no bk) (lcm off)") == 0) {
				frame_buf[i] -= core_mp.no_bk_shift;
			}
		}
	}

	ili_hid_dump_data(frame_buf, 32, cdc_len, core_mp.xch_len,
			  "Mutual CDC combined");
...
static void compare_MaxMin_result(int index, s32 *data)
{
	int x, y;
	if (tItems[index].catalog == TIPRING_RAW ||
	    tItems[index].catalog == TIPRING_P2P) {
...
static int create_mp_test_frame_buffer(int index, int frame_count)
{
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		if (core_mp.tx_delta_buf == NULL) {
			core_mp.tx_delta_buf = kcalloc(core_mp.frame_len,
		...
		if (core_mp.rx_delta_buf == NULL) {
			core_mp.rx_delta_buf = kcalloc(core_mp.frame_len,
						       sizeof(s32), GFP_KERNEL);
		...
		if (core_mp.tx_max_buf == NULL) {
			core_mp.tx_max_buf = kcalloc(core_mp.frame_len,
						     sizeof(s32), GFP_KERNEL);
		...
		if (core_mp.tx_min_buf == NULL) {
			core_mp.tx_min_buf = kcalloc(core_mp.frame_len,
						     sizeof(s32), GFP_KERNEL);
		...
		if (core_mp.rx_max_buf == NULL) {
			core_mp.rx_max_buf = kcalloc(core_mp.frame_len,
						     sizeof(s32), GFP_KERNEL);
		...
		if (core_mp.rx_min_buf == NULL) {
			core_mp.rx_min_buf = kcalloc(core_mp.frame_len,
						     sizeof(s32), GFP_KERNEL);
		...
		if (tItems[index].buf == NULL) {
			tItems[index].buf = vmalloc(
				frame_count * core_mp.frame_len * sizeof(s32));
		...
		if (tItems[index].result_buf == NULL) {
			tItems[index].result_buf = kcalloc(
				core_mp.frame_len, sizeof(s32), GFP_KERNEL);
		...
		if (tItems[index].spec_option == BENCHMARK) {
			if (tItems[index].bch_mrk_max == NULL) {
				tItems[index].bch_mrk_max = (s32 **)kzalloc(
					tItems[index].bch_mrk_frm_num *
						sizeof(s32 *),
					GFP_KERNEL);
				if (ERR_ALLOC_MEM(tItems[index].bch_mrk_max)) {
					ILI_ERR("Failed to allocate bch_mrk_max mem\n");
					ipio_kfree((void **)&tItems[index]
							   .bch_mrk_max);
					return -ENOMEM;
				}

				for (i = 0; i < tItems[index].bch_mrk_frm_num;
				     i++) {
			...
			if (tItems[index].bch_mrk_min == NULL) {
				tItems[index].bch_mrk_min = (s32 **)kzalloc(
					tItems[index].bch_mrk_frm_num *
						sizeof(s32 *),
					GFP_KERNEL);
			...
			if (tItems[index].bench_mark_max == NULL) {
				tItems[index].bench_mark_max =
					kcalloc(core_mp.frame_len, sizeof(s32),
						GFP_KERNEL);
				if (ERR_ALLOC_MEM(
					    tItems[index].bench_mark_max)) {
...
static int pin_test(int index)
{
	...
	if (tItems[index].int_pulse_test == ENABLE) {
		...
		ret = ili_hid_irq_register(IRQF_TRIGGER_RISING);
		if (ret < 0) {
			ret = -EMP_INT;
			goto out;
		}
		...
		ili_hid_irq_unregister();
		ret = ili_hid_irq_register(IRQF_TRIGGER_FALLING);
		if (ret < 0) {
			ret = -EMP_INT;
			goto out;
		}
...
static int mutual_test(int index)
{
	...
	if (tItems[index].catalog == TIPRING_RAW) {
		...
		for (y = 0; y < core_mp.ych_len; y++) {
			for (x = 0; x < core_mp.xch_len; x++) {
				if (tItems[i].catalog == TX_RX_DELTA) {
					core_mp.tx_max_buf[y * core_mp.xch_len +
							   x] = INT_MIN;
					core_mp.rx_max_buf[y * core_mp.xch_len +
							   x] = INT_MIN;
					core_mp.tx_min_buf[y * core_mp.xch_len +
							   x] = INT_MAX;
					core_mp.rx_min_buf[y * core_mp.xch_len +
							   x] = INT_MAX;
				} else {
					tItems[index]
						.max_buf[y * core_mp.xch_len +
							 x] = INT_MIN;
					tItems[index]
						.min_buf[y * core_mp.xch_len +
							 x] = INT_MAX;
				}
			}
		}
		...
		if (tItems[index].spec_option == BENCHMARK) {
			parser_ini_benchmark(tItems[index].bench_mark_max,
					     tItems[index].bench_mark_min,
					     tItems[index].type_option,
					     tItems[index].desp,
					     core_mp.frame_len,
					     BENCHMARK_KEY_NAME);
			dump_benchmark_data(tItems[index].bench_mark_max,
					    tItems[index].bench_mark_min);
		}
	}
	...
	for (i = 0; i < get_frame_cont; i++) {
		...
		case TIPRING_RAW:
			for (j = 0; j < core_mp.tipring_len; j++)
				tItems[index].buf[i * core_mp.tipring_len + j] =
					frame_buf[j];
			break;
		default:
			for (j = 0; j < core_mp.frame_len; j++)
				tItems[index].buf[i * core_mp.frame_len + j] =
					frame_buf[j];
			break;
		}
		if (tItems[index].catalog == TIPRING_RAW)
			compare_MaxMin_result(
				index,
				&tItems[index].buf[i * core_mp.tipring_len]);
		else
			compare_MaxMin_result(
				index,
...
static int peak_to_peak_test(int index)
{
	...
	if (tItems[index].spec_option == BENCHMARK) {
		if (tItems[index].catalog == TIPRING_P2P) {
			...
			for (i = 0; i < tItems[index].bch_mrk_frm_num; i++) {
				if (tItems[index].bch_mrk_multi)
					snprintf(benchmark_str,
						 sizeof(benchmark_str), "%s%d",
						 BENCHMARK_KEY_NAME, i + 1);
				else
					snprintf(benchmark_str,
						 sizeof(benchmark_str), "%s",
						 BENCHMARK_KEY_NAME);
				...
				parser_ini_benchmark(
					...
					tItems[index].bch_mrk_min[i],
					tItems[index].type_option,
					tItems[index].desp, core_mp.frame_len,
					benchmark_str);
	...
	for (y = 0; y < core_mp.ych_len; y++) {
		for (x = 0; x < core_mp.xch_len; x++) {
			tItems[index].max_buf[y * core_mp.xch_len + x] =
				INT_MIN;
			tItems[index].min_buf[y * core_mp.xch_len + x] =
				INT_MAX;
		}
	}
...
static int open_test_sp(int index)
{
	...
	int Charge_AA = 0, Charge_Border = 0, Charge_Notch = 0,
	    full_open_rate = 0;
	char str[512] = { 0 };
	...
	open = kzalloc(tItems[index].frame_count *
			       sizeof(struct mp_test_P540_open),
		       GFP_KERNEL);
	...
	if (frame1_cbk700 == NULL) {
		frame1_cbk700 =
			kcalloc(core_mp.frame_len, sizeof(s32), GFP_KERNEL);
	...
	parser_ini_nodetype(tItems[index].node_type, NODE_TYPE_KEY_NAME,
			    core_mp.frame_len);
	dump_node_type_buffer(tItems[index].node_type, "node type");

	parser_get_int_data(tItems[index].desp, "charge_aa", str, sizeof(str));
	Charge_AA = ili_hid_katoi(str);

	parser_get_int_data(tItems[index].desp, "charge_border", str,
			    sizeof(str));
	Charge_Border = ili_hid_katoi(str);

	parser_get_int_data(tItems[index].desp, "charge_notch", str,
			    sizeof(str));
	Charge_Notch = ili_hid_katoi(str);

	parser_get_int_data(tItems[index].desp, "full open", str, sizeof(str));
	full_open_rate = ili_hid_katoi(str);

	parser_get_int_data(tItems[index].desp, "tvch", str, sizeof(str));
	open_para.tvch = ili_hid_katoi(str);

	parser_get_int_data(tItems[index].desp, "tvcl", str, sizeof(str));
	open_para.tvcl = ili_hid_katoi(str);

	ILI_DBG("AA = %d, Border = %d, Notch = %d, full_open_rate = %d, tvch = %d, tvcl = %d\n",
		Charge_AA, Charge_Border, Charge_Notch, full_open_rate,
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		allnode_open_cdc_result(index, open[i].cbk_700, open[i].dac,
					open[i].tdf_700);
		allnode_open_cdc_result(index, open[i].cbk_250, open[i].dac,
					open[i].tdf_250);
		allnode_open_cdc_result(index, open[i].cbk_200, open[i].dac,
					open[i].tdf_200);
		...
		if (i == 0) {
			ipio_memcpy(frame1_cbk700, open[i].cbk_700,
				    core_mp.frame_len * sizeof(s32),
				    core_mp.frame_len * sizeof(s32));
			ipio_memcpy(frame1_cbk250, open[i].cbk_250,
				    core_mp.frame_len * sizeof(s32),
				    core_mp.frame_len * sizeof(s32));
			ipio_memcpy(frame1_cbk200, open[i].cbk_200,
				    core_mp.frame_len * sizeof(s32),
				    core_mp.frame_len * sizeof(s32));
		}

		ili_hid_dump_data(open[i].cbk_700, 10, core_mp.frame_len,
				  core_mp.xch_len, "cbk 700");
		ili_hid_dump_data(open[i].cbk_250, 10, core_mp.frame_len,
				  core_mp.xch_len, "cbk 250");
		ili_hid_dump_data(open[i].cbk_200, 10, core_mp.frame_len,
				  core_mp.xch_len, "cbk 200");

		for (y = 0; y < core_mp.ych_len; y++) {
			for (x = 0; x < core_mp.xch_len; x++) {
				open[i].charg_rate[addr] =
					open[i].cbk_250[addr] * 100 /
					open[i].cbk_700[addr];
				open[i].full_Open[addr] =
					open[i].cbk_700[addr] -
					open[i].cbk_200[addr];
				addr++;
			}
		...
		ili_hid_dump_data(open[i].charg_rate, 10, core_mp.frame_len,
				  core_mp.xch_len, "origin charge rate");
		ili_hid_dump_data(open[i].full_Open, 10, core_mp.frame_len,
				  core_mp.xch_len, "origin full open");

		addr = 0;
		for (y = 0; y < core_mp.ych_len; y++) {
			for (x = 0; x < core_mp.xch_len; x++) {
				if (full_open_rate_compare(
					    open[i].full_Open, open[i].cbk_700,
					    x, y, tItems[index].node_type[addr],
					    full_open_rate) == false) {
					tItems[index]
						.buf[(i * core_mp.frame_len) +
						     addr] = 0;
		...
		ili_hid_dump_data(&tItems[index].buf[(i * core_mp.frame_len)],
				  10, core_mp.frame_len, core_mp.xch_len,
		...
		for (y = 0; y < core_mp.ych_len; y++) {
			for (x = 0; x < core_mp.xch_len; x++) {
				tItems[index]
					.buf[(i * core_mp.frame_len) + addr] =
					compare_charge(open[i].charg_rate, x, y,
						       tItems[index].node_type,
						       Charge_AA, Charge_Border,
						       Charge_Notch);
				addr++;
			}
		}
		...
		compare_MaxMin_result(
			index, &tItems[index].buf[(i * core_mp.frame_len)]);
	}
...
static int open_test_cap(int index)
{
	...
	isOpenX = (ipio_strcmp(tItems[index].desp, "open test_c (openx enable)") == 0) ?
				ENABLE : DISABLE;
	...
	open_para.gain = ili_hid_katoi(str);
	...
	ILI_INFO("gain = %d, tvch = %d, tvcl = %d, cbk_step = %d, cint = %d\n",
		 open_para.gain, open_para.tvch, open_para.tvcl,
		 open_para.cbk_step, open_para.cint);
	for (i = 0; i < tItems[index].frame_count; i++) {
		open[i].cap_dac =
			kcalloc(core_mp.frame_len, sizeof(s32), GFP_KERNEL);
		open[i].cap_raw =
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if (i == 0) {
			if (isOpenX) {
				ipio_memcpy(openx_cap_dac, open[i].cap_dac,
						core_mp.frame_len * sizeof(s32),
						core_mp.frame_len * sizeof(s32));
				ipio_memcpy(openx_cap_raw, open[i].cap_raw,
						core_mp.frame_len * sizeof(s32),
						core_mp.frame_len * sizeof(s32));
			} else {
				ipio_memcpy(cap_dac, open[i].cap_dac,
						core_mp.frame_len * sizeof(s32),
						core_mp.frame_len * sizeof(s32));
				ipio_memcpy(cap_raw, open[i].cap_raw,
						core_mp.frame_len * sizeof(s32),
						core_mp.frame_len * sizeof(s32));
			}
		}

		ili_hid_dump_data(open[i].dcl_cap, 10, core_mp.frame_len,
				  core_mp.xch_len, "DCL_Cap");
				     ...
				     s32 *avg_result)
{
	...
	u32up_frame = tItems[index].frame_count *
		      tItems[index].highest_percentage / 100;
	u32down_frame = tItems[index].frame_count *
			tItems[index].lowest_percentage / 100;
	...
	for (i = 0; i < core_mp.frame_len; i++) {
		for (j = 0; j < tItems[index].frame_count - 1; j++) {
			for (k = 0; k < (tItems[index].frame_count - 1 - j);
			     k++) {
	...
	if (debug_en) {
		...
		for (i = 0; i < core_mp.frame_len; i++) {
			for (j = u32down_frame;
			     j < tItems[index].frame_count - u32up_frame; j++) {
				if ((i < len) ||
				    (i >= (core_mp.frame_len - len)))
					pr_cont("%d,",
						u32data_buff
	...
	for (i = 0; i < core_mp.frame_len; i++) {
		...
		for (j = u32down_frame;
		     j < tItems[index].frame_count - u32up_frame; j++)
			u32sum_raw_data[i] +=
				u32data_buff[i + j * core_mp.frame_len];

		avg_result[i] =
			u32sum_raw_data[i] / (tItems[index].frame_count -
					      u32down_frame - u32up_frame);
	}
...
static int mp_compare_test_result(int index)
{
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		...
		mp_compare_cdc_result(index, core_mp.tx_max_buf, max_threshold,
				      min_threshold, &test_result);
		mp_compare_cdc_result(index, core_mp.tx_min_buf, max_threshold,
				      min_threshold, &test_result);
		...
		mp_compare_cdc_result(index, core_mp.rx_max_buf, max_threshold,
				      min_threshold, &test_result);
		mp_compare_cdc_result(index, core_mp.rx_min_buf, max_threshold,
				      min_threshold, &test_result);
	} else {
		if (ERR_ALLOC_MEM(tItems[index].buf) ||
		    ERR_ALLOC_MEM(tItems[index].max_buf) ||
		...
		if (tItems[index].spec_option == BENCHMARK) {
			if (tItems[index].catalog == PEAK_TO_PEAK_TEST ||
				...
				for (i = 0; i < cdclen; i++) {
					max_threshold[i] =
						tItems[index].bch_mrk_max[0][i];
					min_threshold[i] =
						tItems[index].bch_mrk_min[0][i];
				}
			} else {
				for (i = 0; i < cdclen; i++) {
					max_threshold[i] =
						tItems[index].bench_mark_max[i];
					min_threshold[i] =
						tItems[index].bench_mark_min[i];
				}
			}
		} else {
			if (tItems[index].catalog == TIPRING_RAW ||
			    tItems[index].catalog == TIPRING_P2P) {
				for (i = 0; i < core_mp.nPxRaw_tipring_len;
				     i++) {
					max_threshold[i] = tItems[index].max1;
					min_threshold[i] = tItems[index].min1;
				}
				for (i = core_mp.nPxRaw_tipring_len;
				     i < core_mp.tipring_len; i++) {
		...
		/* general result */
		if (tItems[index].trimmed_mean &&
		    tItems[index].catalog != PEAK_TO_PEAK_TEST) {
			mp_test_data_sort_average(tItems[index].buf, index,
						  tItems[index].result_buf);
			mp_compare_cdc_result(index, tItems[index].result_buf,
			...
			if (tItems[index].bch_mrk_multi) {
				...
				mp_compare_cdc_result(index,
						      tItems[index].max_buf,
						      max_threshold,
						      min_threshold,
						      &test_result);
				mp_compare_cdc_result(index,
						      tItems[index].min_buf,
						      max_threshold,
						      min_threshold,
						      &test_result);
			}
...
static int mp_show_result(bool lcm_on)
{
	...
	int i, x, y, j, csv_len = 0, pass_item_count = 0, line_count = 0,
			get_frame_cont = 1;
	s32 *max_threshold = NULL, *min_threshold = NULL;
	char *csv = NULL;
	...
	if (parser_get_ini_key_value("pv5_4 command", "date",
				     core_mp.ini_date) < 0)
		ipio_memcpy(core_mp.ini_date, "Unknown", strlen("Unknown"),
			    strlen("Unknown"));
	if (parser_get_ini_key_value("pv5_4 command", "version",
				     core_mp.ini_ver) < 0)
		ipio_memcpy(core_mp.ini_ver, "Unknown", strlen("Unknown"),
			    strlen("Unknown"));
	...
	for (seq = 0; seq < ri.count; seq++) {
		...
		if (tItems[i].item_result == MP_DATA_PASS) {
			pr_info("\n[%s],OK \n\n", tItems[i].desp);
			csv_len += snprintf(csv + csv_len,
					    (CSV_FILE_SIZE - csv_len),
					    "\n[%s],OK\n\n", tItems[i].desp);
		} else {
			pr_info("\n[%s],NG \n\n", tItems[i].desp);
			csv_len += snprintf(csv + csv_len,
		...
		csv_len +=
			snprintf(csv + csv_len, (CSV_FILE_SIZE - csv_len),
				 "Frame count = %d\n", tItems[i].frame_count);

		if (tItems[i].trimmed_mean &&
		    tItems[i].catalog != PEAK_TO_PEAK_TEST) {
		...
		if (tItems[i].spec_option == BENCHMARK) {
			if (tItems[i].catalog == PEAK_TO_PEAK_TEST) {
				for (j = 0; j < core_mp.frame_len; j++) {
					max_threshold[j] =
						tItems[i].bch_mrk_max[0][j];
					min_threshold[j] =
						tItems[i].bch_mrk_min[0][j];
				}

				for (j = 0; j < tItems[i].bch_mrk_frm_num;
				     j++) {
					...
					mp_compare_cdc_show_result(
						i, tItems[i].bch_mrk_max[j],
						csv, &csv_len, TYPE_BENCHMARK,
						max_threshold, min_threshold,
				...
				for (j = 0; j < core_mp.tipring_len; j++) {
					max_threshold[j] =
						tItems[i].bench_mark_max[j];
					min_threshold[j] =
						tItems[i].bench_mark_min[j];
				}
				mp_compare_tip_ring_cdc_show_result(
					i, tItems[i].bench_mark_max, csv,
					&csv_len, TYPE_BENCHMARK, max_threshold,
					min_threshold, "Max_Bench1",
					CSV_FILE_SIZE, TIPRING_X);
				mp_compare_tip_ring_cdc_show_result(
					i, tItems[i].bench_mark_min, csv,
					&csv_len, TYPE_BENCHMARK, max_threshold,
					min_threshold, "Min_Bench1",
					CSV_FILE_SIZE, TIPRING_X);
				mp_compare_tip_ring_cdc_show_result(
					i, tItems[i].bench_mark_max, csv,
					&csv_len, TYPE_BENCHMARK, max_threshold,
					min_threshold, "Max_Bench2",
					CSV_FILE_SIZE, TIPRING_Y);
				mp_compare_tip_ring_cdc_show_result(
					i, tItems[i].bench_mark_min, csv,
					&csv_len, TYPE_BENCHMARK, max_threshold,
					min_threshold, "Min_Bench2",
					CSV_FILE_SIZE, TIPRING_Y);
				...
				for (j = 0; j < tItems[i].bch_mrk_frm_num; j++) {
					mp_compare_tip_ring_cdc_show_result(
						...
						csv, &csv_len, TYPE_BENCHMARK,
						max_threshold, min_threshold,
						get_csv_frame_name("Max_Bench", j + 1),
						CSV_FILE_SIZE, TIPRING_X);
					mp_compare_tip_ring_cdc_show_result(
						i, tItems[i].bch_mrk_min[j],
						csv, &csv_len, TYPE_BENCHMARK,
						max_threshold, min_threshold,
						get_csv_frame_name("Min_Bench", j + 1),
						CSV_FILE_SIZE, TIPRING_X);
				}
				...
				mp_compare_cdc_show_result(
					i, tItems[i].bench_mark_max, csv,
					&csv_len, TYPE_BENCHMARK, max_threshold,
					min_threshold, "Max_Bench",
					CSV_FILE_SIZE);
				mp_compare_cdc_show_result(
					i, tItems[i].bench_mark_min, csv,
					&csv_len, TYPE_BENCHMARK, max_threshold,
					min_threshold, "Min_Bench",
					CSV_FILE_SIZE);
			}
		} else {
			if (tItems[i].catalog == TIPRING_RAW ||
			    tItems[i].catalog == TIPRING_P2P) {
				for (j = 0; j < core_mp.nPxRaw_tipring_len;
				     j++) {
					max_threshold[j] = tItems[i].max1;
					min_threshold[j] = tItems[i].min1;
				}
				for (j = core_mp.nPxRaw_tipring_len;
				     j < core_mp.tipring_len; j++) {
		...
		if (ipio_strcmp(tItems[i].desp, "open test(integration)_sp") == 0) {
			mp_compare_cdc_show_result(i, frame1_cbk700, csv,
						   &csv_len, TYPE_NO_JUGE,
						   max_threshold, min_threshold,
						   "frame1 cbk700",
						   CSV_FILE_SIZE);
			mp_compare_cdc_show_result(i, frame1_cbk250, csv,
						   &csv_len, TYPE_NO_JUGE,
						   max_threshold, min_threshold,
						   "frame1 cbk250",
						   CSV_FILE_SIZE);
			mp_compare_cdc_show_result(i, frame1_cbk200, csv,
						   &csv_len, TYPE_NO_JUGE,
						   max_threshold, min_threshold,
						   "frame1 cbk200",
						   CSV_FILE_SIZE);
		...
		if (ipio_strcmp(tItems[i].desp, "open test_c") == 0) {
			mp_compare_cdc_show_result(i, cap_dac, csv, &csv_len,
						  TYPE_NO_JUGE, max_threshold, min_threshold,
						  "CAP_DAC", CSV_FILE_SIZE);
			mp_compare_cdc_show_result(i, cap_raw, csv, &csv_len,
						  TYPE_NO_JUGE, max_threshold, min_threshold,
						  "CAP_RAW", CSV_FILE_SIZE);
		}

		if (ipio_strcmp(tItems[i].desp, "open test_c (openx enable)") == 0) {
			mp_compare_cdc_show_result(i, openx_cap_dac, csv, &csv_len,
						  TYPE_NO_JUGE, max_threshold, min_threshold,
						  "CAP_DAC", CSV_FILE_SIZE);
			mp_compare_cdc_show_result(i, openx_cap_raw, csv, &csv_len,
						  TYPE_NO_JUGE, max_threshold, min_threshold,
						  "CAP_RAW", CSV_FILE_SIZE);
		}
		...
		if (tItems[i].catalog == TX_RX_DELTA) {
			...
			if (ERR_ALLOC_MEM(tItems[i].buf) ||
			    ERR_ALLOC_MEM(tItems[i].max_buf) ||
		...
		if (tItems[i].catalog == KEY_TEST) {
			...
			mp_compare_cdc_show_result(i, core_mp.tx_max_buf, csv,
						   &csv_len, TYPE_JUGE,
						   max_threshold, min_threshold,
						   "TX Max Hold",
						   CSV_FILE_SIZE);
			mp_compare_cdc_show_result(i, core_mp.tx_min_buf, csv,
						   &csv_len, TYPE_JUGE,
						   max_threshold, min_threshold,
						   "TX Min Hold",
						   CSV_FILE_SIZE);
			...
			mp_compare_cdc_show_result(i, core_mp.rx_max_buf, csv,
						   &csv_len, TYPE_JUGE,
						   max_threshold, min_threshold,
						   "RX Max Hold",
						   CSV_FILE_SIZE);
			mp_compare_cdc_show_result(i, core_mp.rx_min_buf, csv,
						   &csv_len, TYPE_JUGE,
						   max_threshold, min_threshold,
						   "RX Min Hold",
						   CSV_FILE_SIZE);
			...
			for (j = 0; j < get_frame_cont; j++) {
				mp_compare_tip_ring_cdc_show_result(
					...
					&tItems[i].buf[(j *
							core_mp.tipring_len)],
					csv, &csv_len, TYPE_NO_JUGE,
					max_threshold, min_threshold,
					get_csv_frame_name("Frame ", j + 1),
					CSV_FILE_SIZE, TIPRING_X);
			}
			for (j = 0; j < get_frame_cont; j++) {
				mp_compare_tip_ring_cdc_show_result(
			...
			if (tItems[i].trimmed_mean &&
			    tItems[i].catalog != PEAK_TO_PEAK_TEST) {
				mp_compare_cdc_show_result(
					i, tItems[i].result_buf, csv, &csv_len,
					TYPE_JUGE, max_threshold, min_threshold,
					"Mean result", CSV_FILE_SIZE);
			} else {
				if (!tItems[i].bch_mrk_multi) {
					mp_compare_cdc_show_result(
						i, tItems[i].max_buf, csv,
						&csv_len, TYPE_JUGE,
						max_threshold, min_threshold,
						"Max Hold", CSV_FILE_SIZE);
					mp_compare_cdc_show_result(
						i, tItems[i].min_buf, csv,
						&csv_len, TYPE_JUGE,
						max_threshold, min_threshold,
						"Min Hold", CSV_FILE_SIZE);
				}
			}
			if (tItems[i].catalog != PEAK_TO_PEAK_TEST)
				get_frame_cont = tItems[i].frame_count;
			...
			for (j = 0; j < get_frame_cont; j++) {
				mp_compare_cdc_show_result(
					i,
					&tItems[i].buf[(j * core_mp.frame_len)],
					csv, &csv_len, TYPE_NO_JUGE,
					max_threshold, min_threshold,
					get_csv_frame_name("Frame ", j + 1),
					CSV_FILE_SIZE);
	...
	if (pass_item_count == 0) {
		...
		if (lcm_on)
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len),
				 "%s/%s_%s.csv", CSV_LCM_ON_PATH,
				 get_date_time_str(), ret_fail_name);
		else
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len),
				 "%s/%s_%s.csv", CSV_LCM_OFF_PATH,
				 get_date_time_str(), ret_fail_name);
	} else {
...
static void ilitek_tddi_mp_init_item(void)
{
	...
	if (ilits->PenType == POSITION_PEN_TYPE_ON) {
		...
		core_mp.nPxRaw_tipring_len =
			2 * core_mp.nPxRaw * core_mp.xch_len;
		core_mp.nPyRaw_tipring_len =
			2 * core_mp.nPyRaw * core_mp.ych_len;
		core_mp.tipring_len =
			core_mp.nPxRaw_tipring_len + core_mp.nPyRaw_tipring_len;
	}
	...
	if (ilits->PenType == POSITION_PEN_TYPE_ON) {
		ILI_INFO("nPxRaw = %d,nPyRaw = %d\n", core_mp.nPxRaw,
			 core_mp.nPyRaw);
		ILI_INFO(
			"Tip Ring length = %d, nPxRaw buffer length = %d, nPyRaw buffer length = %d\n",
			core_mp.tipring_len, core_mp.nPxRaw_tipring_len,
			core_mp.nPyRaw_tipring_len);
	}
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		...
		if (tItems[i].catalog == MUTUAL_TEST) {
			...
			if (ipio_strcmp(tItems[i].desp,
					"open test(integration)_sp") == 0)
				tItems[i].do_test = open_test_sp;
			else if ((ipio_strcmp(tItems[i].desp, "open test_c") == 0) ||
				 (ipio_strcmp(tItems[i].desp, "open test_c (openx enable)") == 0))
...
static void mp_p2p_td_retry_after_ra_fail(int p2p_td)
{
	...
	ILI_DBG("i = %d, p2p_noise_ret = %d, p2p_noise_run = %d\n", i,
		tItems[i].item_result, tItems[i].run);
...
static void mp_test_run(int index)
{
	...
	tItems[i].spec_option = ili_hid_katoi(str);
	parser_get_int_data(tItems[i].desp, "type option", str, sizeof(str));
	tItems[i].type_option = ili_hid_katoi(str);
	parser_get_int_data(tItems[i].desp, "frame count", str, sizeof(str));
	tItems[i].frame_count = ili_hid_katoi(str);
	parser_get_int_data(tItems[i].desp, "trimmed mean", str, sizeof(str));
	tItems[i].trimmed_mean = ili_hid_katoi(str);
	parser_get_int_data(tItems[i].desp, "lowest percentage", str,
			    sizeof(str));
	tItems[i].lowest_percentage = ili_hid_katoi(str);
	parser_get_int_data(tItems[i].desp, "highest percentage", str,
			    sizeof(str));
	tItems[i].highest_percentage = ili_hid_katoi(str);
	parser_get_int_data(tItems[i].desp, "goldenmode", str, sizeof(str));
	tItems[i].goldenmode = ili_hid_katoi(str);
	parser_get_int_data(tItems[i].desp, "max min mode", str, sizeof(str));
	tItems[i].max_min_mode = ili_hid_katoi(str);

	parser_get_int_data(tItems[i].desp, "retry count", str, sizeof(str));
	tItems[i].retry_cnt = ili_hid_katoi(str);

	if (tItems[i].goldenmode &&
	    (tItems[i].spec_option != tItems[i].goldenmode))
		core_mp.lost_benchmark = true;

	if (core_mp.pre_cmd_sp) {
		parser_get_int_data(tItems[i].desp, "precommand", str,
				    sizeof(str));
		tItems[i].pre_cmd_en = ili_hid_katoi(str);
	}

	/* Get pin test delay time */
	if (tItems[i].catalog == PIN_TEST) {
		parser_get_int_data(tItems[i].desp, "test int pin", str,
				    sizeof(str));
		tItems[i].test_int_pin = ili_hid_katoi(str);
		parser_get_int_data(tItems[i].desp, "int pulse test", str,
				    sizeof(str));
		tItems[i].int_pulse_test = ili_hid_katoi(str);
		parser_get_int_data(tItems[i].desp, "delay time", str,
				    sizeof(str));
		tItems[i].delay_time = ili_hid_katoi(str);
	}

	/* Get TDF value from ini */
	if (tItems[i].catalog == SHORT_TEST) {
		parser_get_int_data(tItems[i].desp, "v_tdf_1", str,
				    sizeof(str));
		tItems[i].v_tdf_1 =
			parser_get_tdf_value(str, tItems[i].catalog);
		parser_get_int_data(tItems[i].desp, "v_tdf_2", str,
				    sizeof(str));
		tItems[i].v_tdf_2 =
			parser_get_tdf_value(str, tItems[i].catalog);
		parser_get_int_data(tItems[i].desp, "h_tdf_1", str,
				    sizeof(str));
		tItems[i].h_tdf_1 =
			parser_get_tdf_value(str, tItems[i].catalog);
		parser_get_int_data(tItems[i].desp, "h_tdf_2", str,
				    sizeof(str));
		tItems[i].h_tdf_2 =
			parser_get_tdf_value(str, tItems[i].catalog);
	} else {
		...
		if (tItems[i].max_min_mode == 1 && tItems[i].spec_option == 1) {
			...
			for (j = 1; j < 11; j++) {
				...
				snprintf(str, sizeof(str), "%s%s%s%d",
					 tItems[index].desp, "_",
					 BENCHMARK_KEY_NAME, j);
				if (parser_get_int_data(str, str, tmp,
							sizeof(tmp)) <= 0)
					break;
	...
	ILI_DBG("%s: run = %d, max = %d, min = %d, frame_count = %d\n",
		tItems[i].desp, tItems[i].run, tItems[i].max, tItems[i].min,
		tItems[i].frame_count);

	ILI_DBG("v_tdf_1 = %d, v_tdf_2 = %d, h_tdf_1 = %d, h_tdf_2 = %d",
		tItems[i].v_tdf_1, tItems[i].v_tdf_2, tItems[i].h_tdf_1,
		tItems[i].h_tdf_2);

	if (tItems[i].catalog == TIPRING_RAW ||
	    tItems[i].catalog == TIPRING_P2P)
		ILI_DBG("max1 = %d, max2 = %d, min1 = %d, min2 = %d\n",
			tItems[i].max1, tItems[i].max2, tItems[i].min1,
			tItems[i].min2);
	...
	if (ipio_strcmp(tItems[i].desp, "peak to peak_td (lcm off)") == 0 &&
		...
		parser_get_int_data(tItems[i].desp, "recheck ptop lcm off", str,
				    sizeof(str));
		ILI_INFO("Peak to Peak TD retry = %d\n", ili_hid_katoi(str));
		core_mp.td_retry = ili_hid_katoi(str);
		if (core_mp.td_retry)
		...
		if (tItems[i].catalog == TX_RX_DELTA) {
			...
			if (tItems[i].spec_option == BENCHMARK) {
				...
				if (tItems[i].bch_mrk_max != NULL) {
					for (j = 0;
					     j < tItems[i].bch_mrk_frm_num; j++)
						if (tItems[i].bch_mrk_max[j] !=
						    NULL) {
							kfree(tItems[i].bch_mrk_max
								      [j]);
							tItems[i].bch_mrk_max[j] =
								NULL;
						}
					kfree(tItems[i].bch_mrk_max);
					tItems[i].bch_mrk_max = NULL;
				...
				if (tItems[i].bch_mrk_min != NULL) {
					for (j = 0;
					     j < tItems[i].bch_mrk_frm_num; j++)
						if (tItems[i].bch_mrk_min[j] !=
						    NULL) {
							kfree(tItems[i].bch_mrk_min
								      [j]);
							tItems[i].bch_mrk_min[j] =
								NULL;
						}
					kfree(tItems[i].bch_mrk_min);
					tItems[i].bch_mrk_min = NULL;
...
static int mp_sort_item(bool lcm_on)
{
	...
	for (i = 0; i < MAX_SECTION_NUM; i++) {
		for (j = 0; j < MP_TEST_ITEM; j++) {
			...
			parser_get_int_data(tItems[j].desp, "enable", str,
					    sizeof(str));
			tItems[j].run = ili_hid_katoi(str);
...
int ili_hid_mp_test_main(char *apk, bool lcm_on)
{
	int i, ret = 0;
	...
	if (ini_info == NULL) {
		ini_info = (struct ini_file_data *)vmalloc(
			sizeof(struct ini_file_data) * PARSER_MAX_KEY_NUM);
...
#if MP_INT_LEVEL
	if (ili_hid_ic_int_trigger_ctrl(true) < 0) {
...
#if MP_INT_LEVEL
	if (ili_hid_ic_int_trigger_ctrl(false) < 0) {
