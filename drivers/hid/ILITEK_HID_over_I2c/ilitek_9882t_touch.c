...
/*gesture info mode*/
struct ili_hid_demo_debug_info_id0 {
	u32 id : 8;
...
void ili_hid_dump_data(void *data, int type, int len, int row_len,
		       const char *name)
{
	int i, row = 31;
	u8 *p8 = NULL;
...
static void dma_clear_reg_setting(void)
{
	...
	if (ili_hid_ice_mode_bit_mask_write(INTR32_ADDR, INTR32_reg_t0_int_en | INTR32_reg_t1_int_en, (0 << 24)) < 0)
		ILI_ERR("Write %lu at %x failed\n", INTR32_reg_t0_int_en | INTR32_reg_t1_int_en, INTR32_ADDR);

	/* 2. clear tdi_err_int_flag */
	if (ili_hid_ice_mode_bit_mask_write(INTR2_ADDR, INTR2_tdi_err_int_flag_clear, (1 << 18)) < 0)
		ILI_ERR("Write %lu at %x failed\n", INTR2_tdi_err_int_flag_clear, INTR2_ADDR);

	/* 3. clear dma channel 0 src1 info */
	if (ili_hid_ice_mode_write(DMA49_reg_dma_ch0_src1_addr, 0x00000000, 4) < 0)
		ILI_ERR("Write 0x00000000 at %x failed\n", DMA49_reg_dma_ch0_src1_addr);
	if (ili_hid_ice_mode_write(DMA50_reg_dma_ch0_src1_step_inc, 0x00, 1) < 0)
		ILI_ERR("Write 0x0 at %x failed\n", DMA50_reg_dma_ch0_src1_step_inc);
	if (ili_hid_ice_mode_bit_mask_write(DMA50_ADDR, DMA50_reg_dma_ch0_src1_format |
			DMA50_reg_dma_ch0_src1_en, BIT(31)) < 0)
		ILI_ERR("Write %lu at %x failed\n", DMA50_reg_dma_ch0_src1_format |
							DMA50_reg_dma_ch0_src1_en, DMA50_ADDR);

	/* 4. clear dma channel 0 trigger select */
	if (ili_hid_ice_mode_bit_mask_write(DMA48_ADDR, DMA48_reg_dma_ch0_trigger_sel, (0 << 16)) < 0)
		ILI_ERR("Write %lu at %x failed\n", DMA48_reg_dma_ch0_trigger_sel, DMA48_ADDR);
	if (ili_hid_ice_mode_bit_mask_write(INTR1_ADDR, INTR1_reg_flash_int_flag, (1 << 25)) < 0)
...
static void dma_trigger_reg_setting(u32 reg_dest_addr, u32 flash_start_addr,
				    u32 copy_size)
{
	int retry = 30;
	u32 stat = 0;
	...
	if (ili_hid_ice_mode_bit_mask_write(DMA48_ADDR, DMA48_reg_dma_ch0_start_clear, (1 << 25)) < 0)
		ILI_ERR("Write %lu at %x failed\n", DMA48_reg_dma_ch0_start_clear, DMA48_ADDR);

	/* 2. set dma channel 0 src1 info */
	if (ili_hid_ice_mode_write(DMA49_reg_dma_ch0_src1_addr, 0x00041010, 4) < 0)
		ILI_ERR("Write 0x00041010 at %x failed\n", DMA49_reg_dma_ch0_src1_addr);
	if (ili_hid_ice_mode_write(DMA50_reg_dma_ch0_src1_step_inc, 0x00, 1) < 0)
		ILI_ERR("Write 0x00 at %x failed\n", DMA50_reg_dma_ch0_src1_step_inc);
	if (ili_hid_ice_mode_bit_mask_write(DMA50_ADDR, DMA50_reg_dma_ch0_src1_format | DMA50_reg_dma_ch0_src1_en, BIT(31)) < 0)
		ILI_ERR("Write %lu at %x failed\n", DMA50_reg_dma_ch0_src1_format | DMA50_reg_dma_ch0_src1_en, DMA50_ADDR);

	/* 3. set dma channel 0 src2 info */
	if (ili_hid_ice_mode_bit_mask_write(DMA52_ADDR, DMA52_reg_dma_ch0_src2_en, (0 << 31)) < 0)
		ILI_ERR("Write %lu at %x failed\n", DMA52_reg_dma_ch0_src2_en, DMA52_ADDR);

	/* 4. set dma channel 0 dest info */
	if (ili_hid_ice_mode_write(DMA53_reg_dma_ch0_dest_addr, reg_dest_addr, 3) < 0)
		ILI_ERR("Write %x at %x failed\n", reg_dest_addr, DMA53_reg_dma_ch0_dest_addr);
	if (ili_hid_ice_mode_write(DMA54_reg_dma_ch0_dest_step_inc, 0x01, 1) < 0)
		ILI_ERR("Write 0x01 at %x failed\n", DMA54_reg_dma_ch0_dest_step_inc);

	if (ili_hid_ice_mode_write(DMA54_ADDR, DMA54_reg_dma_ch0_dest_format | DMA54_reg_dma_ch0_dest_en, BIT(31)) < 0)
		ILI_ERR("Write %lu at %x failed\n", DMA54_reg_dma_ch0_dest_format | DMA54_reg_dma_ch0_dest_en, DMA54_ADDR);

	/* 5. set dma channel 0 trafer info */
	if (ili_hid_ice_mode_write(DMA55_reg_dma_ch0_trafer_counts, copy_size, 4) < 0)
		ILI_ERR("Write %x at %x failed\n", copy_size, DMA55_reg_dma_ch0_trafer_counts);
	if (ili_hid_ice_mode_bit_mask_write(DMA55_ADDR, DMA55_reg_dma_ch0_trafer_mode, (0 << 24)) < 0)
		ILI_ERR("Write %lu at %x failed\n", DMA55_reg_dma_ch0_trafer_mode, DMA55_ADDR);

	/* 6. set dma channel 0 int info */
	if (ili_hid_ice_mode_bit_mask_write(INTR33_ADDR, INTR33_reg_dma_ch0_int_en, (1 << 17)) < 0)
		ILI_ERR("Write %lu at %x failed\n", INTR33_reg_dma_ch0_int_en, INTR33_ADDR);

	/* 7. set dma channel 0 trigger select */
	if (ili_hid_ice_mode_bit_mask_write(DMA48_ADDR, DMA48_reg_dma_ch0_trigger_sel, (1 << 16)) < 0)
		ILI_ERR("Write %lu at %x failed\n", DMA48_reg_dma_ch0_trigger_sel, DMA48_ADDR);

	/* 8. set dma flash setting */
	ili_hid_flash_dma_write(flash_start_addr, (flash_start_addr+copy_size), copy_size);

	/* 9. clear flash and dma ch0 int flag */
	if (ili_hid_ice_mode_bit_mask_write(INTR1_ADDR, INTR1_reg_dma_ch1_int_flag |
			INTR1_reg_flash_int_flag, BIT(16) | BIT(25)) < 0)
		ILI_ERR("Write %lu at %x failed\n", INTR1_reg_dma_ch1_int_flag |
			INTR1_reg_flash_int_flag, INTR1_ADDR);
	if (ili_hid_ice_mode_bit_mask_write(0x041013, BIT(0), 1) < 0) //patch
		ILI_ERR("Write %lu at %x failed\n", BIT(0), 0x041013);

	/* DMA Trigger */
	if (ili_hid_ice_mode_write(FLASH4_reg_rcv_data, 0xFF, 1) < 0)
		ILI_ERR("Trigger DMA failed\n");

	/* waiting for fw reload code completed. */
	while (retry > 0) {
		if (ili_hid_ice_mode_read(INTR1_ADDR, &stat, sizeof(u32)) < 0) {
	...
	/* CS High */
	if (ili_hid_ice_mode_write(FLASH0_reg_flash_csb, 0x1, 1) < 0)
		ILI_ERR("Pull CS High failed\n");
	/* waiting for CS status done */
	mdelay(10);
}

int ili_hid_move_mp_code_flash(void)
{
	int ret = 0;
	...
	ret = ilits->wrapper(cmd, sizeof(u8), data,
			     ilits->protocol->mp_info_len, ON, OFF);
	ili_hid_dump_data(data, 8, ilits->protocol->mp_info_len, 0, "MP overlay info");
	if (ret < 0) {
	...
	ILI_INFO(
		"MP info Overlay: Enable = %d, addr = 0x%x ~ 0x%x, flash addr = 0x%x, mp size = 0x%x\n",
		dma_trigger_enable, overlay_start_addr, overlay_end_addr,
		mp_flash_addr, mp_size);
	...
	ret = ili_hid_ic_check_busy(20, 50, ON); /* Set busy as 0x41 */
	if (ret < 0)
		goto out;
	...
	if (dma_trigger_enable) {
		...
		dma_trigger_reg_setting(0, mp_flash_addr,
			mp_andes_init_size); /* DMA ANDES.INIT */

		dma_clear_reg_setting();

		ILI_INFO("[Move MP.TEXT to DRAM]\n");
		dma_trigger_reg_setting(overlay_end_addr,
					(mp_flash_addr + overlay_start_addr),
					mp_text_size);
		...
		if (ilits->cascade_info_block.nNum != 0) {
			/* DMA Trigger */
			if (ili_hid_ice_mode_write_by_mode(FLASH4_reg_rcv_data,
							   0xFF, 1, BOTH) < 0)
				ILI_ERR("Trigger DMA failed\n");
			/* waiting for fw reload code completed. */
			mdelay(30);
			/* CS High */
			if (ili_hid_ice_mode_write_by_mode(FLASH0_reg_flash_csb,
							   0x1, 1, BOTH) < 0)
				ILI_ERR("Pull CS High failed\n");
			/* waiting for CS status done */
			mdelay(10);
	...
	if (ilits->cascade_info_block.nNum != 0) {
		if (ili_hid_cascade_reset_ctrl(TP_IC_WHOLE_RST_WITHOUT_FLASH, false) < 0)
			ILI_ERR("IC whole reset without flash failed during moving mp code\n");
		// ili_hid_cascade_sync_ctrl(true);
	} else {
		if (ili_hid_reset_ctrl(TP_IC_WHOLE_RST_WITHOUT_FLASH) < 0)
			ILI_ERR("IC whole reset without flash failed during moving mp code\n");
	}
	...
	ret = ili_hid_ic_check_busy(20, 50, ON);
	if (ret < 0)
	...
	if (ret < 0)
		ILI_ERR("set mp report mode to HID\n");
out:
	return ret;
}

int ili_hid_move_mp_code_iram(void)
{
	ILI_INFO("Download MP code to iram\n");
	return ili_hid_fw_upgrade_handler(NULL);
}

int ili_hid_proximity_near(int mode)
{
	int ret = 0;
	...
	case DDI_POWER_ON:
		 ...
		 */
		ret = ili_hid_ic_func_ctrl("sleep", SLEEP_IN);
		if (ret < 0)
...
int ili_hid_proximity_far(int mode)
{
	int ret = 0;
	u8 cmd[2] = { 0 };
	...
	case WAKE_UP_SWITCH_GESTURE_MODE:
		...
		ret = ili_hid_set_tp_data_len(ilits->gesture_mode, true, NULL);
		if (ret < 0)
...
int ili_hid_move_gesture_code_flash(int mode)
{
	int ret = 0;
	...
	ret = ili_hid_set_tp_data_len(mode, true, NULL);

	return ret;
}

void ili_hid_set_gesture_symbol(void)
{
	u8 cmd[7] = { 0 };
	...
	ili_hid_dump_data(cmd, 8, sizeof(cmd), 0, "Gesture symbol");
...
int ili_hid_move_gesture_code_iram(int mode)
{
	...
	/*pre-command for ili_hid_ic_func_ctrl("lpwg", 0x3)*/
	cmd[0] = P5_X_READ_DATA_CTRL;
	cmd[1] = 0x1;
	...
	ret = ili_hid_ic_func_ctrl("lpwg", 0x3);
	if (ret < 0) {
	...
	for (i = 0; i < timeout; i++) {
		...
		ret = ilits->wrapper(cmd_write, sizeof(cmd_write), cmd,
				     sizeof(u8), ON, OFF);
	...
	if (i >= timeout) {
		...
		return ili_hid_gesture_recovery();
	}
	...
	ret = ili_hid_ic_func_ctrl("lpwg", 0x6);
	if (ret < 0) {
...
u8 ili_hid_calc_packet_checksum(u8 *packet, int len)
{
	int i;
	s32 sum = 0;
...
int ili_hid_touch_esd_gesture_flash(void)
{
	int ret = 0, retry = 100;
	u32 answer = 0;
	...
	ILI_INFO(
		"ESD Gesture PWD Addr = 0x%X, PWD = 0x%X, GES_RUN = 0%X, core_ver = 0x%X\n",
		ges_pwd_addr, ges_pwd, ges_run, ilits->chip->core_ver);

	/* write a special password to inform FW go back into gesture mode */
	ret = ili_hid_ice_mode_write(ges_pwd_addr, ges_pwd, pwd_len);
	if (ret < 0) {
	...
	if (ilits->cascade_info_block.nNum != 0) {
		if (ilits->reset == TP_HW_RST_ONLY) {
			...
			ret = ili_hid_cascade_reset_ctrl(ilits->reset, true);
		}
	} else {
		ret = ili_hid_reset_ctrl(ilits->reset);
	}
	...
	do {
		ret = ili_hid_ice_mode_read(ges_pwd_addr, &answer, pwd_len);
		if (ret < 0) {
...
out:
	if (ilits->cascade_info_block.nNum != 0) {
		ili_hid_ice_mode_ctrl_by_mode(DISABLE, ON, BOTH);
		ili_hid_cascade_sync_ctrl(true);
	} else {
	...
	if (ret >= 0) {
		...
		ili_hid_set_tp_data_len(ilits->gesture_mode, false, NULL);
	}

	return ret;
}

int ili_hid_touch_esd_gesture_iram(void)
{
	int ret = 0, retry = 100;
	u32 answer = 0;
	...
	ILI_INFO(
		"ESD Gesture PWD Addr = 0x%X, PWD = 0x%X, GES_RUN = 0x%X, core_ver = 0x%X\n",
		ges_pwd_addr, ges_pwd, ges_run, ilits->chip->core_ver);
...
void ili_hid_demo_debug_info_id0(u8 *buf, size_t len)
{
	struct ili_hid_demo_debug_info_id0 id0;
...
void ili_hid_demo_debug_info_mode(u8 *buf, size_t len)
{
	u8 *info_ptr = NULL;
	u8 info_id = 0, info_len = 0;

	ili_hid_report_ap_mode(buf, len);
	if (ilits->rib.nReportResolutionMode == POSITION_LOW_RESOLUTION) {
		info_ptr = buf + P5_X_DEMO_MODE_PACKET_LEN;
	} else if (ilits->rib.nReportResolutionMode ==
...
static void ilitek_tddi_touch_send_debug_data(u8 *buf, int len)
{
	...
	if (!ilits->netlink && !ilits->dnp && !ilits->dlnp)
		goto out;

	/* Send data to netlink */
	if (ilits->netlink) {
		ili_hid_netlink_reply_msg(buf, len);
		goto out;
	}
	...
	if (ilits->dlnp) {
		...
		if (ilits->dbf_in_idx < TR_BUF_LIST_SIZE) {
			ipio_memcpy(ilits->dbf_in[index].data, buf, len,
				    TR_BUF_SIZE);
			ilits->dbf_in_idx++;
...
static void ilitek_tddi_touch_customer_data_parsing(u8 *buf)
{
	...
	bool palmflag = false;
	ilits->finger = 0;
	if (ilits->rib.nCustomerType == POSITION_CUSTOMER_TYPE_ON &&
	    ilits->tp_data_format == DATA_FORMAT_DEMO) {
		if (ilits->rib.nReportResolutionMode ==
		    POSITION_LOW_RESOLUTION) {
			index = P5_X_5B_LOW_RESOLUTION_LENGTH - others -
				P5_X_INFO_CHECKSUM_LENGTH;
			for (i = 0; i < MAX_TOUCH_NUM; i++) {
				if ((buf[(4 * i) + 1 + P5_X_DEMO_MODE_PACKET_INFO_LEN] ==
				     0xFF) && (buf[(4 * i) + 2 +
					 P5_X_DEMO_MODE_PACKET_INFO_LEN] ==
				     0xFF) && (buf[(4 * i) + 3 +
					 P5_X_DEMO_MODE_PACKET_INFO_LEN] == 0xFF)) {
					continue;
				}
				...
				ILI_DBG("finger = %d, degree = %d, width_major = %d, width_minor = %d\n",
					ilits->finger, axis_info[ilits->finger].degree,
					axis_info[ilits->finger].width_major,
					axis_info[ilits->finger].width_minor);
				ilits->finger++;
			}
			...
			for (i = 0; i < others; i++) {
				ILI_DBG("Other Information byte[%d]:%02x\n", i,
					buf[index + P5_X_CUSTOMER_LENGTH + i]);
			}
		} else if (ilits->rib.nReportResolutionMode ==
			   POSITION_HIGH_RESOLUTION) {
			index = P5_X_DEMO_MODE_PACKET_LEN_HIGH_RESOLUTION -
				others - P5_X_INFO_CHECKSUM_LENGTH;
			for (i = 0; i < MAX_TOUCH_NUM; i++) {
				if ((buf[(5 * i) + 1 +
					 P5_X_DEMO_MODE_PACKET_INFO_LEN] ==
				     0xFF) && (buf[(5 * i) + 2 +
	...
	} else if (ilits->rib.nCustomerType == POSITION_CUSTOMER_TYPE_ON &&
		   ilits->tp_data_format == DATA_FORMAT_DEBUG) {
		if (ilits->rib.nReportResolutionMode ==
		    POSITION_LOW_RESOLUTION) {
			index = P5_X_DEBUG_LOW_RESOLUTION_FINGER_DATA_LENGTH +
				(2 * ilits->xch_num * ilits->ych_num) +
				(ilits->stx * 2) + (ilits->srx * 2) + (2 * 2);
			for (i = 0; i < MAX_TOUCH_NUM; i++) {
				if ((buf[(3 * i) + 1 +
					 P5_X_DEBUG_MODE_PACKET_INFO_LEN] ==
				     0xFF) && (buf[(3 * i) + 2 +
					 P5_X_DEBUG_MODE_PACKET_INFO_LEN] ==
				     0xFF) && (buf[(3 * i) + 3 +
					 P5_X_DEBUG_MODE_PACKET_INFO_LEN] ==
			...
			for (i = 0; i < others; i++) {
			   ...
			   POSITION_HIGH_RESOLUTION) {
			index = P5_X_DEBUG_HIGH_RESOLUTION_FINGER_DATA_LENGTH +
				(2 * ilits->xch_num * ilits->ych_num) +
				(ilits->stx * 2) + (ilits->srx * 2) + (2 * 2);
			for (i = 0; i < MAX_TOUCH_NUM; i++) {
				if ((buf[(4 * i) + 1 +
...
void ili_hid_touch_press(u16 x, u16 y, u16 pressure, u16 id)
{
...
void ili_hid_touch_release(u16 x, u16 y, u16 id)
{
...
void ili_hid_touch_release_all_point(void)
{
	int i;
	ILI_DBG("Touch Release All\n");
	if (MT_B_TYPE) {
		for (i = 0; i < MAX_TOUCH_NUM; i++)
			ili_hid_touch_release(0, 0, i);

		input_report_key(ilits->input, BTN_TOUCH, 0);
		input_report_key(ilits->input, BTN_TOOL_FINGER, 0);
	} else {
		ili_hid_touch_release(0, 0, 0);
	}
...
void ili_hid_report_ap_mode(u8 *buf, int len)
{
	int i = 0;
	u32 xop = 0, yop = 0;
	...
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		if (ilits->rib.nCustomerType == ilits->customertype_off &&
			...
			xop = (((buf[(4 * i) + 1] & 0xF0) << 4) |
			       (buf[(4 * i) + 2]));
			yop = (((buf[(4 * i) + 1] & 0x0F) << 8) |
			       (buf[(4 * i) + 3]));
		} else if (ilits->rib.nCustomerType !=
			...
			xop = (((buf[(4 * i) + 1 +
				     P5_X_DEMO_MODE_PACKET_INFO_LEN] & 0xF0) << 4) |
			       (buf[(4 * i) + 2 + P5_X_DEMO_MODE_PACKET_INFO_LEN]));
			yop = (((buf[(4 * i) + 1 +
				     P5_X_DEMO_MODE_PACKET_INFO_LEN] & 0x0F) << 8) | (buf[(4 * i) + 3 +
				    P5_X_DEMO_MODE_PACKET_INFO_LEN]));
		} else if (ilits->rib.nReportResolutionMode ==
			   POSITION_HIGH_RESOLUTION) {
		...
		if (ilits->trans_xy) {
			...
			if (ilits->rib.nReportResolutionMode ==
			    POSITION_LOW_RESOLUTION) {
				touch_info[ilits->finger].x =
					xop * ilits->panel_wid / TPD_WIDTH;
				touch_info[ilits->finger].y =
					yop * ilits->panel_hei / TPD_HEIGHT;
			} else if (ilits->rib.nReportResolutionMode ==
				   POSITION_HIGH_RESOLUTION) {
				touch_info[ilits->finger].x =
					xop * ilits->panel_wid / ilits->max_x;
				touch_info[ilits->finger].y =
					yop * ilits->panel_hei / ilits->max_y;
			}
		}
		...
		if (MT_PRESSURE) {
			if (ilits->rib.nCustomerType == ilits->customertype_off &&
				...
				touch_info[ilits->finger].pressure = buf[(5 * i) + 5 +
					    P5_X_DEMO_MODE_PACKET_INFO_LEN];
			}
		} else {
	...
	if (ilits->finger) {
		if (MT_B_TYPE) {
			for (i = 0; i < ilits->finger; i++) {
				input_report_key(ilits->input, BTN_TOUCH, 1);
				ili_hid_touch_press(touch_info[i].x, touch_info[i].y,
		...
		if (ilits->last_touch) {
			if (MT_B_TYPE) {
				for (i = 0; i < MAX_TOUCH_NUM; i++) {
					if (ilits->curt_touch[i] == 0 &&
					    ilits->prev_touch[i] == 1)
						ili_hid_touch_release(0, 0, i);
					ilits->prev_touch[i] = ilits->curt_touch[i];
				}
				input_report_key(ilits->input, BTN_TOUCH, 0);
...
static void ili_pen_down(u16 x, u16 y, u16 pressure)
{
	...
	input_report_key(ilits->input_pen, BTN_TOUCH,
			 ilits->pen_info.TipSwitch);
	input_report_key(ilits->input_pen, BTN_TOOL_PEN,
			 ilits->pen_info.InRange);
	input_report_key(ilits->input_pen, BTN_STYLUS,
			 ilits->pen_info.BarelSwitch);
	...
	} else if (ilits->pen_info.TipSwitch == 0 &&
		   ilits->pen_info.InRange == 1) {
		if (ilits->pen_info.distance_cnt == 1) {
			ilits->pen_info.distance_cnt = 2;
		} else {
			ilits->pen_info.distance_cnt = 1;
		}
		input_report_abs(ilits->input_pen, ABS_DISTANCE,
				 ilits->pen_info.distance_cnt);
	}

	input_sync(ilits->input_pen);
}

static void ili_pen_up(void)
{
	ILI_DBG("Pen UP \n");
...
void ili_hid_pen_demo_mode_report_point(u8 *buf, int len)
{
	...
	for (i = 0; i < ilits->touch_num; i++) {
		if (ilits->rib.nCustomerType == ilits->customertype_off &&
		    ilits->rib.nReportResolutionMode ==
			    POSITION_LOW_RESOLUTION) {
			PenStartIdx = P5_X_DEMO_LOW_RESOLUTION_FINGER_DATA_LENGTH +
				P5_X_P_SENSOR_KEY_LENGTH;
		} else if (ilits->rib.nCustomerType != ilits->customertype_off &&
			   ilits->rib.nReportResolutionMode == POSITION_LOW_RESOLUTION) {
			PenStartIdx = P5_X_DEMO_LOW_RESOLUTION_FINGER_DATA_LENGTH +
				P5_X_P_SENSOR_KEY_LENGTH + P5_X_CUSTOMER_LENGTH;
		} else if (ilits->rib.nCustomerType != ilits->customertype_off &&
			   ilits->rib.nReportResolutionMode == POSITION_HIGH_RESOLUTION) {
			PenStartIdx = P5_X_DEMO_HIGH_RESOLUTION_FINGER_DATA_LENGTH +
				P5_X_P_SENSOR_KEY_LENGTH + P5_X_CUSTOMER_LENGTH;
		} else if (ilits->rib.nCustomerType == ilits->customertype_off &&
			   ilits->rib.nReportResolutionMode == POSITION_HIGH_RESOLUTION) {
			PenStartIdx = P5_X_DEMO_HIGH_RESOLUTION_FINGER_DATA_LENGTH +
		...
		if (i >= MAX_TOUCH_NUM) {
			if (buf[len - P5_X_INFO_CHECKSUM_LENGTH - 1] !=
			    ONLY_REPORT_HAND) {
				ILI_DBG("Pen Switch = 0x%X\n", buf[PenStartIdx]);
				if ((buf[PenStartIdx] & TIPSWITCH_HOVER_BIT) ==
				    RELEASE_PEN) {
					if (MT_B_TYPE) {
						ilits->pen_info.active = 0;
						ilits->curt_touch[PEN_INDEX] = 0;
					}
					ILI_DBG("Pen NonActive\n");
					continue;
				} else {
					ilits->pen_info.active = 1;
					ILI_DBG("Pen Active\n");
				}
				...
				if (buf[PenStartIdx + TOUCH_TILT_OFFSET] & NEGITIVE_FLAG) {
					...
					ilits->pen_info.tilt_x = buf[PenStartIdx +
						    TOUCH_TILT_OFFSET] | 0xFFFFFF00;
				} else {
					ilits->pen_info.tilt_x = buf[PenStartIdx +
						    TOUCH_TILT_OFFSET];
				}
				...
				if (buf[PenStartIdx + TOUCH_TILT_OFFSET + 1] & NEGITIVE_FLAG) {
					//Transfer Tilt to Negitive
					ilits->pen_info.tilt_y = buf[PenStartIdx +
						    TOUCH_TILT_OFFSET + 1] | 0xFFFFFF00;
				} else {
					ilits->pen_info.tilt_y =
						buf[PenStartIdx + TOUCH_TILT_OFFSET + 1];
				}

				// Pen ID
				for (pen_id_index = 0;
				     pen_id_index < P5_X_PEN_ID_LEN; pen_id_index++) {
					ilits->pen_info.pen_id[pen_id_index] =
						buf[PenStartIdx + PEN_ID_OFFSET + pen_id_index];
				}
			} else {
				ILI_DBG("Report Switch, Only Report Hand\n");
				continue;
			}
		} else {
			if (buf[len - P5_X_INFO_CHECKSUM_LENGTH - 1] !=
				...
				if (ilits->rib.nCustomerType ==
					...
					xop = (((buf[(4 * i) + 1 +
					       ...
					       (buf[(4 * i) + 2 + P5_X_DEMO_MODE_PACKET_INFO_LEN]));
					yop = (((buf[(4 * i) + 1 +
						     P5_X_DEMO_MODE_PACKET_INFO_LEN] & 0x0F) << 8) |
					       (buf[(4 * i) + 3 + P5_X_DEMO_MODE_PACKET_INFO_LEN]));
				} else if (ilits->rib.nReportResolutionMode ==
					   POSITION_HIGH_RESOLUTION) {
					if ((buf[(5 * i) + 1 +
						 P5_X_DEMO_MODE_PACKET_INFO_LEN] == 0xFF) &&
					...
					xop = ((buf[(5 * i) + 1 +
						    P5_X_DEMO_MODE_PACKET_INFO_LEN] << 8) |
					       (buf[(5 * i) + 2 + P5_X_DEMO_MODE_PACKET_INFO_LEN]));
					yop = ((buf[(5 * i) + 3 +
						    P5_X_DEMO_MODE_PACKET_INFO_LEN] << 8) |
					       (buf[(5 * i) + 4 + P5_X_DEMO_MODE_PACKET_INFO_LEN]));
		...
		if (ilits->pen_info.active == true) {
			if (ilits->trans_xy) {
				...
				if (ilits->rib.nReportResolutionMode ==
				    POSITION_LOW_RESOLUTION) {
					touch_info[PEN_INDEX].x =
						xop * ilits->panel_wid / TPD_WIDTH;
					touch_info[PEN_INDEX].y =
						yop * ilits->panel_hei / TPD_HEIGHT;
				} else if (ilits->rib.nReportResolutionMode ==
					   POSITION_HIGH_RESOLUTION) {
					touch_info[PEN_INDEX].x =
			...
			touch_info[PEN_INDEX].pressure =
				(buf[PenStartIdx + TOUCH_PRESS_OFFSET] << 8) +
				buf[PenStartIdx + TOUCH_PRESS_OFFSET + 1];
			...
			if (MT_PRESSURE) {
				if (ilits->rib.nCustomerType ==
				    ...
				    ilits->rib.nReportResolutionMode ==
					    POSITION_LOW_RESOLUTION) {
					touch_info[ilits->finger].pressure =
						buf[(4 * i) + 4];
				} else if (ilits->rib.nCustomerType !=
						   ilits->customertype_off &&
					   ilits->rib.nReportResolutionMode ==
	...
	if (ilits->pen) {
		...
		//Pen Down
		ili_pen_down(touch_info[PEN_INDEX].x, touch_info[PEN_INDEX].y,
			     touch_info[PEN_INDEX].pressure);
	} else {
		//Pen Up
		if (ilits->curt_touch[PEN_INDEX] == 0 &&
		    ilits->prev_touch[PEN_INDEX] == 1) {
			ili_pen_up();
		}
...
void ili_hid_report_pen_ap_mode(u8 *buf, int len)
{
	ili_hid_pen_demo_mode_report_point(buf, len);
	ilitek_tddi_touch_customer_data_parsing(buf);
	ilitek_tddi_touch_send_debug_data(buf, len);
}

/*Pen report single event, Finger report Mulit event*/
void ili_hid_pen_debug_mode_report_point(u8 *buf, int len, u8 offset)
{
	int i = 0, pen_id_index = 0;
	u32 xop = 0, yop = 0, PenStartIdx = 0;
	...
	for (i = 0; i < ilits->touch_num; i++) {
		if (ilits->rib.nCustomerType == ilits->customertype_off &&
		    ilits->rib.nReportResolutionMode == POSITION_LOW_RESOLUTION) {
			PenStartIdx =
				P5_X_DEBUG_LOW_RESOLUTION_FINGER_DATA_LENGTH +
				ilits->cdc_data_len;
		} else if (ilits->rib.nCustomerType != ilits->customertype_off &&
			   ilits->rib.nReportResolutionMode == POSITION_LOW_RESOLUTION) {
			if (ilits->pen_info.report_type == P5_X_ONLY_PEN_TYPE) {
				PenStartIdx = P5_X_DEBUG_LOW_RESOLUTION_FINGER_DATA_LENGTH +
					ilits->cdc_data_len;
			} else {
				PenStartIdx = P5_X_DEBUG_LOW_RESOLUTION_FINGER_DATA_LENGTH +
					ilits->cdc_data_len + P5_X_CUSTOMER_LENGTH;
			}
		} else if (ilits->rib.nCustomerType != ilits->customertype_off &&
			   ilits->rib.nReportResolutionMode == POSITION_HIGH_RESOLUTION) {
			if (ilits->pen_info.report_type == P5_X_ONLY_PEN_TYPE) {
				PenStartIdx = P5_X_DEBUG_HIGH_RESOLUTION_FINGER_DATA_LENGTH +
					ilits->cdc_data_len;
			} else {
				PenStartIdx = P5_X_DEBUG_HIGH_RESOLUTION_FINGER_DATA_LENGTH +
					ilits->cdc_data_len + P5_X_CUSTOMER_LENGTH;
		...
		if (i >= MAX_TOUCH_NUM) {
			if (buf[len - offset - P5_X_INFO_CHECKSUM_LENGTH - 1] !=
			    ONLY_REPORT_HAND) {
				ILI_DBG("Pen Switch = 0x%X\n", buf[PenStartIdx]);
				if ((buf[PenStartIdx] & TIPSWITCH_HOVER_BIT) ==
				    RELEASE_PEN) {
					if (MT_B_TYPE) {
						ilits->pen_info.active = 0;
						ilits->curt_touch[i] = 0;
						touch_info[i].id = MAX_TOUCH_NUM - 1;
					}
					ILI_DBG("Pen NonActive\n");
					continue;
				} else {
				...
				if (buf[PenStartIdx + TOUCH_TILT_OFFSET + 1] &
				    NEGITIVE_FLAG) {
					//Transfer Tilt to Negitive
					ilits->pen_info.tilt_y = buf[PenStartIdx +
						    TOUCH_TILT_OFFSET + 1] | 0xFFFFFF00;
	...
	if (ilits->pen) {
		if (ilits->pen_info.finger_touch) {
			...
			if (MT_PRESSURE)
				input_report_abs(ilits->input, ABS_MT_PRESSURE,
						 0);
		}
		//Pen Down
		ili_pen_down(touch_info[PEN_INDEX].x, touch_info[PEN_INDEX].y,
	...
	if (ilits->pen == 0) {
		if (ilits->finger) {
			if (MT_B_TYPE) {
				...
				for (i = 0; i < ilits->finger; i++) {
					if (touch_info[i].id < MAX_TOUCH_NUM) {
						ilits->pen_info.finger_touch =
							true;
						input_report_key(ilits->input,
								 BTN_TOUCH, 1);
						ili_hid_touch_press(
							touch_info[i].x,
							touch_info[i].y,
							touch_info[i].pressure,
							touch_info[i].id);
						input_report_key(
							ilits->input,
							BTN_TOOL_FINGER, 1);
...
void ili_hid_report_debug_mode(u8 *buf, int len)
{
...
void ili_hid_report_pen_debug_mode(u8 *buf, int len)
{
	u8 offset = 5;
	...
	ili_hid_pen_debug_mode_report_point(buf + offset, len, offset);
...
void ili_hid_report_debug_lite_mode(u8 *buf, int len)
{
...
void ili_hid_report_proximity_mode(u8 *buf, int len)
{
	...
	if ((ilits->actual_tp_mode == P5_X_FW_AP_MODE) &&
	    (ilits->proxmity_face == true) && (ilits->power_status == true)) {
...
void ili_hid_report_gesture_mode(u8 *buf, int len)
{
	...
	if (ilits->rib.nReportResolutionMode == POSITION_LOW_RESOLUTION) {
		...
		ipio_memcpy(ges, buf, len,
			    P5_X_GESTURE_INFO_LENGTH_HIGH_RESOLUTION);
	}
	...
	if (!transfer) {
		if (ilits->rib.nReportResolutionMode ==
		    POSITION_LOW_RESOLUTION) {
			gc->pos_start.x =
				gc->pos_start.x * ilits->panel_wid / TPD_WIDTH;
			gc->pos_start.y =
				gc->pos_start.y * ilits->panel_hei / TPD_HEIGHT;
			gc->pos_end.x =
				gc->pos_end.x * ilits->panel_wid / TPD_WIDTH;
			gc->pos_end.y =
				gc->pos_end.y * ilits->panel_hei / TPD_HEIGHT;
			gc->pos_1st.x =
				gc->pos_1st.x * ilits->panel_wid / TPD_WIDTH;
			gc->pos_1st.y =
				gc->pos_1st.y * ilits->panel_hei / TPD_HEIGHT;
			gc->pos_2nd.x =
				gc->pos_2nd.x * ilits->panel_wid / TPD_WIDTH;
			gc->pos_2nd.y =
				gc->pos_2nd.y * ilits->panel_hei / TPD_HEIGHT;
			gc->pos_3rd.x =
				gc->pos_3rd.x * ilits->panel_wid / TPD_WIDTH;
			gc->pos_3rd.y =
				gc->pos_3rd.y * ilits->panel_hei / TPD_HEIGHT;
			gc->pos_4th.x =
				gc->pos_4th.x * ilits->panel_wid / TPD_WIDTH;
			gc->pos_4th.y =
				gc->pos_4th.y * ilits->panel_hei / TPD_HEIGHT;
		} else if (ilits->rib.nReportResolutionMode ==
			   POSITION_HIGH_RESOLUTION) {
			gc->pos_start.x = gc->pos_start.x * ilits->panel_wid /
					  ilits->max_x;
			gc->pos_start.y = gc->pos_start.y * ilits->panel_hei /
					  ilits->max_y;
			gc->pos_end.x =
				gc->pos_end.x * ilits->panel_wid / ilits->max_x;
			gc->pos_end.y =
				gc->pos_end.y * ilits->panel_hei / ilits->max_y;
			gc->pos_1st.x =
				gc->pos_1st.x * ilits->panel_wid / ilits->max_x;
			gc->pos_1st.y =
				gc->pos_1st.y * ilits->panel_hei / ilits->max_y;
			gc->pos_2nd.x =
				gc->pos_2nd.x * ilits->panel_wid / ilits->max_x;
			gc->pos_2nd.y =
				gc->pos_2nd.y * ilits->panel_hei / ilits->max_y;
			gc->pos_3rd.x =
				gc->pos_3rd.x * ilits->panel_wid / ilits->max_x;
			gc->pos_3rd.y =
				gc->pos_3rd.y * ilits->panel_hei / ilits->max_y;
			gc->pos_4th.x =
				gc->pos_4th.x * ilits->panel_wid / ilits->max_x;
			gc->pos_4th.y =
				gc->pos_4th.y * ilits->panel_hei / ilits->max_y;
		}
	}

	ILI_INFO("Transfer = %d, Type = %d, clockwise = %d\n", transfer,
		 gc->type, gc->clockwise);
	ILI_INFO(
		"Gesture Points: (%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)\n",
		gc->pos_start.x, gc->pos_start.y, gc->pos_end.x, gc->pos_end.y,
		gc->pos_1st.x, gc->pos_1st.y, gc->pos_2nd.x, gc->pos_2nd.y,
		gc->pos_3rd.x, gc->pos_3rd.y, gc->pos_4th.x, gc->pos_4th.y);

	ilitek_tddi_touch_send_debug_data(buf, len);
}

void ili_hid_report_i2cuart_mode(u8 *buf, int len)
{
	int type = buf[3] & 0x0F;
...
#ifdef ROI
int ili_hid_read_knuckle_roi_data(void)
{
	int i = 0;
	int ret = 0;
	...
	if (ts_dev_data->ts_platform_data->feature_info.roi_info.roi_switch &&
		...
		if (ilits->last_touch != ilits->finger &&
			...
			ret = ili_hid_config_knuckle_roi_ctrl(CMD_ROI_DATA);
			if (ret) {
				ILI_ERR("write data failed, ret = %d\n", ret);
				return ret;
			}
			mdelay(1);
			ret = ilits->wrapper(NULL, 0, ilits->knuckle_roi_data,
					     ROI_DATA_READ_LENGTH, OFF, OFF);
			if (ret) {
...
void ili_hid_report(u8 *raw_data, int size, struct hid_field *field)
{
	...
	if (raw_data[I2C_HID_RID_IDX] == I2C_HID_RID &&
	    raw_data[I2C_HID_GESTURE_LEN_IDX] == I2C_HID_GESTURE_LEN &&
	    raw_data[I2C_HID_PACKET_ID_IDX] == P5_X_GESTURE_PACKET_ID &&
	    raw_data[I2C_HID_GESTURE_TYPE_IDX] == GESTURE_DOUBLECLICK) {
		ILI_INFO("Double Click key event\n");
		input_report_key(ilits->input_hid->input, KEY_GESTURE_POWER, 1);
		input_sync(ilits->input_hid->input);
		input_report_key(ilits->input_hid->input, KEY_GESTURE_POWER, 0);
		input_sync(ilits->input_hid->input);
		ilits->gcoord->type = GESTURE_DOUBLECLICK;
		ilits->gcoord->clockwise = 1;
		ILI_INFO("Type = %d, clockwise = %d\n", ilits->gcoord->type,
			 ilits->gcoord->clockwise);
	} else if (raw_data[I2C_HID_CHECK_MP_INT_IDX] == 0xAD) {
		ilits->hid_report_irq = true;
	} else if (raw_data[I2C_HID_RID_IDX] == I2C_HID_PEN_RID) {
		pen_serial = (raw_data[ILITEK_HID_PEN_SERIAL_4] << 24) |
		...
		input_event(field->hidinput->input, EV_MSC, MSC_TIMESTAMP, timestamp);
	} else {
		return;
	}
}
