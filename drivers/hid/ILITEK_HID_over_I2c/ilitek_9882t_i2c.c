...
static int dma_i2c_alloc(struct i2c_client *client)
{
	if (client != NULL) {
		...
		ilitek_dma_va = (u8 *)dmam_alloc_coherent(
			client->dev, DMA_VA_BUFFER, &ilitek_dma_pa, GFP_KERNEL);
...
static int core_i2c_write(void *buf, int len)
{
	...
	if (len > 8) {
		...
		msgs[0].ext_flag =
			(ilits->i2c->ext_flag | I2C_ENEXT_FLAG | I2C_DMA_FLAG);
	...
	if (atomic_read(&ilits->mp_stat) && txbuf[0] == P5_X_SET_CDC_INIT) {
		check_sum = ili_hid_calc_packet_checksum(txbuf, len);
		...
		mpbuf[len] = check_sum;
		msgs[0].buf = mpbuf;
		ili_hid_dump_data(mpbuf, 8, len + 1, 0, "mp cdc cmd");
		msgs[0].len = len + 1;
	}
...
static int ili_i2c_wrapper(u8 *txbuf, u32 wlen, u8 *rxbuf, u32 rlen,
			   bool spi_irq, bool i2c_irq)
{
	...
	if (ilits->i2c_addr_change == CLIENT) {
		ilits->i2c->addr = TDDI_I2C_CLIENT_ADDR;
	} else {
		ilits->i2c->addr = TDDI_I2C_ADDR;
	}
#endif
	...
	if (ilits->int_pulse)
		ilits->detect_int_stat = ili_hid_ic_check_int_flag;
	else
		ilits->detect_int_stat = ili_hid_ic_check_int_level;
	...
	case RW_SYNC:
		...
		if (ilits->actual_tp_mode == P5_X_FW_TEST_MODE && i2c_irq) {
			rlen += 3;
			ret = ilitek_i2c_read(ilits->tr_buf, rlen);
		} else {
		...
		if (ilits->actual_tp_mode == P5_X_FW_TEST_MODE && i2c_irq) {
			ipio_memcpy(rxbuf, ilits->tr_buf + 3, rlen - 3,
				    TR_BUF_SIZE);
		}
		break;
	case W_ONLY:
...
int ili_hid_core_spi_setup(int num)
{
	ILI_ERR("Not support this interface\n");
	return 0;
}

extern struct ilitek_hwif_info hwif;
			    ...
			    const struct i2c_device_id *id)
{
	...
	ilits = devm_kzalloc(&i2c->dev, sizeof(struct ilitek_ts_hid_data),
			     GFP_KERNEL);
	...
	ilits->detect_int_stat = ili_hid_ic_check_int_pulse;
	...
	ilits->mp_move_code = ili_hid_move_mp_code_flash;
	ilits->gesture_move_code = ili_hid_move_gesture_code_flash;
	ilits->esd_recover = ili_hid_wq_esd_i2c_check;
	ilits->esd_func_ctrl = ENABLE_WQ_ESD;
	ilits->ges_recover = ili_hid_touch_esd_gesture_flash;
...
//int ili_hid_interface_dev_init(struct ilitek_hwif_info *hwif)
int ili_hid_interface_dev_init(struct ilitek_hwif_info *hwif,
			       struct i2c_client *client)
{
	...
	return ilitek_i2c_probe(client, NULL);
}

void ili_hid_interface_dev_exit(struct ilitek_ts_hid_data *ts, bool flag)
{
	struct touch_bus_info *info =
		(struct touch_bus_info *)ilits->hwif->info;
