...
struct mt_application {
	...
	unsigned long *pending_palm_slots; /* slots where we reported palm and need to release */
...
static void mt_get_feature(struct hid_device *hdev, struct hid_report *report)
{
	...
	if (ret < 0) {
		...
		ret = hid_report_raw_event(hdev, HID_FEATURE_REPORT, buf, size,
					   0);
		if (ret)
	...
	case HID_UP_DIGITIZER:
		...
		case HID_DG_HEIGHT:
			if (!(app->quirks & MT_QUIRK_NO_AREA)) {
				 ...
				 */
				if (!test_bit(ABS_MT_ORIENTATION, hi->input->absbit))
		...
		case HID_DG_AZIMUTH:
			...
			input_set_abs_params(hi->input, ABS_MT_ORIENTATION,
						...
						field->logical_maximum / 4,
						cls->sn_move ? field->logical_maximum / cls->sn_move : 0, 0);
			MT_STORE_FIELD(a);
			return 1;
		case HID_DG_CONTACTMAX:
			    ...
			    struct mt_report_data *rdata)
{
	...
	mod_timer(&td->release_timer, jiffies + msecs_to_jiffies(200));
#else
...
static int mt_input_configured(struct hid_device *hdev, struct hid_input *hi)
{
	...
	list_for_each_entry(report, &hi->reports, hidinput_list) {
		...
		if (rdata->is_mt_collection) {
			ret = mt_touch_input_configured(hdev, hi, mt_application);
			if (ret)
				return ret;
		}
	}
...
static void mt_fix_const_fields(struct hid_device *hdev, unsigned int usage)
{
	...
	list_for_each_entry(report,
			    &hdev->report_enum[HID_INPUT_REPORT].report_list, list) {
		if (!report->maxfield)
			continue;
...
static int mt_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	if (ret) {
		FTS_ERROR("cannot start hardware I/O\n");
		return ret;
	}
	...
	ret = fts_hid_probe_entry(hdev, id);
	if (ret){
		FTS_ERROR("fts_hid_probe_entry error\n");
		hid_hw_stop(hdev);
	}
...
static int fts_hid_suspend(struct hid_device *hdev, pm_message_t message)
{
	struct i2c_client *client = hdev->driver_data;
...
static const struct hid_device_id mt_devices[] = {
	{
		HID_I2C_DEVICE(HID_VID_FOCALTECH_SCREEN, HID_PID1_FOCALTECH_SCREEN),
	},
	{
		HID_I2C_DEVICE(HID_VID_FOCALTECH_SCREEN, HID_PID2_FOCALTECH_SCREEN),
	},
	{}
};
...
static struct hid_driver mt_driver = {
	...
	.event = mt_event,
	.report = mt_report,
	.raw_event = fts_raw_event,
	.report_fixup = fts_report_fixup,
#ifdef CONFIG_PM
	.reset_resume = mt_reset_resume,
	.resume = mt_resume,
	.suspend = fts_hid_suspend,
#endif
};
