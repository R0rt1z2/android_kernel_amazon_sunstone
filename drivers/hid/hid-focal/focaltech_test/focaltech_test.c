...
void print_buffer(int *buffer, int length, int line_num)
{
	...
	if ((buffer == NULL) || (length <= 0)) {
		FTS_TEST_ERROR("buffer/length(%d) fail", length);
		return;
	}
	...
	if (tmpbuf == NULL) {
		FTS_TEST_ERROR("kzalloc tmplen fail");
		return;
	}
	...
	for (i = 0; i < length; i = i + tmpline) {
		cnt = 0;
		for (j = 0; j < tmpline; j++) {
			cnt += snprintf(tmpbuf + cnt, tmplen - cnt, "%5d ",
					buffer[i + j]);
...
static int fts_test_bus_read(u8 *cmd, int cmdlen, u8 *data, int datalen)
{
	...
	ret = focal_read(cmd, cmdlen, data, datalen);
	if (ret < 0)
		return ret;
	else
		return 0;
...
static int fts_test_bus_write(u8 *writebuf, int writelen)
{
	...
	ret = focal_write(writebuf, writelen);
	if (ret < 0)
		return ret;
	else
		return 0;
...
static ssize_t tp_test_proc_write(struct file *filp, const char __user *buff,
...
static const struct proc_ops tp_test_proc_fops = {
	...
	.proc_read = seq_read,
	.proc_write = tp_test_proc_write,
};

static int fts_test_func_init(struct fts_hid_data *ts_data)
{
	...
	tp_test_proc =
		proc_create(FTS_PROC_TP_DIFFER, 0444, NULL, &tp_test_proc_fops);
	if (tp_test_proc == NULL) {
		FTS_TEST_ERROR(
			"[focal] %s() - ERROR: create fts_tp_differ proc()  failed.", __func__);
	}
