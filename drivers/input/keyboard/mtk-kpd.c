...
EXPORT_SYMBOL_GPL(get_kpd_swap_vol_key);

void set_kpd_swap_vol_key(bool flag)
{
	kpd_swap_vol_key = flag;
}
EXPORT_SYMBOL_GPL(set_kpd_swap_vol_key);
...
EXPORT_SYMBOL_GPL(kpd_get_linux_key_code);
#endif
	...
	for (i = 0; i < KPD_NUM_MEMS; i++) {
		...
		for (j = 0; j < 16U; j++) {
			...
			if (!keycode)
				continue;
#ifdef CONFIG_KPD_VOLUME_KEY_SWAP
			keycode = kpd_get_linux_key_code(keycode,pressed);
#endif
				...
				struct device_node *node)
{
	...
	pr_debug("kpd_volume_key_hardware_inversed = %s\n",
