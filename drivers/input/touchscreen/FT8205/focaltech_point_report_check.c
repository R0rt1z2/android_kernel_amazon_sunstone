		...
		if (time_after(cur_jiffies, intr_timeout)) {
			if (ts_data->touch_points) {
				...
				if (ts_data->log_level >= 3)
					FTS_DEBUG(
						"prc trigger interval:%dms",
						jiffies_to_msecs(
							cur_jiffies -
							ts_data->intr_jiffies));
			}
			ts_data->prc_mode = 0;
		} else {
			queue_delayed_work(
...
static ssize_t fts_prc_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t fts_prc_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	int count;
	struct fts_ts_data *ts_data = dev_get_drvdata(dev);
