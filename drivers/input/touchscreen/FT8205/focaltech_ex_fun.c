		...
		struct device_attribute *attr, char *buf)
{
	...
	if ((fts_read_reg(FTS_REG_GESTURE_EN, &val)) < 0)
		FTS_ERROR("read reg fail\n");

	count = snprintf(buf, PAGE_SIZE, "Gesture Mode:%s\n",
			 atomic_read(&ts_data->gesture_support) ? "On" : "Off");
		...
		const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		...
		atomic_set(&ts_data->gesture_support, ENABLE);
		panel_gesture_mode_set_by_ft8205(true);
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		FTS_DEBUG("disable gesture");
		atomic_set(&ts_data->gesture_support, DISABLE);
...
static struct attribute *fts_api_attrs[] = {
	&dev_attr_gesture_wakeup.attr,
	NULL
};

static const struct attribute_group fts_api_attribute_group = {
	.attrs = fts_api_attrs,
};
					 ...
					 size_t size, loff_t *pos)
{
	...
	ret = fts_read_reg(FTS_REG_FW_VER, &value);
	if ((ret < 0) || (value == 0xFF) || (value == 0x00))
...
int fts_create_sysfs(struct fts_ts_data *ts_data)
{
	...
	if (touchscreen_link != NULL) {
		...
		if (ret < 0) {
			...
			if (sysfs_create_group(&client->dev.kobj, &fts_api_attribute_group))
...
{
	sysfs_remove_group(&ts_data->dev->kobj, &fts_attribute_group);
	sysfs_remove_group(&ts_data->client->dev.kobj, &fts_api_attribute_group);
	if (tp_fw_version_entry != NULL) {
