...
static int ft8205_ts_disp_notifier_callback(struct notifier_block *self,
	...
	if (ts_data && value) {
		if (event == MTK_DISP_EVENT_BLANK) {
			if (*data == MTK_DISP_BLANK_UNBLANK) {
				FTS_INFO("fts_ts_resume IN");
				if (fts_ts_resume(ts_data->dev) < 0) {
					FTS_ERROR("TP resume failed");
					return -1;
				}
			}
		} else if (event == MTK_DISP_EARLY_EVENT_BLANK) {
			if (*data == MTK_DISP_BLANK_POWERDOWN) {
				FTS_INFO("fts_ts_suspend IN");
				if (fts_ts_suspend(ts_data->dev) < 0) {
					FTS_ERROR("TP suspend failed");
					return -1;
				}
			}
		} else {
			FTS_ERROR("Invalid notification callback event");
			return -1;
		}
	} else {
		FTS_ERROR("ft8205 touch IC can not suspend or resume");
		return -1;
	}
	return 0;
}
...
static int fts_read_touchdata(struct fts_ts_data *ts_data, u8 *buf)
{
	...
	if (event == TOUCH_DEFAULT) {
		...
		touch_max_size =
			(buf[FTS_TOUCH_E_NUM] & 0x0F) * FTS_ONE_TCH_LEN + 2;
	}
#if FTS_PEN_EN
...
static int fts_read_parse_touchdata(struct fts_ts_data *ts_data, u8 *touch_buf)
{
	...
	/*gesture*/
	if (ts_data->suspended && atomic_read(&ts_data->gesture_support)) {
...
static int fts_input_pen_init(struct fts_ts_data *ts_data)
{
	...
	ts_data->pen_etype = STYLUS_HOVER;
	FTS_FUNC_EXIT();
	return 0;
}
#endif
				...
				unsigned long event, void *data)
{
	...
	case FB_BLANK_UNBLANK:
		...
		} else if (FB_EVENT_BLANK == (int)event) {
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	ts_data->disp_notifier.notifier_call = ft8205_ts_disp_notifier_callback;
	ret = mtk_disp_notifier_register("focaltech ic Touch",
					 &ts_data->disp_notifier);
	if (ret) {
...
static int fts_ts_suspend(struct device *dev)
{
	...
	if (atomic_read(&ts_data->gesture_support)) {
