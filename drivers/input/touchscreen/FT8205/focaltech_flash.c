...
#define FTS_FW_NAME_PREX_WITH_REQUEST_KD "focaltech_ts_fw_kd"
#define FTS_FW_NAME_PREX_WITH_REQUEST_TG "focaltech_ts_fw_tg"
...
int fts_flash_read_buf(u32 saddr, u8 *buf, u32 len)
{
	...
	for (i = 0; i < packet_number; i++) {
		...
		if (upg->ts_data->bus_type == BUS_TYPE_I2C) {
			...
			wbuf[4] = BYTE_OFF_8(packet_len);
			wbuf[5] = BYTE_OFF_0(packet_len);
			ret = fts_read(wbuf, FTS_CMD_READ_LEN_SPI, buf + offset,
				       packet_len);
			if (ret < 0) {
				FTS_ERROR(
...
static int fts_get_fw_file_via_request_firmware(struct fts_upgrade *upg)
{
	...
	case FT8205_TG_INX:
		ret = snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin",
			FTS_FW_NAME_PREX_WITH_REQUEST_TG, upg->module_info->vendor_name);
		if (ret >= 0) {
			ret = request_firmware(&fw, fwname, upg->ts_data->dev);
		} else {
			FTS_ERROR("snprintf copy string failed %d",ret);
		}
		break;
	case FT8205_KD_BOE:
		ret = snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin",
			FTS_FW_NAME_PREX_WITH_REQUEST_KD, upg->module_info->vendor_name);
		if (ret >= 0) {
			ret = request_firmware(&fw, fwname, upg->ts_data->dev);
		} else {
			FTS_ERROR("snprintf copy string failed %d",ret);
