...
*
* File Name: Focaltech_test_ft8205.c
*
...
*****************************************************************************/
static int ft8205_short_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	int i = 0;
	...
	ret = short_get_adcdata_incell(TEST_RETVAL_AA, ch_num, byte_num,
				       adcdata);
	if (ret < 0) {
	...
	for (i = 0; i < tdata->node.node_num; i++) {
		tmp_adc = adcdata[i];
		if ((920 - (tmp_adc * 9 / 10)) == 0) {
			adcdata[i] = 3000;
			continue;
		}
		adcdata[i] = (60300 * tmp_adc + 1739100) /
			     (1000 * (920 - (tmp_adc * 9 / 10)));
		if ((adcdata[i] > 3000) && (tmp_adc <= 1023)) {
			adcdata[i] = 3000;
		}
	}
...
test_err:
	...
	/* save test data */
	fts_test_save_data("Short Circuit Test", CODE_SHORT_TEST, adcdata, 0,
			   false, false, *test_result);

	FTS_TEST_FUNC_EXIT();
	return ret;
}

static int ft8205_open_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	int i = 0;
	bool tmp_result = false;
...
static int ft8205_cb_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	bool tmp_result = false;
	int byte_num = 0;
...
test_err:
	...
	/*save test data*/
	fts_test_save_data("CB Test", CODE_CB_TEST, cbdata, 0, false, false,
			   *test_result);
	FTS_TEST_FUNC_EXIT();
	return ret;
}

static int ft8205_rawdata_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	bool tmp_result = false;
	int *rawdata = NULL;
	...
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("rawdata select fail\n");
		goto test_err;
	}
...
static int ft8205_lcdnoise_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	int i = 0;
	bool tmp_result = false;
	...
	for (i = 0; i < FACTORY_TEST_RETRY; i++) {
		...
		if ((ret >= 0) && (TEST_RETVAL_AA == status)) {
			...
			FTS_TEST_DBG("reg%x=%x,retry:%d\n",
				     FACTORY_REG_LCD_NOISE_TEST_STATE, status,
				     i);
		}
		sys_delay(FACTORY_TEST_RETRY_DELAY);
	}
...
test_err:
	...
	/*save test data*/
	fts_test_save_data("LCD Noise Test", CODE_LCD_NOISE_TEST, lcdnoise, 0,
			   false, false, *test_result);
	FTS_TEST_FUNC_EXIT();
	return ret;
}

static int start_test_ft8205(void)
{
	int ret = 0;
	...
	if (true == test_item->short_test) {
		ret = ft8205_short_test(tdata, &temp_result);
	...
	if (true == test_item->open_test) {
		ret = ft8205_open_test(tdata, &temp_result);
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
		}
	}
	...
	if (true == test_item->cb_test) {
		ret = ft8205_cb_test(tdata, &temp_result);
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
		}
	}
	...
	if (true == test_item->rawdata_test) {
		ret = ft8205_rawdata_test(tdata, &temp_result);
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
		}
	}
	...
	if (true == test_item->lcdnoise_test) {
		ret = ft8205_lcdnoise_test(tdata, &temp_result);
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
		}
	}
...
struct test_funcs test_func_ft8205 = {
	.ctype = { 0x26 },
	.hwtype = IC_HW_INCELL,
	.key_num_total = 0,
	.start_test = start_test_ft8205,
};
