...
#define CMD_PEN_RAW_DATA 0x58
#define CMD_PEN_PEAK_TO_PEAK 0x59
...
static struct mp_test_items tItems[MP_TEST_ITEM] = {
	...
	/* PEN TEST */
	{ .desp = "pen raw data",
	  .catalog = PEN_RAW_TEST,
	  .cmd = CMD_PEN_RAW_DATA,
	  .lcm = ON },
	{ .desp = "pen peak to peak",
	  .catalog = PEAK_TO_PEAK_TEST,
	  .cmd = CMD_PEN_PEAK_TO_PEAK,
	  .lcm = ON },
#endif
};
...
{
	ili_dump_data(max_ptr, 32, core_mp.frame_len, 0,
		      "Dump Benchmark Max : ");
	ili_dump_data(min_ptr, 32, core_mp.frame_len, 0,
		      "Dump Benchmark Min : ");
}
...
static void allnode_open_cdc_result(int index, int *buf, int *dac, int *raw)
{
	...
	if (ipio_strcmp(desp, "open test(integration)_sp") == 0) {
		for (i = 0; i < core_mp.frame_len; i++)
			buf[i] = open_sp_formula(dac[i], raw[i], open_para.tvch,
						 open_para.tvcl);
...
static int allnode_open_cdc_data(int mode, int *buf, int index)
{
	...
	u8 *ori = NULL;
	char str[128] = { 0 };
	char *key[] = { "open dac",  "open raw1",     "open raw2",
			"open raw3", "open cap1 dac", "open cap1 raw" };
...
static int allnode_peak_to_peak_cdc_data(int index)
{
	...
	if (frm_buf == NULL) {
		frm_buf = (s32 **)kzalloc(tItems[index].bch_mrk_frm_num *
						  sizeof(s32 *),
					  GFP_KERNEL);
	...
	if (tItems[index].bch_mrk_multi) {
		ili_dump_data(frm_buf[0], 32, core_mp.frame_len,
			      core_mp.xch_len, "Mutual CDC combined[0]/frame1");
		ili_dump_data(frm_buf[1], 32, core_mp.frame_len,
			      core_mp.xch_len, "Mutual CDC combined[1]/frame2");
		ili_dump_data(frm_buf[2], 32, core_mp.frame_len,
			      core_mp.xch_len, "Mutual CDC combined[2]/frame3");
	} else {
		ili_dump_data(frm_buf[0], 32, core_mp.frame_len,
			      core_mp.xch_len, "Mutual CDC combined[0]/frame1");
	}
	...
	for (y = 0; y < core_mp.ych_len; y++) {
		for (x = 0; x < core_mp.xch_len; x++) {
			...
			if (tItems[index].catalog == UNTOUCH_P2P)
				...
				if (core_mp.tx_delta_buf[shift + x] <
				    data[shift + x])
					core_mp.tx_max_buf[shift + x] =
						data[shift + x];

				if (core_mp.tx_delta_buf[shift + x] >
				    data[shift + x])
					core_mp.tx_min_buf[shift + x] =
						data[shift + x];

				/* Rx max/min comparison */
				if (core_mp.rx_delta_buf[shift + x] <
				    data[shift + x])
					core_mp.rx_max_buf[shift + x] =
						data[shift + x];

				if (core_mp.rx_delta_buf[shift + x] >
				    data[shift + x])
					core_mp.rx_min_buf[shift + x] =
						data[shift + x];
			} else {
				if (tItems[index].max_buf[shift + x] <
				    data[shift + x])
					tItems[index].max_buf[shift + x] =
						data[shift + x];

				if (tItems[index].min_buf[shift + x] >
				    data[shift + x])
					tItems[index].min_buf[shift + x] =
						data[shift + x];
			}
	...
	for (y = 0; y < core_mp.ych_len; y++) {
		for (x = 0; x < core_mp.xch_len; x++) {
			if (tItems[i].catalog == TX_RX_DELTA) {
				core_mp.tx_max_buf[y * core_mp.xch_len + x] =
					INT_MIN;
				core_mp.rx_max_buf[y * core_mp.xch_len + x] =
					INT_MIN;
				core_mp.tx_min_buf[y * core_mp.xch_len + x] =
					INT_MAX;
				core_mp.rx_min_buf[y * core_mp.xch_len + x] =
					INT_MAX;
			} else {
...
static int peak_to_peak_test(int index)
{
	...
	ILI_DBG("index = %d, desp = %s bch_mrk_frm_num = %d\n", index,
		tItems[index].desp, tItems[index].bch_mrk_frm_num);
	...
	if (tItems[index].spec_option == BENCHMARK) {
		for (i = 0; i < tItems[index].bch_mrk_frm_num; i++) {
			if (tItems[index].bch_mrk_multi)
				snprintf(benchmark_str, sizeof(benchmark_str),
					 "%s%d", BENCHMARK_KEY_NAME, i + 1);
			else
				snprintf(benchmark_str, sizeof(benchmark_str),
					 "%s", BENCHMARK_KEY_NAME);
			parser_ini_benchmark(tItems[index].bch_mrk_max[i],
					     tItems[index].bch_mrk_min[i],
					     tItems[index].type_option,
					     tItems[index].desp,
					     core_mp.frame_len, benchmark_str);

			dump_benchmark_data(tItems[index].bch_mrk_max[i],
					    tItems[index].bch_mrk_min[i]);
		}
	}
...
#if 0 //ENABLE_PEN_MODE
static int pen_raw_data_test(int index)
{
	ILI_INFO("++++ %s ++++\n", __func__);
	ILI_INFO("---- %s ----\n", __func__);
}

static int pen_raw_test(int index)
{
	int i = 0, j = 0, x = 0, y = 0, ret = 0, get_frame_cont = 1;
...
static int open_test_sp(int index)
{
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		ili_dump_data(open[i].cbk_700, 10, core_mp.frame_len,
			      core_mp.xch_len, "cbk 700");
		ili_dump_data(open[i].cbk_250, 10, core_mp.frame_len,
			      core_mp.xch_len, "cbk 250");
		ili_dump_data(open[i].cbk_200, 10, core_mp.frame_len,
		...
		ili_dump_data(open[i].charg_rate, 10, core_mp.frame_len,
			      core_mp.xch_len, "origin charge rate");
		ili_dump_data(open[i].full_Open, 10, core_mp.frame_len,
		...
		ili_dump_data(&tItems[index].buf[(i * core_mp.frame_len)], 10,
			      core_mp.frame_len, core_mp.xch_len,
...
static int open_test_cap(int index)
{
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		ili_dump_data(open[i].dcl_cap, 10, core_mp.frame_len,
...
static int mp_show_result(bool lcm_on)
{
	...
	for (seq = 0; seq < ri.count; seq++) {
		...
		if (ipio_strcmp(tItems[i].desp, "open test(integration)_sp") ==
		    0) {
		...
		if (ipio_strcmp(tItems[i].desp, "open test_c") == 0) {
			mp_compare_cdc_show_result(i, cap_dac, csv, &csv_len,
						   TYPE_NO_JUGE, max_threshold,
						   min_threshold, "CAP_DAC",
						   CSV_FILE_SIZE);
			mp_compare_cdc_show_result(i, cap_raw, csv, &csv_len,
						   TYPE_NO_JUGE, max_threshold,
						   min_threshold, "CAP_RAW",
						   CSV_FILE_SIZE);
		}
		...
		if (tItems[i].catalog == KEY_TEST) {
			...
			for (j = 0; j < get_frame_cont; j++) {
				...
				snprintf(frame_name, sizeof(frame_name),
					 "Frame %d", (j + 1));
				mp_compare_cdc_show_result(
					i,
...
static void ilitek_tddi_mp_init_item(void)
{
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		...
		if (tItems[i].catalog == MUTUAL_TEST) {
			...
			else if (ipio_strcmp(tItems[i].desp, "open test_c") ==
				 0)
...
#if 0 //ENABLE_PEN_MODE
		else if (tItems[i].catalog == PEN_RAW_TEST) {
			tItems[i].do_test = pen_raw_test;
		}
#endif
