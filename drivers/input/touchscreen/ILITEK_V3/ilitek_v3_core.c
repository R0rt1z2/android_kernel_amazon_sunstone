...
#define DTS_INT_GPIO "irq-gpios"
#define DTS_RESET_GPIO "reset-gpios"
...
void ili_input_register(void)
{
	...
	input_set_abs_params(ilits->input, ABS_MT_POSITION_X,
			     TOUCH_SCREEN_X_MIN, ilits->panel_wid, 0, 0);
	input_set_abs_params(ilits->input, ABS_MT_POSITION_Y,
			     TOUCH_SCREEN_Y_MIN, ilits->panel_hei, 0, 0);
	...
	if (MT_PRESSURE)
		input_set_abs_params(ilits->input, ABS_MT_PRESSURE, 0, 255, 0,
				     0);
	...
	if (MT_B_TYPE) {
		...
		input_set_abs_params(ilits->input, ABS_MT_TRACKING_ID, 0,
				     MAX_TOUCH_NUM, 0, 0);
	}
...
void ili_input_pen_register(void)
{
	...
	input_set_abs_params(ilits->input_pen, ABS_X, TOUCH_SCREEN_X_MIN, ilits->panel_wid, 0,
			     0);
	input_set_abs_params(ilits->input_pen, ABS_Y, TOUCH_SCREEN_Y_MIN, ilits->panel_hei, 0,
			     0);
	input_set_abs_params(ilits->input_pen, ABS_PRESSURE, 0, MAX_PRESSURE, 0,
			     0);
...
static void ilitek_plat_sleep_init(void)
{
	...
	if (active_panel) {
		if (drm_panel_notifier_register(active_panel,
						&ilits->notifier_fb))
						 ...
						 unsigned long event, void *value)
{
	struct ilitek_ts_data *ilits =
		container_of(self, struct ilitek_ts_data, disp_notifier);
	int *data = (int *)value;
...
#else
	ILI_ERR("idme is not ready.\n");
#endif
	vendor_id = (unsigned char)lcm_id;
	ILI_INFO("vendor id = 0x%x\n", vendor_id);
}
...
static int ilitek_plat_probe(void)
{
	...
	get_lcm_id();
	if ((vendor_id != LCM_ILI9882T_TG_HSD) && (vendor_id != LCM_ILI9882T_KD_BOE)) {
		ILI_ERR("[ilitek_plat_probe] It's not ilitek IC, ilitek_plat_probe exit.");
		return -ENODEV;
	}
