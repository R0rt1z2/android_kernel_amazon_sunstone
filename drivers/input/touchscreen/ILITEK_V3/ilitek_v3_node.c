			       ...
			       u32 frame_count)
{
	...
	if (ret < 0) {
		ILI_ERR("Failed to write 0xFA,type command, %d\n", ret);
		goto out;
	}
...
int ili_cascade_rw_tp_reg(u32 mcu, u32 type, u32 addr, u32 wdata, u32 wlen,
			  u32 *rdata)
{
	int ret = 0, len = 0;
	bool mcu_on = 0, read = 0;
	u8 switch_wr_mode[2] = { 0x10, 0x10 };
	u32 read_data, backup_mspi;

	if (ilits->wrapper(switch_wr_mode, sizeof(switch_wr_mode), NULL, 0, OFF,
			   OFF) < 0)
		ILI_ERR("Failed to start Write/Read mode\n");
	...
	if (ilits->slave_wr_ctrl) {
		...
		ili_ice_mode_read(MSPI_REG, &backup_mspi, 1);
		ILI_INFO("Backup Mspi = 0x%x\n", backup_mspi);
		ili_ice_mode_write(MSPI_REG, 0x00, 1);
		atomic_set(&ilits->ice_stat, DISABLE);
		ili_ice_mode_ctrl(ENABLE, OFF); // For AA/AB
	...
	if (type == read) {
		if (ilits->slave_wr_ctrl) {
			ili_mspi_read_slave_register(addr, &read_data,
						     sizeof(u32));
		} else {
			if (ili_ice_mode_read(addr, &read_data, sizeof(u32)) <
			    0) {
		...
		*rdata = read_data;
		...
		if (ilits->slave_wr_ctrl) {
			...
			if (ili_ice_mode_write(addr, wdata, wlen) < 0) {
	...
	if (ilits->slave_wr_ctrl) {
		/* set slave exit ice mode */
		if (ili_mspi_write_slave_register(CONTROL_SLAVE_EXIT_ICE_ADDR,
						  0, 0) < 0) {
			ILI_ERR("Failed to disable Slave ICE mode\n");
		} else {
			ILI_INFO("Disable Slave ICE mode\n");
		}

		/* Recovery mspi */
		ili_ice_mode_write(MSPI_REG, backup_mspi, 1);
	}
	...
	if (ilits->enable_cascade) {
		switch_wr_mode[1] = 0x11;
		if (ilits->wrapper(switch_wr_mode, sizeof(switch_wr_mode), NULL,
				   0, OFF, OFF) < 0)
			ILI_ERR("Failed to end Write/Read mode\n");
	}
...
static ssize_t ilitek_proc_rw_tp_reg_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	if (ilits->enable_cascade) {
		if (ilits->wrapper(switch_wr_mode, sizeof(switch_wr_mode), NULL, 0, OFF, OFF) < 0)
			ILI_ERR("Failed to start Write/Read mode\n");
		mdelay(1);
	}
	...
	if (type == read) {
		if (ilits->enable_cascade == true && ilits->slave_wr_ctrl == true) {
			ili_ice_mode_read(MSPI_REG, &backup_mspi, 1);
			ili_ice_mode_write(MSPI_REG, 0x00, 1);
			...
			ili_mspi_write_slave_register(CONTROL_SLAVE_EXIT_ICE_ADDR, 0, 0);
		...
		if (ilits->enable_cascade) {
			if (ilits->slave_wr_ctrl) {
				...
				ili_ice_mode_read(MSPI_REG, &backup_mspi, 1);
				ili_ice_mode_write(MSPI_REG, 0x01, 1);
			}
		}
					  ...
					  size_t size, loff_t *pos)
{
	...
	if (ilits->enable_cascade) {
		len = ili_cascade_rw_tp_reg(stop_mcu, type, addr, write_data,
					    write_len, NULL);
	} else {
		if (stop_mcu == mcu_on)
			ret = ili_ice_mode_ctrl(ENABLE, ON);
		...
		if (type == read) {
			...
			if (ili_ice_mode_write(addr, write_data, write_len) <
			    0) {
				ILI_ERR("Write data error\n");
				len += snprintf(g_user_buf + len,
						USER_STR_BUFF - len, "%s\n",
	...
	if (ilits->enable_cascade) {
		if (ilits->slave_wr_ctrl) {
			ilits->i2c_addr_change = SLA_ADDR;
			ILI_INFO("Change to Slave Address\n");
		} else {
			ilits->i2c_addr_change = MASTER_ADDR;
			ILI_INFO("Change to Master Address\n");
		}
	}
#endif
...
static int ilitek_proc_debug_message_show(struct seq_file *m, void *v)
{
	...
	ILI_DBG("odi index = %d, dbf index = %d \n", ilits->odi, ilits->dbf);
	if (ilits->odi == ilits->dbf) {
		ILI_ERR("WARNING ! there's no cdc data.\n");
		return -EAGAIN;
	}
					    ...
					    loff_t *pos)
{
	...
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len,
			"[Drive version] = %s\n", DRIVER_VERSION);
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len,
			"[issue_0513] = %s\n", "switch to mp fail issue");
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len,
			"[issue_0615] = %s\n", "delete whole chip reset");
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len,
			"========================================\n");
	if (copy_to_user((char *)buff, g_user_buf, len))
		ILI_ERR("Failed to copy data to user space\n");
					   ...
					   size_t size, loff_t *pos)
{
	...
	if (g_user_buf[1] == EFW_CONVERT_FILE) {
		...
		len += snprintf(
			g_user_buf + len, USER_STR_BUFF - len, "%s\n",
					  ...
					  size_t size, loff_t *pos)
{
	...
	if (ili_ice_mode_ctrl_by_mode(ENABLE, OFF, BOTH) < 0)
		ILI_ERR("Failed to enable ice mode\n");
...
int ili_get_tp_recore_data(bool mcu)
{
	...
	if (fcnt > MAX_FRAME_CNT) {
		ILI_ERR("fcnt = %d is over limit, set to %d", fcnt, MAX_FRAME_CNT);
		fcnt = MAX_FRAME_CNT;
	}
	...
	for (i = 0; i < fcnt; i++) {
		...
		ili_dump_data(raw_ptr, 16, frame_len, ilits->xch_num,
			      "recore_data");
		index--;
		if (index < 0)
			index = fcnt - 1;
...
int ili_tp_data_mode_ctrl(u8 *cmd)
{
	...
	case AP_MODE:
		if (ilits->tp_suspend) {
			if (ili_set_tp_data_len(DATA_FORMAT_GESTURE_DEMO, false,
						NULL) < 0) {
			...
			if (ilits->actual_tp_mode == P5_X_FW_TEST_MODE) {
				...
				if (ili_set_tp_data_len(DATA_FORMAT_DEMO, false,
							NULL) < 0) {
					ILI_ERR("Failed to switch demo mode\n");
					ret = -ENOTTY;
				}
	...
	case DEBUG_MODE:
		...
		if (ilits->tp_suspend) {
			if (ili_set_tp_data_len(DATA_FORMAT_GESTURE_DEBUG,
						false, &cmd[1]) < 0) {
				ILI_ERR("Failed to set debug len from gesture mode\n");
				ret = -ENOTTY;
			}
		} else {
			if (ili_set_tp_data_len(DATA_FORMAT_DEBUG, false,
						&cmd[1]) < 0) {
		...
		break;
	case DEBUG_LITE_ROI:
		if (ili_set_tp_data_len(DATA_FORMAT_DEBUG_LITE_ROI, false,
					&cmd[1]) < 0) {
	...
	case DEBUG_LITE_WINDOW:
		if (ili_set_tp_data_len(DATA_FORMAT_DEBUG_LITE_WINDOW, false,
					&cmd[1]) < 0) {
	...
	case DEBUG_LITE_AREA:
		if (ili_set_tp_data_len(DATA_FORMAT_DEBUG_LITE_AREA, false,
					&cmd[1]) < 0) {
	...
	case DEBUG_PEN_ONLY_MODE:
		ilits->pen_info.pen_type = P5_X_ONLY_PEN_TYPE;
		if (ili_set_pen_data_info(P5_X_NEW_CONTROL_FORMAT,
	...
	case DEBUG_HAND_ONLY_MODE:
		ilits->pen_info.pen_type = P5_X_ONLY_HAND_TYPE;
		if (ili_set_pen_data_info(P5_X_NEW_CONTROL_FORMAT,
				       ...
				       size_t size, loff_t *pos)
{
	...
	if (strncmp(cmd, "hwreset", strlen(cmd)) == 0) {
#if ENABLE_CASCADE
		ili_wdt_reset_status(true, true);
#endif
		if (ili_reset_ctrl(TP_HW_RST_ONLY) < 0) {
			ILI_ERR("TP_HW_RST_ONLY Failed, ret = %d\n", ret);
		}
...
#if ENABLE_CASCADE
		if (ili_cascade_reset_ctrl(TP_IC_WHOLE_RST_WITH_FLASH, true) < 0)
			ILI_ERR("TP_IC_WHOLE_RST_WITH_FLASH Failed\n");
#else
		ili_ice_mode_ctrl(ENABLE, OFF);
...
#if ENABLE_CASCADE
		if (ili_cascade_reset_ctrl(TP_IC_WHOLE_RST_WITHOUT_FLASH, true) < 0)
			ILI_ERR("TP_IC_WHOLE_RST_WITHOUT_FLASH Failed\n");
#else
		ili_ice_mode_ctrl(ENABLE, OFF);
...
#if ENABLE_CASCADE
		if (ili_cascade_reset_ctrl(TP_IC_CODE_RST, true) < 0)
			ILI_ERR("TP_IC_CODE_RST Failed\n");
#else
		ili_ice_mode_ctrl(ENABLE, OFF);
	...
	} else if (strcmp(cmd, "spiwrslavereg") == 0) {
#if ENABLE_CASCADE
		ilits->slave_wr_ctrl = !ilits->slave_wr_ctrl;
		ILI_INFO(" %s slave read ctrl = %x\n",
			 ilits->slave_wr_ctrl ? "Enable" : "Disable",
			 ilits->slave_wr_ctrl);
#endif
		...
		panel_gesture_mode_set_by_ili9882t(atomic_read(&ilits->gesture));
		...
		ilits->edge_palm_para[30] = ili_calc_packet_checksum(
...
static ssize_t gesture_wakeup_store(struct device *dev, struct device_attribute *attr,
...
void ili_node_init(void)
{
	...
	if (proc_dir_ilitek == NULL) {
		ILI_ERR("Proc mkdir ilitek failed\n");
		ret = -ENODEV;
	}
	...
	for (; i < ARRAY_SIZE(iliproc); i++) {
		if (strncmp(iliproc[i].name, "tp_fw_version", strlen(iliproc[i].name)) == 0) {
			iliproc[i].node = proc_create(iliproc[i].name, 0444,
						NULL, iliproc[i].proc_ops);
		} else {
			iliproc[i].node = proc_create(iliproc[i].name, 0644,
						proc_dir_ilitek, iliproc[i].proc_ops);
		}
	...
	if (touchscreen_link != NULL) {
		...
		if (ret < 0) {
			...
			if (sysfs_create_group(&client->dev.kobj, &ili_api_attribute_group))
...
void ili_sysfs_remove_group(struct ilitek_ts_data *ts)
{
