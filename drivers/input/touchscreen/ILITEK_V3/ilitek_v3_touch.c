...
static void dma_clear_reg_setting(void)
{
	...
	if (ili_ice_mode_bit_mask_write(
		    INTR32_ADDR, INTR32_reg_t0_int_en | INTR32_reg_t1_int_en,
		    (0 << 24)) < 0)
		ILI_ERR("Write %lu at %x failed\n",
			INTR32_reg_t0_int_en | INTR32_reg_t1_int_en,
			INTR32_ADDR);

	/* 2. clear tdi_err_int_flag */
	if (ili_ice_mode_bit_mask_write(
		    INTR2_ADDR, INTR2_tdi_err_int_flag_clear, (1 << 18)) < 0)
		ILI_ERR("Write %lu at %x failed\n",
	...
	if (ili_ice_mode_bit_mask_write(DMA50_ADDR,
					DMA50_reg_dma_ch0_src1_format |
						DMA50_reg_dma_ch0_src1_en,
					BIT(31)) < 0)
		ILI_ERR("Write %lu at %x failed\n",
			DMA50_reg_dma_ch0_src1_format |
				DMA50_reg_dma_ch0_src1_en,
	...
	if (ili_ice_mode_bit_mask_write(
		    DMA48_ADDR, DMA48_reg_dma_ch0_trigger_sel, (0 << 16)) < 0)
		ILI_ERR("Write %lu at %x failed\n",
			DMA48_reg_dma_ch0_trigger_sel, DMA48_ADDR);
	if (ili_ice_mode_bit_mask_write(INTR1_ADDR, INTR1_reg_flash_int_flag,
					(1 << 25)) < 0)
		ILI_ERR("Write %lu at %x failed\n", INTR1_reg_flash_int_flag,
			INTR1_ADDR);
				    ...
				    u32 copy_size)
{
	...
	if (ili_ice_mode_bit_mask_write(
		    DMA48_ADDR, DMA48_reg_dma_ch0_start_clear, (1 << 25)) < 0)
		ILI_ERR("Write %lu at %x failed\n",
	...
	if (ili_ice_mode_bit_mask_write(DMA52_ADDR, DMA52_reg_dma_ch0_src2_en,
					(0 << 31)) < 0)
		ILI_ERR("Write %lu at %x failed\n", DMA52_reg_dma_ch0_src2_en,
			DMA52_ADDR);

	/* 4. set dma channel 0 dest info */
	if (ili_ice_mode_write(DMA53_reg_dma_ch0_dest_addr, reg_dest_addr, 3) <
	    0)
		ILI_ERR("Write %x at %x failed\n", reg_dest_addr,
			DMA53_reg_dma_ch0_dest_addr);
	...
	if (ili_ice_mode_write(DMA54_ADDR,
			       DMA54_reg_dma_ch0_dest_format |
				       DMA54_reg_dma_ch0_dest_en,
			       BIT(31)) < 0)
		ILI_ERR("Write %lu at %x failed\n",
			DMA54_reg_dma_ch0_dest_format |
				DMA54_reg_dma_ch0_dest_en,
	...
	if (ili_ice_mode_write(DMA55_reg_dma_ch0_trafer_counts, copy_size, 4) <
	    0)
		ILI_ERR("Write %x at %x failed\n", copy_size,
			DMA55_reg_dma_ch0_trafer_counts);
	if (ili_ice_mode_bit_mask_write(
		    DMA55_ADDR, DMA55_reg_dma_ch0_trafer_mode, (0 << 24)) < 0)
		ILI_ERR("Write %lu at %x failed\n",
			DMA55_reg_dma_ch0_trafer_mode, DMA55_ADDR);

	/* 6. set dma channel 0 int info */
	if (ili_ice_mode_bit_mask_write(INTR33_ADDR, INTR33_reg_dma_ch0_int_en,
					(1 << 17)) < 0)
		ILI_ERR("Write %lu at %x failed\n", INTR33_reg_dma_ch0_int_en,
			INTR33_ADDR);

	/* 7. set dma channel 0 trigger select */
	if (ili_ice_mode_bit_mask_write(
		    DMA48_ADDR, DMA48_reg_dma_ch0_trigger_sel, (1 << 16)) < 0)
		ILI_ERR("Write %lu at %x failed\n",
			DMA48_reg_dma_ch0_trigger_sel, DMA48_ADDR);

	/* 8. set dma flash setting */
	ili_flash_dma_write(flash_start_addr, (flash_start_addr + copy_size),
			    copy_size);

	/* 9. clear flash and dma ch0 int flag */
	if (ili_ice_mode_bit_mask_write(INTR1_ADDR,
					INTR1_reg_dma_ch1_int_flag |
						INTR1_reg_flash_int_flag,
					BIT(16) | BIT(25)) < 0)
		ILI_ERR("Write %lu at %x failed\n",
			INTR1_reg_dma_ch1_int_flag | INTR1_reg_flash_int_flag,
			INTR1_ADDR);
...
int ili_move_mp_code_flash(void)
{
	...
	ili_dump_data(data, 8, ilits->protocol->mp_info_len, 0,
		      "MP overlay info");
	if (ret < 0) {
	...
	ILI_DBG("MP info Overlay: Enable = %d, addr = 0x%x ~ 0x%x, flash addr = 0x%x, mp size = 0x%x\n",
	...
	if (dma_trigger_enable) {
		...
		ILI_DBG("MP andes init size = %d , MP text size = %d\n",
			mp_andes_init_size, mp_text_size);

		dma_clear_reg_setting();

		ILI_DBG("[Move ANDES.INIT to DRAM]\n");
...
{
	ILI_DBG("Download MP code to iram\n");
...
int ili_proximity_near(int mode)
{
	...
	case DDI_POWER_ON:
		...
		break;
	case DDI_POWER_OFF:
		ILI_DBG("DDI POWER OFF, do nothing\n");
		break;
	default:
...
int ili_proximity_far(int mode)
{
	...
	if (!ilits->prox_near) {
		ILI_DBG("No proximity near event, break\n");
		return 0;
	}
	...
	case WAKE_UP_SWITCH_GESTURE_MODE:
		...
		ILI_DBG("write prepare gesture command 0xF6 0x0A\n");
		ret = ilits->wrapper(cmd, 2, NULL, 0, ON, OFF);
		if (ret < 0) {
			ILI_DBG("write prepare gesture command error\n");
			break;
		}
...
int ili_move_gesture_code_flash(int mode)
{
	...
	ILI_DBG("Gesture mode = %d\n", mode);
	ret = ili_set_tp_data_len(mode, true, NULL);
	if (ret < 0)
		ILI_ERR("Enter gesture mode failed\n");
	return ret;
}
...
int ili_move_gesture_code_iram(int mode)
{
	...
	for (i = 0; i < timeout; i++) {
		...
		if (cmd[0] == 0x91) {
			ILI_DBG("Ready to load gesture code\n");
			break;
		}
		mdelay(2);
	}
...
int ili_touch_esd_gesture_flash(void)
{
	...
	ILI_DBG("ESD Gesture PWD Addr = 0x%X, PWD = 0x%X, GES_RUN = 0%X, core_ver = 0x%X\n",
	...
	if (ilits->reset == TP_HW_RST_ONLY) {
		...
		ret = ili_cascade_reset_ctrl(ilits->reset, true);
	}
#else
	ret = ili_reset_ctrl(ilits->reset);
#endif
...
#if ENABLE_CASCADE
	ili_cascade_sync_ctrl(false);
	if (ili_ice_mode_ctrl_by_mode(ENABLE, ON, BOTH) < 0)
	...
	ILI_DBG("0x%X Enter gesture successfully\n", answer);

out:
#if ENABLE_CASCADE
	if (ili_ice_mode_ctrl_by_mode(DISABLE, ON, BOTH) < 0)
		ILI_ERR("Failed to disable ice mode\n");
	ili_cascade_sync_ctrl(true);
#else
...
int ili_touch_esd_gesture_iram(void)
{
	...
	ILI_DBG("ESD Gesture PWD Addr = 0x%X, PWD = 0x%X, GES_RUN = 0x%X, core_ver = 0x%X\n",
		ges_pwd_addr, ges_pwd, ges_run, ilits->chip->core_ver);
	...
	ILI_DBG("Enter gesture successfully\n");
...
void ili_demo_debug_info_id0(u8 *buf, size_t len)
{
	...
	ILI_DBG("id0 len = %d,strucy len = %d", (int)len, (int)sizeof(id0));

	ILI_DBG("id = %d\n", id0.id);
	ILI_DBG("app_sys_powr_state_e = %d\n", id0.sys_powr_state_e);
	ILI_DBG("app_sys_state_e = %d\n", id0.sys_state_e);
	ILI_DBG("tp_state_e = %d\n", id0.tp_state_e);

	ILI_DBG("touch_palm_state_e = %d\n", id0.touch_palm_state);
	ILI_DBG("app_an_statu_e = %d\n", id0.app_an_statu_e);
	ILI_DBG("app_sys_bg_err = %d\n", id0.app_sys_bg_err);
	ILI_DBG("g_b_wrong_bg = %d\n", id0.g_b_wrong_bg);
	ILI_DBG("reserved0 = %d\n", id0.reserved0);

	ILI_DBG("normal_mode = %d\n", id0.normal_mode);
	ILI_DBG("charger_mode = %d\n", id0.charger_mode);
	ILI_DBG("glove_mode = %d\n", id0.glove_mode);
	ILI_DBG("stylus_mode = %d\n", id0.stylus_mode);
	ILI_DBG("multi_mode = %d\n", id0.multi_mode);
	ILI_DBG("noise_mode = %d\n", id0.noise_mode);
	ILI_DBG("palm_plus_mode = %d\n", id0.palm_plus_mode);
	ILI_DBG("floating_mode = %d\n", id0.floating_mode);

	ILI_DBG("algo_pt_status0 = %d\n", id0.algo_pt_status0);
	ILI_DBG("algo_pt_status1 = %d\n", id0.algo_pt_status1);
	ILI_DBG("algo_pt_status2 = %d\n", id0.algo_pt_status2);
	ILI_DBG("algo_pt_status3 = %d\n", id0.algo_pt_status3);
	ILI_DBG("algo_pt_status4 = %d\n", id0.algo_pt_status4);
	ILI_DBG("algo_pt_status5 = %d\n", id0.algo_pt_status5);
	ILI_DBG("algo_pt_status6 = %d\n", id0.algo_pt_status6);
	ILI_DBG("algo_pt_status7 = %d\n", id0.algo_pt_status7);
	ILI_DBG("algo_pt_status8 = %d\n", id0.algo_pt_status8);
	ILI_DBG("algo_pt_status9 = %d\n", id0.algo_pt_status9);
	ILI_DBG("reserved2 = %d\n", id0.reserved2);

	ILI_DBG("hopping_flg = %d\n", id0.hopping_flg);
	ILI_DBG("hopping_index = %d\n", id0.hopping_index);
...
void ili_demo_debug_info_mode(u8 *buf, size_t len)
{
	...
	ILI_DBG("info len = %d ,id = %d\n", info_len, info_id);
	if (info_id == 0) {
		ilits->demo_debug_info[info_id](&info_ptr[1], info_len);
	} else {
		ILI_DBG("Not support this id %d\n", info_id);
	}
}
...
static void ilitek_tddi_touch_send_debug_data(u8 *buf, int len)
{
	...
	ILI_DBG("netlink = %d, dnp = %d, dlnp = %d\n", ilits->netlink,
		ilits->dnp, ilits->dlnp);
	...
	if (ilits->dnp) {
		...
		ILI_DBG("len = %d, buf[0] = 0x%X, buf[1] = 0x%X, buf[2] = 0x%X \n",
			len, buf[0], buf[1], buf[2]);
		ipio_memcpy(ilits->dbl[index].data, buf, len, TR_BUF_SIZE);
		ILI_DBG("dbl[%d].data = 0x%X, dbl[%d].data = 0x%X, dbl[%d].data = 0x%X \n",
			index, ilits->dbl[index].data[0], index,
			ilits->dbl[index].data[1], index,
			ilits->dbl[index].data[2]);
...
static void ilitek_tddi_touch_customer_data_parsing(u8 *buf)
{
	...
	if (ilits->rib.nCustomerType == POSITION_CUSTOMER_TYPE_ON &&
		...
		if (ilits->rib.nReportResolutionMode ==
			...
			palmflag = ((buf[index + P5_X_CUSTOMER_LENGTH + 2] &
				     0x08) >>
				    3);
			ILI_DBG("palmflag = %d\n", palmflag);
	...
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		if (ilits->rib.nCustomerType == POSITION_CUSTOMER_TYPE_OFF &&
			...
			yop = (((buf[(4 * i) + 1 +
			   ...
			   POSITION_HIGH_RESOLUTION) {
			if ((buf[(5 * i) + 1 + P5_X_DEMO_MODE_PACKET_INFO_LEN] ==
			     0xFF) &&
			    (buf[(5 * i) + 2 + P5_X_DEMO_MODE_PACKET_INFO_LEN] ==
			     0xFF) &&
			    (buf[(5 * i) + 3 + P5_X_DEMO_MODE_PACKET_INFO_LEN] ==
			     0xFF) &&
			...
			xop = ((buf[(5 * i) + 1 + P5_X_DEMO_MODE_PACKET_INFO_LEN]
				<< 8) |
			       (buf[(5 * i) + 2 +
				    P5_X_DEMO_MODE_PACKET_INFO_LEN]));
			yop = ((buf[(5 * i) + 3 + P5_X_DEMO_MODE_PACKET_INFO_LEN]
				<< 8) |
			       (buf[(5 * i) + 4 +
				    P5_X_DEMO_MODE_PACKET_INFO_LEN]));
		}
	...
	if (ilits->finger) {
		if (MT_B_TYPE) {
			for (i = 0; i < ilits->finger; i++) {
				input_report_key(ilits->input, BTN_TOUCH, 1);
				ili_touch_press(touch_info[i].x,
						touch_info[i].y,
						touch_info[i].pressure,
						touch_info[i].id);
				input_report_key(ilits->input, BTN_TOOL_FINGER,
						 1);
			}
			for (i = 0; i < MAX_TOUCH_NUM; i++) {
...
#if ENABLE_PEN_MODE
static void ili_touch_press_debug_mode(u16 x, u16 y, u16 pressure, u16 id)
{
	ILI_DBG("Touch Press: id = %d, x = %d, y = %d, p = %d\n", id, x, y,
		pressure);
	...
	if (MT_B_TYPE) {
		input_mt_slot(ilits->input, id);
#if ENABLE_PEN_MODE
		if (id == (MAX_TOUCH_NUM - 1) && ilits->pen_info.active == 1) {
			input_mt_report_slot_state(ilits->input, MT_TOOL_PEN,
						   true);
		} else {
			input_mt_report_slot_state(ilits->input, MT_TOOL_FINGER,
						   true);
		}
#else
		input_mt_report_slot_state(ilits->input, MT_TOOL_FINGER, true);
...
static void ili_touch_release_debug_mode(u16 x, u16 y, u16 id)
{
	ILI_DBG("Touch Release: id = %d, x = %d, y = %d\n", id, x, y);
	...
	if (MT_B_TYPE) {
		input_mt_slot(ilits->input, id);
#if ENABLE_PEN_MODE
		if (id == (MAX_TOUCH_NUM - 1) && ilits->pen_info.active == 0) {
			input_mt_report_slot_state(ilits->input, MT_TOOL_PEN,
						   false);
		} else {
			input_mt_report_slot_state(ilits->input, MT_TOOL_FINGER,
...
static void ili_pen_down(u16 x, u16 y, u16 pressure)
{
	ILI_DBG("Pen Down: x = %d, y = %d, pressure = %d, TipSwitch = %d, InRange = %d\n",
		x, y, pressure, ilits->pen_info.TipSwitch,
	...
	if (ilits->pen_info.TipSwitch) {
		...
		ILI_DBG("ABS_DISTANCE = 0\n");
	} else if (ilits->pen_info.InRange) {
		input_report_abs(ilits->input_pen, ABS_DISTANCE, 1);
		ILI_DBG("ABS_DISTANCE = 1\n");
	}
...
void ili_pen_demo_mode_report_point(u8 *buf, int len, u16 offset)
{
	...
	for (i = 0; i < ilits->touch_num; i++) {
		if (ilits->rib.nReportResolutionMode ==
			...
			if (i >= MAX_TOUCH_NUM) {
				...
				if (buf[len - (offset + 2)] !=
					...
					if (buf[(5 * i) + TOUCH_TILT_OFFSET] &
					    NEGITIVE_FLAG) {
						//Transfer Tilt to Negitive
						ilits->pen_info.tilt_x =
							buf[(5 * i) +
							    TOUCH_TILT_OFFSET] |
							0xFFFFFF00;
					} else {
					...
					if (buf[(5 * i) + TOUCH_TILT_OFFSET + 1] &
					    NEGITIVE_FLAG) {
						//Transfer Tilt to Negitive
		...
		if (ilits->pen_info.active == true) {
			...
			touch_info[PEN_INDEX].pressure =
				(buf[(5 * i) + TOUCH_PRESS_OFFSET + 1] << 8) +
				buf[(5 * i) + TOUCH_PRESS_OFFSET + 2];
			...
			if (MT_PRESSURE) {
				if (ilits->rib.nCustomerType ==
					...
					touch_info[ilits->finger].pressure =
					   ...
					   ilits->rib.nReportResolutionMode ==
						   POSITION_LOW_RESOLUTION) {
					touch_info[ilits->finger].pressure = buf
						[(4 * i) + 4 +
						 P5_X_DEMO_MODE_PACKET_INFO_LEN];
				} else if (ilits->rib.nReportResolutionMode ==
					   POSITION_HIGH_RESOLUTION) {
					touch_info[ilits->finger].pressure = buf[(5 * i) + 4];
				}
			} else {
				touch_info[ilits->finger].pressure = 1;
			}
...
void ili_pen_debug_mode_report_point(u8 *buf, int len, int offset)
{
	int i = 0;
	int finger_data_offset = 0;
	u8 self_key = 2;
	u32 xop = 0, yop = 0;
	...
	for (i = 0; i < ilits->touch_num; i++) {
		...
		if (ilits->rib.nReportResolutionMode ==
		    POSITION_HIGH_RESOLUTION) {
			if (ilits->pen_info.pen_type == REPORT_HAND_PEN) {
				if (buf[len - (offset + 2)] ==
				    ONLY_REPORT_HAND) {
					if ((buf[(4 * i)] == 0xFF) &&
					    (buf[(4 * i) + 1] == 0xFF) &&
			...
			} else if (ilits->pen_info.pen_type ==
				   P5_X_ONLY_PEN_TYPE) {
				ILI_DBG("Report Switch = 0x%X\n",
					buf[len - (offset + 2)]);
				if ((buf[len - (offset + 2)] | 0x01) ==
				    ONLY_REPORT_PEN) {
					ILI_DBG("Pen Switch = 0x%X\n", buf[i]);
					if ((buf[i] & 0x11) == RELEASE_PEN) {
						if (MT_B_TYPE) {
							ilits->pen_info.active =
								0;
							ilits->curt_touch[i] =
					...
					ILI_DBG("Pen No Touch\n");
					continue;
				}
			} else {
				finger_data_offset =
					(P5_X_DEBUG_HIGH_RESOLUTION_FINGER_DATA_LENGTH +
				...
				if (i == MAX_TOUCH_NUM) {
					...
					if (buf[(len - (offset + 2))] ==
					    ...
					    buf[(len - (offset + 2))] ==
						    REPORT_HAND_PEN) {
						ILI_DBG("Pen Switch = 0x%X\n",
							buf[finger_data_offset]);
						if ((buf[finger_data_offset] &
						     0x11) == RELEASE_PEN) {
							if (MT_B_TYPE) {
								ilits->pen_info
									.active =
						...
						xop = ((buf[finger_data_offset +
							    2]
							<< 8) |
						       (buf[finger_data_offset +
							    3]));
						yop = ((buf[finger_data_offset +
							    4]
							<< 8) |
						       (buf[finger_data_offset +
							    5]));
					} else {
						// ILI_DBG("Report Switch = 0x%X, Only Report Hand\n", buf[len - (offset + 2)]);
						continue;
					}
				} else {
					if (buf[len - (offset + 2)] ==
						...
						// ILI_DBG("Report Switch = 0x%X, Only Report Pen\n", buf[len - (offset + 2)]);
						continue;
					}
				}
			}
		...
		touch_info[ilits->finger].id = i;
		//Pen ID = 9
		if (touch_info[ilits->finger].id == MAX_TOUCH_NUM) {
			touch_info[ilits->finger].id = MAX_TOUCH_NUM - 1;
		}
		...
		if (MT_PRESSURE) {
			 ...
			 */
			if (ilits->pen_info.pen_type == P5_X_ONLY_PEN_TYPE) {
				touch_info[ilits->finger].pressure =
					(buf[TOUCH_PRESS_OFFSET] << 8) +
					buf[TOUCH_PRESS_OFFSET + 1];
			} else if (ilits->pen_info.pen_type ==
				   P5_X_HAND_PEN_TYPE) {
				if (i >= MAX_TOUCH_NUM) {
					touch_info[ilits->finger].pressure =
	...
	if (ilits->finger) {
		if (MT_B_TYPE) {
			for (i = 0; i < ilits->finger; i++) {
				ili_touch_press_debug_mode(
					touch_info[i].x, touch_info[i].y,
					touch_info[i].pressure,
					touch_info[i].id);
				if ((touch_info[i].id == (MAX_TOUCH_NUM - 1) ||
				     ilits->pen_info.pen_type ==
					     P5_X_ONLY_PEN_TYPE) &&
				    ilits->pen_info.active == 1) {
					input_report_key(ilits->input,
							 BTN_TOUCH, 1);
			...
			for (i = 0; i < ilits->touch_num; i++) {
				if (ilits->curt_touch[i] == 0 &&
					...
					if ((touch_info[i].id ==
						...
						ili_touch_release_debug_mode(
							0, 0, touch_info[i].id);
						input_report_key(ilits->input,
								 BTN_TOOL_PEN,
								 0);
						ILI_DBG("Release Pen, id = %d\n",
							touch_info[i].id);
					} else {
						ili_touch_release_debug_mode(
							0, 0, i);
		...
		if (ilits->last_touch) {
			if (MT_B_TYPE) {
				...
				ili_touch_release_debug_mode(0, 0, 0);
			}
			input_sync(ilits->input);
			ilits->last_touch = 0;
		}
...
{
	int offset = P5_X_ID_TYPE_LEN - 1;

	ili_pen_demo_mode_report_point(buf + offset, len, offset);
	ilitek_tddi_touch_send_debug_data(buf, len);
}
...
void ili_report_pen_debug_mode(u8 *buf, int len)
{
	int offset = 0;
	if (ilits->pen_info.pen_type == P5_X_ONLY_PEN_TYPE) {
		offset = P5_X_ID_TYPE_LEN + ((ilits->stx + ilits->srx) * 2);
		ilits->touch_num = MAX_PEN_NUM;
	} else if (ilits->pen_info.pen_type == P5_X_ONLY_HAND_TYPE) {
		offset = P5_X_ID_TYPE_LEN;
		ilits->touch_num = MAX_TOUCH_NUM;
	} else {
		offset = P5_X_ID_TYPE_LEN;
		ilits->touch_num = MAX_TOUCH_NUM + MAX_PEN_NUM;
	}
	ILI_DBG("Pen Type = 0x%X, Max Touch Num = %d, Pen Data Mode = %d\n",
		ilits->pen_info.pen_type, ilits->touch_num,
...
void ili_report_gesture_mode(u8 *buf, int len)
{
	...
	ILI_DBG("gesture code = 0x%x, score = %d\n", gc->code, score);

	switch (gc->code) {
	case GESTURE_DOUBLECLICK:
		ILI_DBG("Double Click key event\n");
		input_report_key(input, KEY_GESTURE_POWER, 1);
		input_sync(input);
		input_report_key(input, KEY_GESTURE_POWER, 0);
		input_sync(input);
	...
	ILI_DBG("Transfer = %d, Type = %d, clockwise = %d\n", transfer,
		gc->type, gc->clockwise);
	ILI_DBG("Gesture Points: (%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)\n",
...
int ili_read_knuckle_roi_data(void)
{
	...
	ILI_DBG("roi_switch = %d, roi_support =%d\n",
		...
		ts_dev_data->ts_platform_data->feature_info.roi_info
			.roi_supported);

	ILI_DBG("last_fingers = %d, cur_fingers =%d\n", ilits->last_touch,
		ilits->finger);
