...
static int ilitek_tddi_fw_iram_read(u8 *buf, u32 start, int len)
{
	...
	for (cnt = 0, addr = start; cnt < loop; cnt++, addr += limit) {
		...
		if (ilits->wrapper(NULL, 0, buf + cnt * limit, tmp_len, OFF,
				   OFF) < 0) {
...
{
	if (ili_ice_mode_bit_mask_write(FLASH0_ADDR, FLASH0_reg_preclk_sel,
					(2 << 16)) < 0)
		ILI_ERR("Write %lu at %x failed\n", FLASH0_reg_preclk_sel,
			FLASH0_ADDR);

	if (ili_ice_mode_bit_mask_write(FLASH4_ADDR,
					FLASH4_reg_flash_dma_trigger_en,
					(0 << 24)) < 0)
		ILI_ERR("Write %lu at %x failed\n",
			FLASH4_reg_flash_dma_trigger_en, FLASH4_ADDR);

	if (ili_ice_mode_bit_mask_write(FLASH0_ADDR, FLASH0_reg_rx_dual,
					(0 << 24)) < 0)
		ILI_ERR("Write %lu at %x failed\n", FLASH0_reg_rx_dual,
			FLASH0_ADDR);
...
static int ilitek_tddi_flash_read_int_flag(void)
{
	...
	do {
		if (ili_ice_mode_read(INTR1_ADDR & BIT(25), &data,
				      sizeof(u32)) < 0)
...
void ili_flash_dma_write(u32 start, u32 end, u32 len)
{
	...
	if (ili_ice_mode_write(FLASH2_reg_tx_data, (start & 0xFF0000) >> 16,
			       1) < 0)
	...
	if (ili_ice_mode_write(FLASH2_reg_tx_data, (start & 0x00FF00) >> 8, 1) <
	    0)
		ILI_ERR("Write %x at %x failed\n", (start & 0x00FF00) >> 8,
			FLASH2_reg_tx_data);
...
void ilitek_tddi_flash_protect(bool enable, bool mcu)
{
	...
	case 0xEF: //W25 Flash
		if (ilits->flash_devid == 0x6012 ||
			...
			if (enable) {
				if (ili_ice_mode_write(FLASH2_ADDR, 0x7E, 1) <
				    0)
...
static int ilitek_tddi_flash_fw_crc_check(u8 *pfw, bool check_fw_ver)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		if (ili_fw_read_hw_crc(start_addr,
				       end_addr - start_addr - crc_byte_len + 1,
				       &hw_crc) < 0) {
...
static int ilitek_tddi_fw_flash_program(u8 *pfw)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		for (addr = fbi[i].start; addr < fbi[i].end; addr += page) {
			...
			if (skip) {
				if (ili_ice_mode_write(FLASH_BASED_ADDR, 0x1,
						       1) < 0)
					ILI_ERR("Write cs high failed\n");
				return UPDATE_FAIL;
			}
			...
			if (ilits->wrapper(buf, page + 4, NULL, 0, OFF, OFF) <
			    0) {
			...
			if (ilits->flash_mid == 0xEF) {
				...
				if (ilitek_tddi_flash_poll_busy(
					    TIMEOUT_PROGRAM) < 0)
					return UPDATE_FAIL;
			}
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		for (addr = fbi[i].start; addr <= fbi[i].end;
			...
			if (addr == fbi[AP].start) {
				...
				if (ili_ice_mode_write(FLASH2_ADDR, 0xD8, 1) <
				    0)
				...
				if (ili_ice_mode_write(FLASH2_ADDR, 0x20, 1) <
				    0)
...
static int ilitek_fw_flash_upgrade(u8 *pfw, bool mcu)
{
	...
	if (ret == UPDATE_PASS) {
		...
		if (ili_ice_mode_ctrl_by_mode(DISABLE, OFF, BOTH) < 0) {
			if (ilits->reset == TP_HW_RST_ONLY) {
				ili_wdt_reset_status(false, true);
				if (ili_reset_ctrl(ilits->reset) < 0) {
					ILI_ERR("TP reset failed during flash progam\n");
					return -EFW_REST;
				}
		...
		ILI_INFO(
			"Flash FW and target file FW are different, do upgrade\n");
	}
...
static void ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	ilits->rib.nCustomerType = (ilits->chip->core_ver >= CORE_VER_1470) ?
					   ilits->rib.nCustomerType :
						 POSITION_CUSTOMER_TYPE_OFF;
#if ENABLE_PEN_MODE
	ilits->rib.nPenType = (ilits->chip->core_ver >= CORE_VER_1470) ?
				      (ilits->rib.nPenType >> 6) :
					    POSITION_PEN_TYPE_OFF;

	ILI_INFO(
		"report_info_block : nReportByPixel = %d, nIsHostDownload = %d, nIsSPIICE = %d, nIsSPISLAVE = %d, nIsI2C = %d\n",
		ilits->rib.nReportByPixel, ilits->rib.nIsHostDownload,
		ilits->rib.nIsSPIICE, ilits->rib.nIsSPISLAVE,
		ilits->rib.nIsI2C);

	ILI_INFO(
		"report_info_block : nReserved00 = %d, nReportResolutionMode = %d, nCustomerType = %d, nPenType = %d, nReserved02 = %x,  nReserved03 = %x\n",
		ilits->rib.nReserved00, ilits->rib.nReportResolutionMode,
		ilits->rib.nCustomerType, ilits->rib.nPenType,
...
#else
	ILI_INFO(
		"report_info_block : nReportByPixel = %d, nIsHostDownload = %d, nIsSPIICE = %d, nIsSPISLAVE = %d\n",
		ilits->rib.nReportByPixel, ilits->rib.nIsHostDownload,
		ilits->rib.nIsSPIICE, ilits->rib.nIsSPISLAVE);

	ILI_INFO(
		"report_info_block : nIsI2C = %d, nReserved00 = %d, nCustomerType = %d, nReportResolutionMode = %d, nReserved02 = %x,  nReserved03 = %x\n",
		ilits->rib.nIsI2C, ilits->rib.nReserved00,
		ilits->rib.nCustomerType, ilits->rib.nReportResolutionMode,
		ilits->rib.nReserved02, ilits->rib.nReserved03);
