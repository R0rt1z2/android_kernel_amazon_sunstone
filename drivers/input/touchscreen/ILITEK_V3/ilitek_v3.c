	...
	if (!strcmp(psy->desc->name, "usb")) {
		if (psy && val == POWER_SUPPLY_PROP_STATUS) {
			ret = power_supply_get_property(
				psy, POWER_SUPPLY_PROP_PRESENT, &prop);
			if (ret < 0) {
				...
				if (ilits->usb_plug_status != prop.intval) {
					...
					if (!ilits->tp_suspend &&
						...
						queue_work(
							ilits->charger_notify_wq,
							&ilits->update_charger);
				}
			}
		}
	}
...
void ilitek_plat_charger_init(void)
{
	...
	ilits->charger_notify_wq =
		create_singlethread_workqueue("ili_charger_wq");
	...
	ilits->notifier_charger.notifier_call =
		ilitek_charger_notifier_callback;
	...
	if (ilits->fw_upgrade_mode == UPGRADE_IRAM)
		...
		if (ilits->reset == TP_HW_RST_ONLY) {
			...
			if (ili_reset_ctrl(ilits->reset) < 0)
				ILI_ERR("TP Reset failed during ili_resume_by_ddi_work\n");
		} else {
...
void ili_resume_by_ddi(void)
{
	...
	if (!ilits->eng_flow) {
		...
		if (ilits->reset == TP_HW_RST_ONLY) {
			ili_wdt_reset_status(true, true);
			if (ili_reset_ctrl(ilits->reset) < 0)
				ILI_ERR("TP Reset failed during ili_resume_by_ddi\n");
		} else {
			if (ili_cascade_reset_ctrl(ilits->reset, true) < 0)
...
static int ili_set_knuckle_roi_switch(bool roi_enable)
{
	...
	ret = ili_config_knuckle_roi_ctrl(roi_enable ? CMD_ENABLE :
							     CMD_DISABLE);
	if (ret) {
...
int ili_switch_tp_mode(u8 mode)
{
	...
	case P5_X_FW_AP_MODE:
		...
		if (ilits->fw_upgrade_mode == UPGRADE_IRAM) {
			...
			if (ilits->reset == TP_HW_RST_ONLY) {
				ili_wdt_reset_status(true, true);
				if (ili_reset_ctrl(ilits->reset) < 0)
					ILI_ERR("TP Reset failed during ili_switch_tp_mode\n");
			} else {
				if (ili_cascade_reset_ctrl(ilits->reset, true) < 0)
	...
	case P5_X_FW_GESTURE_MODE:
		...
		if (ges_dbg) {
			...
			ili_set_tp_data_len(DATA_FORMAT_GESTURE_DEBUG, false,
					    NULL);
		}
		break;
...
int ili_sleep_handler(int mode)
{
	...
	if (atomic_read(&ilits->fw_stat) || atomic_read(&ilits->mp_stat)) {
		ILI_INFO(
			"fw upgrade or mp still running, ignore sleep requst\n");
	...
	case TP_RESUME:
		...
		if ((ilits->proxmity_face == false) &&
			...
			if (ili_set_tp_data_len(DATA_FORMAT_DEMO, false, NULL) <
			    0)
...
int ili_fw_upgrade_handler(void *data)
{
	...
	ilitek_tddi_flash_protect(DISABLE,
				  OFF); // flash un-protect for i2c project
	...
	if (ret != 0) {
		...
		if ((ilits->usb_plug_status) &&
			...
			ret = ili_ic_func_ctrl(
				"plug", !ilits->usb_plug_status); // plug in
			if (ret < 0) {
				ILI_ERR("Write plug in failed\n");
			}
		}
...
int ili_set_tp_data_len(int format, bool send, u8 *data)
{
	...
	} else if (ilits->rib.nCustomerType != POSITION_CUSTOMER_TYPE_OFF &&
	...
	case DATA_FORMAT_DEMO:
		...
		if (ilits->rib.nPenType == POSITION_PEN_TYPE_ON) {
			...
			ilits->pen_info.pen_type = P5_X_HAND_PEN_TYPE;
			ilits->pen_info.pen_data_mode = HAND_PEN_DEMO_MODE;
			len = P5_X_HAND_PEN_TYPE_PACKET_LEN;
		} else {
	...
	case DATA_FORMAT_DEBUG:
		...
		if (ilits->rib.nPenType == POSITION_PEN_TYPE_ON) {
			...
			if (data_type == P5_X_FW_RAW_DATA_MODE) {
				...
				len += (ilits->xch_num * ilits->ych_num * 2) +
				       ((ilits->stx + ilits->srx) * 2) +
				       (2 * self_key) + P5_X_PEN_DATA_LEN +
			...
			len += (2 * ilits->xch_num * ilits->ych_num) +
			       (ilits->stx * 2) + (ilits->srx * 2) +
			       (2 * self_key) + 16;
		}
#else
		len += (2 * ilits->xch_num * ilits->ych_num) +
		       (ilits->stx * 2) + (ilits->srx * 2) + (2 * self_key) +
		       16;
#endif
...
#if ENABLE_PEN_MODE
		if (ilits->rib.nCustomerType != POSITION_CUSTOMER_TYPE_OFF &&
		    ilits->rib.nPenType == POSITION_PEN_TYPE_OFF) {
	...
	if (ctrl == DATA_FORMAT_DEBUG_LITE_CMD) {
		...
		if (ilits->chip->core_ver >= CORE_VER_1700) {
			...
			ILI_INFO(
				"Switch Debug Hand Pen CMD: 0x%X, 0x%X, 0x%X, 0x%X, 0x%X, 0x%X, 0x%X, 0x%X\n",
		...
		if (ilits->chip->core_ver >= CORE_VER_1700) {
			...
			ILI_INFO(
				"Switch Debug Hand Pen CMD: 0x%X, 0x%X, 0x%X\n",
				cmd[0], cmd[1], cmd[2]);
		} else {
			cmd[0] = P5_X_MODE_CONTROL;
			cmd[1] = ctrl;
...
#if ENABLE_PEN_MODE
int ili_set_pen_data_info(u8 header, u8 ctrl, u8 type)
{
	int len = 0;
	u8 ret = 0, self_key = 2;
	u8 cmd[3] = { 0 };
	...
	switch (ilits->pen_info.pen_type) {
	case P5_X_ONLY_PEN_TYPE:
		len = P5_X_ID_TYPE_LEN + ((ilits->stx + ilits->srx) * 2) +
		      P5_X_PEN_DATA_LEN;

		if (type == P5_X_FW_RAW_DATA_MODE) {
			len += ((ilits->pen_info_block.nPxRaw * ilits->xch_num *
				 2) +
				(ilits->pen_info_block.nPyRaw * ilits->ych_num *
				 2)) * 2 +
			       4;
			ilits->pen_info.pen_data_mode = PEN_ONLY_RAW_DATA_MODE;
		} else {
			len += ((ilits->pen_info_block.nPxVa * ilits->xch_num *
				 2) +
				(ilits->pen_info_block.nPyVa * ilits->ych_num *
				 2)) * 2 +
			       4;
	...
	case P5_X_ONLY_HAND_TYPE:
		...
		len = P5_X_DEBUG_HIGH_RESOLUTION_FINGER_DATA_LENGTH +
		      ...
		      ((ilits->stx + ilits->srx) * 2) + (2 * self_key); // 3281
		break;
	...
	ILI_INFO("Packet Length = %d, Pen Type = 0x%X, Pen Data Mode = %d\n",
		 ilits->tp_data_len, ilits->pen_info.pen_type,
		 ilits->pen_info.pen_data_mode);
...
int ili_report_handler(void)
{
	...
	if (!ilits->boot || !ilits->report || atomic_read(&ilits->tp_reset) ||
		...
		ILI_INFO(
			"boot: %d, report: %d, rst: %d, fw: %d, switch: %d, mp: %d, sleep: %d, esd: %d, igr:%d\n",
			ilits->boot, ilits->report,
	...
	if (ilits->actual_tp_mode == P5_X_FW_GESTURE_MODE) {
		...
		if (ilits->pm_suspend) {
			...
			ret = wait_for_completion_timeout(
				&ilits->pm_completion, msecs_to_jiffies(700));
			if (!ret) {
...
#endif
			if (ilits->actual_tp_mode == P5_X_FW_GESTURE_MODE &&
			    atomic_read(&ilits->gesture) && !ilits->prox_near) {
...
#if ENABLE_PEN_MODE
		if (trdata[3] == P5_X_HAND_PEN_TYPE) {
...
#if ENABLE_PEN_MODE
		if (trdata[3] == P5_X_ONLY_HAND_TYPE ||
		    trdata[3] == P5_X_ONLY_PEN_TYPE ||
		    trdata[3] == P5_X_HAND_PEN_TYPE) {
...
int ili_cascade_reset_ctrl(int reset_mode, bool enter_ice)
{
	...
	ili_wdt_reset_status(enter_ice, false);
...
extern unsigned char vendor_id;
...
int ili_tddi_init(void)
{
	...
	if (ili_ice_mode_ctrl(DISABLE, OFF) < 0)
		ILI_ERR("Failed to disable Slave ice mode failed during init\n");
