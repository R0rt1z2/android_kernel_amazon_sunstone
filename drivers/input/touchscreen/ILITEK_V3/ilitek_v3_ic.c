...
int ili_config_get_knuckle_roi_status(void)
{
	...
	msleep(1);
	ret = ilits->wrapper(NULL, 0, &roi_status, sizeof(roi_status), OFF,
			     OFF);
	if (ret) {
...
int ili_ice_mode_ctrl(bool enable, bool mcu)
{
	...
	if (enable) {
		...
		if (ilits->wrapper(cmd_open, sizeof(cmd_open), NULL, 0, OFF,
				   OFF) < 0) {
	...
	if (ilits->ice_flash_cs_ctrl) {
		...
		ret = ilits->wrapper(cmd_close, sizeof(cmd_close), NULL, 0, OFF,
				     OFF);
		if (ret < 0) {
...
void ili_mspi_set_slave_to_qual_mode(void)
{
	...
	if (ili_ice_mode_write(MSPI_INIT_EN_ADDR, backup_mspi_init_en & 0xFE,
			       1) < 0)
...
void ili_wdt_reset_status(bool enter_ice, bool exit_ice)
{
	...
	if (ili_ice_mode_write_by_mode(WDT_ADDR, 0x01, 1, BOTH) > 0) {
		ILI_INFO("Clear Master & SLA WDT Status\n");
	}
	...
	for (i = 0; i < FUNC_CTRL_NUM; i++) {
		if (func_ctrl[i].rec_state == ENABLE) {
			ILI_DBG("reset func ctrl %s, record status = %d, cmd = %d\n",
				...
				func_ctrl[i].rec_cmd);
			if (ili_ic_func_ctrl(func_ctrl[i].name,
...
int ili_ic_whole_reset(bool mcu, bool withflash)
{
	...
	if (!withflash) {
		ret = ili_ice_mode_write(ilits->chip->reset_addr, 0xA0009878, sizeof(u32));

		mdelay(3);
	}

	ret = ili_ice_mode_write(ilits->chip->reset_addr,
...
void ili_ic_get_pc_counter(int stat)
{
	...
	if (ilits->enable_cascade) {
		...
		ili_cascade_rw_tp_reg(stop_mcu, read, pc_addr, 0, 0, &pc);
		ili_cascade_rw_tp_reg(stop_mcu, read, latch_addr, 0, 0, &latch);
...
int ili_ic_check_busy(int count, int delay, bool irq)
{
	...
	do {
		...
		if (ilits->wrapper(&cmd[1], sizeof(u8), &busy, sizeof(u8), irq,
				   OFF) < 0)
...
int ili_ic_get_project_id(u8 *pdata, int size)
{
	...
	if (ili_ice_mode_write(0x041008, (RSV_BK_ST_ADDR & 0xFF0000) >> 16, 1) <
	    0)
		ILI_ERR("Write address failed\n");
	if (ili_ice_mode_write(0x041008, (RSV_BK_ST_ADDR & 0x00FF00) >> 8, 1) <
	    0)
		ILI_ERR("Write address failed\n");
...
int ili_ic_get_panel_info(void)
{
		...
		ilits->trans_xy = (ilits->chip->core_ver >= CORE_VER_1430 &&
				   (ilits->rib.nReportByPixel > 0)) ?
					  ON :
						OFF;
#if ENABLE_PEN_MODE
		if (ilits->rib.nPenType == POSITION_PEN_TYPE_ON) {
			ilits->touch_num = MAX_TOUCH_NUM + MAX_PEN_NUM;
			ilits->tp_data_len = P5_X_HAND_PEN_TYPE_PACKET_LEN;
			ILI_INFO(
...
#if ENABLE_PEN_MODE
			ilits->rib.nPenType = buf[1] >> 6;

			if (ilits->rib.nPenType == POSITION_PEN_TYPE_ON) {
				ilits->touch_num = MAX_TOUCH_NUM + MAX_PEN_NUM;
				ilits->tp_data_len =
					P5_X_HAND_PEN_TYPE_PACKET_LEN;
				ILI_INFO(
					"Panel support Pen Mode, Packet Len = %d, Touch Number = %d\n",
					ilits->tp_data_len, ilits->touch_num);
			}
	...
	if (buf[0] != cmd) {
		ilits->cascade_info_block.nDisable = 0;
...
#if ENABLE_CASCADE
void ili_check_enable_cascade(void)
{
	u32 enable_cascade;

	ili_ice_mode_ctrl(ENABLE, ON);

	ili_ice_mode_read(ENABLE_CASCADE_ADDR, &enable_cascade, sizeof(u32));
	ILI_INFO("Enable Casecade = 0x%x, Check Slave exist.\n",
		 enable_cascade);
	ilits->enable_cascade = enable_cascade >> 2;
	ILI_INFO("Enable Casecade = 0x%x, Check Slave exist.\n",
		 ilits->enable_cascade);

	ili_ice_mode_ctrl(DISABLE, ON);
}

int ili_slave_ic_get_info(void)
{
	int ret = 0;
#if (TDDI_INTERFACE == BUS_I2C)
	...
	if (ili_ice_mode_read(ilits->chip->pid_addr, &ilits->chip->slave_pid,
			      sizeof(u32)) < 0)
...
#else
	ili_mspi_read_slave_register(TDDI_PID_ADDR, &ilits->chip->slave_pid,
				     sizeof(u32));
