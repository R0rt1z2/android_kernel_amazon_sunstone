/*
 * Copyright (C) 2010 - 2022Novatek, Inc.
 *
 * $Revision: 77558 $
 *
...
int32_t Write_Flash(void)
{
	...
	for (i = 0; i < count; i++) {
		...
		for (j = 0; j < min(fw_need_write_size - i * 256, (size_t)256);
		     j += 32) {
		...
		for (k = 0;
		     k < min(fw_need_write_size - Flash_Address, (size_t)256);
		     k++)
		...
		buf[4] = (Flash_Address & 0xFF);
		buf[5] = 0x00;
		buf[6] = min(fw_need_write_size - Flash_Address, (size_t)256) -
			 1;
		buf[7] = tmpvalue;
...
void Boot_Update_Firmware(struct work_struct *work)
{
	...
	case LCM_NT36523B_TG_INX:
		ret = snprintf(firmware_name, sizeof(firmware_name)-1,
			"%s", BOOT_UPDATE_FIRMWARE_NAME_INX);
		break;
	case LCM_NT36523B_KD_BOE:
		ret = snprintf(firmware_name, sizeof(firmware_name)-1,
			"%s", BOOT_UPDATE_FIRMWARE_NAME_BOE);
		break;
	case LCM_NT36523B_KD_HSD:
		ret = snprintf(firmware_name, sizeof(firmware_name)-1,
			"%s", BOOT_UPDATE_FIRMWARE_NAME_HSD);
		break;
	default:
		NVT_ERR("Invalid panel, exit.\n");
		return;
	}
	...
	NVT_LOG("vendor_id = %d loading. firmware name = %s\n", vendor_id, firmware_name);
