		...
		struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0)
		NVT_LOG("snprintf copy string failed %d", ret);
		...
		const char *buf, size_t count)
{
	...
	if (kstrtou8(buf, 10, &mode) || mode > 1 || !ts->bTouchIsAwake) {
		NVT_ERR("mode:%d Awake:%d\n", mode, ts->bTouchIsAwake);
		return -EINVAL;
	}

	switch (mode) {
	case 0:
		NVT_LOG("Disable WKG Mode\n");
		atomic_set(&ts->wkg, 0);
		panel_gesture_mode_set_by_nt36523b(false);
		break;
	case 1:
		NVT_LOG("Enable WKG Mode\n");
		atomic_set(&ts->wkg, 1);
		panel_gesture_mode_set_by_nt36523b(true);
		break;
	default:
		NVT_LOG("Not valid input\n");
		break;
	}
...
int32_t nvt_extra_proc_init(void)
{
	...
	NVT_proc_fw_version_entry =
		proc_create(NVT_FW_VERSION, 0440, NULL, &nvt_fw_version_fops);
	...
	NVT_proc_baseline_entry =
		proc_create(NVT_BASELINE, 0440, NULL, &nvt_baseline_fops);
	...
	NVT_proc_raw_entry = proc_create(NVT_RAW, 0440, NULL, &nvt_raw_fops);
	...
	NVT_proc_diff_entry = proc_create(NVT_DIFF, 0440, NULL, &nvt_diff_fops);
	...
	if (ts->pen_support) {
		NVT_proc_pen_diff_entry = proc_create(NVT_PEN_DIFF, 0440, NULL,
						      &nvt_pen_diff_fops);
	...
	if (touchscreen_link != NULL) {
		...
		if (ret < 0) {
			NVT_ERR("Failed to create sysfs link for input_dev %d\n", ret);
		} else {
			if (sysfs_create_group(&client->dev.kobj, &nvt_api_attribute_group))
				NVT_ERR("Failed to create gesture_wakeup gesture sysfs");
			else
				NVT_LOG("create gesture_wakeup Succeeded!\n");
		}
	}
...
void nvt_extra_proc_deinit(void)
{
	...
	sysfs_remove_group(&ts->client->dev.kobj, &nvt_api_attribute_group);
}
#endif
