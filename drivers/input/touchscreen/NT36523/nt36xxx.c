...
struct drm_bridge *nvt_panel_bridge;
//static int32_t nvt_register_panel_bridge(struct drm_bridge *bridge);
//static void nvt_unregister_panel_bridge(struct drm_bridge *bridge);
...
{
	NVT_proc_entry = proc_create(DEVICE_NAME, 0440, NULL, &nvt_flash_fops);
...
static void nvt_parse_dt(struct device *dev)
{
	...
	ts->stylus_resol_double =
		of_property_read_bool(np, "novatek,stylus-resol-double");
	NVT_LOG("novatek,stylus-resol-double=%d\n", ts->stylus_resol_double);

	ret = of_property_read_u32(np, "novatek,trigger-type", &ts->int_trigger_type);
	if (ret) {
		NVT_ERR("invalid trigger-type\n");
	}
}
#else
static void nvt_parse_dt(struct device *dev)
...
static int nt36523_ts_disp_notifier_callback(struct notifier_block *self,
					    unsigned long event, void *value)
{
	struct nvt_ts_data *ts =
		container_of(self, struct nvt_ts_data, disp_notifier);
	int *data = (int *)value;

	if (ts && value) {
		if (event == MTK_DISP_EVENT_BLANK) {
			if (*data == MTK_DISP_BLANK_UNBLANK) {
				NVT_LOG("nvt_ts_resume IN");
				if (nvt_ts_resume(&ts->input_dev->dev) < 0) {
					NVT_ERR("TP resume failed");
					return -1;
				}
			}
		} else if (event == MTK_DISP_EARLY_EVENT_BLANK) {
			if (*data == MTK_DISP_BLANK_POWERDOWN) {
				NVT_LOG("nvt_ts_suspend IN");
				if (nvt_ts_suspend(&ts->input_dev->dev) < 0) {
					NVT_ERR("TP suspend failed");
					return -1;
				}
			}
		} else {
			NVT_ERR("Invalid notification callback event");
			return -1;
		}
	} else {
		NVT_ERR("nt36523 touch IC can not suspend or resume");
		return -1;
	}
	return 0;
}
...
static void nvt_panel_bridge_enable(struct drm_bridge *bridge)
{
	NVT_LOG("%s\n", __func__);
	nvt_ts_resume(&ts->input_dev->dev);
}

static void nvt_panel_bridge_disable(struct drm_bridge *bridge)
{
	NVT_LOG("%s\n", __func__);
	...
	if (bridge->encoder && bridge->encoder->crtc) {
		...
		if (drm_atomic_crtc_effectively_active(crtc_state)) {
			NVT_LOG("crtc is still active\n");
			return;
		}
	}
	nvt_ts_suspend(&ts->input_dev->dev);
}

static const struct drm_bridge_funcs nvt_panel_bridge_funcs = {
	.enable = nvt_panel_bridge_enable,
	.disable = nvt_panel_bridge_disable,
};
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (vendor_id != LCM_NT36523B_TG_INX &&
	    vendor_id != LCM_NT36523B_KD_HSD &&
	    vendor_id != LCM_NT36523B_KD_BOE) {
		NVT_ERR("Invalid display vendor id\n");
		return -ENODEV;
	}
	...
	NVT_LOG("novatek,trigger-type=%d\n", ts->int_trigger_type);

	//---request and config GPIOs---
#if NVT_TOUCH_SUPPORT_HW_RST
	NVT_LOG("request and config GPIOs\n");
	gpio_set_value(ts->reset_gpio, 1);
	msleep(10);
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
	ts->disp_notifier.notifier_call = nt36523_ts_disp_notifier_callback;
	ret = mtk_disp_notifier_register("novatek ic Touch",
					 &ts->disp_notifier);
	if (ret) {
...
//ovatek_nvt.ko,/drivers/input/touchscreen/NT36xxx/novatek_nvt.ko,vendor,Y,Y,user/userdebug/eng
/*******************************************************
Description:
...
#if defined(CONFIG_DRM_PANEL_NOTIFY)
