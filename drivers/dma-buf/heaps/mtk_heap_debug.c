...
#if defined(CONFIG_MTK_IOMMU_MT8XXX)
	{"mtk_svp", NULL, 0},
#else
...
static int dma_buf_init_procfs(void)
{
	...
	if (!dma_heap_proc_root) {
		pr_err("%s failed to create procfs root dir.\n", __func__);
		return -1;
	}
	...
	proc_file = proc_create_data("all_heaps",
				     ...
				     NULL);
	if (!proc_file) {
		pr_err("Failed to create debug file for all_heaps:%ld\n",
		       PTR_ERR(proc_file));
		return -2;
	}
	...
	if (!dma_heaps_dir) {
		pr_err("%s failed to create procfs heaps dir.\n",
	...
	dma_heaps_stats = proc_create_data("stats",
					   ...
					   NULL);
	if (!dma_heaps_stats) {
		pr_err("%s failed to create procfs stats dir:%ld\n",
	...
	if (!dma_heaps_stat_pid) {
		pr_err("%s failed to create procfs rss_pid dir:%ld\n",
