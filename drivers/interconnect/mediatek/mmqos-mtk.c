...
int mtk_mmqos_dbg_level;
EXPORT_SYMBOL(mtk_mmqos_dbg_level);
module_param(mtk_mmqos_dbg_level, int, 0644);
		...
		if (!IS_ERR_OR_NULL(opp)) {
			...
			if (comm_node->comm_reg == NULL) {
				...
				if (comm_node->high_volt <= 0) {
					pr_notice("get common high volt fail\n");
					return;
				}
			}
...
int mtk_mmqos_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < mmqos_desc->num_nodes; i++) {
		...
		case MTK_MMQOS_NODE_LARB:
			...
			id = node->id & (MTK_LARB_NR_MAX - 1);
			if (id >= 0 && id < MTK_LARB_NR_MAX)
				larb_dev = smi_imu->larb_imu[id].dev;
			if (larb_dev) {
