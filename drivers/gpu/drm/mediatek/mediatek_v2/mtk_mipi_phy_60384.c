...
struct ext_mipi_phy_data {
	u32 offset_add;
};

static const struct ext_mipi_phy_data ext_mipiphy_data_04 = {
	.offset_add = 0x04,
};

static const struct ext_mipi_phy_data ext_mipiphy_data_94 = {
	.offset_add = 0x94,
};

static const struct of_device_id ext_mipi_phy_match[] = {
	{ .compatible = "mediatek,ext-mipi-phy-04", .data = &ext_mipiphy_data_04},
	{ .compatible = "mediatek,ext-mipi-phy-94", .data = &ext_mipiphy_data_94},
	{},
};
...
struct ext_mipi_phy {
	...
	struct clk *pll;
	const struct ext_mipi_phy_data *data;
};
				    ...
				    u8 write_addr, u8 write_data)
{
	...
	writel(dev_addr << 0x1, ext_phy->regs + ext_phy->data->offset_add);
	...
	writel(0x1, ext_phy->regs + 0x14);
	writel(0x1, ext_phy->regs + 0x18);
	writel((dev_addr << 0x1) + 1, ext_phy->regs + ext_phy->data->offset_add);
	writel(0x1, ext_phy->regs + 0x24);
	while ((readl(ext_phy->regs + 0xC) & 0x1) != 0x1)
		;
	writel(0xFF, ext_phy->regs + 0xC);
...
static int ext_mipi_phy_pll_prepare(struct clk_hw *hw)
{
	...
	if (ext_phy->data == NULL)
		pr_info("ext_phy->data is null %s\n", __func__);
	else
		pr_info("ext_phy->data is ok %s\n", __func__);
...
static int ext_mipi_phy_probe(struct platform_device *pdev)
{
	...
	of_id = of_match_device(ext_mipi_phy_match, &pdev->dev);
	if (!of_id) {
		dev_err(dev, "ext_phy_60384 match failed\n");
		return -EPROBE_DEFER;
	}

	ext_phy->data = (struct ext_mipi_phy_data *)of_id->data;
