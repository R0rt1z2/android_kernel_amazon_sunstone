...
#define MT8188_MMSYS_SODI_REQ_MASK	0xE4
#define MT8188_SODI_HRT_FIFO_SEL_ALL	REG_FLD_MSB_LSB(1, 0)
...
#define MT8188_MMSYS_EMI_REQ_CTL	0xE8
...
#define SMI_LARB_NON_SEC_CON 0xf80
...
static const struct mtk_ddp_comp_match mtk_ddp_matches[DDP_COMPONENT_ID_MAX] = {
	...
	{DDP_COMPONENT_DSC1_VIRTUAL0, MTK_DISP_VIRTUAL, -1, NULL, 0},
	...
	{DDP_COMPONENT_MERGE4, MTK_DISP_MERGE, 4, NULL, 0},
	{DDP_COMPONENT_MERGE5, MTK_DISP_MERGE, 5, NULL, 0},
	...
	{DDP_COMPONENT_PSEUDO_OVL, MTK_DISP_PSEUDO_OVL, -1, NULL, 0},
	{DDP_COMPONENT_ETHDR, MTK_DISP_ETHDR, -1, NULL, 0},
	{DDP_COMPONENT_DMDP_WROT0, MTK_DISP_DMDP_WROT0, -1, NULL, 0},
	{DDP_COMPONENT_VDO1_VIRTUAL0, MTK_DISP_VIRTUAL, -1, NULL, 0},
	{DDP_COMPONENT_SINA_VIRTUAL0, MTK_DISP_VIRTUAL, -1, NULL, 0},
	{DDP_COMPONENT_SINB_VIRTUAL0, MTK_DISP_VIRTUAL, -1, NULL, 0},
};
					 ...
					 enum mtk_ddp_comp_type comp_type)
{
	...
	if (comp_type < 0 || mtk_ddp_comp_stem[comp_type] == 0)
		goto end;
	...
	DDPINFO("%s(): undefined component type: %d\n", __func__, comp_type);
	return -EINVAL;
}
		      ...
		      const struct mtk_ddp_comp_funcs *funcs)
{
	...
	if (comp_id < 0 || comp_id >= DDP_COMPONENT_ID_MAX) {
		DDPINFO("%s(): error: invalid component\n", __func__);
		ret = -EINVAL;
		goto end;
	}
	...
	if (IS_ERR(comp->clk)) {
		comp->clk = NULL;
		DDPINFO("%s(): warning: failed to get the clock\n", __func__);
	}
	...
	if (of_address_to_resource(node, 0, &res) != 0) {
		DDPINFO("%s(): error: missing reg in node: %s\n",
	...
	if (comp_id == DDP_COMPONENT_DPI0 || comp_id == DDP_COMPONENT_DPI1 ||
	    comp_id == DDP_COMPONENT_DP_INTF0 || comp_id == DDP_COMPONENT_DP_INTF1 ||
	...
	DDPINFO("%s(): %s: 0x%lx -> 0x%lx (pa)\n",
		__func__, node->full_name,
		(unsigned long)comp->regs,
		(unsigned long)comp->regs_pa);

	/* handle cmdq related resources */
	comp_pdev = of_find_device_by_node(node);
	if (!comp_pdev) {
		DDPINFO("%s(): warning: waiting for comp device %s\n",
			__func__, node->full_name);
		ret = -EPROBE_DEFER;
		goto end;
	}
...
int mtk_ddp_comp_register(struct drm_device *drm, struct mtk_ddp_comp *comp)
{
	...
	if (drm == NULL || comp == NULL) {
		DDPDBG("%s(): NULL pointer\n", __func__);
		goto end;
	}

	if (comp->id < 0) {
		DDPDBG("%s(): invalid index %d\n",
			__func__, comp->id);
		goto end;
	}
	...
	ret = 0;
	DDPDBG("%s(): component %d registered\n",
		__func__, comp->id);
end:
	return ret;
}
...
void mtk_ddp_comp_clk_prepare(struct mtk_ddp_comp *comp)
{
	...
	if (ret)
		DDPMSG("larb_dev enable failed:%s\n",
			mtk_dump_comp_str(comp));
			       ...
			       struct cmdq_pkt *handle)
{
	...
	while (1) {
		...
		cmdq_pkt_write(handle, NULL,
			res.start + SMI_LARB_NON_SEC_CON + port * 4, 0, 1);
#else
		cmdq_pkt_write(handle, NULL,
			res.start + SMI_LARB_NON_SEC_CON + port * 4, 1, 1);
#endif
		index++;
	}
}
	...
	if (id == DDP_COMPONENT_ID_MAX) { /* config when top clk on */
		if (!en) {
			...
			SET_VAL_MASK(sodi_req_val, sodi_req_mask,
						1, SODI_REQ_SEL_RDMA0_PD_MODE);
			SET_VAL_MASK(sodi_req_val, sodi_req_mask,
						0, SODI_REQ_VAL_RDMA0_PD_MODE);
			SET_VAL_MASK(sodi_req_val, sodi_req_mask,
						1, SODI_REQ_SEL_RDMA1_PD_MODE);
			SET_VAL_MASK(sodi_req_val, sodi_req_mask,
						0, SODI_REQ_VAL_RDMA1_PD_MODE);
			SET_VAL_MASK(emi_req_val, emi_req_mask,
						0, HRT_URGENT_CTL_SEL_ALL);
			SET_VAL_MASK(emi_req_val, emi_req_mask,
						0, HRT_URGENT_CTL_VAL_ALL);
			SET_VAL_MASK(emi_req_val, emi_req_mask,
						0x3F, DVFS_HALT_MASK_SEL_ALL);
		} else {
			SET_VAL_MASK(sodi_req_val, sodi_req_mask,
						0, SODI_REQ_SEL_ALL);
			SET_VAL_MASK(sodi_req_val, sodi_req_mask,
			    ...
			    struct cmdq_pkt *handle, void *data)
{
	...
	if (handle == NULL) {
		...
		v = (readl(priv->config_regs + MT8188_MMSYS_SODI_REQ_MASK)
			& (~sodi_req_mask));
		v += (sodi_req_val & sodi_req_mask);
		writel_relaxed(v, priv->config_regs + MT8188_MMSYS_SODI_REQ_MASK);

		v = (readl(priv->config_regs +  MT8188_MMSYS_EMI_REQ_CTL)
			& (~emi_req_mask));
		v += (emi_req_val & emi_req_mask);
		writel_relaxed(v, priv->config_regs +  MT8188_MMSYS_EMI_REQ_CTL);
	} else {
		cmdq_pkt_write(handle, NULL, priv->config_regs_pa +
			MT8188_MMSYS_SODI_REQ_MASK, sodi_req_val, sodi_req_mask);
		cmdq_pkt_write(handle, NULL, priv->config_regs_pa +
			MT8188_MMSYS_EMI_REQ_CTL, emi_req_val, emi_req_mask);
	}
}
