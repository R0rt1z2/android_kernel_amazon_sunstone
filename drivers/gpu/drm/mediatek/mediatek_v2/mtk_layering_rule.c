...
/*Layer only support yuv*/
static uint16_t layer_yuv_only_mapping_table[HRT_TB_NUM] = {
	0x0003, 0x0, 0x0, 0x0
};
/*Layer only support rgb*/
static uint16_t layer_rgb_only_mapping_table[HRT_TB_NUM] = {
	0x000C, 0x0, 0x0, 0x0
};
				  ...
				  int param)
{
	...
	case DISP_HW_LAYER_TB:
		...
		break;
	case DISP_HW_LAYER_YUV_ONLY_TB:
		map = layer_yuv_only_mapping_table[addon_data->hrt_type];
		break;
	case DISP_HW_LAYER_RGB_ONLY_TB:
		map = layer_rgb_only_mapping_table[addon_data->hrt_type];
		break;
	default:
		break;
	}
