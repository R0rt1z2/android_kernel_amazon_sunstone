...
{
	if (snprintf(buf, buf_len, "%s_%s", mtk_dump_comp_str(comp), qos_event) < 0)
		DDPINFO("%s(): error: snprintf\n", __func__);
}
			     ...
			     unsigned int bandwidth, unsigned int bw_mode)
{
	...
	// enlarge the number for possible EMI bandwidth lose
			...
			const char *caller)
{
	...
	dev_pm_opp_put(opp);
	crtc_voltage[index] = volt;
	volt = (crtc_voltage[0] < crtc_voltage[1]) ? crtc_voltage[1] : crtc_voltage[0];
	DDPINFO("%s crtc0 volt = %d, crtc1 volt = %d\n", caller, crtc_voltage[0], crtc_voltage[1]);
	DDPINFO("%s set volt %d\n", caller, volt);
...
//Temp solution for clock setting
void mtk_drm_temp_solution_clk_adjust(struct drm_crtc *crtc)
{
#ifdef IF_ZERO
	struct clk *clk;
	struct clk *parent;

	DDPINFO("%s mode width:%d, height:%d, vrefresh:%d\n", __func__,
			crtc->state->adjusted_mode.hdisplay,
			crtc->state->adjusted_mode.vdisplay,
			drm_mode_vrefresh(&(crtc->state->adjusted_mode)));

	if (crtc->state->adjusted_mode.hdisplay >= 3840) {
		clk = __clk_lookup("vpp_sel");
		parent = __clk_lookup("tvdpll1_ck");
		clk_set_parent(clk, parent);
		parent = __clk_lookup("univpll_d6");
		clk = __clk_lookup("ethdr_sel");
		clk_set_parent(clk, parent);
		DDPINFO("switch to 416MHz");
	} else {
		clk = __clk_lookup("vpp_sel");
		parent = __clk_lookup("mainpll_d5_d2");
		clk_set_parent(clk, parent);
		clk = __clk_lookup("ethdr_sel");
		parent = __clk_lookup("mmpll_d5_d4");
		clk_set_parent(clk, parent);
		DDPINFO("switch to 218MHz");
	}
#endif
}
...
#ifndef MTK_DISP_MMDVFS_SUPPORT
	if (drm_crtc_index(crtc) == CRTC_ID_EXTERNAL) {
		mtk_drm_temp_solution_clk_adjust(crtc);
		return;
	}
#endif
