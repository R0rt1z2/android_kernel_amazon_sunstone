...
#define SMI_LARB_NON_SEC_CON(port) (0xf80 + 4 * (port))
...
bool mtk_drm_set_cwb_roi(struct mtk_rect rect)
{
	...
	if (!cwb_info->buffer[0].fb) {
		...
		cwb_info->buffer[0].fb  =
			mtk_drm_framebuffer_create(
			crtc->dev, &mode, &mtk_gem->base);
		DDPMSG("[capture] b[0].addr_mva:%llu, addr_va:0x%llx\n",
			cwb_info->buffer[0].addr_mva,
			cwb_info->buffer[0].addr_va);
...
extern int mtk_dsi_bist_pattern_test(struct mtk_ddp_comp *comp, unsigned int color);
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "helper", 6) == 0) {
		...
		DDPPR_ERR("enable dsi pattern: 0x%08x\n", pattern);
		...
		DDPINFO("Enter set_lp_cmd! dbg_buf=%lu\n", strlen(dbg_buf));
		...
		ret = sscanf(opt, fmt,
			...
			&para[10], &para[11], &para[12], &para[13], &para[14], &para[15]);
...
#define SPRINTF_DEV_ATTR(fmt, arg...) \
		...
		if (count > 10) {
			DDPPR_ERR("Count should no more than 10!\n");
			return;
		}
		...
		mtk_drm_suspend(drm_dev->dev);
	} else if (strncmp(opt, "resume", 6) == 0) {
		mtk_drm_resume(drm_dev->dev);
	}
}
...
static char STR_HELP[] =
	"USAGE:\n"
	"       echo [ACTION]>/sys/kernel/debug/mtkfb\n"
	"ACTION:\n"
	"       regr:addr\n              :regr:0xf400c000\n"
	...
	"       mmp\n"
	"       dump_reg:moduleID\n dump_path:mutexID\n  dpfd_ut1:channel\n"
	"       get_lp_cmd:addr,num          :get_lp_cmd:0xda,02\n"
	"       set_dsi_cmd:type:addr,value          :set_dsi_cmd:hs:0x51,0xf\n"
	"       esd_check on/off          :esd_check 0\n";
