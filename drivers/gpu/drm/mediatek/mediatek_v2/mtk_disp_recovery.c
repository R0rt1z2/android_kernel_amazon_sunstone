...
int mtk_drm_esd_recover(struct drm_crtc *crtc)
{
...
#endif
	CRTC_MMP_EVENT_START(crtc_id, esd_recovery, 0, 0);
	if (crtc->state && !crtc->state->active) {
		DDPMSG("%s: crtc is inactive\n", __func__);
		return 0;
	}
	CRTC_MMP_MARK(crtc_id, esd_recovery, 0, 1);
	output_comp = mtk_ddp_comp_request_output(mtk_crtc);
	...
	CRTC_MMP_MARK(crtc_id, esd_recovery, 0, 2);
	...
	CRTC_MMP_MARK(crtc_id, esd_recovery, 0, 3);

	mtk_ddp_comp_io_cmd(output_comp, NULL, CONNECTOR_PANEL_ENABLE, NULL);

	CRTC_MMP_MARK(crtc_id, esd_recovery, 0, 4);
	...
	mtk_drm_idlemgr_kick(__func__, &mtk_crtc->base, 0);
	CRTC_MMP_MARK(crtc_id, esd_recovery, 0, 5);

done:
	CRTC_MMP_EVENT_END(crtc_id, esd_recovery, 0, ret);
	...
	while (1) {
		...
		ret = wait_event_interruptible_timeout(
			...
			(atomic_read(&esd_ctx->target_time) ||
				esd_ctx->chk_mode == READ_EINT), HZ/10);
		if (ret < 0) {
