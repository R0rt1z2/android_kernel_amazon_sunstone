			   ...
			   enum DISP_DEBUG_LEVEL debug_level)
{
	...
	for (i = 0; i < HRT_DISP_TYPE_NUM; i++) {
		...
		DDPDBG("%s(disp-%d): mod=%d, layers=%d, G(%d,%d), hrt=0x%x(%u)\n",
			__func__, i,
		...
		for (j = 0; j < disp_info->layer_num[i]; j++) {
			...
			DDPDBG("%s(%d:%d): %d, %dx%d(%d,%d)>%dx%d(%d,%d), %x, %x, %d, %d, %d, %d\n",
				__func__, i, j,
				layer_info->ovl_id,
...
static void dump_disp_trace(struct drm_mtk_layering_info *disp_info)
{
	...
	for (i = 0; i < HRT_DISP_TYPE_NUM; i++) {
		...
		n = snprintf(msg, 1000 - 1, "D%d,ovp:%d,dal:%d,LN:%d,G(%d,%d)",
	...
	for (i = start_idx; i <= end_idx; i++) {
		...
		if ((lc->src_height != lc->dst_height) ||
			...
			if (priv->data->mmsys_id == MMSYS_MT8188) {
				if (disp_idx == CRTC_ID_EXTERNAL &&
				    lc->src_height <= lc->dst_height &&
				    lc->src_width  <= lc->dst_width) {
					// vdo1 supports layer resizing but only scaling up
					continue;
				}
			}
			if (disp_info->gles_head[disp_idx] == -1 ||
...
int mtk_rollback_all_resize_layer_to_GPU(
	...
	int disp_idx)
{
	mtk_rollback_resize_layer_to_GPU_range(dev,
		disp_info, disp_idx, 0, disp_info->layer_num[disp_idx] - 1);

	return 0;
}
#if LAYER_RULE_BOTTOM_UP
				     ...
				     int disp, int ovl_limit)
{
	...
	available_ovl_num = ovl_limit;
	for (i = disp_info->layer_num[disp] - 1; i >= 0;
	     i--) {
		layer_info = &disp_info->input_config[disp][i];
		if (!is_extended_layer(layer_info)) {
			if (mtk_is_gles_layer(disp_info, disp, i)) {
				      ...
				      int disp, int ovl_limit)
{
	...
	available_ovl_num = ovl_limit;
	for (i = 0; i < info->gles_tail[disp]; i++) {
		l_info = &info->input_config[disp][i];
		if (is_extended_layer(l_info))
			continue;
		available_ovl_num--;
			   ...
			   int available)
{
...
#else
	available_ovl_num =
		_rollback_to_GPU_top_down(info, disp, available_ovl_num);
	if (has_gles_layer)
		available_ovl_num = _rollback_to_GPU_bottom_up(
			info, disp, available_ovl_num);
#endif


	if ((info->layer_num[disp] > available) &&
		mtk_is_layer_id_valid(info, disp, info->gles_head[disp]) == false) {
	...
	if ((info->layer_num[disp] > available) &&
		mtk_is_layer_id_valid(info, disp, info->gles_tail[disp]) == false) {
		dump_disp_info(info, DISP_DEBUG_LEVEL_CRITICAL);
	...
	if (mtk_is_layer_id_valid(info, disp, info->gles_tail[disp])) {
			      ...
			      int disp_idx)
{
	...
	uint16_t l_tb;
	uint16_t only_yuv_cnt;
	uint16_t rgb_num_limit;
	uint16_t rgb_gpu_cnt = 0;
	unsigned int rgb_layer_gpu[12] = {0};
...
retry:
	...
	/*filter by rgb only count*/
	only_yuv_cnt = mtk_get_phy_layer_limit(l_rule_ops->get_mapping_table(
		dev, disp_idx, DISP_HW_LAYER_YUV_ONLY_TB, 0));
	rgb_num_limit = ovl_num_limit - only_yuv_cnt;

	if (rgb_num_limit != ovl_num_limit) {
	...
	if (rgb_gpu_cnt > rgb_num_limit) {
		//decrease one ovl layer for gles layers
		if (!has_gles_layer)
			rgb_num_limit--;

		for (i = rgb_num_limit; i < rgb_gpu_cnt; i++) {
			mtk_rollback_layer_to_GPU(disp_info,
				disp_idx, rgb_layer_gpu[i]);
#ifdef HRT_DEBUG_LEVEL2
			DDPMSG("RGB only layer exceed, rollback %i\n", rgb_layer_gpu[i]);
#endif
		}
		phy_ovl_cnt = get_phy_ovl_layer_cnt(disp_info, disp_idx);
	}
...
static int mtk_lye_get_comp_id(int disp_idx, struct drm_device *drm_dev,
			       int layer_map_idx)
{
	if (disp_idx == SECONDARY_DISP_ID)
		return DDP_COMPONENT_PSEUDO_OVL;
	else
		...
		if (disp_idx == 0) {
			...
			/* When open VDS path switch feature, primary OVL have OVL0 only */
				...
				return DDP_COMPONENT_OVL0;
		} else if (disp_idx == SECONDARY_DISP_ID {
...
static void clear_layer(struct drm_mtk_layering_info *disp_info)
{
	...
	for (di = 0; di < HRT_DISP_TYPE_NUM; di++) {
		...
		for (i = disp_info->layer_num[di] - 1; i >= 0; i--) {
		...
		for (j = 0; j < MAX_CLEAR_LAYER; j++) {
			if (k >= 0 && k < disp_info->layer_num[di]) {
				c = &disp_info->input_config[di][k];
				if (k == disp_info->gles_head[di])
					disp_info->gles_head[di]++;
				else if (k == disp_info->gles_tail[di])
					disp_info->gles_tail[di]--;

				c->layer_caps |= MTK_DISP_CLIENT_CLEAR_LAYER;

				DDPDBG("%s:D%d:g%d,%d\n", __func__, di, disp_info->gles_head[di],
					disp_info->gles_tail[di]);
				...
				if ((c->src_width < c->dst_width &&
					...
					if(!mtk_has_layer_cap(c, MTK_MDP_RSZ_LAYER)) {
						c->layer_caps &= ~MTK_DISP_CLIENT_CLEAR_LAYER;
						DDPPR_ERR("%s:remove clear(rsz), caps:0x%08x\n",
				  ...
				  struct drm_device *drm_dev)
{
	...
	int prev_comp_id = -1;
	int i, j, k;
	int clear_idx[MAX_CLEAR_LAYER] = {-1, -1};
	int ori_layer_map;
	uint16_t yuv_only_map = 0;
	uint16_t rgb_only_map = 0;
	int gles_head, gles_tail;
	int clear = -1;
	...
	for (i = 0, k = 0; i < disp_info->layer_num[disp_idx]; i++) {
		layer_info = &disp_info->input_config[disp_idx][i];
		if (mtk_has_layer_cap(layer_info,
				      MTK_DISP_CLIENT_CLEAR_LAYER)) {
			if (k < MAX_CLEAR_LAYER)
				clear_idx[k++] = i;
			else
				DDPPR_ERR("too many clear layer:%d\n", k);
		}
	}

	for (k = 0; k < MAX_CLEAR_LAYER; k++)
		DDPDBG("clear_idx:%d\n", clear_idx[k]);

	ori_layer_map = layer_map;
	yuv_only_map = l_rule_ops->get_mapping_table(
		drm_dev, disp_idx, DISP_HW_LAYER_YUV_ONLY_TB, 0);
	rgb_only_map = l_rule_ops->get_mapping_table(
		drm_dev, disp_idx, DISP_HW_LAYER_RGB_ONLY_TB, 0);
	...
	for (i = 0; i < disp_info->layer_num[disp_idx]; i++) {
		...
		if (!mtk_is_gles_layer(disp_info, disp_idx, i) &&
			mtk_is_yuv(layer_info->src_fmt) && layer_map_idx&rgb_only_map) {
			/*Find first match layer*/
			if (layer_map & (ori_layer_map & (~rgb_only_map))) {
				layer_map_idx = HRT_GET_FIRST_SET_BIT(layer_map &
								(ori_layer_map & (~yuv_only_map)));

				layer_map &= ~(layer_map_idx-1);
			} else {
				/*Can't find suitable layer, rollback to GPU*/
				gles_head = disp_info->gles_head[disp_idx];
				gles_tail = disp_info->gles_tail[disp_idx];
				mtk_rollback_layer_to_GPU(disp_info, disp_idx, i);

				if ((gles_tail != -1) && (i > gles_tail)) {
					for (j = i; j > gles_tail; j--)
						layer_info->ovl_id =
						disp_info->input_config[disp_idx][gles_tail].ovl_id;
					continue;
				}
			}

		} else if ((mtk_is_gles_layer(disp_info, disp_idx, i) ||
					!mtk_is_yuv(layer_info->src_fmt)) &&
					layer_map_idx&yuv_only_map) {

			/*Find first match layer*/
			if (layer_map & (ori_layer_map & (~yuv_only_map))) {
				layer_map_idx = HRT_GET_FIRST_SET_BIT(layer_map &
								(ori_layer_map & (~yuv_only_map)));
			   ...
			   struct drm_device *drm_dev)
{
	...
	for (disp_idx = 0; disp_idx < HRT_DISP_TYPE_NUM; disp_idx++) {
		...
		if (clear)
			ovl_cnt++;
		layer_map = l_rule_ops->get_mapping_table(
			drm_dev, disp_idx, DISP_HW_LAYER_TB, MAX_PHY_OVL_CNT);
...
void lye_search_lc_tgt_layer(struct mtk_lye_ddp_state *lye_state,
		struct drm_mtk_layering_info *disp_info)
{
	/*for mt8188 */
	int i;
	struct drm_mtk_layer_config *c = NULL;
	uint32_t rsz_layer_cnt = 0;

	lye_state->lc_tgt_layer = 0;

	if (lye_state->scn[HRT_PRIMARY] == ONE_SCALING) {
		for (i = 0; i < disp_info->layer_num[HRT_PRIMARY]; i++) {
			if (!(mtk_is_gles_layer(disp_info, HRT_PRIMARY, i))) {
				...
				if (is_extended_layer(c))
					continue;

				if (mtk_has_layer_cap(c, MTK_DISP_RSZ_LAYER) &&
					is_rsz_valid(c)) {
					lye_state->lc_tgt_layer = c->ovl_id;
					rsz_layer_cnt++;
				}
			}

			if (rsz_layer_cnt > 1) {
				DDPPR_ERR("layering rule fail, [%d] layer caps rsz\n", rsz_layer_cnt);
				c->layer_caps &= ~MTK_DISP_RSZ_LAYER;
			}
		}
	}
		      ...
		      int crtc_mask)
{
	...
	//search lc tgt layer
	lye_state.lc_tgt_layer = 0;
	lye_search_lc_tgt_layer(&lye_state, l_info);
			...
			bool has_pq)
{
	...
	if (disp_info->layer_num[HRT_SECONDARY] > 0)
		mtk_rollback_all_resize_layer_to_GPU(dev, disp_info, HRT_SECONDARY);
	if (disp_info->layer_num[HRT_THIRD] > 0)
		mtk_rollback_all_resize_layer_to_GPU(dev, disp_info, HRT_THIRD);
			       ...
			       int debug_mode, struct drm_device *dev)
{
	...
	if (get_layering_opt(LYE_OPT_RPO)) {
		...
		mtk_rollback_all_resize_layer_to_GPU(dev, &layering_info, HRT_PRIMARY);
		mtk_rollback_all_resize_layer_to_GPU(dev, &layering_info, HRT_SECONDARY);
		mtk_rollback_all_resize_layer_to_GPU(dev, &layering_info, HRT_THIRD);
	}
