...
static void write_phy_layer_sec_addr_cmdq(struct mtk_ddp_comp *comp,
				      struct cmdq_pkt *handle, int id,
				      dma_addr_t addr, uint32_t buf_size, uint32_t offset)
{
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
	struct mtk_disp_ovl *ovl = comp_to_ovl(comp);
	uint32_t size, meta_type, regs_addr;

	size = buf_size;
	regs_addr = comp->regs_pa + DISP_REG_OVL_ADDR(ovl, id);
	meta_type = CMDQ_IWC_H_2_MVA;

	cmdq_sec_pkt_write_reg(handle, regs_addr,
		addr, meta_type, offset, size, 0);
#endif
}
...
static void write_ext_layer_sec_addr_cmdq(struct mtk_ddp_comp *comp,
				      struct cmdq_pkt *handle, int id,
				      dma_addr_t addr, uint32_t buf_size, uint32_t offset)
{
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
	...
	size = buf_size;
	regs_addr = comp->regs_pa + DISP_REG_OVL_EL_ADDR(ovl, id);
	meta_type = CMDQ_IWC_H_2_MVA;
			       ...
			       struct cmdq_pkt *handle)
{
	...
	if (ext_lye_idx != LYE_NORMAL) {
		...
		if (state->pending.is_sec && pending->addr)
			write_ext_layer_sec_addr_cmdq(comp, handle, id,
				pending->addr, buf_size, offset);
		else
		...
		if (state->pending.is_sec && pending->addr)
			write_phy_layer_sec_addr_cmdq(comp, handle, lye_idx,
				pending->addr, buf_size, offset);
		else
...
//		       comp->regs_pa + DISP_REG_OVL_RDMA_CTRL(idx), 0x0, ~0);
		...
		if (pending->prop_val[PLANE_PROP_COMPRESS])
			temp_bw = (unsigned long long) ALIGN(pending->width, 32) *
				ALIGN(pending->height, 16);
		else
			...
			struct cmdq_pkt *handle)
{
	...
	DDPDBG("%s:%d, addr:0x%llx, pitch:%d, vpitch:%d\n",
...
static void
mtk_ovl_addon_rsz_config_embed(struct mtk_ddp_comp *comp, enum mtk_ddp_comp_id prev,
			 enum mtk_ddp_comp_id next, struct mtk_rect rsz_src_roi,
			 struct mtk_rect rsz_dst_roi, struct cmdq_pkt *handle, uint8_t lc_tgt_layer)
{
	unsigned int value = 0, mask = 0;

	if ((prev == -1) && (next == DDP_COMPONENT_RSZ0)) {
		int lc_x = rsz_dst_roi.x, lc_y = rsz_dst_roi.y;
		int lc_w = rsz_dst_roi.width, lc_h = rsz_dst_roi.height;
		...
		//add pq out sel, enable pq out opt
		SET_VAL_MASK(value, mask, (lc_tgt_layer & 0x3), DATAPATH_CON_FLD_PQ_OUT_SEL);
		SET_VAL_MASK(value, mask, 1, DATAPATH_CON_FLD_PQ_OUT_EN);
		SET_VAL_MASK(value, mask, 1, DATAPATH_CON_FLD_PQ_OUT_OPT);
		cmdq_pkt_write(handle, comp->cmdq_base,
				   comp->regs_pa + DISP_REG_OVL_DATAPATH_CON,
				   value, mask);
		...
		// disable pq out path
		SET_VAL_MASK(value, mask, (0 & 0x3), DATAPATH_CON_FLD_PQ_OUT_SEL);
		SET_VAL_MASK(value, mask, 0, DATAPATH_CON_FLD_PQ_OUT_EN);
		SET_VAL_MASK(value, mask, 0, DATAPATH_CON_FLD_PQ_OUT_OPT);
		cmdq_pkt_write(handle, comp->cmdq_base,
		   comp->regs_pa + DISP_REG_OVL_DATAPATH_CON,
		   value, mask);
				 ...
				 struct cmdq_pkt *handle)
{
	...
	if ((addon_config->config_type.module == DISP_RSZ) &&
		...
		mtk_ovl_addon_rsz_config_embed(comp, prev, next, config->rsz_src_roi,
					 config->rsz_dst_roi, handle, config->lc_tgt_layer);
	}
}
...
static void mtk_ovl_backup_info_cmp(struct mtk_ddp_comp *comp, bool *compare)
{
	...
	for (i = 0; i < mtk_ovl_layer_num(comp); i++) {
		...
		if (!cur_info[i].layer_en) {
			DDPMSG("%s:layer%d,en %d,size 0x%x,addr 0x%llx\n",
		...
		DDPMSG("%s:layer%d,en %d,size 0x%x, addr %llu\n", __func__, i,
...
static int mtk_lk_ovl_status(struct mtk_ddp_comp *comp)
{
	void __iomem *baddr = comp->regs;

	unsigned int ovl_en = readl(DISP_REG_OVL_EN + baddr);

	unsigned int ovl_poweoff_flag = readl(DISP_REG_OVL_DUMMY_REG + baddr);

	if ((ovl_en == 0) || (ovl_poweoff_flag == 0x87654321)) {
		DDPMSG("ovl disable during lk, (%d 0x%x)!!!\n",
			ovl_en, ovl_poweoff_flag);
		return 0;
	}  else {
		DDPMSG("ovl enable during lk, (%d 0x%x)!!!\n",
			ovl_en, ovl_poweoff_flag);
		return 1;
	}
}
			  ...
			  enum mtk_ddp_io_cmd io_cmd, void *params)
{
	...
	case CHECK_LK_LOGO_STATUS:
	{
		u32 *enable;
		enable = (u32 *)params;
		*enable = mtk_lk_ovl_status(comp);
	}
	default:
		break;
	}
					 ...
					 bool is_ext_layer)
{
	...
	DDPDUMP("compr_en:%u, pitch_msb:0x%x, hdr_addr:0x%llx, hdr_pitch:0x%x\n",
				...
				bool is_ext_layer)
{
	...
	DDPDUMP("%s_L%d:(%u,%u,%ux%u)\n",
		...
		(src_size >> 16) & 0xfff);
	DDPDUMP("pitch=%u,addr=0x%llx,source=%s,aen=%u,alpha=%u,cl=0x%x\n",
		pitch & 0xffff,
...
static const struct compress_info compr_info_mt8195  = {
	.name = "AFBC_V1_2_MTK_1",
	.l_config = &compr_l_config_AFBC_V1_2,
};
...
static const struct of_device_id mtk_disp_ovl_driver_dt_match[] = {
	...
	{.compatible = "mediatek,mt8188-disp-ovl",
	 .data = &mt8195_ovl_driver_data}, // same as 8195
	{},
};
