...
#define WDMA_CON_SWAP BIT(15)
#define WDMA_UNI_CONFIG BIT(16)
#define WDMA_UFO_DCP_ENABLE BIT(17)
#define WDMA_UFO_DCP_10BIT BIT(18)
#define WDMA_INT_MTX_SEL (0x1f << 23)
...
#define MEM_MODE_INPUT_SWAP BIT(15)
...
static int mtk_wdma_is_busy(struct mtk_ddp_comp *comp)
{
	...
	ret = ((tmp & REG_FLD_MASK(FLOW_CTRL_DBG_FLD_WDMA_STA_FLOW_CTRL)) != 0x1);

	DDPINFO("%s() returns %d: regs=0x%x\n", __func__, ret, tmp);
...
static void write_dst_addr(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle,
			 int id, dma_addr_t addr, uint32_t offset, uint32_t size)
{
	struct mtk_disp_wdma *wdma = comp_to_wdma(comp);

	DDPDBG("%s(comp-%d): id=%d, addr=0x%llx, offset=0x%x, size=%u\n",
		__func__, comp->id, id, addr, offset, size);

	if (comp->fb && mtk_drm_fb_is_secure(comp->fb)) {
		DDPDBG("%s(comp-%d): is sec\n", __func__, comp->id);
		cmdq_sec_pkt_write_reg(handle,
			comp->regs_pa + DISP_REG_WDMA_DST_ADDRX(id),
			addr, CMDQ_IWC_H_2_MVA, offset, size, 0);
	} else {
		mtk_ddp_write(comp, addr + offset,
			DISP_REG_WDMA_DST_ADDRX(id), handle);
	}
			      ...
			      void *handle)
{
	...
	u_off += comp->fb->offsets[0];
	v_off += comp->fb->offsets[0];

	write_dst_addr(comp, handle, 1, dstAddress, u_off, u_size);
	if (has_v)
		write_dst_addr(comp, handle, 2, dstAddress, v_off, u_size);
			    ...
			    struct mtk_ddp_config *cfg, struct cmdq_pkt *handle)
{
	...
	if (comp->mtk_crtc) {
		...
		crtc_state = comp->mtk_crtc->base.state;
		state = to_mtk_crtc_state(crtc_state);
		need_skip = state->prop_val[CRTC_PROP_SKIP_CONFIG];
	} else {
	...
	DDPINFO("%s fmt:0x%x, con:0x%x addr:0x%llx\n", __func__,
	...
	if (comp->mtk_crtc && comp->mtk_crtc->sec_on)
		mtk_ddp_write(comp, 0, DISP_REG_WDMA_INTEN, handle);
	else
		mtk_ddp_write(comp, inten, DISP_REG_WDMA_INTEN, handle);
	...
	mtk_ddp_write_mask(comp, WDMA_UNI_CONFIG,
		DISP_REG_WDMA_CFG, WDMA_UNI_CONFIG, handle);

	if (is_yuv(comp->fb->format->format)) {
		wdma_config_yuv420(comp, comp->fb->format->format,
				comp->fb->pitches[0], cfg->h,
				addr, handle);
		mtk_ddp_write_mask(comp, 0, DISP_REG_WDMA_CFG,
				WDMA_UFO_DCP_ENABLE | WDMA_UFO_DCP_10BIT, handle);
		//mtk_ddp_write_mask(comp, WDMA_CT_EN,
		//		DISP_REG_WDMA_CFG, WDMA_CT_EN, handle);
		mtk_ddp_write_mask(comp, (0b0010 << 23),
				 ...
				 struct cmdq_pkt *handle)
{
	...
	write_dst_addr(comp, handle, 0, addr, 0, 0);
	...
	DDPMSG("[capture] config addr:0x%llx, roi:(%d,%d,%d,%d)\n",
...
int mtk_wdma_analysis(struct mtk_ddp_comp *comp)
{
	...
	DDPDUMP("pitch=(W=%d,UV=%d),addr=(%llu,%llu,%llu),cfg=0x%x\n",
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case WDMA_WRITE_DST_ADDR0:
	{
		...
		write_dst_addr(comp, handle, 0, addr, 0, 0);
		DDPMSG("[capture] update addr:0x%llx\n", addr);
...
static const struct mtk_disp_wdma_data mt8195_wdma_driver_data = {
	.fifo_size_1plane = 325,
	.fifo_size_uv_1plane = 31,
	.fifo_size_2plane = 228,
	.fifo_size_uv_2plane = 109,
...
static const struct mtk_disp_wdma_data mt8188_wdma_driver_data = {
	.fifo_size_1plane = 325,
	.fifo_size_uv_1plane = 31,
	.fifo_size_2plane = 228,
	.fifo_size_uv_2plane = 109,
...
static const struct of_device_id mtk_disp_wdma_driver_dt_match[] = {
	...
	{.compatible = "mediatek,mt8195-disp-wdma",
	 .data = &mt8195_wdma_driver_data},
	{.compatible = "mediatek,mt8188-disp-wdma",
	 .data = &mt8188_wdma_driver_data},
	{},
};
