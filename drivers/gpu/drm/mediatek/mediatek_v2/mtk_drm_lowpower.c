...
static void mtk_drm_vdo_mode_enter_idle(struct drm_crtc *crtc)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt,
		...
		mtk_disp_mutex_inten_disable_cmdq(mtk_crtc, mtk_crtc->mutex[0], handle);
...
static void mtk_drm_vdo_mode_leave_idle(struct drm_crtc *crtc)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt,
				   MTK_DRM_OPT_IDLEMGR_DISABLE_ROUTINE_IRQ)) {
		mtk_disp_mutex_inten_enable_cmdq(mtk_crtc, mtk_crtc->mutex[0], handle);
		for_each_comp_in_cur_crtc_path(comp, mtk_crtc, i, j)
...
#define LEN 50
	...
	ret = snprintf(name, LEN, "mtk_drm_disp_idlemgr-%d", index);
	if (ret < 0)
		DDPPR_ERR("invalid idlemgr name\n");
