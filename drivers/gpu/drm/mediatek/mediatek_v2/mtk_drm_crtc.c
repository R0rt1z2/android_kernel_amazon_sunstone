...
static void mtk_drm_crtc_destroy(struct drm_crtc *crtc)
{
	...
	mtk_disp_mutex_put(mtk_crtc, mtk_crtc->mutex[0]);
...
void mtk_drm_crtc_dump(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6879:
		...
		break;
	case MMSYS_MT8188:
		mmsys_config_dump_reg_mt8188(mtk_crtc);
		break;
	case MMSYS_MT8195:
		break;
	default:
...
int mtk_drm_aod_setbacklight(struct drm_crtc *crtc, unsigned int level)
{
	...
	if (!mtk_crtc->enabled) {
		/* 1. power on mtcmos */
		mtk_drm_top_clk_prepare_enable(crtc->dev, crtc_id);
	...
	if (!mtk_crtc->enabled) {
		...
		mtk_drm_top_clk_disable_unprepare(crtc->dev, crtc_id);
	}
...
bool mtk_crtc_is_dual_pipe(struct drm_crtc *crtc)
{
	...
	if ((drm_crtc_index(crtc) == CRTC_ID_EXTERNAL) &&
		(crtc->state->adjusted_mode.hdisplay == 1920*2) &&
		(!mtk_crtc->use_second_mutex)) {
		DDPFUNC();
		return true;
	} else
		return false;
...
void mtk_crtc_ddp_prepare(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (drm_crtc_index(crtc) == CRTC_ID_VIRTUAL) {
#else
	if (drm_crtc_index(crtc) == CRTC_ID_EXTERNAL) {
#endif
		writel_relaxed(0x220000, mtk_crtc->config_regs + 0xE10);
		mtk_disp_mutex_prepare(mtk_crtc, mtk_crtc->mutex[0]);
	}
}
...
#ifdef CONFIG_MTK_WFD_OVER_VDO1
	if (drm_crtc_index(&mtk_crtc->base) == CRTC_ID_VIRTUAL)
#else
	if (drm_crtc_index(&mtk_crtc->base) == CRTC_ID_EXTERNAL)
#endif
	{
#ifdef MTK_DISP_MMDVFS_SUPPORT
		...
		mtk_drm_set_mmclk(&mtk_crtc->base, 0, __func__);
#endif
		mtk_disp_mutex_unprepare(mtk_crtc, mtk_crtc->mutex[0]);
	}
}
				      ...
				      struct cmdq_pkt *cmdq_handle)
{
	...
	for (i = 0; i < addon_data->module_num; i++) {
		...
		if (addon_module->type == ADDON_BETWEEN &&
			...
			if (state->rsz_src_roi.width <= state->rsz_dst_roi.width &&
				state->rsz_src_roi.height <= state->rsz_dst_roi.height)
			...
			enum CRTC_DDP_PATH ddp_path)
{
	...
	if (!mtk_crtc->mutex[ddp_path]) {
		DDPMSG("%s, invalid mutex\n", __func__);
		return -EINVAL;
	}

	mutex_id = mtk_crtc->mutex[ddp_path]->id;

	DDPDBG("%s(crtc-%d): path=%d, mutex_id=%d\n",
		__func__, crtc_id, ddp_path, mutex_id);

	for_each_comp_in_crtc_target_path(comp, mtk_crtc, i, ddp_path)
		if (mtk_ddp_comp_is_output(comp))
			break;

	if (!comp) {
		DDPINFO("%s(crtc-%d): no output component for path %d\n",
			__func__, crtc_id, ddp_path);
		return -EINVAL;
	}

	if (comp->id == DDP_COMPONENT_DSI0 || comp->id == DDP_COMPONENT_DSI1) {
		return mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base) ?
	...
	DDPINFO("%s(crtc-%d): no frame done event for component %d\n",
			__func__, crtc_id, comp->id);
	return -EINVAL;
}
			   ...
			   struct cmdq_pkt *handle)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6983 ||
		...
		priv->data->mmsys_id == MMSYS_MT8188) {
	...
	if (drm_crtc_index(crtc) != 2) {
		...
		if (!mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base)) {
			if (cb_data->msync2_enable)
				...
				if (mtk_crtc->pre_eof_time < mtk_crtc->eof_time)
					mtk_release_present_fence(session_id,
				...
				mtk_crtc->pre_eof_time = mtk_crtc->eof_time;
			}
		}
	}
...
void mtk_crtc_start_trig_loop(struct drm_crtc *crtc)
{
	...
	if (mtk_crtc_is_frame_trigger_mode(crtc)) {
		...
		/*Trigger*/
		mtk_disp_mutex_enable_cmdq(mtk_crtc, mtk_crtc->mutex[0], cmdq_handle,
		...
		mtk_disp_mutex_submit_sof(mtk_crtc, mtk_crtc->mutex[0]);
		if (crtc_id == 0) {
		...
		} else if (crtc_id == CRTC_ID_EXTERNAL) {
			cmdq_pkt_wfe(cmdq_handle, mtk_crtc->gce_obj.event[EVENT_VDO_EOF]);
		}
		...
		if (mtk_crtc_with_sodi_loop(crtc)) {
			...
			if (inst)
				*inst = *inst | CMDQ_REG_SHIFT_ADDR(jump_pa);
				...
				DDPPR_ERR("cmdq pkt buffer is NULL\n");
...
void mtk_crtc_disconnect_embed_addon_module(struct drm_crtc *crtc)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_crtc_state *crtc_state = to_mtk_crtc_state(crtc->state);
	struct cmdq_pkt *handle;
	...
	for (i = 0; i < addon_data->module_num; i++) {
		...
		if (addon_module->type == ADDON_EMBED &&
		    addon_module->module == DISP_RSZ)
			mtk_addon_disconnect_embed(
				crtc, mtk_crtc->ddp_mode, addon_module, &addon_config,
				handle);
	}
	cmdq_pkt_flush(handle);
	cmdq_pkt_destroy(handle);
...
void mtk_crtc_disable_embed_lc_layer(struct drm_crtc *crtc, int idx,
	struct mtk_ddp_comp *comp,	void *cmdq_handle)
{
	const struct mtk_addon_scenario_data *addon_data;
	const struct mtk_addon_module_data *addon_module;
	union mtk_addon_config addon_config;
	...
	for (i = 0; i < addon_data->module_num; i++) {
		...
		if ((addon_module->type == ADDON_EMBED) &&
			(addon_module->module == DISP_RSZ) &&
			(idx == lye_state->lc_tgt_layer))
			mtk_ddp_comp_layer_off(comp, 4, LYE_LC, cmdq_handle);
	}
}
...
void mtk_crtc_connect_default_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	for_each_comp_in_crtc_path_bound(comp, mtk_crtc, i, j, 1) {
		...
		mtk_ddp_add_comp_to_path(
			mtk_crtc, ddp_comp[j], prev_id, next_id);
	}

	DDPDBG("%s(crtc-%d): is dc mode: %d\n",
		__func__, crtc_id, mtk_crtc_is_dc_mode(crtc));

	/* add module in mutex */
	if (mtk_crtc_is_dc_mode(crtc)) {
		for_each_comp_in_crtc_target_path(comp, mtk_crtc, i,
						  DDP_FIRST_PATH)
			mtk_disp_mutex_add_comp(mtk_crtc, mtk_crtc->mutex[1], comp->id);
		for_each_comp_in_crtc_target_path(comp, mtk_crtc, i,
						  DDP_SECOND_PATH)
			mtk_disp_mutex_add_comp(mtk_crtc, mtk_crtc->mutex[0], comp->id);
	} else {
		for_each_comp_in_crtc_target_path(comp, mtk_crtc, i,
						  DDP_FIRST_PATH)
			mtk_disp_mutex_add_comp(mtk_crtc, mtk_crtc->mutex[0], comp->id);
	...
	if (!mtk_crtc_is_frame_trigger_mode(crtc))
		mtk_disp_mutex_enable(mtk_crtc, mtk_crtc->mutex[0]);
...
void mtk_crtc_restore_plane_setting(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (mtk_drm_dal_enable() && crtc_id == 0)
...
static int mtk_crtc_get_primary_layer(struct mtk_drm_crtc *mtk_crtc, int *primary_layer)
{
	int i, j;
	struct mtk_ddp_comp *comp;
	int ret = -EINVAL;
	...
	for_each_comp_in_cur_crtc_path(comp, mtk_crtc, i, j) {
		ret = mtk_ddp_comp_io_cmd(comp, NULL, OVL_GET_PRIMARY_LAYER,
			primary_layer);
		if (ret >= 0)
			return ret;
	}
...
void mtk_crtc_disconnect_default_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (!mtk_crtc_is_frame_trigger_mode(crtc))
		mtk_disp_mutex_disable(mtk_crtc, mtk_crtc->mutex[0]);
	...
	if (mtk_crtc_is_dc_mode(crtc)) {
		for_each_comp_in_crtc_target_path(comp, mtk_crtc, i,
						  DDP_FIRST_PATH)
			mtk_disp_mutex_remove_comp(mtk_crtc, mtk_crtc->mutex[1],
						  comp->id);
		for_each_comp_in_crtc_target_path(comp, mtk_crtc, i,
						  DDP_SECOND_PATH)
			mtk_disp_mutex_remove_comp(mtk_crtc, mtk_crtc->mutex[0],
						  comp->id);
	} else {
		for_each_comp_in_crtc_target_path(comp, mtk_crtc, i,
						  DDP_FIRST_PATH)
...
void mtk_drm_crtc_enable(struct drm_crtc *crtc)
{
	...
	if (mtk_crtc->bootlogo) {
		if (output_comp)
			mtk_ddp_comp_io_cmd(output_comp, NULL, BOOTLOGO_DISABLE, NULL);
		mtk_drm_crtc_disable(crtc, true);
		mtk_crtc->bootlogo = false;
	}
...
#ifndef MTK_DRM_BRINGUP_STAGE
	/* 1. power on mtcmos */
	DDPDBG("%s(crtc-%d): 1: power on mtcmos\n", __func__, crtc_id);
	mtk_drm_top_clk_prepare_enable(crtc->dev, crtc_id);
#endif

	/* 2. prepare modules would be used in this CRTC */
	DDPDBG("%s(crtc-%d): 2: prepare modules\n", __func__, crtc_id);
	...
	if (crtc_id == 2) {
		struct cmdq_client *client;
		DDPDBG("%s(crtc-%d): 3: power on cmdq client\n", __func__, crtc_id);
		client = mtk_crtc->gce_obj.client[CLIENT_CFG];
	...
	if (mtk_crtc_with_trigger_loop(crtc)) {
		DDPDBG("%s(crtc-%d): 4: trigger loop\n", __func__, crtc_id);
		if (mtk_crtc_with_sodi_loop(crtc) &&
			(!mtk_crtc_is_frame_trigger_mode(crtc)))
	...
	/* 5. connect path */
	DDPDBG("%s(crtc-%d): 5: connect path\n", __func__, crtc_id);
				...
				struct drm_crtc_state *old_crtc_state)
{
	...
	/* hold wakelock */
	mtk_drm_crtc_wk_lock(crtc, 1, __func__, __LINE__);
	pr_notice("[METRICS_DISP] display ON\n");
...
void mtk_drm_crtc_init_para(struct drm_crtc *crtc)
{
	...
	if (!timing) {
		DDPDBG("%s:%d CRTC%d timing is null\n", __func__, __LINE__, crtc_id);
		return;
	}
...
static uint32_t mtk_crtc_first_ovl_status_check(struct mtk_drm_crtc *mtk_crtc)
{
...
static void mtk_drm_crtc_disable_dsi(struct mtk_drm_crtc *mtk_crtc)
{
	struct mtk_ddp_comp *output_comp = NULL;
	struct cmdq_pkt *cmdq_handle = NULL;
...
void mtk_drm_crtc_first_enable(struct drm_crtc *crtc)
{
	...
	/*if lk ovl disable, no need to enable clk*/
	if (mtk_crtc_first_ovl_status_check(mtk_crtc) == 0) {
		DDPMSG("crtc%d skip for lk no display %s\n", crtc_id, __func__);
		mtk_drm_top_clk_disable_unprepare(crtc->dev, crtc_id);
		mtk_drm_crtc_disable_dsi(mtk_crtc);
		return;
	}
...
/* release all fence */
	...
	/* release wakelock */
	mtk_drm_crtc_wk_lock(crtc, 0, __func__, __LINE__);
	pr_notice("[METRICS_DISP] display OFF\n");
...
void mtk_crtc_disable_secure_state(struct drm_crtc *crtc)
{
	...
	comp = mtk_crtc_get_comp(&(mtk_crtc->base), DDP_FIRST_PATH, 0);
	...
	cmdq_sec_pkt_set_data(cmdq_handle, sec_engine, sec_engine, sec_scenario,
		CMDQ_METAEX_NONE);
						...
						struct mtk_crtc_state *crtc_state)
{
	...
	if (mtk_crtc->sec_on) {
		...
		if (drm_crtc_index(crtc) == 0)
			sec_scenario = CMDQ_SEC_PRIMARY_DISP;
		else if (drm_crtc_index(crtc) == 1)
			sec_scenario = CMDQ_SEC_SUB_DISP;
		else if (drm_crtc_index(crtc) == 2) {
			sec_scenario = CMDQ_SEC_PRIMARY_MEMOUT;
			sec_engine |= (1ULL << CMDQ_SEC_DISP_WDMA0);
		}

		cmdq_sec_pkt_set_data(cmdq_handle, sec_engine,
			sec_engine,
			sec_scenario,
			CMDQ_METAEX_NONE);
	}
		...
		enum MSYNC_RECORD_TYPE type, u64 time)
{
	...
	if (msync_dy->record_index < 0 ||
		msync_dy->record_index >= MSYNC_MAX_RECORD) {
		DDPPR_ERR("[Msync] invalid record index\n");
		return;
	}
			       ...
			       struct mtk_plane_state *plane_state)
{
	...
	DDPDBG("%s(crtc-%d): plane=%d, comp=%d, state->comp=%d\n",
		__func__, drm_crtc_index(crtc),
	...
	if (plane_state->pending.enable) {
		...
		if (comp_state->comp_id) {
			...
			mtk_ddp_comp_layer_off(comp, comp_state->lye_id,
					comp_state->ext_lye_id, cmdq_handle);
			mtk_crtc_disable_embed_lc_layer(crtc, comp_state->lye_id,
				comp, cmdq_handle);
		} else {
				      ...
				      struct drm_crtc_state *old_crtc_state)
{
	...
	if ((priv->data->shadow_register) == true) {
		mtk_disp_mutex_acquire(mtk_crtc, mtk_crtc->mutex[0]);
		mtk_crtc_ddp_config(crtc);
		mtk_disp_mutex_release(mtk_crtc, mtk_crtc->mutex[0]);
	}
	...
	if (!mtk_crtc_is_frame_trigger_mode(crtc) &&
		...
		if (cmdq_handle->cmd_buf_size >= 4096) {
			...
			pr_info("[Msync]cmdq pkt size = %lu\n", cmdq_handle->cmd_buf_size);
		}
	}
...
static void mtk_drm_crtc_attach_property(struct drm_crtc *crtc)
{
	...
	if (num == 0) {
		for (i = 0; i < CRTC_PROP_MAX; i++) {
			...
			DDPINFO("created property:%s, flags:0x%x\n",
	...
	for (i = 0; i < CRTC_PROP_MAX; i++) {
		...
		crtc_prop = &(mtk_crtc_property[i]);
		if (!prop) {
			DDPINFO("%s(crtc-%d): attach property %s = %lu\n",
				__func__, index, crtc_prop->name, crtc_prop->val);
			     ...
			     unsigned int pipe)
{
	...
	DDPINFO("%s(crtc-%d)+\n", __func__, pipe);
	DDPINFO("%s(crtc-%d): primary plane  = %p\n", __func__, pipe, primary);
	DDPINFO("%s(crtc-%d): curosr  plane  = %p\n", __func__, pipe, cursor);
	ret = drm_crtc_init_with_planes(drm, &mtk_crtc->base, primary, cursor,
					&mtk_crtc_funcs, NULL);
	if (ret) {
		DDPDBG("%s(): error: drm_crtc_init_with_planes() returns %d\n",
			__func__, ret);
...
{
	int len, crtc_id = drm_crtc_index(&mtk_crtc->base);
	char output_comp[20];
	...
	case EVENT_STREAM_DIRTY:
		len = snprintf(buf, buf_len, "disp_token_stream_dirty%d", crtc_id);
		break;
	case EVENT_SYNC_TOKEN_SODI:
		len = snprintf(buf, buf_len, "disp_token_sodi%d", crtc_id);
		break;
	case EVENT_STREAM_EOF:
		len = snprintf(buf, buf_len, "disp_token_stream_eof%d", crtc_id);
		break;
	case EVENT_VDO_EOF:
		len = snprintf(buf, buf_len, "disp_mutex%d_eof", crtc_id);
		break;
	case EVENT_CMD_EOF:
	...
	case EVENT_TE:
		...
		break;
	case EVENT_ESD_EOF:
		len = snprintf(buf, buf_len, "disp_token_esd_eof%d", crtc_id);
		break;
	case EVENT_RDMA0_EOF:
		len = snprintf(buf, buf_len, "disp_rdma0_eof%d", crtc_id);
		break;
	case EVENT_WDMA0_EOF:
		len = snprintf(buf, buf_len, "disp_wdma0_eof%d", crtc_id);
		break;
	case EVENT_WDMA1_EOF:
		len = snprintf(buf, buf_len, "disp_wdma1_eof%d", crtc_id);
		break;
	case EVENT_STREAM_BLOCK:
		len = snprintf(buf, buf_len, "disp_token_stream_block%d", crtc_id);
		break;
	case EVENT_CABC_EOF:
		len = snprintf(buf, buf_len, "disp_token_cabc_eof%d", crtc_id);
		break;
	case EVENT_DSI0_SOF:
		len = snprintf(buf, buf_len, "disp_dsi0_sof%d", crtc_id);
		break;
	/*Msync 2.0*/
	case EVENT_SYNC_TOKEN_VFP_PERIOD:
		len = snprintf(buf, buf_len, "disp_token_vfp_period%d", crtc_id);
		break;
	case EVENT_GPIO_TE1:
		len = snprintf(buf, buf_len, "disp_gpio_te1");
		break;
	case EVENT_SYNC_TOKEN_DISP_VA_START:
		len = snprintf(buf, buf_len, "disp_token_disp_va_start%d", crtc_id);
		break;
	case EVENT_SYNC_TOKEN_DISP_VA_END:
		len = snprintf(buf, buf_len, "disp_token_disp_va_end%d", crtc_id);
		break;
	default:
				  ...
				  struct mtk_drm_crtc *mtk_crtc)
{
	...
	for (i = 0; i < CLIENT_TYPE_MAX; i++) {
		...
		DDPINFO("cmdq mailbox client:%d index:%d\n", i, index);
			...
			const struct mtk_crtc_path_data *path_data)
{
	...
	int primary_layer = 0;
	enum mtk_ddp_comp_id comp_id;
	const unsigned int *dispsys_map = priv->reg_data->dispsys_map;

	DDPMSG("%s(crtc-%d)+\n", __func__, pipe);

	DDPMSG("%s(crtc-%d): path_data = %p\n", __func__, pipe, path_data);

	if (!path_data) {
		DDPINFO("%s(): empty path\n", __func__);
		return 0;
	}
	...
	for_each_comp_id_in_path_data(comp_id, path_data, i, j, p_mode) {
		...
		DDPDBG("%s(crtc-%d): check component: %d\n", __func__, pipe, comp_id);
	...
	mtk_crtc->is_dual_pipe = false;
	mtk_crtc->use_first_mutex = false;
	mtk_crtc->use_second_mutex = false;

	// 8195 only, deprecated since 8188
	if (priv->data->mmsys_id == MMSYS_MT8195 &&
		pipe == 1 &&
		priv->side_config_regs != NULL) {
		DDPDBG("%s(crtc-%d): on path two mutex\n", __func__, pipe);
		mtk_crtc->one_path_two_mutex_en = true;
	}

	for (i = 0; i < DDP_MODE_NR; i++) {

		DDPDBG("%s(crtc-%d): allocate memory for mode: %d\n",
			__func__, pipe, i);
	...
	for (i = 0; i < DDP_PATH_NR; i++) {
		...
		DDPDBG("%s(crtc-%d): get mutex for path: %d/%d\n",
			__func__, pipe, i+1, DDP_PATH_NR);

		mtk_crtc->mutex[i] = mtk_disp_mutex_get(priv->mutex_dev, pipe);
	...
	for_each_comp_id_in_path_data(comp_id, path_data, i, j, p_mode) {
		...
		DDPDBG("%s(crtc-%d): init component: %d\n",
			__func__, pipe, comp_id);
		...
		comp = priv->ddp_comp[comp_id];
		if (!comp) {
			DDPINFO("Component %s not initialized\n", node->full_name);
			return -ENODEV;
		}

		// check if crtc has any compoent using the second mutex
		if (dispsys_map && dispsys_map[comp_id])
			mtk_crtc->use_second_mutex = true;
		else
			mtk_crtc->use_first_mutex = true;
		...
		if ((j == 0) && (p_mode == DDP_MAJOR) &&
		    (comp_id == DDP_COMPONENT_RDMA0 ||
		     ...
		     comp_id == DDP_COMPONENT_PSEUDO_OVL))
		{
			rdma_memory_mode = comp->comp_mode;
			*rdma_memory_mode = true;
			if (comp_id != DDP_COMPONENT_PSEUDO_OVL)
	...
	for_each_wb_comp_id_in_path_data(comp_id, path_data, i, p_mode) {
		...
		DDPDBG("%s(crtc-%d): init wb component: %d\n",
			__func__, pipe, comp_id);
	...
	DDPDBG("%s(crtc-%d): use_first_mutex=%d\n",
		__func__, pipe, mtk_crtc->use_first_mutex);
	DDPDBG("%s(crtc-%d): use_second_mutex=%d\n",
		__func__, pipe, mtk_crtc->use_second_mutex);

	DDPDBG("%s(crtc-%d): allocate memory for the planes\n", __func__, pipe);
	...
	mtk_crtc_get_primary_layer(mtk_crtc, &primary_layer);

	for (zpos = 0; zpos < mtk_crtc->layer_nr; zpos++) {
		type = (zpos == primary_layer) ? DRM_PLANE_TYPE_PRIMARY
				   : (zpos == (mtk_crtc->layer_nr - 1UL))
					     ? DRM_PLANE_TYPE_CURSOR
					     : DRM_PLANE_TYPE_OVERLAY;
		DDPDBG("%s(crtc-%d): init plane-%d: type=%s\n",
			__func__, pipe, zpos,
		...
		ret = mtk_plane_init(drm_dev, &mtk_crtc->planes[zpos], zpos,
				     BIT(pipe), type);
		if (ret) {
			DDPINFO("%s(crtc-%d): failed to init plane: %d\n",
				__func__, pipe, ret);
			return ret;
		}
	...
	if (mtk_crtc->layer_nr == 1UL) {
		...
		ret = mtk_drm_crtc_init(
			drm_dev, mtk_crtc, &mtk_crtc->planes[primary_layer].base,
	...
	output_comp = mtk_ddp_comp_request_output(mtk_crtc);
	if (output_comp) {
		DDPDBG("%s(crtc-%d): output component = %d\n",
			__func__, pipe, output_comp->id);
		mtk_ddp_comp_io_cmd(output_comp, NULL, REQ_PANEL_EXT,
				    &mtk_crtc->panel_ext);
	} else {
		DDPDBG("%s(crtc-%d): warning: no output component\n",
			__func__, pipe);
	}
...
int mtk_crtc_lcm_ATA(struct drm_crtc *crtc)
{
	...
	if (!mtk_crtc_is_frame_trigger_mode(crtc)) {
		...
		mtk_disp_mutex_trigger(mtk_crtc, mtk_crtc->mutex[0], cmdq_handle);
...
static bool __is_side_vdosys(
	struct mtk_drm_crtc *mtk_crtc,
	enum mtk_ddp_comp_id comp_id)
{
	if (unlikely(!mtk_crtc))
		pr_info("%s(): NULL crtc\n", __func__);
	else if (mtk_crtc->dispsys_num > 1 &&
		mtk_crtc->mmsys_reg_data  &&
		mtk_crtc->mmsys_reg_data->dispsys_map &&
		comp_id < DDP_COMPONENT_ID_MAX &&
		mtk_crtc->mmsys_reg_data->dispsys_map[comp_id])
		return true;
	return false;
}

void __iomem *mtk_crtc_get_regs(
	struct mtk_drm_crtc *mtk_crtc,
	enum mtk_ddp_comp_id comp_id)
{
	return __is_side_vdosys(mtk_crtc, comp_id) ?
		mtk_crtc->side_config_regs :
		mtk_crtc->config_regs;
}

resource_size_t mtk_crtc_get_regs_pa(
	struct mtk_drm_crtc *mtk_crtc,
	enum mtk_ddp_comp_id comp_id)
{
	return __is_side_vdosys(mtk_crtc, comp_id) ?
