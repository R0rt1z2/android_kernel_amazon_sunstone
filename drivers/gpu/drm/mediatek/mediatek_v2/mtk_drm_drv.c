...
#define MTK_DRM_DISP_TOP_CLOCK_ALL 255
				    ...
				    struct drm_atomic_state *old_state)
{
	...
	for_each_old_plane_in_state(old_state, plane, old_plane_state, i) {
		...
		if (comp_state[drm_crtc_index(plane_state->crtc)][idx].layer_caps
			& MTK_DISP_RSZ_LAYER) {
...
{
	int i;
	bool sec_in[MAX_CRTC]  = {0};
	bool sec_out[MAX_CRTC] = {0};
	static bool sec_in_prev[MAX_CRTC];
	...
	for_each_old_plane_in_state(new_state, plane, new_plane_state, i) {
		if (plane->state->crtc) {
			...
			if (plane->state->fb
				...
				sec_in[drm_crtc_index(&mtk_crtc->base)] = true;
		}
	}
	...
	for_each_old_crtc_in_state(new_state, crtc, new_crtc_state, i) {
		...
		if (mtk_crtc_check_out_sec(crtc))
			sec_out[drm_crtc_index(crtc)] = true;

		if (sec_in_prev[i] && !sec_in[i])
			mtk_crtc_disable_secure_state(&mtk_crtc->base);

		sec_in_prev[i] = sec_in[i];
		 ...
		 */
		if (!mtk_crtc->sec_on && (sec_in[i] || sec_out[i]))
			mtk_drm_idlemgr_kick(__func__, crtc, false);

		mtk_crtc->sec_on = sec_in[i] || sec_out[i];
	}
}
#endif
...
static bool mtk_drm_is_enable_from_lk(int crtc_id)
{
	/* TODO: check if target CRTC has been turn on in LK */
	int ret = false;

#ifndef CONFIG_MTK_SUB_AS_MAIN
	if (crtc_id == 0)
	...
	DDPDBG("%s(crtc-%d): return %d\n", __func__, crtc_id, ret);
	return ret;
}
					 ...
					 struct drm_atomic_state *state)
{
	 ...
	 */
	for_each_old_crtc_in_state(state, crtc, old_crtc_state, i) {
		if (!mtk_drm_is_enable_from_lk(drm_crtc_index(crtc)))
			return false;
	}
...
static const struct mtk_addon_module_data addon_rsz_data_v4[] = {
	{DISP_RSZ, ADDON_EMBED, DDP_COMPONENT_OVL0},
};
...
static const enum mtk_ddp_comp_id mt8195_mtk_ddp_dual_main[] = {
	DDP_COMPONENT_OVL1,		DDP_COMPONENT_RDMA1,
	DDP_COMPONENT_COLOR1,		DDP_COMPONENT_CCORR1,
	DDP_COMPONENT_AAL1,		DDP_COMPONENT_GAMMA1,
	DDP_COMPONENT_DITHER1,
};

static const enum mtk_ddp_comp_id mt8195_mtk_ddp_main_wb_path[] = {
	DDP_COMPONENT_OVL0,		DDP_COMPONENT_WDMA0,
};


static const enum mtk_ddp_comp_id mt8195_mtk_ddp_main_minor[] = {
	DDP_COMPONENT_OVL1,		DDP_COMPONENT_OVL0,
	DDP_COMPONENT_WDMA0,
};


static const enum mtk_ddp_comp_id mt8195_mtk_ddp_main_minor_sub[] = {
	DDP_COMPONENT_RDMA0,		DDP_COMPONENT_COLOR0,
	DDP_COMPONENT_CCORR0,		DDP_COMPONENT_AAL0,
	DDP_COMPONENT_GAMMA0,		DDP_COMPONENT_DITHER0,
#ifndef CONFIG_MTK_SUB_AS_MAIN
	DDP_COMPONENT_DSI0,		DDP_COMPONENT_PWM0,
#endif
};
...
static const enum mtk_ddp_comp_id mt8195_mtk_ddp_third[] = {
	DDP_COMPONENT_OVL1, DDP_COMPONENT_WDMA1,
};

static const struct mtk_addon_scenario_data mt8195_addon_main[ADDON_SCN_NR] = {
	[NONE] = {
			.module_num = 0,
			.hrt_type = HRT_TB_TYPE_GENERAL1,
		},
...
static const struct mtk_addon_scenario_data mt8195_addon_main_dual[ADDON_SCN_NR] = {
	[NONE] = {
			.module_num = 0,
			.hrt_type = HRT_TB_TYPE_GENERAL1,
		},
...
static const struct mtk_addon_scenario_data mt8195_addon_ext[ADDON_SCN_NR] = {
	[NONE] = {
		.module_num = 0,
		.hrt_type = HRT_TB_TYPE_GENERAL0,
	},
...
static const enum mtk_ddp_comp_id mt8188_mtk_ddp_main_wb_path[] = {
	DDP_COMPONENT_OVL0,		DDP_COMPONENT_WDMA0,
};


static const enum mtk_ddp_comp_id mt8188_mtk_ddp_main_minor[] = {
	DDP_COMPONENT_OVL0,		DDP_COMPONENT_WDMA0,
};


static const enum mtk_ddp_comp_id mt8188_mtk_ddp_main_minor_sub[] = {
	DDP_COMPONENT_RDMA0,		DDP_COMPONENT_COLOR0,
	DDP_COMPONENT_CCORR0,		DDP_COMPONENT_AAL0,
	DDP_COMPONENT_GAMMA0,		DDP_COMPONENT_DITHER0,
	DDP_COMPONENT_POSTMASK0,	DDP_COMPONENT_DSI0,
	DDP_COMPONENT_PWM0,

};

static const struct mtk_addon_scenario_data mt8188_addon_main[ADDON_SCN_NR] = {
	[NONE] = {
			.module_num = 0,
			.hrt_type = HRT_TB_TYPE_GENERAL1,
		},
...
static const struct mtk_crtc_path_data mt8195_mtk_main_path_data = {
	.path[DDP_MAJOR][0] = mt8195_mtk_ddp_main,
	.path_len[DDP_MAJOR][0] = ARRAY_SIZE(mt8195_mtk_ddp_main),
	.path_req_hrt[DDP_MAJOR][0] = true,
	.dual_path[0] = mt8195_mtk_ddp_dual_main,
	.dual_path_len[0] = ARRAY_SIZE(mt8195_mtk_ddp_dual_main),
	.wb_path[DDP_MAJOR] = mt8195_mtk_ddp_main_wb_path,
	.wb_path_len[DDP_MAJOR] = ARRAY_SIZE(mt8195_mtk_ddp_main_wb_path),
	//.path[DDP_MINOR][0] = mt8195_mtk_ddp_main_minor,
	//.path_len[DDP_MINOR][0] = ARRAY_SIZE(mt8195_mtk_ddp_main_minor),
	//.path_req_hrt[DDP_MINOR][0] = false,
	//.path[DDP_MINOR][1] = mt8195_mtk_ddp_main_minor_sub,
	//.path_len[DDP_MINOR][1] = ARRAY_SIZE(mt8195_mtk_ddp_main_minor_sub),
	//.path_req_hrt[DDP_MINOR][1] = true,
	.addon_data = mt8195_addon_main,
	.addon_data_dual = mt8195_addon_main_dual,
};

static const struct mtk_crtc_path_data mt8195_mtk_ext_path_data = {
	.path[DDP_MAJOR][0] = mt8195_mtk_ddp_ext,
	.path_len[DDP_MAJOR][0] = ARRAY_SIZE(mt8195_mtk_ddp_ext),
	.path_req_hrt[DDP_MAJOR][0] = true,
	.addon_data = mt8195_addon_ext,
};

static const struct mtk_crtc_path_data mt8195_mtk_third_path_data = {
	.path[DDP_MAJOR][0] = mt8195_mtk_ddp_third,
	.path_len[DDP_MAJOR][0] = ARRAY_SIZE(mt8195_mtk_ddp_third),
	.addon_data = mt8195_addon_ext,
};

static const struct mtk_crtc_path_data mt8188_mtk_main_path_data = {
	.path[DDP_MAJOR][0] = mt8188_mtk_ddp_main,
	.path_len[DDP_MAJOR][0] = ARRAY_SIZE(mt8188_mtk_ddp_main),
	.path_req_hrt[DDP_MAJOR][0] = true,
	.wb_path[DDP_MAJOR] = mt8188_mtk_ddp_main_wb_path,
	.wb_path_len[DDP_MAJOR] = ARRAY_SIZE(mt8188_mtk_ddp_main_wb_path),
	.path[DDP_MINOR][0] =  NULL,
	.path_len[DDP_MINOR][0] = 0,
	.path_req_hrt[DDP_MINOR][0] = false,
	.path[DDP_MINOR][1] = NULL,
	.path_len[DDP_MINOR][1] = 0,
	.path_req_hrt[DDP_MINOR][1] = false,
	.addon_data = mt8188_addon_main,
};
...
const struct mtk_session_mode_tb mt8195_mode_tb[MTK_DRM_SESSION_NUM] = {
	[MTK_DRM_SESSION_DL] = {
		.en = 1,
		.ddp_mode = {DDP_MAJOR, DDP_MAJOR, DDP_MAJOR},
	},
...
const struct mtk_session_mode_tb mt8188_mode_tb[MTK_DRM_SESSION_NUM] = {
	[MTK_DRM_SESSION_DL] = {
		.en = 1,
		.ddp_mode = {DDP_MAJOR, DDP_MAJOR, DDP_MAJOR},
	},
...
static const struct mtk_fake_eng_reg mt8195_fake_eng_reg[] = {
		{.CG_idx = 0, .CG_bit = 29, .share_port = true},
		{.CG_idx = 0, .CG_bit = 30, .share_port = true},
};

static const struct mtk_fake_eng_data mt8195_fake_eng_data = {
	.fake_eng_num =  ARRAY_SIZE(mt8195_fake_eng_reg),
	.fake_eng_reg = mt8195_fake_eng_reg,
};

static const struct mtk_fake_eng_reg mt8188_fake_eng_reg[] = {
		{.CG_idx = 0, .CG_bit = 29, .share_port = true},
		{.CG_idx = 0, .CG_bit = 30, .share_port = true},
};

static const struct mtk_fake_eng_data mt8188_fake_eng_data = {
	.fake_eng_num =  ARRAY_SIZE(mt8188_fake_eng_reg),
	.fake_eng_reg = mt8188_fake_eng_reg,
};
...
static const struct mtk_mmsys_driver_data mt8195_mmsys_driver_data = {
	.main_path_data = &mt8195_mtk_main_path_data,
	.ext_path_data = &mt8195_mtk_ext_path_data,
	.third_path_data = &mt8195_mtk_third_path_data,
	.fake_eng_data = &mt8195_fake_eng_data,
	.mmsys_id = MMSYS_MT8195,
	.mode_tb = mt8195_mode_tb,
	.sodi_config = mt8195_mtk_sodi_config,
};

static const struct mtk_mmsys_driver_data mt8188_mmsys_driver_data = {
	.mmsys_id = MMSYS_MT8188,
	.main_path_data = &mt8188_mtk_main_path_data,
	.ext_path_data = &mtk_ext_path_data,
#ifdef CONFIG_MTK_WFD_OVER_VDO1
	.third_path_data = &mtk_vrt_path_data,
#else
	.third_path_data = NULL,
#endif
	.fake_eng_data = &mt8188_fake_eng_data,
	.mode_tb = mt8188_mode_tb,
	.sodi_config = mt8188_mtk_sodi_config,
};
...
static void mtk_drm_get_top_clk(struct mtk_drm_private *priv)
{
	...
	if (!priv->top_clk) {
		dev_err(dev, "Can't allocate top_clk buffer\n");
		return;
	}

	priv->top_clk_related_sub_sys = devm_kmalloc_array(dev, priv->top_clk_num,
					   sizeof(*priv->top_clk_related_sub_sys), GFP_KERNEL);

	if (!priv->top_clk_related_sub_sys) {
		dev_err(dev, "Can't allocate top_clk_related_sub_sys buffer\n");
		return;
	}
	...
	for (i = 0; i < priv->top_clk_num; i++) {
		...
		/*Get related sub sys info*/
		if (of_property_read_u32_index(node, "clock-relation",
				i, &priv->top_clk_related_sub_sys[i]) < 0) {
			priv->top_clk_related_sub_sys[i] = MTK_DRM_DISP_TOP_CLOCK_ALL;
			DDPINFO("No sub sys related info for clk %d", i);
		}
	}
...
void mtk_drm_top_clk_prepare_enable(struct drm_device *drm, int crtc_id)
{
	...
	if (priv->power_dev[crtc_id]) {
		pm_runtime_get_sync(priv->power_dev[crtc_id]);
		DDPINFO("power on crtc:%d power domain", crtc_id);
	}
	...
	for (i = 0; i < priv->top_clk_num; i++) {
		...
		if (priv->top_clk_related_sub_sys[i] == crtc_id) {
			ret = clk_prepare_enable(priv->top_clk[i]);
			if (ret)
				DDPPR_ERR("top clk prepare enable failed:%d\n", i);
		}

		if (priv->top_clk_related_sub_sys[i] == MTK_DRM_DISP_TOP_CLOCK_ALL) {
			ret = clk_prepare_enable(priv->top_clk[i]);
			if (ret)
				DDPPR_ERR("top clk prepare enable failed:%d\n", i);
		}
	...
	if (priv->data->sodi_config && crtc_id == 0)
...
void mtk_drm_top_clk_disable_unprepare(struct drm_device *drm, int crtc_id)
{
	...
	if (priv->power_dev[crtc_id]) {
		pm_runtime_put_sync(priv->power_dev[crtc_id]);
		DDPINFO("power off crtc:%d power domain", crtc_id);
	}
}
	...
	if (chosen_node) {
		...
		if (videolfb_tag) {
			...
			*fps = videolfb_tag->fps;
			...
			DDPINFO("[DT][videolfb] videolfb_tag not found\n");
			prop = (void *)of_get_property(chosen_node,
...
#if (defined CONFIG_MTK_DPTX_SUPPORT)
		ret = mtk_drm_dp_get_info(dev, info);
#endif
	} else if (s_type == MTK_SESSION_MEMORY) {
		return ret;
	} else {
		DDPDBG("invalid session type:0x%08x\n", info->session_id);
		ret = -EINVAL;
	}
...
static int mtk_drm_kms_init(struct drm_device *drm)
{
	...
	/*Get Power Device*/
	for (i = 0; i < MAX_CRTC ; ++i) {
		if (private->power_node[i]) {
			pdev = of_find_device_by_node(private->power_node[i]);
			of_node_put(private->power_node[i]);
			private->power_node[i] = NULL;

			if (!pdev) {
				dev_err(drm->dev, "Waiting for display power device %d\n", i);
				return -EPROBE_DEFER;
			}
			private->power_dev[i] = &pdev->dev;
		}
	}
...
static const struct drm_ioctl_desc mtk_ioctls[] = {
...
#endif
#ifdef CONFIG_DRM_MEDIATEK_HDMI
	DRM_IOCTL_DEF_DRV(MTK_HDMI_ENABLE, mtk_drm_ioctl_enable_hdmi,
			DRM_UNLOCKED),
#endif
#ifdef CONFIG_MTK_DPTX_SUPPORT
	DRM_IOCTL_DEF_DRV(MTK_DP_ENABLE, mtk_drm_ioctl_enable_dp,
			  DRM_UNLOCKED),
#endif
#ifdef CONFIG_MTK_MML
	DRM_IOCTL_DEF_DRV(MTK_MML_GEM_SUBMIT, mtk_drm_ioctl_mml_gem_submit,
			  DRM_UNLOCKED | DRM_AUTH | DRM_RENDER_ALLOW),
#endif

#ifdef IFZERO // fix me: conflict with hdmi, ethdr
	...
	DRM_IOCTL_DEF_DRV(MTK_FACTORY_LCM_AUTO_TEST, mtk_drm_fm_lcm_auto_test,
			  DRM_UNLOCKED),
#endif
	DRM_IOCTL_DEF_DRV(MTK_ETHDR_SET_METADATA, mtk_drm_ioctl_set_hdr_metadata,
			  DRM_UNLOCKED),
	DRM_IOCTL_DEF_DRV(MTK_ETHDR_SET_HDR_ON_OFF, mtk_drm_ioctl_set_hdr_on_off,
			  DRM_UNLOCKED),
};
...
static int mtk_drm_bind(struct device *dev)
{
	...
	if (ret < 0) {
		DDPINFO("%s(): error: mtk_drm_kms_init\n", __func__);
		goto err_free;
	}

	ret = drm_dev_register(drm, 0);
	if (ret < 0) {
		DDPINFO("%s(): error: drm_dev_register\n", __func__);
		goto err_deinit;
	}
...
static const struct of_device_id mtk_ddp_comp_dt_ids[] = {
	...
	{.compatible = "mediatek,mt6983-disp-wdma",
	 .data = (void *)MTK_DISP_WDMA},
	{.compatible = "mediatek,mt8195-disp-wdma",
	 .data = (void *)MTK_DISP_WDMA},
	...
	{.compatible = "mediatek,mt6879-disp-pwm",
	 .data = (void *)MTK_DISP_PWM},
	{.compatible = "mediatek,mt8195-disp-pwm",
	 .data = (void *)MTK_DISP_PWM},
...
static int mtk_drm_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		DDPDBG("dispsys_num not found, set to 1\n", ret);
		dispsys_num = 1;
	}
	...
	if (IS_ERR(private->config_regs)) {
		ret = PTR_ERR(private->config_regs);
		DDPDBG("Failed to ioremap mmsys-config resource: %d\n", ret);
		return ret;
	}
	private->config_regs_pa = mem->start;

	DDPDBG("%s(): config_regs    = %p\n", __func__, private->config_regs);
	DDPDBG("%s(): config_regs_pa = %llx\n", __func__, private->config_regs_pa);

	if (dispsys_num <= 1) {
		DDPDBG("%s(): dispsys_num = %d, skip side display\n",
			__func__, dispsys_num);
		goto SKIP_SIDE_DISP;
	}
	...
	DDPDBG("%s(): side_config_regs    = %p\n",
		__func__, private->side_config_regs);
	DDPDBG("%s(): side_config_regs_pa = %llx\n",
		__func__, private->side_config_regs_pa);
...
SKIP_SIDE_DISP:
	...
	for_each_child_of_node(dev->of_node->parent, node) {
		...
		if (!of_device_is_available(node)) {
			DDPDBG("%s(): skip disabled component %s\n",
		...
		if (comp_type == MTK_DISP_POWER_DEV) {
			uint crtc_index;
			struct device_node *power_node = of_node_get(node);

			DDPINFO("%s line:%d, MTK_DISP_POWER_DEV\n", __func__, __LINE__);

			if (power_node) {
				if (!of_property_read_u32(power_node, "index", &crtc_index)) {
					private->power_node[crtc_index] = power_node;
					DDPINFO("getpower node for crtc:%d", crtc_index);
				}
			}
			continue;
		}
		...
		if (comp_type >= MTK_DDP_COMP_TYPE_MAX) {
			dev_err(dev, "invalid comp_type:%d\n", comp_type);
		} else if (comp_type == MTK_DISP_OVL     ||
			...
			comp_type == MTK_DPI             ||
#ifndef DRM_BYPASS_PQ
			comp_type == MTK_DISP_TDSHP      ||
			comp_type == MTK_DISP_CHIST      ||
			comp_type == MTK_DISP_C3D        ||
			comp_type == MTK_DISP_COLOR      ||
			comp_type == MTK_DISP_CCORR      ||
			comp_type == MTK_DISP_GAMMA      ||
			comp_type == MTK_DISP_AAL        ||
			comp_type == MTK_DISP_DITHER     ||
			comp_type == MTK_DISP_CM         ||
			comp_type == MTK_DISP_SPR        ||
			...
			dev_info(dev, "%s(): add component %s (type=%d, id=%d) to match\n",
				 __func__, node->full_name, comp_type, comp_id);
...
{
	if (mtk_drm_sys_suspend(dev) < 0)
		DDPINFO("mtk_drm_sys_suspend failed!!");
}

void mtk_drm_resume(struct device *dev)
{
	mtk_drm_sys_resume(dev);
}
#endif
...
static struct platform_driver *const mtk_drm_drivers[] = {
	...
	//&mtk_lvds_tx_driver,
#endif
	&mtk_pseudo_ovl_larb_driver,
	&mtk_disp_pseudo_ovl_driver,
	&mtk_ethdr_driver,
	&mtk_disp_merge_driver,
	&mtk_disp_power_dev_driver,
};
