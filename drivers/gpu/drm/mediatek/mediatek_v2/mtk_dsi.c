...
#define DSI_REG_BASE 0x1c008000
#define MIPITX_REG_BASE 0x11c80000
...
#define DSI_SHADOW_GROUP_EN 0xC04
#define DSI_SHADOW_SOURCE_SEL 0xC08

#define DSI_CMDQ_0 0xD00

#define CONFIG (0xff << 0)
#define SHORT_PACKET 0
#define LONG_PACKET 2
#define TYPE_COMMAND BIT(0)
...
#define VDO0_LCM_RST_B			0x50
#define VDO0_GLOBAL0_CG_CLR0		0x108
#define VDO0_GLOBAL0_CG_CLR1		0x118
#define VDO0_GLOBAL0_CG_CLR2		0x128

#define MMSYS_LCM0_RST_B BIT(0)
#define MMSYS_LCM1_RST_B BIT(1)
#endif
...
#define DSI_GERNERIC_SHORT_READ_RESPONSE_1 0x11
#define DSI_GERNERIC_SHORT_READ_RESPONSE_2 0x12
#define DSI_DCS_SHORT_READ_RESPONSE_1 0x21
#define DSI_DCS_SHORT_READ_RESPONSE_2 0x22
#define DSI_GERNERIC_LONG_READ_RESPONSE 0x1A
#define DSI_DCSC_LONG_READ_RESPONSE 0x1C
#define DSI_ACKNOWLEDGE_AND_ERROR 0x02
...
static void mtk_dsi_dphy_timconfig(struct mtk_dsi *dsi, void *handle)
{
	...
	writel(0x02000102, dsi->regs + DSI_PHY_TIMECON0);
	writel(0x010A0308, dsi->regs + DSI_PHY_TIMECON1);
	writel(0x02000100, dsi->regs + DSI_PHY_TIMECON2);
	writel(0x00010701, dsi->regs + DSI_PHY_TIMECON3);
	return;
#endif
...
#ifdef CONFIG_FPGA_EARLY_PORTING
static void mtk_dsi_mmsys_reset(struct mtk_dsi *dsi)
{
	DRM_DEBUG_DRIVER("\n");

	writel(0x0, dsi->vdosys_regs + VDO0_LCM_RST_B);

	usleep_range(1000, 1100);

	writel(0x3, dsi->vdosys_regs + VDO0_LCM_RST_B);
}

static void mtk_dsi_mmsys_clock_on(struct mtk_dsi *dsi)
{
	DRM_DEBUG_DRIVER("\n");

	writel(0xFFFFFFFF, dsi->vdosys_regs + VDO0_GLOBAL0_CG_CLR0);
	writel(0xFFFFFFFF, dsi->vdosys_regs + VDO0_GLOBAL0_CG_CLR1);
	writel(0xFFFFFFFF, dsi->vdosys_regs + VDO0_GLOBAL0_CG_CLR2);
}
#endif
...
static unsigned int mtk_dsi_default_rate(struct mtk_dsi *dsi)
{
	...
	if (priv && priv->data && (priv->data->mmsys_id == MMSYS_MT6983) && (dsi->d_rate != 0)) {
...
static int mtk_dsi_set_data_rate(struct mtk_dsi *dsi)
{
	...
	ret = clk_set_rate(dsi->hs_clk, mipi_tx_rate);
		if (ret < 0)
			dev_info(dsi->dev, "Failed to set data rate: %d\n", ret);
...
#ifdef CONFIG_FPGA_EARLY_PORTING
	writel(FORCE_COMMIT | DSI_READ_WORKING, dsi->regs + dsi->driver_data->reg_shadow_ofs);
	DDPINFO("dsi shadow 0x%x\n", readl(dsi->regs + dsi->driver_data->reg_shadow_ofs));

	writel(0x0, dsi->regs + DSI_SHADOW_GROUP_EN);
	DDPINFO("dsi shadow 0x%x\n", readl(dsi->regs + DSI_SHADOW_GROUP_EN));
#endif
...
int mtk_dsi_bist_pattern_test(struct mtk_ddp_comp *comp, unsigned int color)
{
	...
	writel(color, dsi->regs + DSI_SELF_PAT_CON0);
	pr_notice("DSI_BIST_Pattern_Test SELF_PAT_MODE 0x%x\n",
		readl(dsi->regs + DSI_SELF_PAT_CON0));

	if ((readl(dsi->regs + DSI_MODE_CTRL) & MODE_FLD_REG_MODE_CON) == CMD_MODE) {
		struct DSI_T0_INS t0;

		t0.CONFG = TYPE_COMMAND | HSTX;
		t0.Data_ID = DSI_DCS_LONG_PACKET_ID;
		t0.Data0 = DCS_WRITE_MEMORY_START;
		t0.Data1 = 0;
		temp =&t0;

		writel(AS_UINT32(temp), dsi->regs + DSI_CMDQ_0);
		writel(1, dsi->regs + DSI_CMDQ_SIZE);
...
#ifdef CONFIG_FPGA_EARLY_PORTING
	mtk_dsi_mmsys_clock_on(dsi);
	mtk_dsi_mmsys_reset(dsi);
#endif
...
static ssize_t mtk_dsi_show_info(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct attribute *pattr = &(attr->attr);
	int len = 0;
	int temp_len = 0;
	void *reg_base = ioremap(DSI_REG_BASE, 0x4);
	int k;
	unsigned int reg_val;

	if (strcmp(pattr->name, "register") == 0) {
		SPRINTF_DEV_ATTR("--dump dsi register--\n");

		if (DISP_REG_GET_FIELD(MODE_FLD_REG_MODE_CON,
				   reg_base + DSI_MODE_CTRL)) {
			/* VDO mode */
			reg_val = (readl(reg_base + DSI_STATE_DBG7)) & 0xfff;
			SPRINTF_DEV_ATTR("state7(vdo mode):%s\n",
				mtk_dsi_vdo_mode_parse_state(reg_val));
		} else {
			reg_val = (readl(reg_base + DSI_STATE_DBG6)) & 0xffff;
			SPRINTF_DEV_ATTR("state6(cmd mode):%s\n",
				mtk_dsi_cmd_mode_parse_state(reg_val));
		}
		reg_val = (readl(reg_base + DSI_STATE_DBG8)) & 0x3fff;
		SPRINTF_DEV_ATTR("state8 WORD_COUNTER(cmd mode):%u\n", reg_val);
		reg_val = (readl(reg_base + DSI_STATE_DBG9)) & 0x3fffff;
		SPRINTF_DEV_ATTR("state9 LINE_COUNTER(cmd mode):%u\n", reg_val);

		SPRINTF_DEV_ATTR("== dsi REGS ==\n");
		for (k = 0; k < 0x200; k += 16) {
			SPRINTF_DEV_ATTR("0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n", k,
				readl(reg_base + k),
				readl(reg_base + k + 0x4),
				readl(reg_base + k + 0x8),
				readl(reg_base + k + 0xc));
		}

		SPRINTF_DEV_ATTR("- DSI CMD REGS -\n");
		for (k = 0; k < 32; k += 16) {
			SPRINTF_DEV_ATTR("0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n", k + 0xD00,
				readl(reg_base + DSI_CMDQ_0 + k),
				readl(reg_base + DSI_CMDQ_0 + k + 0x4),
				readl(reg_base + DSI_CMDQ_0 + k + 0x8),
				readl(reg_base + DSI_CMDQ_0 + k + 0xc));
		}
		reg_base = ioremap(MIPITX_REG_BASE, 0x4);
		SPRINTF_DEV_ATTR("== MIPI REGS ==\n");
		for (k = 0; k < 0x120; k += 16) {
			SPRINTF_DEV_ATTR("0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n", k,
				readl(reg_base + k),
				readl(reg_base + k + 0x4),
				readl(reg_base + k + 0x8),
...
static DEVICE_ATTR(register, 0444, mtk_dsi_show_info, NULL);
static struct device_attribute *dsi_attr_list[] = {
	&dev_attr_register,
};

static int mtk_dsi_core_create_attr(struct device *dev)
{
	int err = 0;

	if (!dev)
		return -EINVAL;
	err = device_create_file(dev, dsi_attr_list[0]);
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	struct mtk_panel_params *cur_panel_params;
...
#ifndef CONFIG_MTK_DISP_NO_LK
static int mtk_dsi_clk_enable(struct mtk_dsi *dsi)
{
	int ret;

	ret = clk_prepare_enable(dsi->engine_clk);
	if (ret < 0) {
		pr_info("Failed to enable engine clock: %d\n", ret);
		return ret;
	}

	ret = clk_prepare_enable(dsi->digital_clk);
	if (ret < 0) {
		pr_info("Failed to enable digital clock: %d\n", ret);
		return ret;
	}

	ret = clk_prepare_enable(dsi->hs_clk);
	if (ret < 0) {
		pr_info("Failed to enable hs_clk clock: %d\n", ret);
		return ret;
	}
	return ret;
}

static void mtk_dsi_clk_disable(struct mtk_dsi *dsi)
{
	clk_disable_unprepare(dsi->engine_clk);
	clk_disable_unprepare(dsi->digital_clk);
				...
				unsigned int para_size)
{
	...
	if (!mtk_dsi_is_cmd_mode(comp)) {
		...
		mtk_disp_mutex_trigger(comp->mtk_crtc, comp->mtk_crtc->mutex[0], handle);
			...
			struct mtk_ddic_dsi_msg *cmd_msg)
{
	...
	if (dsi_mode == 0) { /* CMD mode LP */
		...
		mtk_disp_mutex_trigger(comp->mtk_crtc, comp->mtk_crtc->mutex[0], cmdq_handle);
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	if (fps_chg_index & DYNFPS_DSI_MIPI_CLK) {
		...
		if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_MSYNC2_0)
			&& dsi->ext && dsi->ext->params && dsi->ext->params->msync2_enable) {
			if (state->prop_val[CRTC_PROP_MSYNC2_0_ENABLE] != 0) {
				DDPDBG("[Msync]%s,%d\n", __func__, __LINE__);
...
static irqreturn_t dsi_te1_irq_handler(int irq, void *data)
{
	...
	/* ESD check */
	if (mtk_crtc && mtk_crtc->esd_ctx && atomic_read(&mtk_crtc->d_te.esd_te1_en) == 1) {
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case DSI_VFP_DEFAULT_MODE:
	{
		...
		if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_MSYNC2_0)
			&& panel_ext && panel_ext->params && panel_ext->params->msync2_enable) {
			if (state->prop_val[CRTC_PROP_MSYNC2_0_ENABLE] != 0) {
				DDPDBG("[Msync]%s,%d\n", __func__, __LINE__);
				if (dsi->mipi_hopping_sta &&
					panel_ext->params->dyn.max_vfp_for_msync_dyn)
	...
	case DSI_VFP_EARLYSTOP:
	{
		...
		DDPDBG("[Msync] VFP_EARLYSTOP = 0x%x\n", value);
	...
	case DUAL_TE_INIT:
	{
		...
		break;
	case DSI_CLK_DISABLE:
	{
		mtk_dsi_clk_disable(dsi);
	}
		break;
	default:
		break;
...
static int mtk_dsi_bind(struct device *dev, struct device *master, void *data)
{
	...
	if (dsi->is_slave) {
		dev_info(dev, "DSI is slave\n");
		return 0;
	}

	prev_endpoint = NULL;
	do {
		endpoint = of_graph_get_next_endpoint(dsi->dev->of_node, prev_endpoint);
		if (endpoint) {
			...
			if (of_device_is_available(remote_node))
				pr_info("remote_node %p is state = ok\n", remote_node);
			else
				pr_err("remote_node %p is state = NG\n", remote_node);

			dsi->bridge = of_drm_find_bridge(remote_node);
			dsi->panel = of_drm_find_panel(remote_node);
			if ((dsi->panel == ERR_PTR(-ENODEV)) || (dsi->panel == ERR_PTR(-EPROBE_DEFER)))
				dsi->panel = NULL;
...
static const struct of_device_id mtk_dsi_of_match[] = {
	...
	{.compatible = "mediatek,mt8195-dsi", .data = &mt6885_dsi_driver_data},
...
static int mtk_dsi_probe(struct platform_device *pdev)
{
	...
	ret = mtk_dsi_core_create_attr(&(pdev->dev));
	if (ret) {
		pr_notice(" create device attribute fail!\n");
		goto error;
	}
...
#ifdef CONFIG_FPGA_EARLY_PORTING
	dsi->vdosys_regs = of_iomap(dev->of_node, 1);
	DRM_DEBUG_DRIVER("vdosys regs 0x%p\n",
		dsi->vdosys_regs);
	if (IS_ERR(dsi->vdosys_regs)) {
		ret = PTR_ERR(dsi->vdosys_regs);
		dev_info(dev, "Failed to iomap vdosys regs: %d\n", ret);
		goto error;
	}
#endif
		...
		unsigned char force_update)
{
	...
	if (packet_type == DSI_GERNERIC_LONG_READ_RESPONSE
		|| packet_type == DSI_DCSC_LONG_READ_RESPONSE) {
	...
	} else if (packet_type == DSI_GERNERIC_SHORT_READ_RESPONSE_1
		|| packet_type == DSI_DCS_SHORT_READ_RESPONSE_1) {
		recv_data_cnt = 1;
		memcpy((void *)msg.rx_buf,
			(void *)&read_data[0].byte1, recv_data_cnt);

	} else if (packet_type == DSI_GERNERIC_SHORT_READ_RESPONSE_2
		|| packet_type == DSI_DCS_SHORT_READ_RESPONSE_2) {
		recv_data_cnt = 2;
		if (recv_data_cnt > msg.rx_len)
			recv_data_cnt = msg.rx_len;
	...
	} else if (packet_type == DSI_ACKNOWLEDGE_AND_ERROR) {
		DDPPR_ERR("read return type is 0x02, re-read\n");
	} else {
		DDPPR_ERR("read return type is non-recognite, type = 0x%x\n",
				packet_type);
