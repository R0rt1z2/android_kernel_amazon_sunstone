	...
	struct cmdq_pkt *handle, unsigned int cmd, void *data)
{
	...
	case WRITE_REG:
	{
		...
		if (comp->mtk_crtc->is_dual_pipe) {
			...
			cmdq_pkt_write(handle, comp->cmdq_base,
				pa, wParams->val, wParams->mask);
			DDPINFO("dual pipe pa:0x%x(va:0x%lx) = 0x%x (0x%x) comp->regs_pa:(0x%lx)\n",
...
static const struct mtk_disp_color_data mt8188_color_driver_data = {
	.color_offset = DISP_COLOR_START_MT8188,
	.support_color21 = true,
	.support_color30 = true,
	.reg_table = {0x1c003000, 0x1c004000, 0x1c005000,
			0x1c006000, 0x1c007000},
	.color_window = 0x40185E57,
	.support_shadow = false,
	.need_bypass_shadow = false,
};
