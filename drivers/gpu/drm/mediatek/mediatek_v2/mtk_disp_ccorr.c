	...
	struct cmdq_pkt *handle, int32_t matrix[16], int32_t hint, bool fte_flag)
{
	...
	if (g_disp_ccorr_coef[id] == NULL) {
		g_disp_ccorr_coef[id] = kmalloc(sizeof(struct DRM_DISP_CCORR_COEF_T), GFP_KERNEL);
		for (i = 0; i < 3; i++) {
			for (j = 0; j < 3; j++) {
				/* Copy Color Matrix */
				g_disp_ccorr_coef[id]->coef[i][j] = g_ccorr_color_matrix[i][j];
			}
		}
	}
...
static const struct mtk_disp_ccorr_data mt8188_ccorr_driver_data = {
	.support_shadow = false,
	.need_bypass_shadow = true,
};
