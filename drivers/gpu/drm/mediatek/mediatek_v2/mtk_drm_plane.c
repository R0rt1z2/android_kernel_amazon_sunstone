...
static const u32 formats[] = {
	...
	DRM_FORMAT_UYVY,     DRM_FORMAT_VYUY,     DRM_FORMAT_P010,
...
mtk_plane_duplicate_state(struct drm_plane *plane)
{
	...
	for (i = 0; i < PLANE_PROP_MAX; i++) {
		state->prop_val[i] = old_state->prop_val[i];
		DDPDBG("%s, %d, dup plane prop val\n",
			__func__, __LINE__);
	}
				    ...
				    struct drm_plane_state *old_state)
{
	...
	int i, src_x, src_y, src_w, src_h, dst_x, dst_y, dst_w, dst_h;
	...
	src_x = plane->state->src.x1;
	src_y = plane->state->src.y1;
	...
	if (crtc_index != CRTC_ID_EXTERNAL && (src_w < dst_w || src_h < dst_h)) {
#ifdef IF_ZERO // modify dst x,y for rsz between case
	...
	state->pending.offset1 = fb->offsets[1];
	...
	state->pending.src_w = src_w;
	state->pending.src_h = src_h;
