...
//gpu stall counter
#if defined(CONFIG_GPU_MT8188) || defined(CONFIG_GPU_MT6853)
...
static int _mtk_mfg_update_counter(void)
{
	...
	if (ret != PMU_NG) {
		...
		gpu_freq = gpufreq_get_cur_freq(TARGET_DEFAULT);
#else
...
static int mali_get_gpu_pmu_init(struct GPU_PMU *pmus,
