...
static unsigned int g_segment_table;
...
EXPORT_SYMBOL_GPL(mt_gpufreq_get_shader_present);
...
void mt_gpufreq_dump_infra_status(void)
{
	...
	gpufreq_pr_info("[GPU_DFD] freq=%d(%d) vgpu=%d(%d) vsram_gpu=%d(%d)\n",
	...
	if (g_sleep) {
		...
		gpufreq_pr_info("[GPU_DFD] pwr info 0x%x:0x%08x %08x\n",
			0x10006000 + 0x174,
			readl(g_sleep + 0x174),
			readl(g_sleep + 0x178));
	}
	...
	// 0x10000000
	if (g_topckgen) {
		gpufreq_pr_info("[GPU_DFD] CLK_CFG_5 0x%x:0x%08x\n",
			0x10000000 + 0x5C,
			readl(g_topckgen + 0x5C));
	}

	// 0x1000C000
	if (g_apmixed_base) {
		gpufreq_pr_info("[GPU_DFD] MFGPLL_CON* 0x%x:0x%08x %08x\n",
			0x1000C000 + 0x340,
			readl(g_apmixed_base + 0x340),
			readl(g_apmixed_base + 0x344));
	}

	// 0x13FBF000
	if (g_mfg_base && g_cg_on) {
		if (1) /*(g_dfd_force_dump == 1 || g_dfd_force_dump == 3)*/ {
			start = 0x000;
			...
			unsigned int limit_idx, enum mt_gpufreq_kicker kicker)
{
	...
	if (limit_idx > g_segment_min_opp_idx ||
		...
		ret |= (1 << 1);
		gpufreq_pr_info("out of segment opp range, %d (%d ~ %d)\n",
				limit_idx, g_segment_max_opp_idx, g_segment_min_opp_idx);
	}
...
static void mt_gpufreq_mtcmos_control(enum mt_power_state power)
{
	...
	if (power == POWER_ON) {
		...
		if (shader_present & MFG4_SHADER_STACK4) {
			ret = pm_runtime_get_sync(g_mtcmos->pm_domain_devs[2]);
			if (ret != 0)
				gpufreq_pr_info("failed enable mtcmos_mfg5 (%d)\n", ret);
		}

	} else {
		if (shader_present & MFG4_SHADER_STACK4) {
			ret = pm_runtime_put_sync(g_mtcmos->pm_domain_devs[2]);
			if (ret != 0)
				gpufreq_pr_info("failed disable mtcmos_mfg6 (%d)\n", ret);
		}
...
#if MT_GPUFREQ_DFD_ENABLE
	/*
	 * MFG_MISC_CON[19:19] = mfg_dfd_triggered
	 * MFG_MISC_CON[18:18] = mfg_dfd_complete
	 */
...
void mt_gpufreq_devfreq_target(unsigned int limited_freq)
{
	...
	mt_gpufreq_target(limited_idx, KIR_POLICY);
}
...
static void __mt_gpufreq_clock_switch(unsigned int freq_new)
{
	...
	if (ret)
		gpufreq_pr_info("@%s:failed set target[%d] erro[%d]\n",
		...
		unsigned int idx, unsigned int freq, unsigned int volt)
{
	...
	gpufreq_pr_debug("idx: %d, dynamic: %d, leakage: %d, total: %d\n",
...
static int __mt_gpufreq_init_table(struct platform_device *pdev)
{
	unsigned int i;
	struct opp_table_info *opp_table = NULL;

#if IS_ENABLED(CONFIG_MTK_DEVINFO)
	const char *efuse_field[EFUSE_FIELD_CNT] = {
	"segment_table0", "segment_table1", "segment_table2"};
	struct nvmem_cell *efuse_cell[EFUSE_FIELD_CNT];
	unsigned int *efuse_buf[EFUSE_FIELD_CNT] = {0};
	size_t efuse_len[EFUSE_FIELD_CNT] = {0};
	int j;
#endif

	g_segment_table = SEGMENT_TABLE_1;
	g_segment_id = MT8188_SEGMENT;
	g_shader_present = MT_GPU_SHADER_PRESENT_3;

#if IS_ENABLED(CONFIG_MTK_DEVINFO)
	for (j = 0; j < EFUSE_FIELD_CNT; j++) {
		efuse_cell[j] = nvmem_cell_get(&pdev->dev, efuse_field[j]);
		if (IS_ERR(efuse_cell[j])) {
			gpufreq_pr_info("@%s: cannot get efuse_cell\n", __func__);
			return PTR_ERR(efuse_cell[j]);
		}

		efuse_buf[j] =
			(unsigned int *)nvmem_cell_read(efuse_cell[j], &efuse_len[j]);
		nvmem_cell_put(efuse_cell[j]);
		if (IS_ERR(efuse_buf[j])) {
			gpufreq_pr_info("@%s: cannot get efuse_buf\n", __func__);
			return PTR_ERR(efuse_buf[j]);
		}
		gpufreq_pr_info("@%s: get efuse_len[%d]=%zu,efuse_buf[%d]=0x%x.\n",
			__func__, j, efuse_len[j], j, *efuse_buf[j]);
	}

	/* 0x11F2_0040[22] & 0x11F2_004C[11]  */
	if (((*efuse_buf[0] & 0x00400000) == 0x00400000)
		|| ((*efuse_buf[1] & 0x00000800) == 0x00000800)) {
		g_shader_present = MT_GPU_SHADER_PRESENT_2;
	} else {
		g_shader_present = MT_GPU_SHADER_PRESENT_3;
	}

	/* 0x11F2 0580[7:5] = 3'b011 */
	if ((*efuse_buf[2] & 0x000000E0) == 0x000000060)
		g_segment_table = SEGMENT_TABLE_3;
	/* 0x11F2 0580[7:5] = 3'b100 */
	else if ((*efuse_buf[2] & 0x000000E0) == 0x00000080)
		g_segment_table = SEGMENT_TABLE_2;
	else
		g_segment_table = SEGMENT_TABLE_1;
	...
	if (g_segment_table == SEGMENT_TABLE_3)
		opp_table = g_opp_table_segment_3;
	else if (g_segment_table == SEGMENT_TABLE_2)
		opp_table = g_opp_table_segment_2;
	else
		opp_table = g_opp_table_segment_1;

	if (g_segment_id == MT8188_SEGMENT)
...
static int __mt_gpufreq_init_mtcmos(struct platform_device *pdev)
{
	...
	if (g_mtcmos->num_domains != GPU_CORE_NUM) {
		gpufreq_pr_info("@%s: Incorrect number of power domains: %d provided, %d needed\n",
			__func__, g_mtcmos->num_domains, GPU_CORE_NUM);
		ret = -EINVAL;
		goto err;
	}

	if (mt_gpufreq_get_shader_present() == MT_GPU_SHADER_PRESENT_2)
		num_domains = 2;
...
static int __mt_gpufreq_init_pmic(struct platform_device *pdev)
{
	...
	g_pmic->reg_vgpu =
			devm_regulator_get(&pdev->dev, "vgpu");
	...
	g_pmic->reg_vsram_gpu =
			devm_regulator_get(&pdev->dev, "vsram_gpu");
...
{
	// 0x1000C000, MFGPLL is from APMIXED and its parent clock is from XTAL
	g_apmixed_base = __mt_gpufreq_of_ioremap("mediatek,mt8188-apmixedsys", 0);
	if (!g_apmixed_base) {
	...
	// 0x13FBF000
	g_mfg_base = __mt_gpufreq_of_ioremap("mediatek,mt8188-mfgcfg", 0);
	if (!g_mfg_base) {
	...
	// 0x10006000
	g_sleep = __mt_gpufreq_of_ioremap("mediatek,mt8188-sleep", 0);
	if (!g_sleep) {
	...
	// 0x10000000
	g_topckgen = __mt_gpufreq_of_ioremap("mediatek,mt8188-topckgen", 0);
	if (!g_topckgen) {
	...
	// 0x10001000
	g_infracfg_ao = __mt_gpufreq_of_ioremap("mediatek,mt8188-infracfg_ao", 0);
	if (!g_infracfg_ao) {
...
static int __mt_gpufreq_pdrv_probe(struct platform_device *pdev)
{
	...
	/* init opp table */
	ret = __mt_gpufreq_init_table(pdev);
	if (ret)
		return ret;
...
#if MT_GPUFREQ_DFD_ENABLE
	/* if dfd is triggered, power off BUCK to clear it */
	if (__mt_gpufreq_is_dfd_triggered()) {
...
static int __init __mt_gpufreq_init(void)
{
	...
	gpufreq_pr_info("start to initialize gpufreq driver\n");
