...
static int vcu_sec_handle_get(struct mtk_vcu *vcu, unsigned long arg)
{
	int ret;
	unsigned char *user_data_addr = NULL;
	...
	mtk_vcu_get_dma_addr(vcu->dev, sec_handle_data.fd,
...
static int vcu_gce_get_inst_id(u64 gce_handle, u32 core_id)
{
	int i, temp;
	...
	for (i = 0; i < VCODEC_INST_MAX; i++) {
		if (vcu_ptr->gce_info[i].user_hdl == gce_handle) {
			temp = atomic_read(&vcu_ptr->gce_info[i].flush_done[core_id]);
			mutex_unlock(&vcu_ptr->vcu_share);
	...
	for (i = 0; i < VCODEC_INST_MAX; i++) {
		if (vcu_ptr->gce_info[i].v4l2_ctx == ctx) {
			...
			for (j = 0; j < GCE_THNUM_MAX; j++) {
				temp2 = atomic_read(&vcu_ptr->gce_info[i].flush_done[j]);
				atomic_set(&vcu_ptr->gce_info[i].flush_done[j], 0);
			}
...
static void vcu_gce_flush_callback(struct cmdq_cb_data data)
{
	...
	vcu = buff->vcu_ptr;
	j = vcu_gce_get_inst_id(buff->cmdq_buff.gce_handle, core_id);
	...
	struct mtk_vcu_queue *q, unsigned long arg)
{
	...
	if (buff.cmdq_buff.codec_type >= VCU_CODEC_MAX ||
		...
		pr_info("[VCU] %s invalid core(th) id %u\n",
			__func__, core_id);
		mutex_unlock(&vcu->gce_cmds_mutex[codec_type]);
		return -EINVAL;
	}
	...
	if (cl == NULL) {
		pr_info("[VCU] %s gce thread is null id %u type %u\n",
			__func__, core_id,
	...
	gce_idx = vcu_gce_get_inst_id(buff.cmdq_buff.gce_handle, core_id);
	...
	if (buff.cmdq_buff.codec_type == VCU_VENC &&
		...
		while (lock != 0) {
			lock = vcu->cbf.enc_lock(vcu->gce_info[gce_idx].v4l2_ctx, core_id,
	...
	pr_debug("[VCU][%u] %s: buff %p type %u cnt %u order %u hndl %llx %d %d\n",
...
static int vcu_wait_gce_callback(struct mtk_vcu *vcu, unsigned long arg)
{
	...
	if (obj.codec_type >= VCU_CODEC_MAX ||
		obj.core_id >= vcu->gce_th_num[obj.codec_type]) {
		pr_info("[VCU] %s invalid core(th) id %u\n",
	...
	vcu_dbg_log("[VCU] %s: type %u handle %llx\n",
	 ...
	 */
	j = vcu_gce_get_inst_id(obj.gce_handle, core_id);
...
static int mtk_vcu_release(struct inode *inode, struct file *file)
{
	...
	if (vcu_queue == NULL) {
		pr_info("%s vcu_queue is null\n", __func__);
		return 0;
	}

	vcu_dev = (struct mtk_vcu *)vcu_queue->vcu;
	if (vcu_queue)
		mtk_vcu_mem_release(vcu_queue, vcu_dev->clt_venc[0]);
...
static int mtk_vcu_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	vcu_dev = (struct mtk_vcu *)vcu_queue->vcu;
	vcu_dbg_log("[VCU] %s vma->start 0x%lx, end 0x%lx, pgoff 0x%lx, pa_start:0x%lx\n",
		 __func__, vma->vm_start, vma->vm_end, vma->vm_pgoff, pa_start);
				   ...
				   unsigned long arg)
{
	...
	case VCU_GET_SEC_HANDLE:
		ret = vcu_sec_handle_get(vcu_dev, arg);
		break;
	default:
...
static int mtk_vcu_probe(struct platform_device *pdev)
{
	...
	dev_dbg(dev, "[VCU] GCE clt_base %p clt_vdec %d %p %p clt_venc %d %p %p sec %p %pdev %p",
