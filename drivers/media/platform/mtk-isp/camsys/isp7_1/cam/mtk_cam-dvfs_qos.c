...
void mtk_cam_dvfs_update_clk(struct mtk_cam_device *cam)
{
	...
	if (dvfs->clklv_num) {
		...
		if (dvfs->reg_vmm) {
			...
			err = isp_direct_set_voltage(dvfs->reg_vmm,
...
#endif
			if (err) {
				dev_dbg(cam->dev, "[%s] adjust voltage fail\n", __func__);
				return;
			}
		}
	}
...
void mtk_cam_dvfs_init(struct mtk_cam_device *cam)
{
	...
	for (i = 0; i < dvfs_info->clklv_num; i++) {
		dev_dbg(cam->dev, "[%s] idx=%d, clk=%d volt=%d\n",
	...
	of_property_for_each_string(
		...
		if (num_clksrc >= MAX_CAM_OPP_STEP) {
			dev_dbg(dev, "Too many items in clk_src array\n");
			return;
		}
		dev_dbg(dev, "clksrc name(%s)\n", clksrc_name);
...
void mtk_cam_qos_bw_calc(struct mtk_cam_ctx *ctx, unsigned long raw_dmas)
{
	...
	dev_dbg(cam->dev, "[%s] engine_id(%d) enable_dmas(0x%lx) raw_dmas(0x%lx), updated_raw_dmas(0x%lx)\n",
	...
	if (ctx->sensor) {
		...
		dev_dbg(cam->dev, "[%s] FPS:%lu/%lu:%lu, H:%lu, VB:%lu\n",
			 __func__, fi.interval.denominator, fi.interval.numerator,
			 fps, height, vblank);
	} else {
		dev_dbg(cam->dev, "[%s] error: ctx->sensor is null\n",
			__func__);
		return;
	}
	...
	for (i = 0; i < MTKCAM_IPI_RAW_ID_MAX; i++) {
		...
		case MTKCAM_IPI_RAW_IMGO:
			...
			if (unlikely(debug_mmqos))
				dev_dbg(cam->dev, "[%16s] qos_idx:%2d ipifmt/bits/plane/w/h : %2d/%2d/%d/%5d/%5d BW(B/s)(avg:%lu,peak:%lu)\n",
		...
		case MTKCAM_IPI_RAW_META_STATS_CFG:
			...
			dvfs_info->qos_bw_avg[qos_port_id] =
				dvfs_info->qos_bw_peak[qos_port_id] = PBW_MB_s;
			if (unlikely(debug_mmqos))
				dev_dbg(cam->dev, "[%16s] qos_idx:%2d BW(B/s):%lu\n",
				  raw_mmqos->port[qos_port_id % raw_qos_port_num],
	...
	for (i = 0; i < ctx->used_mraw_num; i++) {
		for (j = MTKCAM_IPI_MRAW_META_STATS_CFG; j < MTKCAM_IPI_MRAW_ID_MAX; j++) {
			...
			case MTKCAM_IPI_MRAW_META_STATS_CFG:
				...
				if (unlikely(debug_mmqos))
					dev_dbg(cam->dev, "[%16s] qos_idx:%2d BW(B/s)(avg:%lu,peak:%lu)\n",
	...
	for (i = 0; i < raw_qos_port_num; i++) {
		qos_port_id = engine_id * raw_qos_port_num + i;
		if (unlikely(debug_mmqos))
			dev_dbg(cam->dev, "[%s] port idx/name:%2d/%16s BW(kB/s)(avg:%lu,peak:%lu)\n",
	...
	for (i = 0; i < MTK_CAM_SV_PORT_NUM; i++) {
		if (dvfs_info->sv_qos_bw_avg[i] != 0) {
			if (unlikely(debug_mmqos))
				dev_dbg(cam->dev, "[%s] port idx:%2d BW(kB/s)(avg:%lu,peak:%lu)\n",
				  __func__, i,
...
void mtk_cam_qos_init(struct mtk_cam_device *cam)
{
	...
	for (raw_num = 0; raw_num < cam->num_raw_drivers; raw_num++) {
		...
		dev_dbg(raw_dev->dev, "[%s] raw_engine_id=%d, port_num=%d\n",
	...
	for (engine_id = CAMSV_START; engine_id < CAMSV_END; engine_id++) {
		...
		dev_dbg(sv_dev->dev, "[%s] sv_engine_id=%d, port_num=%d\n",
			 __func__, engine_id, i);
	}
#ifdef FULL_CAM_SUBDEV
	i = 0;
	for (engine_id = MRAW_START; engine_id < MRAW_END; engine_id++) {
		...
		dev_dbg(mraw_dev->dev, "[%s] mraw_engine_id=%d, port_num=%d\n",
			 __func__, engine_id, i);
	}
#endif
#endif
...
void mtk_cam_qos_bw_reset(struct mtk_cam_ctx *ctx, unsigned int enabled_sv)
{
	...
	dev_dbg(cam->dev, "[%s] enabled_raw(%d),used_sv_num(%d),used_mraw_num(%d)\n",
