...
static bool mtk_cam_req_frame_sync_start(struct mtk_cam_request *req)
{
	...
	if (ctx_cnt > 1) {  /* multi sensor case */
		...
		if (synced_cnt < 2) {
			...
			dev_dbg(cam->dev, "%s:%s: sensor is not ready\n",
...
struct mtk_camsv_device *get_hdr_sv_dev(struct mtk_cam_ctx *ctx, int exp_order)
{
	...
	for (i = MTKCAM_SUBDEV_CAMSV_END - 1; i >= MTKCAM_SUBDEV_CAMSV_START; i--) {
		if (ctx->pipe->enabled_raw & (1 << i)) {
			...
			if (dev_sv == NULL) {
				dev_dbg(ctx->cam->dev, "[%s] camsv device not found\n", __func__);
				continue;
			}
				   ...
				   struct mtk_cam_request_stream_data *s_data)
{
	...
	if (mtk_cam_is_stagger(ctx)) {
		...
		for (i = MTKCAM_SUBDEV_CAMSV_START ; i < MTKCAM_SUBDEV_CAMSV_END ; i++) {
			...
			if (src_pad_idx == -1) {
				dev_dbg(cam->dev, "camsv pipeline hwcap not found\n");
				ret = -EINVAL;
				goto fail_switch_stop;
			}
...
void mtk_cam_req_seninf_change(struct mtk_cam_request *req)
{
	...
	dev_dbg(cam->dev, "%s, req->ctx_used:0x%x, req->ctx_link_update:0x%x\n",
	...
	for (i = 0; i < cam->max_stream_num; i++) {
		if (req->ctx_used & (1 << i) && (req->ctx_link_update & (1 << i))) {
			...
			dev_dbg(cam->dev, "%s: pipe(%d): switch seninf: %s--> %s\n",
			...
			dev_dbg(cam->dev, "%s: pipe(%d): update BW for %s\n",
	...
	for (i = 0; i < cam->max_stream_num; i++) {
		if (req->ctx_used & (1 << i) && req->ctx_link_update & (1 << i)) {
			...
			dev_dbg(cam->dev, "%s: pipe(%d): Enable VF of raw:%d\n",
				 __func__, stream_id, raw_dev->id);

			/* Enable CMOS */
			dev_dbg(raw_dev->dev, "enable CMOS\n");
			...
			dev_dbg(raw_dev->dev, "%s: stream off seninf:%s\n",
		...
		struct mtk_cam_request_stream_data *req_stream_data)
{
	...
	if (!ctx->sensor) {
		dev_dbg(cam->dev, "%s: ctx(%d): no sensor found\n",
			__func__, ctx->stream_id);
	} else {
		...
		struct mtk_cam_ctx *ctx, struct mtk_cam_request_stream_data *req_stream_data)
{
	...
	if (type != EXPOSURE_CHANGE_NONE && config_exposure_num == 3) {
		...
		if (sv_main_id < MTKCAM_SUBDEV_CAMSV_START) {
			dev_info(ctx->cam->dev,
			"[%s] Failed to get main sv pipe id\n", __func__);
			return -EINVAL;
		}
		sv_sub_id = get_sub_sv_pipe_id(ctx->cam, ctx->pipe->enabled_raw);
		if (sv_sub_id < MTKCAM_SUBDEV_CAMSV_START) {
			dev_info(ctx->cam->dev,
			"[%s] Failed to get sub sv pipe id\n", __func__);
			return -EINVAL;
		}
		switch (type) {
...
static int mtk_cam_hdr_switch_toggle(struct mtk_cam_ctx *ctx, int raw_feature)
{
	...
	raw_dev = get_master_raw_dev(ctx->cam, ctx->pipe);
	sv_main_id = get_main_sv_pipe_id(ctx->cam, ctx->pipe->enabled_raw);
	if (sv_main_id < MTKCAM_PIPE_CAMSV_0) {
		dev_dbg(ctx->cam->dev, "%s Failed to get sv main pipe\n",
			 __func__);
		return 0;
	}
	...
	if (raw_feature == STAGGER_3_EXPOSURE_LE_NE_SE ||
		...
		sv_sub_id = get_sub_sv_pipe_id(ctx->cam, ctx->pipe->enabled_raw);
		if (sv_sub_id < MTKCAM_PIPE_CAMSV_0) {
			dev_dbg(ctx->cam->dev, "%s Failed to get sv sub pipe\n",
				 __func__);
			return 0;
		}
		...
		struct mtk_camsys_irq_info *irq_info)
{
	...
	if (state_inner) {
		...
		if (frame_idx_inner > atomic_read(&sensor_ctrl->isp_request_seq_no) ||
			...
			dev_dbg_ratelimited(raw_dev->dev, "[SOF] frame done work too late frames. req(%d),ts(%lu)\n",
			...
			dev_dbg_ratelimited(raw_dev->dev, "[SOF] frame done reading lost %d frames. req(%d),ts(%lu)\n",
			...
			&& irq_info->fbc_cnt == 0) {
			dev_dbg_ratelimited(raw_dev->dev, "[SOF] frame done reading lost frames. req(%d),ts(%lu)\n",
				req_stream_data->frame_seq_no, irq_info->ts_ns / 1000);
			mtk_cam_set_timestamp(req_stream_data,
						      time_boot - 1000, time_mono - 1000);
	...
	if (que_cnt > 0) {
					...
					*current_state = state_rec[0];
					...
					if (req_stream_data->feature.switch_prev_frame_done) {
						...
						*current_state = state_rec[0];
						...
						dev_dbg(raw_dev->dev, "[switch] prev not done :%lu\n",
							irq_info->ts_ns / 1000);
		...
		if (working_req_found && state_rec[0]) {
			if (state_rec[0]->estate == E_STATE_READY) {
				dev_dbg(raw_dev->dev, "[SOF] sensor delay ts:%lu\n",
					irq_info->ts_ns / 1000);
				req_stream_data = mtk_cam_ctrl_state_to_req_s_data(state_rec[0]);
			...
			if (state_rec[0]->estate == E_STATE_SENINF)
				dev_dbg(raw_dev->dev, "[SOF] sensor switch delay\n");
			...
			if (state_rec[0]->estate == E_STATE_CQ_SCQ_DELAY) {
				state_transition(state_rec[0], E_STATE_CQ_SCQ_DELAY,
						E_STATE_OUTER);
				dev_dbg(raw_dev->dev, "[SOF] SCQ_DELAY state:%d ts:%lu\n",
		...
		int frame_idx_inner)
{
	...
	if (que_cnt > 0 && state_rec[0]) {
		if (state_rec[0]->estate == E_STATE_TS_READY) {
			dev_dbg(ctx->cam->dev, "[TS-SOF] sensor delay\n");
			return STATE_RESULT_PASS_CQ_SW_DELAY;
		}
	}
	/* Trigger high resolution timer to try sensor setting */
				       ...
				       unsigned int dequeued_frame_seq_no)
{
	...
	if (ctx->sensor) {
		...
		if (state_handle_ret != STATE_RESULT_TRIGGER_CQ) {
			dev_dbg(cam->dev, "[TS-SOF] SV-ENQUE drop s:%d deq:%d\n",
			      ...
			      unsigned int pipe_id)
{
	...
	if (list_empty(&ctx->composed_buffer_list.list)) {
		...
		if (state_sensor == NULL) {
			dev_dbg(raw_dev->dev, "[M2M P1 Don] Invalid state_sensor\n");
			return;
		}
				       ...
				       struct mtk_camsys_irq_info *irq_info)
{
			...
			if (ctx->pipe->stagger_path == STAGGER_DCIF) {
				...
				for (sv_i = MTKCAM_SUBDEV_CAMSV_START;
					...
					if (ctx->pipe->enabled_raw & (1 << sv_i)) {
						...
						if (!buf_entry) {
							dev_err(camsv_dev->dev, "%s: buf_entry not found", __func__);
							return;
						}
						mtk_cam_sv_enquehwbuf(camsv_dev,
	...
	if (ctx->used_sv_num && is_apply) {
		if (mtk_cam_sv_apply_all_buffers(ctx, irq_info->ts_ns) == 0)
			dev_dbg(raw_dev->dev, "sv apply next buffer failed");
	}
	if (ctx->used_mraw_num && is_apply) {
		if (mtk_cam_mraw_apply_all_buffers(ctx, irq_info->ts_ns) == 0)
			dev_dbg(raw_dev->dev, "mraw apply next buffer failed");
	}
}
			...
			struct mtk_camsys_irq_info *irq_info)
{
	...
	if (state_switch && STAGGER_DYNAMIC_SWITCH == 0) {
		...
		if (req_stream_data->feature.switch_prev_frame_done == 0) {
			dev_dbg(raw_dev->dev, "[%s] prev not done :%lu\n",
	...
	if (req->flags & MTK_CAM_REQ_FLAG_SENINF_IMMEDIATE_UPDATE) {
		for (i = 0; i < cam->max_stream_num; i++) {
			if ((req->ctx_used & 1 << i) && (req->ctx_link_update & 1 << i)) {
				...
				raw_dev = get_master_raw_dev(ctx->cam, ctx->pipe);
				req_stream_data = mtk_cam_req_get_s_data(req, stream_id, 0);
				dev_dbg(cam->dev, "%s: toggle for rawi\n", __func__);
				   ...
				   unsigned int frame_seq_no_outer)
{
	...
	if (raw_dev->sof_count == 0) {
		...
		if (!req_stream_data) {
			dev_dbg(raw_dev->dev, "Failed to get req s_data\n");
			return;
		}
				  ...
				  unsigned int dequeued_frame_seq_no)
{
	...
	if (!ctx->sensor) {
		dev_dbg(cam->dev, "%s: no sensor found in ctx:%d, req:%d",
			       ...
			       unsigned int dequeued_frame_seq_no)
{
	...
	for (i = 0; i < cam->max_stream_num; i++) {
		if ((req->ctx_used & 1 << i) && (req->ctx_link_update & (1 << i))) {
			...
			if (req_stream_data->state.estate == E_STATE_CAMMUX_OUTER_CFG_DELAY) {
				 ...
				 * queue_work(cam->link_change_wq, &req->link_work);
				 */
				dev_dbg(raw_dev->dev, "Exchange streams at req(%d), update link ctx (0x%x)\n",
	...
	dev_dbg(raw_dev->dev, "%s:req(%d) no link update data found!\n",
		__func__, frame_seq);
...
void mtk_cam_meta1_done_work(struct work_struct *work)
{
	...
	if (!ctx->streaming) {
		spin_unlock(&ctx->streaming_lock);
		dev_dbg(ctx->cam->dev, "%s: skip for stream off ctx:%d\n",
			 __func__, ctx->stream_id);
		return;
	}
	spin_unlock(&ctx->streaming_lock);
...
void mtk_cam_sv_work(struct work_struct *work)
{
	...
	if (s_data->pipe_id < MTKCAM_SUBDEV_CAMSV_START) {
		dev_dbg(ctx->cam->dev, "%s Failed to get sv pipe\n",
			 __func__);
		return;
	}
			       ...
			       unsigned int pipe_id)
{
	...
	if (!req_stream_data) {
		dev_dbg(ctx->cam->dev, "%s:ctx-%d:pipe-%d:s_data not found!\n",
	...
	struct mtk_cam_ctx *ctx, unsigned int dequeued_frame_seq_no, u64 ts_ns)
{
	...
	if (ctx->stream_id >= MTKCAM_SUBDEV_CAMSV_START &&
		...
		if (mtk_cam_sv_apply_next_buffer(ctx,
			...
			dev_dbg(camsv_dev->dev, "sv apply next buffer failed");
		}
	} else {
	...
	unsigned int pipe_id)
{
	...
	spin_unlock(&ctx->first_cq_lock);
	dev_dbg(ctx->cam->dev, "[1st-CQD] all done:%d, pipe_id:%d (using raw/mraw:%d/%d)\n",
				       ...
				       struct mtk_camsys_irq_info *irq_info)
{
	...
	if (!ctx) {
		dev_info(raw_dev->dev, "cannot find ctx\n");
		return -EINVAL;
	}
	...
	if (irq_info->irq_type & (1 << CAMSYS_IRQ_FRAME_START)) {
		if (atomic_read(&raw_dev->vf_en) == 0) {
			dev_dbg(raw_dev->dev, "skip sof event when vf off\n");
			return 0;
		}
					...
					struct mtk_camsys_irq_info *irq_info)
{
	...
	if (!ctx) {
		dev_info(mraw_dev->dev, "cannot find ctx\n");
		return -EINVAL;
	}
					 ...
					 struct mtk_camsys_irq_info *irq_info)
{
	...
	if (camsv_dev->pipeline->hw_scen &
		...
		if (!ctx) {
			pr_err("%s: failed, ctx can't be NULL\n", __func__);
			return -EINVAL;
		}
			...
			if (camsv_dev->pipeline->exp_order == 0) {
				if (ctx->pipe->stagger_path == STAGGER_DCIF) {
					...
					if (raw_dev->sof_count == 1) {
						...
						if (!camsv_dev_s) {
							dev_err(camsv_dev->dev, "failed to get camsv_dev_s\n");
							return -EINVAL;
						}
			 ...
			 struct mtk_camsys_irq_info *irq_info)
{
	...
	case CAMSYS_ENGINE_SENINF:
		...
		if (irq_info->irq_type & (1 << CAMSYS_IRQ_FRAME_DROP))
			dev_dbg(cam->dev, "MTK_CAMSYS_ENGINE_SENINF_TAG engine:%d type:0x%x\n",
				  ...
				  struct mtk_cam_ctx *ctx)
{
	...
	mtk_cam_set_sensor_full(req_stream_data, &ctx->sensor_ctrl);
	dev_dbg(ctx->cam->dev, "[mstream] Initial sensor timer setup, seq_no(%d)\n",
				req_stream_data->frame_seq_no);
	if (mtk_cam_is_mstream_m2m(ctx))
				  ...
				  struct mtk_cam_ctx *ctx)
{
	...
	if (mtk_cam_is_subsample(ctx))
		state_transition(&req_stream_data->state,
			E_STATE_READY, E_STATE_SUBSPL_READY);
	dev_dbg(ctx->cam->dev, "Directly setup sensor req:%d\n",
		req_stream_data->frame_seq_no);
}
...
int mtk_camsys_ctrl_start(struct mtk_cam_ctx *ctx)
{
	...
	dev_dbg(ctx->cam->dev, "[%s] ctx:%d/raw_dev:0x%x drained/sensor (%d)%d/%d\n",
...
void mtk_camsys_ctrl_stop(struct mtk_cam_ctx *ctx)
{
	...
	dev_dbg(ctx->cam->dev, "[%s] ctx:%d/raw_dev:0x%x\n",
