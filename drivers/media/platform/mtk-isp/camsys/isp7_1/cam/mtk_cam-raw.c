...
#define PURE_M2M_PROCESS_MARGIN_D 10000
	...
	pr_debug("%s:width:%lld height:%lld interval:%d/%d prate:%lld\n",
...
static int mtk_raw_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MTK_CAM_RAW_RESOURCE_UPDATE:
		...
		dev_dbg(dev, "%s:pipe(%d):streaming(%d), sensor_mode_update(%d)\n",
			 __func__, pipeline->id, pipeline->subdev.entity.stream_count,
...
static int mtk_raw_pde_get_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MTK_CAM_PDE_INFO:
		...
		break;
	default:
		dev_dbg(dev, "%s(id:0x%x,val:%d) is not handled\n",
	...
	if (~raw_rst_stat & RST_STAT_RAWI_R2) { /* RAWI_R2 */
		...
		ret = ((chasing_stat & RAWI_R2_SMI_REQ_ST) == 0 &&
		 ...
		 (readl(dev->base + REG_CTL_MOD6_EN) & 0x1))
			? true:false;
		dev_dbg(dev->dev, "%s: chasing_stat: 0x%llx ret=%d\n",
...

{
	...
	if (readx_poll_timeout(readl, dev->base_inner + offset,
			       ...
			       1 /* sleep, us */,
			       10000 /* timeout, us*/) < 0) {
		dev_dbg(dev->dev, "%s: wait vf off timeout: TG_VF_CON 0x%x\n",
			 __func__, chk_val);
...
void stream_on(struct mtk_raw_device *dev, int on)
{
	...
	if (on) {
		if (!pipe->res_config.enable_hsf_raw) {
			...
			dev_dbg(dev->dev, "VF on - REG_TG_TIME_STAMP_CNT val:%d fps(30x):%d\n",
			val, fps_ratio);
			if (dev->stagger_en)
			...
			if (feature & MTK_CAM_FEATURE_TIMESHARE_MASK ||
				...
				dev_dbg(dev->dev, "[%s] M2M view finder disable\n", __func__);
			} else {
				       ...
				       int *clk_idx)
{
	...
	if (debug_raw_num > 0) {
		dev_dbg(cam->dev, "DEBUG: force raw_num_used: %d\n",
			 debug_raw_num);
		res->raw_num_used = debug_raw_num;
	}

	if (debug_pixel_mode >= 0) {
		dev_dbg(cam->dev, "DEBUG: force debug_pixel_mode (log2): %d\n",
			 debug_pixel_mode);
		res->tgo_pxl_mode = debug_pixel_mode;
	}

	if (debug_clk_idx >= 0) {
		dev_dbg(cam->dev, "DEBUG: force debug_clk_idx: %d\n",
			 debug_clk_idx);
...
static irqreturn_t mtk_thread_irq_raw(int irq, void *data)
{
	...
	if (unlikely(atomic_cmpxchg(&raw_dev->is_fifo_overflow, 1, 0)))
		dev_dbg(raw_dev->dev, "msg fifo overflow\n");
				...
				int dequeued_frame_seq_no)
{
	...
	if (s_data) {
		...
		if (tg_full_sel == 1)
			...
			dev_dbg(raw_dev->dev, "tg_full_sel 0x%x\n", __func__, tg_full_sel);
	} else {
		dev_dbg(raw_dev->dev,
					  ...
					  int dequeued_frame_seq_no)
{
	...
	if (s_data) {
		...
		dev_dbg(raw_dev->dev, "%s: req(%d) can't be found for dump\n",
			__func__, dequeued_frame_seq_no);
			    ...
			    struct mtk_raw_device *raw)
{
	...
	for (i = 0; i < larbs; i++) {
		...
		if (WARN_ON(!larb_pdev)) {
			...
			dev_dbg(dev, "failed to get larb pdev\n");
			continue;
		}
		of_node_put(larb_node);
		...
		if (!link)
			dev_dbg(dev, "unable to link smi larb%d\n", i);
	}
	...
	if (ret) {
		dev_dbg(dev, "failed to register notifier block.\n");
		return ret;
	}
#endif
	return 0;
...
#else
	dev_dbg(raw->cam_dev, "%s raw_status:0x%x Available Engine:A/B:%d/%d\n",
		 __func__, res_status,
			!(res_status & (1 << MTKCAM_SUBDEV_RAW_0)),
			!(res_status & (1 << MTKCAM_SUBDEV_RAW_1)));
#endif
			...
			struct mtk_raw_stagger_select *result)
{
	...
	for (i = 0;  i < cam->max_stream_num; i++) {
		...
		if (ctx_chk != ctx && ctx_chk->streaming && mtk_cam_is_stagger(ctx_chk)) {
			for (m = 0; m < STAGGER_MAX_STREAM_NUM; m++) {
				...
				dev_dbg(cam->dev, "[%s:stagger check] i:%d/m:%d; mode:%d\n",
				__func__, i, m, mode);
	...
	dev_dbg(cam->dev, "[%s:counter] num:%d; this ctx order is :%d (%d/%d/%d/%d)\n",
	...
	for (i = 0; i < stagger_ctx_num + 1; i++) {
		if (stagger_order_mask[i] == 0) {
			...
			dev_dbg(cam->dev, "[%s:plan:%d] raw_status 0x%x, stagger_select_raw_mask:0x%x mode:0x%x\n",
	...
	for (m = MTKCAM_SUBDEV_RAW_0; m < RAW_PIPELINE_NUM; m++) {
		...
		if (stagger_select.raw_select & mask) { /*check stagger raw select mask*/
			...
			dev_dbg(cam->dev, "[%s:select] traversed current raw %d/0x%x, stagger_select_raw_mask:0x%x\n",
			    ...
			    int raw_status)
{
	...
	if (!s_raw_pipe_data) {
		dev_err(ctx->cam->dev, "%s: s_raw_pipe_data not found\n", __func__);
		return -EINVAL;
	}
	...
	if (!pipe) {
		dev_err(ctx->cam->dev, "%s: pipe not found\n", __func__);
		return -EINVAL;
	}
		       ...
		       struct mtkcam_ipi_input_param *cfg_in_param)
{
	...
	if (mtk_cam_is_stagger(ctx)) {
		...
		for (m = MTKCAM_SUBDEV_RAW_0; m < ARRAY_SIZE(pipe->raw->devs); m++) {
			...
			dev_dbg(cam->dev, "[%s] checking idx:%d pipe_id:%d pipe_chk_id:%d\n",
		...
		if (selected) {
			dev_dbg(cam->dev, "[%s] Timeshared (%d)- enabled_raw:0x%x as pipe:%d enabled_raw:0x%x\n",
...
static int mtk_raw_sd_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	dev_dbg(raw->cam_dev, "%s:raw-%d: en %d, dev 0x%x dmas 0x%x\n",
					  ...
					  struct mtk_cam_resource *res)
{
	...
	user_ptr = (void *)addr;
	if (!user_ptr) {
		dev_dbg(sd->v4l2_dev->dev, "%s: mtk_cam_resource is null\n",
			__func__);
		return false;
	}

	if (copy_from_user(res, user_ptr, sizeof(*res))) {
		dev_dbg(sd->v4l2_dev->dev, "%s: copy_from_user failedm user_ptr:%p\n",
			__func__, user_ptr);
		return false;
	}
			   ...
			   struct v4l2_subdev_format *fmt)
{
	...
	if (!source_fmt) {
		dev_dbg(dev,
			"%s(%d): Set fmt pad:%d(%s), no fmt on raw pipeline\n",
			__func__, fmt->which, fmt->pad, node->desc.name);
		return -EINVAL;
	}
			   ...
			   struct v4l2_subdev_format *fmt)
{
	...
	if (req) {
		...
		dev_dbg(cam->dev, "sd:%s pad:%d pending success, req fd(%d)\n",
			sd->name, fmt->pad, v4l2_subdev_format_request_fd(fmt));
	} else {
		dev_dbg(cam->dev, "sd:%s pad:%d pending failed, req fd(%d) invalid\n",
			sd->name, fmt->pad, v4l2_subdev_format_request_fd(fmt));
		return -EINVAL;
	}
			       ...
			       struct v4l2_device *v4l2_dev)
{
	...
	snprintf_ret = snprintf(sd->name, sizeof(sd->name),
	...
	if (ret < 0) {
		dev_dbg(dev, "Failed to register subdev: %d\n", ret);
		return ret;
	}
...
int mtk_raw_setup_dependencies(struct mtk_raw *raw)
{
	...
	for (i = 0; i < cam_dev->num_raw_drivers; i++) {
		...
		if (!consumer || !supplier) {
			dev_dbg(dev, "failed to get raw/yuv dev for id %d\n", i);
			continue;
		}
		...
		link = device_link_add(consumer, supplier,
				       ...
				       DL_FLAG_PM_RUNTIME);
		if (!link) {
			dev_dbg(dev, "Unable to create link between %s and %s\n",
...
static int mtk_raw_runtime_resume(struct device *dev)
{
	...
	for (i = 0; i < drvdata->num_clks; i++) {
		...
		if (ret) {
			dev_dbg(dev, "enable failed at clk #%d, ret = %d\n",
				 i, ret);
			i--;
			while (i >= 0)
				clk_disable_unprepare(drvdata->clks[i--]);
...
static int mtk_yuv_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_dbg(dev, "mtk_yuv_of_probe failed\n");
		return ret;
	}
...
int mtk_cam_translation_fault_callback(int port, dma_addr_t mva, void *data)
{
	...
	dev_dbg(dev, "=================== [CAMSYS M4U] Dump Begin ==================\n");

	dev_dbg(dev, "M4U TF port %d iova %pad frame_seq_no %d raw id %d vf %d\n",
