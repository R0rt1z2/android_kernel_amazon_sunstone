...
#include "mtk_cam-seninf-dualctl.h"
...
#define PIPE_DPTZ_DEBUG 1
static int senint_ctrl_need_init = 1;
static int senint_user[SENINF_SESSION_MAX] = {0};
struct mutex seninf_ctl_lock;

int dualSeninfCtl(enum SENINFSESSION session, int op, struct seninf_ctx *ctx)
{
	int res = SENINF_PASS;

	if ((ctx->port != CSI_PORT_1A) && (ctx->port != CSI_PORT_1))
		return SENINF_NOCTL;

	mutex_lock(&seninf_ctl_lock);
	switch (op) {
	case SENINF_GET:
		res = (senint_user[session] == 0) ? SENINF_ACTIVE : SENINF_PASS;
		senint_user[session] += 1;
		break;

	case SENINF_RELEASE:
		res = (senint_user[session] == 1) ? SENINF_ACTIVE : SENINF_PASS;
		if (senint_user[session] > 0)
			senint_user[session] -= 1;
		break;
	}
	dev_dbg(ctx->dev, "-- [DPTZ] Seninf session %d op %d, user = %d\n",
			 session, op, senint_user[session]);
	mutex_unlock(&seninf_ctl_lock);

	if (PIPE_DPTZ_DEBUG) {
		int i = 0;

		for (i = SENINF_INIT; i < SENINF_SESSION_MAX; i++)
			dev_dbg(ctx->dev, "-- [DPTZ] senint_user[%d] = %d\n", i, senint_user[i]);
	}

	return res;
}

int dualSeninfUser(enum SENINFSESSION session, struct seninf_ctx *ctx)
{
	int usr = 0;
	...
	mutex_lock(&seninf_ctl_lock);
	usr = senint_user[session];
	dev_dbg(ctx->dev, "-- [DPTZ] Get Seninf session %d user = %d\n",
			 session, senint_user[session]);
	mutex_unlock(&seninf_ctl_lock);
			   ...
			   const char *buf, size_t count)
{
	...
	if (num_para != REG_OPS_CMD_MAX_NUM) {
		dev_dbg(dev, "Wrong command parameter number\n");
		goto ERR_DEBUG_OPS_STORE;
	}
	...
	if (strncmp("SET_REG", arg[REG_OPS_CMD_ID], sizeof("SET_REG")) == 0) {
		...
		for (i = 0; i < CSI_PORT_MAX_NUM; i++) {
			...
			if (ret < 0)
				dev_dbg(dev, "%s Failed to read csi_names: %d\n",
			 __func__, ret);
...
static int seninf_dfs_init(struct seninf_dfs *dfs, struct device *dev)
{
	...
	if (IS_ERR(dfs->reg)) {
		dev_dbg(dev, "can't get dvfsrc-vmm\n");
	...
	for (i = 0; i < dfs->cnt; i++) {
		dev_dbg(dev, "dfs[%d] freq %ld volt %ld\n",
...
#if IS_ENABLED(CONFIG_REGULATOR_MTK_VMM)
	ret = isp_direct_set_voltage(dfs->reg, dfs->volts[i],
				     dfs->volts[dfs->cnt - 1], VMM_ISP_SENINF);
#else
	ret = regulator_set_voltage(dfs->reg, dfs->volts[i],
			      dfs->volts[dfs->cnt - 1]);
#endif
	dev_dbg(ctx->dev, "freq %ld require %ld selected %ld\n",
	...
	if (core->pm_domain_cnt == 1)
		...
		for (i = 0; i < core->pm_domain_cnt; i++) {
			...
			if (IS_ERR_OR_NULL(core->pm_domain_devs[i])) {
				dev_dbg(core->dev, "%s: fail to probe pm id %d\n",
					__func__, i);
...
static int seninf_core_pm_runtime_put(struct seninf_core *core)
{
	...
	if (core->pm_domain_cnt == 1)
		...
		for (i = core->pm_domain_cnt - 1; i >= 0; i--) {
			if (core->pm_domain_devs && core->pm_domain_devs[i])
...
static int get_seninf_ops(struct device *dev, struct seninf_core *core)
{
	...
	if (ret) {
		...
		dev_dbg(dev, "%s: seninf_num = %d, mux_num = %d, cam_mux_num = %d, pref_mux_num =%d\n",
			__func__,
			g_seninf_ops->seninf_num,
			g_seninf_ops->mux_num,
			g_seninf_ops->cam_mux_num,
	...
	for (i = 0; i < SENINF_PHY_VER_NUM; i++) {
		if (!strcasecmp(ver, csi_phy_versions[i])) {
			...
			dev_dbg(dev, "%s: mtk_csi_phy_2_0 = 0x%x mtk_csi_phy_3_0 = 0x%x\n",
			__func__,
			&mtk_csi_phy_2_0, &mtk_csi_phy_3_0);

			dev_dbg(dev, "%s: mtk_csi_phy_ver = %s i = %d 0x%x ret = %d\n",
			__func__,
...
static int seninf_core_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_dbg(dev, "%s: failed to create sub devices\n", __func__);
		return ret;
	}
	...
	if (ret) {
		dev_dbg(dev, "%s: failed to init dfs\n", __func__);
		//return ret;
	}
#endif
	...
	if (ret)
		dev_dbg(dev, "failed to create sysfs status\n");

	ret = device_create_file(dev, &dev_attr_debug_ops);
	if (ret)
		dev_dbg(dev, "failed to create sysfs debug ops\n");
	...
	if (IS_ERR(core->seninf_kworker_task)) {
		dev_dbg(dev, "%s: failed to start seninf kthread worker\n",
			__func__);
...
static int dev_read_csi_efuse(struct seninf_ctx *ctx)
{
	...
	dev_dbg(ctx->dev, "ctx->port = %d\n", ctx->port);
	if (IS_ERR(cell)) {
		if (PTR_ERR(cell) == -EPROBE_DEFER) {
			dev_dbg(ctx->dev, "read csi efuse returned with error cell %d\n",
	...
	nvmem_cell_put(cell);
	if (IS_ERR(buf)) {
		dev_dbg(ctx->dev, "read csi efuse returned with error buf\n");
		return PTR_ERR(buf);
	}
	ctx->m_csi_efuse = *buf;
	kfree(buf);
	dev_dbg(ctx->dev, "Efuse Data: 0x%08x\n", ctx->m_csi_efuse);
...
static int set_test_model(struct seninf_ctx *ctx, char enable)
{
	...
	if (ctx->is_test_model == 1) {
		...
		dev_dbg(ctx->dev, "testmodel %d invalid\n", ctx->is_test_model);
		return -1;
	}

	for (; i < vc_used; ++i) {
		if (!vc[i]) {
			dev_dbg(ctx->dev, "vc not found\n");
			return -1;
		}
	}
	...
	if (enable) {
		...
		for (i = 0; i < vc_used; ++i) {
			...
			dev_dbg(ctx->dev, "test mode mux %d, cam %d, pixel mode %d\n",
...
static int config_hw(struct seninf_ctx *ctx)
{
...
#ifdef MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	if (dualSeninfCtl(SENINF_STREAM_ONOFF, SENINF_GET, ctx)) {
		dev_dbg(ctx->dev, "[DPTZ] seninf port[%d] stream on\n",
			ctx->port);
#endif
	...
	g_seninf_ops->_set_csi_mipi(ctx);
		...
		dev_dbg(ctx->dev, "[DPTZ] seninf port[%d] skip stream on\n",
			ctx->port);
#endif
	...
	for (i = 0; i < vcinfo->cnt; i++) {
		...
		if (!vc->enable) {
			dev_dbg(ctx->dev, "vc[%d] pad %d. skip\n",
		...
		dev_dbg(ctx->dev, "ctx->pad2cam[%d] %d vc->out_pad %d vc->cam %d, i %d",
		...
		if (vc->cam != 0xff) {
			...
			dev_dbg(ctx->dev, "vc[%d] pad %d intf %d mux %d cam %d\n",
				 i, vc->out_pad, intf, vc->mux, vc->cam,
				vc_sel, dt_sel);

#ifdef SENSOR_SECURE_MTEE_SUPPORT
			if (ctx->is_secure == 1) {
				dev_dbg(ctx->dev, "Sensor kernel init seninf_ca");
				if (!seninf_ca_open_session())
					dev_dbg(ctx->dev, "seninf_ca_open_session fail");

				dev_dbg(ctx->dev, "Sensor kernel ca_checkpipe");
				seninf_ca_checkpipe(ctx->SecInfo_addr);
			}
#endif

		} else
			dev_dbg(ctx->dev, "not set camtg yet, vc[%d] pad %d intf %d mux %d cam %d\n",
					 i, vc->out_pad, intf, vc->mux, vc->cam);
	}
	return 0;
}
				    ...
				    int fps_n, int fps_d, s64 *result)
{
	...
	if ((width + hblank - k) == 0) {
		dev_err(dev, "error: width(%lld) + hblank(%lld) - k(%lld) == 0\n", width, hblank, k);
		return -EINVAL;
	}
				   ...
				   s64 *result)
{
	...
	if (ret) {
		dev_dbg(ctx->dev, "no get_fmt in %s\n", sd->name);
		return ret;
	}
	...
	if (ret) {
		dev_dbg(ctx->dev, "no g_frame_interval in %s\n", sd->name);
		return ret;
	}

	ctrl = v4l2_ctrl_find(sd->ctrl_handler, V4L2_CID_HBLANK);
	if (!ctrl) {
		dev_dbg(ctx->dev, "no hblank in %s\n", sd->name);
		return -EINVAL;
	}
	...
	if (!ctrl) {
		dev_dbg(ctx->dev, "no vblank in %s\n", sd->name);
		return -EINVAL;
	}
			  ...
			  s64 *result)
{
	...
	if (!ctrl) {
		dev_dbg(ctx->dev, "no cust pixel rate in subdev %s\n", sd->name);
		return -EINVAL;
	}
			  ...
			  s64 *result)
{
	...
	if (!ctrl) {
		dev_dbg(ctx->dev, "no pixel rate in subdev %s\n", sd->name);
		return -EINVAL;
	}
...
static int get_mbus_config(struct seninf_ctx *ctx, struct v4l2_subdev *sd)
{
	...
	if (ret) {
		dev_dbg(ctx->dev, "no g_mbus_config in %s\n",
			 sd->entity.name);
		return -1;
	}
...
int update_isp_clk(struct seninf_ctx *ctx)
{
...
#ifdef MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	if (!vc) {
		dev_dbg(ctx->dev, "[DPTZ] %s use PAD_SRC_RAW1\n", __func__);
		vc = mtk_cam_seninf_get_vc_by_pad(ctx, PAD_SRC_RAW1);
	}
#endif
	if (!vc) {
		dev_dbg(ctx->dev, "failed to get vc\n");
		return -1;
	}
	dev_dbg(ctx->dev,
	...
	if (ctx->customized_pixel_rate)
		...
		dev_dbg(ctx->dev, "failed to get pixel_rate\n");
		return -EINVAL;
	}
	...
	if (i == dfs->cnt) {
		dev_dbg(ctx->dev, "mux is overrun. please adjust pixelmode\n");
		return -EINVAL;
	}

	ret = seninf_dfs_set(ctx, dfs->freqs[i]);
	if (ret) {
		dev_dbg(ctx->dev, "failed to set freq\n");
		return ret;
	}
...
static int seninf_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		...
		if (ret < 0) {
			dev_dbg(ctx->dev, "%s pm_runtime_get_sync ret %d\n", __func__, ret);
		...
		ret = config_hw(ctx);
		if (ret) {
			dev_dbg(ctx->dev, "config_seninf_hw ret %d\n", ret);
			return ret;
		}
		...
		if (ret) {
			dev_dbg(ctx->dev, "sensor stream-on ret %d\n", ret);
			return  ret;
		}
		...
		if (ret) {
			dev_dbg(ctx->dev, "sensor stream-off ret %d\n", ret);
			return ret;
		}
#ifdef SENSOR_SECURE_MTEE_SUPPORT
		if (ctx->is_secure == 1) {
			dev_dbg(ctx->dev, "sensor kernel ca_free");
			seninf_ca_free();

			dev_dbg(ctx->dev, "close seninf_ca");
			ret_gz = seninf_ca_close_session();
			ctx->is_secure = 0;
		}
#endif
#ifdef MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	if (dualSeninfCtl(SENINF_STREAM_ONOFF, SENINF_RELEASE, ctx)) {
		dev_dbg(ctx->dev, "[DPTZ] seninf port[%d] stream off\n",
			ctx->port);
#endif
		g_seninf_ops->_set_idle(ctx);
		mtk_cam_seninf_release_mux(ctx);
		...
		dev_dbg(ctx->dev, "[DPTZ] seninf port[%d] skip stream off\n",
			ctx->port);
		mtk_cam_seninf_release_mux(ctx);
		seninf_dfs_set(ctx, 0);
	}
					 ...
					 struct v4l2_async_subdev *asd)
{
	...
	if (ret) {
		dev_dbg(ctx->dev, "failed to create subdev nodes\n");
		return ret;
	}
					   ...
					   struct v4l2_async_subdev *asd)
{
	...
	dev_dbg(ctx->dev, "%s is unbounded\n", sd->entity.name);
}
...
static int seninf_test_pattern(struct seninf_ctx *ctx, u32 pattern)
{
	...
	case 0:
		...
		ctx->is_test_model = 0;
		mtk_cam_seninf_get_vcinfo(ctx);
		dev_dbg(ctx->dev, "test pattern off\n");
		break;
	case 1:// RAW only
	...
	case 5:// 1 RAW + 1 W channel
		...
		dev_dbg(ctx->dev, "test pattern on\n");
		break;
	default:
		break;
	}
...
static int mtk_cam_seninf_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	default:
		ret = 0;
		dev_dbg(ctx->dev, "%s Unhandled id:0x%x, val:0x%x\n",
...
static int seninf_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	if (ctx->open_refcnt == 1)
		dev_dbg(ctx->dev, "%s open_refcnt %d\n", __func__, ctx->open_refcnt);
...
static int register_subdev(struct seninf_ctx *ctx, struct v4l2_device *v4l2_dev)
{
	...
	if (strlen(dev->of_node->name) > 16)
		snprintf_ret = snprintf(sd->name, sizeof(sd->name), "%s-%s",
	...
	if (ret < 0) {
		dev_dbg(dev, "failed to init pads\n");
		return ret;
	}

	ret = v4l2_device_register_subdev(v4l2_dev, sd);
	if (ret < 0) {
		dev_dbg(dev, "failed to register subdev\n");
		return ret;
	}
	...
	if (ret < 0) {
		dev_dbg(dev, "failed to register notifier\n");
...
static int seninf_probe(struct platform_device *pdev)
{
...
#ifdef MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	if (senint_ctrl_need_init) {
		mutex_init(&seninf_ctl_lock);
		senint_ctrl_need_init = 0;
		dev_dbg(dev, "[DPTZ] -- senint_ctrl_init --\n");
	}
#endif
	...
	if (ret) {
		dev_dbg(dev, "get_csi_port ret %d\n", ret);
		return ret;
	}
	...
	//dev_dbg(dev, "seninf dphy settle delay dt = %u\n",
	//	 ctx->seninf_dphy_settle_delay_dt);
	...
	if (ret < 0)
		dev_dbg(dev, "Failed to read efuse data\n");
#endif

	ret = seninf_initialize_controls(ctx);
	if (ret) {
		dev_dbg(dev, "Failed to initialize controls\n");
		return ret;
	}

	ret = component_add(dev, &seninf_comp_ops);
	if (ret < 0) {
		dev_dbg(dev, "component_add failed\n");
		goto err_free_handler;
	}
...
static int runtime_resume(struct device *dev)
{
	...
	if (core->refcnt == 1) {
		if (ctx->core->dfs.reg)
			res = regulator_enable(ctx->core->dfs.reg);
...
int mtk_cam_seninf_check_timeout(struct v4l2_subdev *sd, u64 time_after_sof)
{
	...
	if (!ctrl) {
		dev_dbg(ctx->dev, "no timeout value in subdev %s\n", sd->name);
		return -EINVAL;
	}
	...
	dev_dbg(ctx->dev, "%s time_after_sof %llu frame_time %llu in us ret %d ratio %d val %d\n",
		__func__,
		time_after_sof,
		frame_time,
		ret,
...
int mtk_cam_seninf_dump(struct v4l2_subdev *sd)
{
	...
	if (ctx->streaming) {
		...
		dev_dbg(ctx->dev, "%s should not dump during stream off\n", __func__);
