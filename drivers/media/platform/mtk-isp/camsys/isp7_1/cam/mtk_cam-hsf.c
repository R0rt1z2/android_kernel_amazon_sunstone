...
static unsigned int get_ccu_device(struct mtk_cam_hsf_ctrl *handle_inst)
{
	...
	if (rproc_np) {
		...
		pr_info("handle_inst.ccu_pdev = 0x%p\n", handle_inst->ccu_pdev);
	...
	unsigned int sub_cq_size, unsigned int sub_cq_offset)
{
...
#ifdef USING_CCU
	pr_info("ccu_pdev = 0x%px\n", (void *)hsf_config->ccu_pdev);
	ret = mtk_ccu_rproc_ipc_send(
		hsf_config->ccu_pdev,
		MTK_CCU_FEATURE_CAMSYS,
	...
	if (ret != 0)
		pr_info("error: CCU trigger CQ failure. tg:%d cq_src:0x%lx cq_dst:0x%lx cq_size:%d\n",
		pData.tg, pData.src_addr, pData.dst_addr, pData.cq_size);
	else
		pr_info("after CCU trigger CQ. tg:%d cq_src:0x%lx cq_dst:0x%lx initial_value = %d\n",
...
int mtk_cam_hsf_config(struct mtk_cam_ctx *ctx, unsigned int raw_id)
{
	...
	if (!dev) {
		dev_info(cam->dev, "Find raw dev failed\n");
		return -1;
	}
