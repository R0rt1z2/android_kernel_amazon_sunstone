			      ...
			      void __iomem *if_base, void __iomem *ana_base)
{
	...
	for (j = SENINF_1; j < _seninf_ops->seninf_num; j++) {
		...
		ctx->reg_if_tg[j] = if_base + 0x0f00 + (0x1000 * j);
...
static int mtk_cam_seninf_cammux(struct seninf_ctx *ctx, unsigned int cam_mux)
{
	...
	dev_info(ctx->dev, " %s cam_mux %u EN 0x%x IRQ_EN 0x%x IRQ_STATUS 0x%x\n",
		__func__,
		cam_mux,
...
static u32 mtk_cam_seninf_get_cammux_err(struct seninf_ctx *ctx, unsigned int cam_mux)
{
	void *pSeninf_cam_mux_pcsr = NULL;
	u32 ret = 0;
				 ...
				 int vc_sel, int dt_sel, int vc_en, int dt_en)
{
	...
	dev_info(ctx->dev, "%s cam_mux %u vc 0x%x dt 0x%x, vc_en %d dt_en %d\n",
		__func__,
