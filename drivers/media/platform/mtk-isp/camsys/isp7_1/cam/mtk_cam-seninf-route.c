...
struct seninf_vc *mtk_cam_seninf_get_vc_by_pad(struct seninf_ctx *ctx, int idx)
{
	...
	// fore return vc
	if ((vcinfo->vc[0].out_pad) == 2 && (idx == 1)) {
		dev_dbg(ctx->dev, "[DPTZ debug] Cannot find raw0 pad vc, return raw1 pad vc\n",
			__func__, idx);
		return &vcinfo->vc[0];
	}
#endif
...
int mtk_cam_seninf_get_csi_param(struct seninf_ctx *ctx)
{
	...
	if (!ctrl) {
		dev_dbg(ctx->dev, "%s, no V4L2_CID_MTK_CSI_PARAM %s\n",
	...
	ret = get_ctrl(ctrl);
	dev_dbg(ctx->dev, "%s get_ctrl ret:%d 0x%x|0x%x|0x%x|0x%x\n", __func__,
...
int mtk_cam_seninf_get_vcinfo(struct seninf_ctx *ctx)
{
	...
	if (!ctrl) {
		dev_dbg(ctx->dev, "%s, no V4L2_CID_MTK_FRAME_DESC %s\n",
			__func__, sensor_sd->name);
		return -EINVAL;
	}
	...
	if (ret || fd.type != MTK_MBUS_FRAME_DESC_TYPE_CSI2 || !fd.num_entries) {
		dev_dbg(ctx->dev, "%s get_ctrl ret:%d num_entries:%d type:%d\n", __func__,
	...
	for (i = 0; i < fd.num_entries; i++) {
		...
		default:
			if (vc->dt == 0x2a || vc->dt == 0x2b ||
				...
				default:
#ifdef MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
					ret = dualSeninfUser(SENINF_STREAM_ONOFF, ctx);
					if (ret == 0) {
						dev_dbg(ctx->dev, "[DPTZ] seninf port[%d] assign PAD_SRC_RAW0\n",
							ctx->port);
						vc->out_pad = PAD_SRC_RAW0;
					} else if (ret == 1) {
						dev_dbg(ctx->dev, "[DPTZ] seninf port[%d] assign PAD_SRC_RAW1\n",
							ctx->port);
						vc->out_pad = PAD_SRC_RAW1;
					} else
#endif
				...
				dev_dbg(ctx->dev, "unknown desc %d, dt 0x%x\n",
					desc, vc->dt);
				continue;
			}
			break;
		...
		dev_dbg(ctx->dev, "%s vc[%d] vc 0x%x dt 0x%x pad %d exp %dx%d grp 0x%x code 0x%x\n",
			__func__,
...
int _mtk_cam_seninf_set_camtg(struct v4l2_subdev *sd, int pad_id, int camtg, bool disable_last)
{
	...
	if (ctx->streaming && vc_en && vc->cam != camtg) {
		...
		if (ctx->is_secure == 1) {
			...
			dev_dbg(ctx->dev, "Sensor Secure CA");
			...
			dev_dbg(ctx->dev, "%s: pad %d mux %d cam %d -> %d\n",
		...
		dev_dbg(ctx->dev, "%s: pad_id %d, camtg %d, ctx->streaming %d, vc_en %d, vc->cam %d\n",
...
#ifdef MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	dev_dbg(ctx->dev, "[DPTZ debug] %s: no raw0 data in vc channel, try raw1\n", __func__);
	for (i = 0; i < vcinfo->cnt; i++) {
		vc = &vcinfo->vc[i];
		if (vc->out_pad == PAD_SRC_RAW1) /* DPTZ case */
			return vc->cam;
	}
#endif
	dev_dbg(ctx->dev, "%s: no raw data in vc channel\n", __func__);
	return -1;
}
...
int reset_sensor(struct seninf_ctx *ctx)
{
	...
	if (!ctrl) {
		dev_dbg(ctx->dev, "V4L2_CID_MTK_SENSOR_RESET %s\n",
			sensor_sd->name);
		return -EINVAL;
	}
...
int notify_fsync_cammux_usage(struct seninf_ctx *ctx)
{
	...
	if (!ctrl) {
		dev_dbg(ctx->dev, "no fsync map id in %s\n",
			sensor_sd->name);
		return -EINVAL;
	}

	dev_dbg(ctx->dev, "raw cammux usage = %d\n", cam_idx);
