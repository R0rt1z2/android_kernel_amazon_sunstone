...
struct mtk_cam_seninf_ops {
	...
	int (*_is_cammux_used)(struct seninf_ctx *ctx, unsigned int cam_mux);
	int (*_cammux)(struct seninf_ctx *ctx, unsigned int cam_mux);
	int (*_disable_cammux)(struct seninf_ctx *ctx, unsigned int cam_mux);
	...
	int (*_get_cammux_ctrl)(struct seninf_ctx *ctx, unsigned int cam_mux);
	u32 (*_get_cammux_res)(struct seninf_ctx *ctx, unsigned int cam_mux);
	int (*_set_cammux_vc)(struct seninf_ctx *ctx, unsigned int cam_mux,
					 int vc_sel, int dt_sel, int vc_en, int dt_en);
	int (*_set_cammux_src)(struct seninf_ctx *ctx, int src,
					  unsigned int target, int exp_hsize, int exp_vsize);
	int (*_set_vc)(struct seninf_ctx *ctx, unsigned int seninfIdx,
				  struct seninf_vcinfo *vcinfo);
	int (*_set_mux_ctrl)(struct seninf_ctx *ctx, unsigned int mux,
					int hsPol, int vsPol, int src_sel,
					int pixel_mode);
	int (*_set_mux_crop)(struct seninf_ctx *ctx, unsigned int mux,
					int start_x, int end_x, int enable);
	int (*_is_mux_used)(struct seninf_ctx *ctx, unsigned int mux);
	int (*_mux)(struct seninf_ctx *ctx, unsigned int mux);
	int (*_disable_mux)(struct seninf_ctx *ctx, unsigned int mux);
	int (*_disable_all_mux)(struct seninf_ctx *ctx);
	int (*_set_cammux_chk_pixel_mode)(struct seninf_ctx *ctx,
							 unsigned int cam_mux, int pixelMode);
	int (*_set_test_model)(struct seninf_ctx *ctx,
					  unsigned int mux, unsigned int cam_mux, int pixelMode);
	int (*_set_csi_mipi)(struct seninf_ctx *ctx);
	int (*_poweroff)(struct seninf_ctx *ctx);
	int (*_reset)(struct seninf_ctx *ctx, unsigned int seninfIdx);
	int (*_set_idle)(struct seninf_ctx *ctx);
	int (*_get_mux_meter)(struct seninf_ctx *ctx, unsigned int mux,
	...
	int (*_set_cammux_next_ctrl)(struct seninf_ctx *ctx, int src, unsigned int target);
	int (*_update_mux_pixel_mode)(struct seninf_ctx *ctx, unsigned int mux, int pixel_mode);
	int (*_irq_handler)(int irq, void *data);
	int (*_set_sw_cfg_busy)(struct seninf_ctx *ctx, bool enable, int index);
	int (*_set_cam_mux_dyn_en)(struct seninf_ctx *ctx, bool enable, unsigned int cam_mux,
					int index);
	...
	int (*_enable_cam_mux_vsync_irq)(struct seninf_ctx *ctx, bool enable, unsigned int cam_mux);
