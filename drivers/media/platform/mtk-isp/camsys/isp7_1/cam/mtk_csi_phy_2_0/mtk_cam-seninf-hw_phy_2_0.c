			      ...
			      void __iomem *if_base, void __iomem *ana_base)
{
	...
	for (j = SENINF_1; j < _seninf_ops->seninf_num; j++) {
		ctx->reg_if_ctrl[j] = if_base + 0x0200 + (0x1000 * j);
		ctx->reg_if_tg[j] = if_base + 0x0600 + (0x1000 * j);
		ctx->reg_if_csi2[j] = if_base + 0x0a00 + (0x1000 * j);
	}
...
static int mtk_cam_seninf_init_port(struct seninf_ctx *ctx, int port)
{
	...
	if (port == CSI_PORT_1A) {
		dev_info(ctx->dev, "[DPTZ debug] %s reassign portNum to PORT_1.\n", __func__);
		portNum = CSI_PORT_1;
	}
#endif
...
static int mtk_cam_seninf_is_cammux_used(struct seninf_ctx *ctx, unsigned int cam_mux)
{
	...
	if (cam_mux >= _seninf_ops->cam_mux_num) {
		dev_err(ctx->dev, "%s err cam_mux %u >= SENINF_CAM_MUX_NUM %d\n",
			__func__,
			cam_mux,
...
static int mtk_cam_seninf_cammux(struct seninf_ctx *ctx, unsigned int cam_mux)
{
	void *pSeninf_cam_mux = ctx->reg_if_cam_mux;
	u32 temp;
	...
	dev_info(ctx->dev, "cam_mux %u EN 0x%x IRQ_EN 0x%x IRQ_STATUS 0x%x\n",
		 cam_mux,
...
static int mtk_cam_seninf_disable_cammux(struct seninf_ctx *ctx, unsigned int cam_mux)
{
	...
	if ((1 << cam_mux) & temp) {
		SENINF_WRITE_REG(base, SENINF_CAM_MUX_EN,
			temp & (~(1 << cam_mux)));
		dev_info(ctx->dev, "cammux %u EN %x IRQ_EN %x IRQ_STATUS %x",
			 cam_mux,
...
static int mtk_cam_seninf_get_cammux_ctrl(struct seninf_ctx *ctx, unsigned int cam_mux)
{
	void *pSeninf_cam_mux = ctx->reg_if_cam_mux;
	...
	default:
		dev_err(ctx->dev, "invalid cam_mux %u", cam_mux);
		return -EINVAL;
	}
...
static u32 mtk_cam_seninf_get_cammux_res(struct seninf_ctx *ctx, unsigned int cam_mux)
{
	if (cam_mux >= _seninf_ops->cam_mux_num) {
		dev_err(ctx->dev, "%s err cam_mux %u >= SENINF_CAM_MUX_NUM %d\n",
			__func__,
...
static u32 mtk_cam_seninf_get_cammux_exp(struct seninf_ctx *ctx, unsigned int cam_mux)
{
	if (cam_mux >= _seninf_ops->cam_mux_num) {
		dev_err(ctx->dev, "%s err cam_mux %u >= SENINF_CAM_MUX_NUM %d\n",
			__func__,
...
static int mtk_cam_seninf_set_cammux_vc(struct seninf_ctx *ctx, unsigned int cam_mux,
				 int vc_sel, int dt_sel, int vc_en, int dt_en)
{
	...
	dev_info(ctx->dev, "cam_mux %u vc 0x%x dt 0x%x, vc_en %d dt_en %d\n",
...
static int mtk_cam_seninf_set_cammux_next_ctrl(struct seninf_ctx *ctx, int src, unsigned int target)
{
...
static int mtk_cam_seninf_set_cammux_src(struct seninf_ctx *ctx, int src, unsigned int target,
				  int exp_hsize, int exp_vsize)
{
...
#if	DEBUG_CAM_MUX_SWITCH
	dev_info(ctx->dev, "%s cam_mux %u src %d exp_hsize %d, exp_hsize %d\n",
	...
	default:
		dev_err(ctx->dev, "invalid src %d target %u", src, target);
		return -EINVAL;
	}
...
static int mtk_cam_seninf_set_vc(struct seninf_ctx *ctx, unsigned int intf,
...
static int mtk_cam_seninf_set_mux_ctrl(struct seninf_ctx *ctx, unsigned int mux,
...
static int mtk_cam_seninf_update_mux_pixel_mode(struct seninf_ctx *ctx, unsigned int mux,
				int pixel_mode)
{
	unsigned int temp = 0;
	void *pSeninf_mux;
	...
	dev_info(ctx->dev, "%s mux %u SENINF_MUX_CTRL_1(0x%x), SENINF_MUX_OPT(0x%x)",
		__func__,
		mux,
		SENINF_READ_REG(pSeninf_mux, SENINF_MUX_CTRL_1),
		SENINF_READ_REG(pSeninf_mux, SENINF_MUX_OPT));
...
static int mtk_cam_seninf_set_mux_crop(struct seninf_ctx *ctx, unsigned int mux,
				int start_x, int end_x, int enable)
{
	...
	dev_info(ctx->dev, "mux %u, start %d, end %d, enable %d\n",
		 mux, start_x, end_x, enable);

	return 0;
}

static int mtk_cam_seninf_is_mux_used(struct seninf_ctx *ctx, unsigned int mux)
{
	void *pSeninf_mux = ctx->reg_if_mux[mux];
...
static int mtk_cam_seninf_mux(struct seninf_ctx *ctx, unsigned int mux)
{
	void *pSeninf_mux = ctx->reg_if_mux[mux];
...
static int mtk_cam_seninf_disable_mux(struct seninf_ctx *ctx, unsigned int mux)
{
	int i;
	void *pSeninf_mux = ctx->reg_if_mux[mux];
...
static int mtk_cam_seninf_set_cammux_chk_pixel_mode(struct seninf_ctx *ctx,
					     unsigned int cam_mux, int pixel_mode)
{
	void *mpSeninfCamMux_base = ctx->reg_if_cam_mux;
...
#if	DEBUG_CAM_MUX_SWITCH
	dev_info(ctx->dev, "%s cam_mux %u chk pixel_mode %d\n",
	...
	default:
		dev_err(ctx->dev, "invalid cam_mux %u pixel_mode %d\n",
			cam_mux, pixel_mode);
		return -EINVAL;
	}
...
static int mtk_cam_seninf_set_test_model(struct seninf_ctx *ctx,
				  unsigned int mux, unsigned int cam_mux, int pixel_mode)
{
	unsigned int intf;
	void *pSeninf;
	void *pSeninf_tg;
...
static int csirx_phyA_power_on(struct seninf_ctx *ctx, unsigned int portIdx, int en)
{
...
static int csirx_seninf_top_setting(struct seninf_ctx *ctx)
{
	...
	case CSI_PORT_1A:
		...
		break;
#ifndef MTK_CAM_DUAL_PIPE_DPTZ_SUPPORT
	case CSI_PORT_1A:
#endif
	case CSI_PORT_1B:
		SENINF_BITS(pSeninf_top, SENINF_TOP_PHY_CTRL_CSI1,
...
static int mtk_cam_seninf_reset(struct seninf_ctx *ctx, unsigned int seninfIdx)
{
	int i;
	void *pSeninf_mux;
	...
	dev_info(ctx->dev, "reset seninf %u\n", seninfIdx);
...
static int mtk_cam_seninf_get_mux_meter(struct seninf_ctx *ctx, unsigned int mux,
...
static int mtk_cam_seninf_debug(struct seninf_ctx *ctx)
{
	...
	for (j = CSI_PORT_0; j < CSI_PORT_3; j++) {
		...
		dev_info(ctx->dev,
			"Csi%d_Cphy_Top: DPHY_RX_CLOCK_LANE_FSM(0x%x) DPHY_RX_DATA_LANE_FSM(0x%x)\n",
			j,
			SENINF_READ_REG(base_cphy, DPHY_RX_CLOCK_LANE_FSM),
			SENINF_READ_REG(base_cphy, DPHY_RX_DATA_LANE_FSM));
	}
	...
	if (core->csi_irq_en_flag) {
		list_for_each_entry(ctx, &core->list, list) {
			...
			if ((ctx->data_not_enough_cnt) >= (core->detection_cnt) ||
				...
				if (cnt1 < 0)
					dev_err(ctx->dev, "%s: snprintf error\n", __func__);
				cnt_tmp += cnt1;
				...
				if (cnt7 < 0)
					dev_err(ctx->dev, "%s: snprintf error\n", __func__);
				cnt_tmp += cnt7;
				snprintf_ret = snprintf(seninf_name+cnt_tmp, 256-cnt_tmp,
					"   size_err_count: %d",
					ctx->size_err_cnt);
				if (snprintf_ret < 0)
					dev_err(ctx->dev, "%s: snprintf error\n", __func__);
...
static int mtk_cam_seninf_set_cam_mux_dyn_en(
	struct seninf_ctx *ctx, bool enable, unsigned int cam_mux, int index)
{
	void *pSeninf_cam_mux = ctx->reg_if_cam_mux;
	int tmp = 0;
...
static int mtk_cam_seninf_enable_cam_mux_vsync_irq(struct seninf_ctx *ctx, bool enable,
	unsigned int cam_mux)
{
	void *pSeninf_cam_mux = ctx->reg_if_cam_mux;
	int tmp = 0;
