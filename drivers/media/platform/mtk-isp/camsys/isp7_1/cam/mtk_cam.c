					    ...
					    int pipe_id, int index)
{
	...
	struct mtk_cam_video_device *node;
	struct vb2_buffer *vb;
	int i, buf_state, buf_start = 0, buf_end;
	...
	if (is_raw_subdev(pipe_id)) {
		...
		pr_err("%s: unexpected pipe_id", __func__);
		return;
	}
	...
	dev_dbg(cam->dev,
		"%s:%s: pipe_id(%d) buf_state(%d) buf_ret_cnt(%u)\n", __func__,
		...
		for (i = MTKCAM_SUBDEV_CAMSV_START; i < MTKCAM_SUBDEV_CAMSV_END; i++) {
			if (enabled_raw & (1 << i)) {
				if (config_param->n_maps < ARRAY_SIZE(config_param->maps)) {
					...
					dev_dbg(ctx->cam->dev, "hw mapping pipe_id:%d exp_order:%d",
						i,
						config_param->maps[config_param->n_maps].exp_order);
					config_param->n_maps++;
				} else
					dev_dbg(ctx->cam->dev, "hw mapping is over size(enable_raw:%d)",
		...
		struct mtk_cam_request_stream_data *s_data)
{
	...
	if (!virt_addr_valid(s_data->working_buf) ||
	    (s_data->working_buf->buffer.va == (void *)NULL) ||
	...
	if (!pTimestamp) {
		dev_dbg(ctx->cam->dev,
			 "%s:ctx(%d): Failed to get timestamp addr\n",
			 __func__, ctx->stream_id);
		return;
	}
			      ...
			      const struct v4l2_format *cfg_fmt)
{
	...
	if (!buf_entry) {
		dev_dbg(cam->dev, "%s: No img buf availablle: req:%d\n",
				   ...
				   struct mtk_cam_buffer *buf)
{
	...
	if (!raw_pipline) {
		dev_dbg(cam->dev, "%s: Failed to get raw pipeline\n", __func__);
		return;
	}
				     ...
				     struct mtk_cam_buffer *buf)
{
	...
	if (feature & MTK_CAM_FEATURE_SUBSAMPLE_MASK) {
		...
		if (is_mtk_format(cfg_fmt->fmt.pix_mp.pixelformat)) {
			...
			if (!mtk_info) {
				dev_dbg(cam->dev, "%s: Failed to get mtk format info\n",
				 __func__);
				return -EINVAL;
			}
			...
			if (!v4l2_info) {
				dev_dbg(cam->dev, "%s: Failed to get v4l2 format info\n",
				 __func__);
				return -EINVAL;
			}
			      ...
			      struct mtk_cam_request *req)
{
	...
	list_for_each_entry_safe(obj, obj_prev, &req->req.objects, list) {
		...
		if (ctx == NULL) {
			dev_dbg(cam->dev, "%s FAIL: ctx is NULL", __func__);
			return -EINVAL;
		}
	...
	for (i = 0; i < cam->max_stream_num; i++) {
		...
		if (!raw_pipe_data) {
			dev_err(cam->dev, "raw_pipe_data not found\n");
			return -EINVAL;
		}
			      ...
			      unsigned int notification)
{
	...
	if (IS_ERR(req)) {
		dev_dbg(cam->dev, "%s:req fd(%d) is invalid\n", __func__, request_fd);
		return v4l2_pipeline_link_notify(link, flags, notification);
	}
					  ...
					  struct mtk_raw_pipeline *pipe)
{
	...
	if (!dev_master) {
		pr_err("%s: failed, dev_master can't be NULL\n", __func__);
		return NULL;
	}
...
struct mtk_raw_device *get_slave_raw_dev(struct mtk_cam_device *cam,
					 struct mtk_raw_pipeline *pipe)
{
	struct device *dev_slav = NULL;
	unsigned int i;

	for (i = 0; i < cam->num_raw_drivers - 1; i++) {
		if (pipe->enabled_raw & (1 << i)) {
			dev_slav = cam->raw.devs[i + 1];
			break;
		}
	}

	if (!dev_slav) {
		pr_err("%s: failed, dev_slav can't be NULL\n", __func__);
		return NULL;
	}

	return dev_get_drvdata(dev_slav);
}
	...
	if ((ctx->composed_frame_seq_no == 1 && !mtk_cam_is_time_shared(ctx)) ||
		...
		if (mtk_cam_is_with_w_channel(ctx)) {
			if (mtk_cam_sv_rgbw_apply_next_buffer(buf_entry->s_data) == 0)
				dev_dbg(raw_dev->dev, "rgbw: sv apply next buffer failed");
		}
				...
				int len, void *priv, u32 src)
{
	...
	if (ipi_msg->ack_data.ack_cmd_id == CAM_CMD_FRAME) {
		...
		dev_dbg(dev, "%s:ctx(%d): session destroyed",
			 __func__, ctx->stream_id);
	}
...
static void isp_tx_frame_worker(struct work_struct *work)
{
	...
	if (!s_raw_pipe_data) {
		pr_info("%s req_stream_data(%p), s_raw_pipe_data(%p), dropped\n",
				__func__, req_stream_data, s_raw_pipe_data);
		return;
	}
	...
	if (!buf_entry) {
		dev_dbg(cam->dev, "%s: No CQ buf availablle: req:%d(%s)\n",
			 __func__, req_stream_data->frame_seq_no,
			 req->req.debug_str);
		WARN_ON(1);
		return;
	...
	for (i = 0; i < ctx->used_mraw_num; i++) {
		...
		if (!mraw_buf_entry) {
			dev_dbg(cam->dev, "%s: No Mraw working buf availablle: req:%d(%s)\n",
				__func__, req_stream_data->frame_seq_no,
				req->req.debug_str);
			WARN_ON(1);
			return;
	...
	struct mtk_cam_request *req, unsigned int idx)
{
	...
	for (i = 0 ; i < ctx->used_sv_num ; i++) {
		...
		if (buf_entry == NULL) {
			dev_dbg(ctx->cam->dev, "%s FAIL: no free sv buffer", __func__);
			return false;
		}
					  ...
					  int stream_id)
{
	...
	if (!s_raw_pipe_data) {
		dev_dbg(ctx->cam->dev, "%s: Failed to get raw pipe data\n", __func__);
		return;
	}
	...
	if (ctx->used_raw_num) {
		...
		if (mtk_cam_is_time_shared(ctx)) {
			...
			// stream_on(raw_dev, 0);
			dev_dbg(ctx->cam->dev, "%s: Disable cammux: %s\n", __func__,
			    ...
			    struct mtkcam_ipi_input_param *cfg_in_param)
{
	...
	if (ctx->streaming) {
		for (i = 0; i < ARRAY_SIZE(raw->devs); i++) {
			if (pipe->enabled_raw & 1 << i) {
				dev_dbg(raw->cam_dev, "%s: power off raw (%d) for reset\n",
						 __func__, i);
	...
	for (i = 0; i < ARRAY_SIZE(raw->devs); i++) {
		if (pipe->enabled_raw & 1 << i) {
			dev_dbg(raw->cam_dev, "%s: power on raw (%d)\n", __func__, i);
	...
	if (ret < 0) {
		...
		while (i >= 0) {
			if (pipe->enabled_raw & 1 << i) {
				dev_dbg(raw->cam_dev, "%s: power off raw (%d)\n",
						 __func__, i);
				pm_runtime_put_sync(raw->devs[i]);
			}
			i--;
	...
	dev_dbg(raw->cam_dev, "ctx_id %d used_raw_dev 0x%x pipe_id %d\n",
			    ...
			    struct mtkcam_ipi_input_param *cfg_in_param)
{
	...
	s_raw_pipe_data = mtk_cam_s_data_get_raw_pipe_data(s_data);
	if (!s_raw_pipe_data) {
		dev_err(ctx->cam->dev, "%s: failed to get raw_pipe_data (pipe:%d, seq:%d)\n",
	...
	bool streaming, bool config_pipe)
{
	...
	s_raw_pipe_data = mtk_cam_s_data_get_raw_pipe_data(s_data);
	if (!s_raw_pipe_data) {
		dev_dbg(dev, "Failed to get raw pipe data\n");
		return -EINVAL;
	}
	feature = s_raw_pipe_data->res.raw_res.feature;
	...
	if (config_pipe && mtk_cam_feature_is_stagger(feature)) {
		...
		if (idle_pipes == 0) {
			dev_dbg(cam->dev, "no available sv pipes(scen:%d/req_amount:%d)",
				hw_scen, req_amount);
			return -EINVAL;
		}
	}
	...
	for (i = 0; i < RAW_PIPELINE_NUM; i++)
		if (raw->pipelines[i].enabled_raw & 1 << raw_dev->id) {
			...
			if (raw->pipelines[i].res_config.raw_num_used != 1) {
				struct mtk_raw_device *raw_dev_slav =
						get_slave_raw_dev(cam, ctx->pipe);
				if (!raw_dev_slav) {
					dev_err(dev, "raw_dev_slav not found\n");
					return -EINVAL;
				}
				raw_dev_slav->pipeline = &raw->pipelines[i];
				dev_dbg(dev, "twin master/slave raw_id:%d/%d\n",
					raw_dev->id, raw_dev_slav->id);
				if (raw->pipelines[i].res_config.raw_num_used == 3) {
					struct mtk_raw_device *raw_dev_slav2 =
						get_slave2_raw_dev(cam, ctx->pipe);
					if (!raw_dev_slav2) {
						dev_err(dev, "raw_dev_slav2 not found\n");
						return -EINVAL;
					}
					raw_dev_slav2->pipeline = &raw->pipelines[i];
					dev_dbg(dev, "triplet m/s/s2 raw_id:%d/%d/%d\n",
						raw_dev->id, raw_dev_slav->id, raw_dev_slav2->id);
				}
			}
			break;
		}
...
int mtk_cam_s_data_sv_dev_config(struct mtk_cam_request_stream_data *s_data)
{
	...
	s_raw_pipe_data = mtk_cam_s_data_get_raw_pipe_data(s_data);
	if (!s_raw_pipe_data) {
		dev_dbg(cam->dev, "%s Failed to get raw pipe data", __func__);
		return -EINVAL;
	}
	feature = s_raw_pipe_data->res.raw_res.feature;
	...
	for (i = MTKCAM_SUBDEV_CAMSV_START; i < MTKCAM_SUBDEV_CAMSV_END; i++) {
		...
		if (used_pipes & (1 << i)) {
			...
			if (ret) {
				dev_dbg(cam->dev, "%s failed(pipe:%d)", __func__, i);
				return ret;
			}
		}
	}
...
static int isp_composer_init(struct mtk_cam_ctx *ctx, unsigned int pipe_id)
{
	...
	snprintf_ret = snprintf(msg->name, RPMSG_NAME_SIZE, "mtk-camsys\%d", pipe_id);
	if (snprintf_ret < 0)
	...
	if (IS_ERR(ctx->rpmsg_dev->rpdev.ept)) {
		dev_dbg(dev, "%s failed rpmsg_create_ept, ctx:%d\n",
			 __func__, ctx->stream_id);
		goto faile_release_msg_dev;
	}

	dev_dbg(dev, "%s initialized composer of ctx:%d, ipi_id:%d\n",
...
static int mtk_cam_runtime_resume(struct device *dev)
{
	...
	if (dvfs_info->reg_vmm)
		return regulator_enable(dvfs_info->reg_vmm);
				      ...
				      struct mtk_cam_video_device *node)
{
	...
	dev_dbg(cam->dev, "%s:ctx(%d): triggered by %s\n",
	...
	if (is_first_ctx) {
		...
		dev_dbg(cam->dev, "%s: power on camsys\n", __func__);
	...
	if (IS_ERR(ctx->sensor_worker_task)) {
		dev_dbg(cam->dev, "%s:ctx(%d): could not create sensor_worker_task\n",
			 __func__, ctx->stream_id);
...
void mtk_cam_stop_ctx(struct mtk_cam_ctx *ctx, struct media_entity *entity)
{
	...
	if (ctx->session_created) {
		...
		if (wait_for_completion_timeout(
			...
			dev_dbg(cam->dev, "%s:ctx(%d): complete timeout\n",
			__func__, ctx->stream_id);
	}
...
int mtk_cam_ctx_stream_on(struct mtk_cam_ctx *ctx)
{
	...
	if (ctx->used_raw_num) {
		if (!ctx->pipe) {
			dev_dbg(cam->dev, "Failed to get pipe\n");
			return -EINVAL;
		}
		...
		if (mtk_cam_feature_is_stagger(feature_active)) {
			...
			for (i = MTKCAM_SUBDEV_CAMSV_START ; i < MTKCAM_SUBDEV_CAMSV_END ; i++) {
				...
				if (used_pipes & (1 << i)) {
					//HSF control
					if (mtk_cam_is_hsf(ctx)) {
						dev_dbg(cam->dev, "error: un-support hsf stagger mode\n");
						goto fail_img_buf_release;
					}
					...
					dev_dbg(cam->dev, "seninf_set_camtg(src_pad:%d/i:%d/camtg:%d)",
						src_pad_idx, i,
						cam->sv.pipelines[
							i - MTKCAM_SUBDEV_CAMSV_START].cammux_id);
					ret = mtk_cam_sv_dev_config(
			...
			for (i = MTKCAM_SUBDEV_CAMSV_START ; i < MTKCAM_SUBDEV_CAMSV_END ; i++) {
				if (used_pipes & (1 << i)) {
					...
					if (ret)
						goto fail_img_buf_release;
					src_pad_idx++;
					dev_dbg(cam->dev, "[TS] scen:0x%x/enabled_raw:0x%x/i(%d)",
					hw_scen, ctx->pipe->enabled_raw, i);
				}
			}
		} else if (ctx->pipe && mtk_cam_is_with_w_channel(ctx)) {
			for (i = MTKCAM_SUBDEV_CAMSV_START ; i < MTKCAM_SUBDEV_CAMSV_END ; i++) {
				if (ctx->pipe->enabled_raw & 1 << i) {
					int hw_scen =
		...
		if (mtk_cam_feature_is_stagger(feature_active)) {
			...
			if (mtk_cam_is_hsf(ctx)) {
				//HSF control
				dev_dbg(cam->dev, "enabled_hsf_raw =%d\n",
					ctx->pipe->res_config.enable_hsf_raw);
					ret = mtk_cam_hsf_config(ctx, raw_dev->id);
					if (ret != 0) {
						dev_dbg(cam->dev, "Error:enabled_hsf fail\n");
						goto fail_img_buf_release;
					}
...
fail_pipe_off:
	/* stop pipe subdevs here makes restart camera app error */
	/*
	for (i = 0; i < MAX_PIPES_PER_STREAM && ctx->pipe_subdevs[i]; i++)
	...
	for (i = 0;  i < cam->max_stream_num; i++) {
		...
		if (ctx_chk->pipe) {
			...
			ts_id_chk = pipe_chk->feature_active &
					MTK_CAM_FEATURE_TIMESHARE_MASK;
			dev_dbg(cam->dev, "[%s] i:%d pipe/ts:%d/0x%x chk_pipe/ts:%d/0x%x\n",
...
int mtk_cam_ctx_stream_off(struct mtk_cam_ctx *ctx)
{
	...
	if (ctx->pipe && watchdog_scenario(ctx))
		mtk_ctx_watchdog_stop(ctx);

	dev_dbg(cam->dev, "%s: ctx-%d:  composer_cnt:%d, streaming_pipe:0x%x\n",
	...
	if (ctx->used_raw_num) {
		if (ctx->pipe && mtk_cam_is_hsf(ctx)) {
			ret = mtk_cam_hsf_uninit(ctx);
			if (ret != 0) {
		...
		if (ctx->pipe && ctx->pipe->res_config.raw_num_used != 1) {
			struct mtk_raw_device *raw_dev_slav =
						get_slave_raw_dev(cam, ctx->pipe);
			if (!raw_dev_slav) {
				dev_err(cam->dev, "raw_dev_slav not found\n");
				return -EINVAL;
			}
			stream_on(raw_dev_slav, 0);
			if (ctx->pipe->res_config.raw_num_used == 3) {
				struct mtk_raw_device *raw_dev_slav2 =
					get_slave2_raw_dev(cam, ctx->pipe);
				if (!raw_dev_slav2) {
					dev_err(cam->dev, "raw_dev_slav2 not found\n");
					return -EINVAL;
				}
				stream_on(raw_dev_slav2, 0);
			}
		}
	}
	for (i = 0 ; i < ctx->used_sv_num ; i++) {
	...
	if (ctx->pipe && mtk_cam_feature_is_stagger(feature)) {
		unsigned int hw_scen = mtk_raw_get_hdr_scen_id(ctx);
				   ...
				   struct v4l2_subdev *seninf)
{
	...
	for (i = 0; i < cam->max_stream_num; i++) {
		if (i >= MTKCAM_SUBDEV_RAW_START &&
			...
			dev_dbg(cam->dev, "create pad link %s %s\n",
...
static int mtk_cam_master_bind(struct device *dev)
{
	...
	snprintf_ret = snprintf(media_dev->bus_info, sizeof(media_dev->bus_info),
		 "platform:%s", dev_name(dev));
	if (snprintf_ret < 0)
		dev_err(dev, "snprintf error\n");
...
static struct component_match *mtk_cam_match_add(struct device *dev)
{
	...
	dev_dbg(dev, "#: raw %d, yuv %d, larb %d, sv %d, seninf %d, mraw %d\n",
...
static void mtk_cam_ctx_watchdog_worker(struct work_struct *work)
{
	...
	if (atomic_read(&ctx->watchdog_dumped)) {
		...
		if (timeout) {
			dev_dbg(ctx->cam->dev, "%s:ctx(%d): timeout, VF(%d) vsync count(%d) sof count(%d) start dump (%dx100ms)\n",
				__func__, ctx->stream_id, atomic_read(&raw->vf_en),
				raw->vsync_count, raw->sof_count, watchdog_cnt);
			if (is_abnormal_vsync)
				dev_dbg(ctx->cam->dev, "abnormal vsync\n");
			...
			dev_dbg(ctx->cam->dev, "%s:ctx(%d): not timeout, for long exp (%dx100ms)\n",
...
static void mtk_ctx_watchdog(struct timer_list *t)
{
	...
	if (atomic_read(&ctx->watchdog_dumped)) {
		...
		if (watchdog_dump_cnt < 4) {
			dev_dbg_ratelimited(ctx->cam->dev, "%s:ctx(%d): timeout! VF(%d) vsync count(%d) sof count(%d) watcgdog_cnt(%d)(+%dms)\n",
				__func__, ctx->stream_id, atomic_read(&raw->vf_en),
				raw->vsync_count, raw->sof_count, watchdog_cnt,
			...
			dev_dbg_ratelimited(ctx->cam->dev, "%s:ctx(%d): dump > 3! watchdog_dump_cnt(%d)(+%dms)\n",
...
{
	dev_dbg(ctx->cam->dev, "%s:ctx(%d):start the watchdog, timeout setting(%d)\n",
...
{
	dev_dbg(ctx->cam->dev, "%s:ctx(%d):stop the watchdog\n",
		__func__, ctx->stream_id);
			  ...
			  struct v4l2_subdev_format *sink_fmt)
{
	...
	if (ret)
		dev_dbg(dev, "%s: link validate failed pad/code/w/h: SRC(%d/0x%x/%d/%d), SINK(%d:0x%x/%d/%d)\n",
...
static int mtk_cam_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_dbg(dev, "fail to register_sub_drivers\n");
		goto fail_destroy_mutex;
	}
