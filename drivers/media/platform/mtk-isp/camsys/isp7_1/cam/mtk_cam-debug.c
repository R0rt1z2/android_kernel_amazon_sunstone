			      ...
			      struct mtk_cam_device *cam, int content_size)
{
	...
	for (i = 0; i < cam->max_stream_num; i++) {
		...
		if (ret < 0)
			dev_info(cam->dev, "%s Failed to read name: %d\n",
			 __func__, ret);
...
static void mtk_cam_exception_work(struct work_struct *work)
{
	...
	if (ret < 0)
		dev_info(ctx->cam->dev, "%s Failed to read title_desc: %d\n",
			 __func__, ret);
	...
	ret = snprintf(warn_desc, 48, "%s:ctx(%d):req(%d):%s",
		 ...
		 dbg_work->desc);
	if (ret < 0)
		dev_info(ctx->cam->dev, "%s Failed to read warn_desc: %d\n",
			 __func__, ret);
		     ...
		     unsigned int dump_flag, char *desc, bool smi_dump)
{
	...
	snprintf_ret = snprintf(dbg_work->desc, MTK_CAM_DEBUG_DUMP_DESC_SIZE - 1, desc);
	if (snprintf_ret < 0)
		dev_err(ctx->cam->dev, "%s: snprintf error\n", __func__);
