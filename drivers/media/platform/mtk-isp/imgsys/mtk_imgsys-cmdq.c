...
void imgsys_cmdq_streamon(struct mtk_imgsys_dev *imgsys_dev)
{
	...
	dev_dbg(imgsys_dev->dev, "%s: cmdq stream on (%d)\n", __func__, is_stream_off);
	is_stream_off = 0;
...
static void imgsys_cmdq_cb_work(struct work_struct *work)
{
	...
	if (cb_param->err != 0)
		pr_info(
			"%s: [ERROR] cb(%p) error(%d) gid(%d) for frm(%d/%d) blk(%d/%d) lst(%d/%d) earlycb(%d) task(%d/%d/%d) ofst(%x/%x/%x/%x/%x)",
...
void imgsys_cmdq_task_cb(struct cmdq_cb_data data)
{
	...
	if (cb_param->err != 0) {
		...
		if (err_idx >= cb_param->task_cnt) {
			pr_info(
				"%s: [ERROR] can't find task in task list! cb(%p) error(%d) for frm(%d/%d) blk(%d/%d) erridx(%d/%d) task(%d/%d/%d) ofst(%x/%x/%x/%x/%x)",
				__func__, cb_param, cb_param->err, cb_param->group_id,
				cb_param->frm_idx, cb_param->frm_num,
				cb_param->blk_idx, cb_param->blk_num,
		...
		pr_info(
			"%s: [ERROR] cb(%p) error(%d) gid(%d) for frm(%d/%d) blk(%d/%d) lst(%d/%d/%d) earlycb(%d) erridx(%d/%d) task(%d/%d/%d) ofst(%x/%x/%x/%x/%x)",
			__func__, cb_param, cb_param->err, cb_param->group_id,
			cb_param->frm_idx, cb_param->frm_num,
			cb_param->blk_idx, cb_param->blk_num,
			cb_param->isBlkLast, cb_param->isFrmLast, cb_param->isTaskLast,
		...
		if (event <= IMGSYS_CMDQ_HW_EVENT_END) {
			isHWhang = 1;
			pr_info(
...
void mtk_imgsys_mmdvfs_uninit(struct mtk_imgsys_dev *imgsys_dev)
{
	...
	if (IS_ERR_OR_NULL(dvfs_info->reg))
		...
		ret = isp_direct_set_voltage(dvfs_info->reg, volt, INT_MAX, VMM_ISP_IMG);
	#else
