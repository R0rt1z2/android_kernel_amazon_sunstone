...
void imgsys_main_init(struct mtk_imgsys_dev *imgsys_dev)
{
	...
	if (of_address_to_resource(imgsys_dev->dev->of_node,
		REG_MAP_E_ADL_A, &adl)) {
		dev_info(imgsys_dev->dev, "%s of_address_to_resource fail, devnode(%s).\n",
				__func__, imgsys_dev->dev->of_node->name);
		return;
	}
	if (adl.start) {
		...
		adlARegBA = NULL;
		adlBRegBA = NULL;
		dev_dbg(imgsys_dev->dev, "%s Do not have ADL hardware.\n", __func__);
	}
