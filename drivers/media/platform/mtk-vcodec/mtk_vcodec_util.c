	...
	const char *param_key, const char *param_val, enum mtk_vcodec_log_index index)
{
	...
	if (pram == NULL) {
		mtk_v4l2_err("param  alloc fail");
		mutex_unlock(plist_mutex);
		return;
	}
	if (strlen(param_key) >= (LOG_PARAM_INFO_SIZE - 1))
		cpy_size = (LOG_PARAM_INFO_SIZE - 1);
	else
		cpy_size = strlen(param_key);
	strncpy(pram->param_key, param_key, cpy_size);

	if (strlen(param_val) >= (LOG_PARAM_INFO_SIZE - 1))
		cpy_size = (LOG_PARAM_INFO_SIZE - 1);
	else
		cpy_size = strlen(param_val);
	strncpy(pram->param_val, param_val, cpy_size);
