	...
	struct mtk_vcodec_mem *current_bs)
{
	...
	for (i = 0; i < MAX_GEN_BUF_CNT; ++i) {
		if (ctx->dma_buf_list[i].va && ctx->dma_buf_list[i].dmabuf) {
			int *dma_va = ctx->dma_buf_list[i].va;
...
int mtk_vdec_put_fb(struct mtk_vcodec_ctx *ctx, enum mtk_put_buffer_type type)
{
	...
	if (ctx == NULL || ctx->m2m_ctx == NULL){
		mtk_v4l2_debug(0, "ctx or m2m_ctx is NULL");
		return 0;
	}
...
static void mtk_vdec_worker(struct work_struct *work)
{
	...
	if (src_buf_info->lastframe == EOS) {
		...
		if (dst_buf_info && !ctx->input_driven) {
			mutex_lock(&ctx->buf_lock);
			if (dst_buf_info->used) {
				for (i = 0; i < num_planes; i++) {
					dma_buf_put(dst_buf->planes[i].dbuf);
					mtk_v4l2_debug(4, "[Ref cnt] id=%d Ref put dma %p",
						dst_buf->planes[i].dbuf);
				}
			}
			dst_buf_info->used = false;
			mtk_v4l2_debug(1, "reset used flag in EOS");
			mutex_unlock(&ctx->buf_lock);
		}
