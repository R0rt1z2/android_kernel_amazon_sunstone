...
int mtk_vcodec_init_dec_pm(struct mtk_vcodec_dev *mtkdev)
{
	...
	mtk_v4l2_debug(2, "%s at: %d", __func__, __LINE__);
...
void mtk_vcodec_dec_clock_on(struct mtk_vcodec_pm *pm, int hw_id)
{
	...
	void __iomem *vdec_racing_addr = NULL;
	int clk_id = 0;
	struct mtk_vdec_clks_data *clks_data = NULL;
	...
	for (j = 0; j < clks_data->main_clks_len; j++) {
		...
		if (clk_id >= 0 && clk_id < MTK_MAX_VDEC_CLK_COUNT) {
