					  ...
					  void *out)
{
	...
	if (!ctx->drv_handle) {
		...
		drv_handle_exist = 0;
		mtk_v4l2_debug(2, "%s init drv_handle = 0x%lx",
	...
	unsigned int core_id, unsigned long *flags)
{
	...
	if (ctx->enc_params.use_irq == 1)
		venc_lock(ctx, core_id, ctx->enc_params.svp_mode);
	...
	struct venc_done_result *result)
{
	...
	if (ctx->enc_params.use_irq == 1)
		venc_encode_prepare(ctx, MTK_VENC_CORE_0, &flags);
	vcodec_trace_begin("%s", __func__);
	ret = ctx->enc_if->encode(ctx->drv_handle, opt, frm_buf,
							  bs_buf, result);
	if (ctx->enc_params.use_irq == 1)
		venc_encode_unprepare(ctx, MTK_VENC_CORE_0, &flags);
	vcodec_trace_end();
	return ret;
}
...
int venc_lock(void *ctx_lock, unsigned int core_id, bool sec)
{
...
void venc_unlock(void *ctx_unlock, unsigned int core_id)
{
