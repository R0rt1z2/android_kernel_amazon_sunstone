...
static void mtk_vdec_do_gettimeofday(struct timespec64 *tv)
{
...
long long get_time_us(void)
{
	...
	mtk_vdec_do_gettimeofday(&tv);
	return (1000000LL * tv.tv_sec + tv.tv_nsec);
}
...
/**
 * free_next_hist - free next codec_history and shift next one forward
 *                  (not used)
 *
...
long long est_next_submit(struct codec_history *hist)
{
	...
	/* Add 2x estimated gap for next job */
	if (hist->cur_cnt == 1)
