...
#define ENABLE_SLBC 0
struct platform_device *venc_pm_dev[MTK_VENC_MAX_LARB_COUNT] = { NULL };
...
void mtk_vcodec_enc_clock_on(struct mtk_vcodec_ctx *ctx, int core_id)
{
	...
	for (i = 0; i < MTK_VENC_MAX_LARB_COUNT; i++) {
		if (venc_pm_dev[i]) {
			ret = pm_runtime_get_sync(&venc_pm_dev[i]->dev);
			if (ret < 0)
	...
	if (core_id == MTK_VENC_CORE_0 ||
		...
		for (j = 0; j < clks_data->core_clks_len; j++) {
			...
			if (clk_id >= 0 && clk_id < MTK_MAX_VENC_CLK_COUNT) {
...
void mtk_vcodec_enc_clock_off(struct mtk_vcodec_ctx *ctx, int core_id)
{
	...
	for (i = 0; i < MTK_VENC_MAX_LARB_COUNT; i++) {
		if (venc_pm_dev[i])
			pm_runtime_put_sync(&venc_pm_dev[i]->dev);
	}
#endif
}
