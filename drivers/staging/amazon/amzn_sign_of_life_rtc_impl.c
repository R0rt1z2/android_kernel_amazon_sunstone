 ...
 *
 * Copyright (C) 2022 Amazon Technologies Inc. All rights reserved.
...
static char __lcr_cmdline[COMMAND_LINE_SIZE];
static char *lcr_cmdline = __lcr_cmdline;

static const char *lcr_get_cmd(void)
{
	struct file *fd;
	mm_segment_t fs;
	loff_t pos = 0;

	if (__lcr_cmdline[0] != 0)
		return lcr_cmdline;
	...
	if (IS_ERR(fd)) {
		pr_err("Unable to open /proc/cmdline (%ld)",
				PTR_ERR(fd));
		set_fs(fs);
		return lcr_cmdline;
	}
	kernel_read(fd, (void *)lcr_cmdline, COMMAND_LINE_SIZE, &pos);
	filp_close(fd, NULL);
	fd = NULL;
	set_fs(fs);
	return lcr_cmdline;
...
int get_android_bootreason(void)
{
	char bootreason_str[32] = {0};
	char *ptr = NULL, *ptr_e = NULL;
	char keyword[] = "androidboot.bootreason";
	int size = 0;

	ptr = strstr(lcr_get_cmd(), keyword);
	if (ptr != 0) {
		ptr_e = strstr(ptr, " ");
		if (ptr_e == 0)
			goto end;
		...
		if (size <= 0)
			goto end;
		strncpy(bootreason_str, ptr + strlen(keyword), size);
		bootreason_str[size] = '\0';
		pr_err("bootreason_str is %s\n", bootreason_str);
		if (!strcmp(POWER_KEY_BOOT, bootreason_str))
			lcr_boot = COLDBOOT_BY_POWER_KEY;
		else if (!strcmp(USB_BOOT, bootreason_str))
			lcr_boot = COLDBOOT_BY_USB;
		else if (!strcmp(WATCHDOG_SW, bootreason_str))
			lcr_boot = WARMBOOT_BY_KERNEL_WATCHDOG;
		else if (!strcmp(WATCHDOG_HW, bootreason_str))
			lcr_boot = WARMBOOT_BY_HW_WATCHDOG;
		else if (!strcmp(TWO_SEC_REBOOT, bootreason_str))
			lcr_boot = WARMBOOT_BY_2SEC_REBOOT;
		else if (!strcmp(POWER_LOSS, bootreason_str))
			lcr_boot = SHUTDOWN_BY_SUDDEN_POWER_LOSS;
		else if (!strcmp(OVER_CUR_PRO, bootreason_str))
			lcr_boot = SHUTDOWN_BY_OCP;
		else if (!strcmp(KERNEL_PANIC, bootreason_str))
			lcr_boot = WARMBOOT_BY_KERNEL_PANIC;
		else
