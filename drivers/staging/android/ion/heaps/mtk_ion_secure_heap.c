...
static
struct ion_sec_heap ion_svp_heap[] = {
	{	.name = "ion_svp_heap",
...
#elif defined(SECMEM_KERNEL_API)
static struct ion_sec_heap *ion_svp_heap;
				 ...
				 unsigned long flags)
{
	struct ion_sec_heap *sec_heap =
		container_of(heap, struct ion_sec_heap, heap);
	...
	pr_debug("%s enter name:%s, size:%lu\n", __func__, heap->name, size);
	...
	ION_DBG("sec hdl %u (%s), sz 0x%lx (%s) -- %llu ns\n",
		sec_handle, heap->name, size, buf_info->task_comm, tm_end);
...
static void ion_sec_heap_free(struct ion_buffer *buffer)
{
	...
	pr_debug("%s enter name:%s, size:%zu\n",
		 __func__, heap->name, buffer->size);
...
static struct ion_heap *__ion_sec_heap_create(struct ion_sec_heap *sec_heap)
{
	pr_info("%s enter\n", __func__);
	...
	memcpy(&sec_heap->heap.buf_ops, &mtk_ion_sec_buf_ops,
	       sizeof(struct dma_buf_ops));
...
static int ion_sec_heap_create(void)
{
	...
	for (i = 0; i < heap_total; i++) {
		heap = __ion_sec_heap_create(&ion_svp_heap[i]);
		if (IS_ERR(heap))
			return PTR_ERR(heap);

		heap->name = ion_svp_heap[i].name;
		ret = ion_device_add_heap(heap);
		pr_info("type_id:%d, name:%s, heap_id:%u (ret %d)\n",
			ion_svp_heap[i].type, heap->name, heap->id, ret);
	}
	...
	ion_svp_heap = kzalloc(sizeof(struct ion_sec_heap) * heap_total,
			       GFP_KERNEL);
	if (!ion_svp_heap)
		return -ENOMEM;
	...
	for (i = 0; i < heap_total; i++) {
		...
		ion_svp_heap[i].ssmr_id = ssmr_id;
		heap = __ion_sec_heap_create(&ion_svp_heap[i]);
		if (IS_ERR(heap))
			return PTR_ERR(heap);
