...
static int force_dvfs_set(void *data, u64 val)
{
	...
	ISP_LOGI("Force max dvfs control(%llu)", val);
...
int isp_apmcu_set_voltage(struct regulator *reg, int min_uV, int max_uV)
{
	...
	ret = 0;
	regulator = rdev_get_drvdata(reg->rdev);
	if (!regulator) {
	...
	if (target_voltage != current_info->voltage_target) {
		for (i = 0; i < table->opp_num; i++) {
		...
		if (cur_opp_idx < prev_opp_idx) {
			...
			/* Then update voltage */
			regulator_set_voltage(vmm_reg, target_voltage, VMM_MAX_VOLTAGE);
...
EXPORT_SYMBOL(isp_apmcu_set_voltage);

int apmcu_set_voltage(struct regulator_dev *rdev,
