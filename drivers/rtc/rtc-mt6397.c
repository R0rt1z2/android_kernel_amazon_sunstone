...
static struct mt6397_rtc *mt6397_rtc_proxy;

int mtk_rtc_mark_sw_lprst(void)
{
	...
	data |= RTC_SPAR0_LPRST;

	ret = regmap_write(rtc->regmap, rtc->addr_base + RTC_SPAR0, data);
	if(ret < 0)
		goto err_exit;
...
EXPORT_SYMBOL(mtk_rtc_mark_sw_lprst);

int mtk_rtc_write(u16 addr, u16 data)
{
	struct mt6397_rtc *rtc = mt6397_rtc_proxy;
	int ret = -1;

	if (rtc)
		ret = regmap_write(rtc->regmap, rtc->addr_base + (u32)addr, (u32)data);

	return ret;
}
EXPORT_SYMBOL(mtk_rtc_write);

u16 mtk_rtc_read(u16 addr)
{
	struct mt6397_rtc *rtc = mt6397_rtc_proxy;
	u32 data = 0;

	if (rtc)
		regmap_read(rtc->regmap, rtc->addr_base + (u32)addr, &data);

	return (u16)data;
}
EXPORT_SYMBOL(mtk_rtc_read);

void mtk_rtc_write_trigger_out(void)
{
	struct mt6397_rtc *rtc = mt6397_rtc_proxy;
	if (rtc)
		mtk_rtc_write_trigger(rtc);
}
EXPORT_SYMBOL(mtk_rtc_write_trigger_out);

void mtk_rtc_acquire_lock(void)
{
	struct mt6397_rtc *rtc = mt6397_rtc_proxy;
	if (rtc)
		mutex_lock(&rtc->lock);
}
EXPORT_SYMBOL(mtk_rtc_acquire_lock);

void mtk_rtc_release_lock(void)
{
	struct mt6397_rtc *rtc = mt6397_rtc_proxy;
	if (rtc)
		mutex_unlock(&rtc->lock);
}
EXPORT_SYMBOL(mtk_rtc_release_lock);
