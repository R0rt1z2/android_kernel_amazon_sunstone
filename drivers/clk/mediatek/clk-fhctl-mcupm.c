		...
		unsigned int new_dds, int postdiv)
{
	...
	if (has_err) {
		...
		FHNOT("domain<%s>, fh_id<%d>\n",
				domain_name, fh_id);
		FHNOT("ret<%d>, ack<%x>, tr<%x>\n",
				ret, ack_data, d->tr_id);
		FHNOT("con_pcw<%x>, new_dds<%x>\n",
				con_pcw, new_dds);
...
static int mcupm_init_v1(struct pll_dts *array, struct match *match)
{
	...
	if (!ipi_inited) {
		...
		ret = mtk_ipi_register(get_mcupm_ipidev(), CH_S_FHCTL, NULL,
				NULL, (void *)&ack_data);
		if (ret) {
			FHNOT("[MCUPM] ipi_register fail, ret %d\n", ret);
			return -1;
		}
		ipi_inited = true;
		FHDBG("\n");
...
struct hdlr_data_v1 hdlr_data_8188 = {
	.reg_tr = NULL,
};
static struct fh_hdlr mcupm_hdlr_8188 = {
	.ops = &mcupm_ops_v1,
	.data = &hdlr_data_8188,
};
static struct match mt8188_match = {
	.name = "mediatek,mt8188-fhctl",
	.hdlr = &mcupm_hdlr_8188,
	.init = &mcupm_init_v1,
};
