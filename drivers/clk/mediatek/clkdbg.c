...
static u32 *read_spm_pwr_status(void)
{
	...
	static u32 val[PWR_STA_GROUP_NR] = {0};
	...
	if (clkdbg_ops == NULL || clkdbg_ops->get_spm_pwr_status  == NULL) {
		...
		val[0] = clk_readl(pwr_sta) & clk_readl(pwr_sta_2nd);
	} else {
		val[0] = clkdbg_ops->get_spm_pwr_status();
	}
...
static void show_pwr_status(u32 *pwr_status)
{
	...
	if (clkdbg_ops->get_all_pwr_status) {
		...
		for (i = 0; i < 32; i++) {
			const char *st = (pwr_status[0] & BIT(i)) != 0U ? "ON" : "OFF";
...
static int clkdbg_dump_power_domain_status(struct seq_file *s, void *v)
{
	...
	dev = dev_from_name(dev_name);
	if (dev != NULL) {
		bool r = clkdbg_get_power_domain_status(dev);
