...
int pwr_hw_is_on(enum PWR_STA_TYPE type, u32 mask)
{
	...
	if (type == PWR_STA) {
		if ((pval[PWR_STA] & mask) != mask &&
			...
			return -1;
	} else if (type == CPU_PWR_STA) {
		if ((pval[CPU_PWR_STA] & mask) != mask &&
				(pval[CPU_PWR_STA2] & mask) != mask)
			return 0;
		else if ((pval[CPU_PWR_STA] & mask) == mask &&
				(pval[CPU_PWR_STA2] & mask) == mask)
			return 1;
		else
			return -1;
...
static bool clkchk_hw_is_on(struct clk_hw *hw)
{
...
bool clkchk_pvdck_is_enabled(struct provider_clk *pvdck)
{
	...
	if (clkchk_pvdck_is_on(pvdck) == 1) {
		hw = __clk_get_hw(pvdck->ck);
		p_hw = hw ? clk_hw_get_parent(hw) : NULL;
...
static int clk_chk_dev_pm_suspend(struct device *dev)
{
	...
	if (clkchk_ops && clkchk_ops->dump_pwr_status)
		clkchk_ops->dump_pwr_status();
