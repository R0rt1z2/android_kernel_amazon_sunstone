...
static struct pll_dts *parse_dt(struct platform_device *pdev)
{
	...
	for_each_child_of_node(root, map) {
		...
		ret = of_property_read_string(map, "domain",
					      (const char **)&domain);
		if (ret) {
			FHNOT("Cannot get property: domain(%d)\n", ret);
			continue;
		}

		ret = of_property_read_string(map, "method",
					      (const char **)&method);
		if (ret) {
			FHNOT("Cannot get property: method(%d)\n", ret);
			continue;
		}
		...
		FHDBG("---------------------\n");
		for_each_child_of_node(map, of_pll) {
			int fh_id = INITIAL_ID;
			int pll_id = INITIAL_ID;
			int perms, ssc_rate;
