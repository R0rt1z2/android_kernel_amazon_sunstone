...
static char* ntc_name[]={"ap_ntc", "sdcard_ntc","pm6365_ntc","usb_ntc","charge_ntc"};
...
static int get_little_core_max_power()
{
	unsigned int num_cpus = 0;
	int index = 0;

	index = (cpufreq_cdev[CPU_CLUSTER_0])->max_level;
	num_cpus = cpumask_weight(cpufreq_cdev[CPU_CLUSTER_0]->policy->related_cpus);

	return cpufreq_cdev[CPU_CLUSTER_0]->em->table[index].power * num_cpus;
}
...
static int set_cpufreq_thermal_power(unsigned int budget)
{
	...
	int i, ret = 0;
	unsigned int cpu_buget[CPU_CLUSTER_NUM];

	if (!cpufreq_cdev[0])
		for (i = 0; i < CPU_CLUSTER_NUM; i++)
			vs_init_cpufreq_limit_power(i, cpu_polices[i]);

	little_cpu_max_power = get_little_core_max_power();
	little_cpu_min_power = get_cpu_cluster_min_power(CPU_CLUSTER_0);
	large_cpu_min_power = get_cpu_cluster_min_power(CPU_CLUSTER_1);

	cpu_buget[CPU_CLUSTER_0] = little_cpu_min_power;
	cpu_buget[CPU_CLUSTER_1] = large_cpu_min_power;
	remain_budget = budget - (little_cpu_min_power + large_cpu_min_power);
	if (remain_budget > 0) {
		if (remain_budget <= (little_cpu_max_power - cpu_buget[CPU_CLUSTER_0])) {
			cpu_buget[CPU_CLUSTER_0] += remain_budget;
		} else {
			cpu_buget[CPU_CLUSTER_0] = little_cpu_max_power;
			cpu_buget[CPU_CLUSTER_1] = budget - little_cpu_max_power;
		}
	} else {
		pr_notice("VS temp is to high,power is less than Sum of large and small cores!");
	}
	pr_info("budget:%u buget[0]:%u buget[1]:%u\n",budget,
		cpu_buget[CPU_CLUSTER_0],cpu_buget[CPU_CLUSTER_1]);
