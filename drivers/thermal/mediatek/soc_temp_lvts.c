...
static int soc_thermal_notify(struct thermal_zone_device *thermal,
...
static void enable_all_sensing_points(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		if ((tc[i].ts_offset == 1) && (num == 1))
			...
			writel(ENABLE_SENSING_POINT(num),
			       LVTSMONCTL0_0 + base);
	}
}
...
static int lvts_register_thermal_zones(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_sensor + 1; i++) {
		...
		if (IS_ERR(tzdev)) {
			...
			tzdev->ops->notify = soc_thermal_notify;
		}

		ret = thermal_add_hwmon_sysfs(tzdev);
		if (ret)
			dev_warn(dev, "Error: lvts tz %d thermal_add_hwmon_sysfs fail\n"
						, lvts_tz->id);
	}
...
#define MT8188_NUM_LVTS (ARRAY_SIZE(mt8188_tc_settings))
...
static void mt8188_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	for (i = 0 ; i < MT8188_NUM_TS; i++)
		dev_info(dev, "[lvts_cal] g_count_r[%d] = %u\n",
			i, cal_data->count_r[i]);
	for (i = 0 ; i < LVTS_CONTROLLER_NUM; i++)
		dev_info(dev, "[lvts_cal] g_count_rc[%d] = %u\n",
			i, cal_data->count_r[i]);
}

static struct tc_settings mt8188_tc_settings[] = {
	[LVTS_MCU_CONTROLLER0] = {
