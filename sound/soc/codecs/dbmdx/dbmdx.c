...
#ifdef DBMDX_UC_USE_CPU_FREQ_QOS
...
#define DRIVER_VERSION				"4.1.20230131"
...
#define DBMDX_METRICS_LOG_MAX_SIZE (128)
#endif
...
#ifdef DBMDX_UC_USE_CPU_FREQ_QOS
static int cpufreq_policy_num = 0;
static struct cpufreq_list_struct cpufreq_policy_list;
static struct freq_qos_request *cpufreq_min_request = NULL;
...
#ifdef DBMDX_UC_USE_CPU_FREQ_QOS
static void CPU_freq_deinit(void)
{
	int i = 0;
	struct cpufreq_list_struct *cpufreq_policy_node = NULL;
	struct cpufreq_list_struct *cpufreq_policy_node_tmp = NULL;

	list_for_each_entry_safe(cpufreq_policy_node,
		cpufreq_policy_node_tmp,
		&cpufreq_policy_list.list,
		list) {
		cpufreq_cpu_put(cpufreq_policy_node->policy);
		list_del(&cpufreq_policy_node->list);
		kfree(cpufreq_policy_node);
	}

	if (cpufreq_min_request) {
		for (i = 0; i < cpufreq_policy_num; i ++)
			freq_qos_remove_request(&(cpufreq_min_request[i]));
		kfree(cpufreq_min_request);
		cpufreq_min_request = NULL;
	}
}

static void CPU_freq_init(void)
{
	...
	cpufreq_policy_num = 0;
	INIT_LIST_HEAD(&cpufreq_policy_list.list);
	for_each_possible_cpu(cpu) {
		policy = cpufreq_cpu_get(cpu);
		if (policy) {
			pr_debug("%s, policy[%d]: first:%d, min:%d, max:%d",
				__func__, cpufreq_policy_num, cpu, policy->min, policy->max);
			cpufreq_policy_num ++;
			cpufreq_policy_node = (struct cpufreq_list_struct *)
				kzalloc(sizeof(struct cpufreq_list_struct), GFP_KERNEL);
			cpufreq_policy_node->policy = policy;
			cpufreq_policy_node->cpu_no = cpu;
			list_add(&cpufreq_policy_node->list, &cpufreq_policy_list.list);
			cpu = cpumask_last(policy->related_cpus);
			cpufreq_cpu_put(policy);
		}
	}

	cpufreq_min_request = kcalloc(cpufreq_policy_num,
				sizeof(struct freq_qos_request),
				GFP_KERNEL);

	if (cpufreq_min_request) {
		list_for_each_entry(cpufreq_policy_node,
			&cpufreq_policy_list.list,
			list) {
			freq_qos_add_request(&cpufreq_policy_node->policy->constraints,
				&(cpufreq_min_request[i]),
				FREQ_QOS_MIN,
				cpufreq_policy_node->policy->min);
			i ++;
		}
	} else {
		CPU_freq_deinit();
		pr_warn("%s: cannot get cpu policy\n", __func__);
	}
}

static void CPU_freq_max(void)
{
	int i = 0;
	struct cpufreq_list_struct *cpufreq_policy_node = NULL;
	...
	if (cpufreq_min_request) {
		...
		list_for_each_entry(cpufreq_policy_node,
			&cpufreq_policy_list.list,
			list) {
			freq_qos_update_request(&(cpufreq_min_request[i]),
				cpufreq_policy_node->policy->max);
			i ++;
		}
...
static void CPU_freq_default(void)
{
	int i = 0;
	struct cpufreq_list_struct *cpufreq_policy_node = NULL;

	/* restore CPU freq to default */
	if (cpufreq_min_request) {
		mutex_lock(&cpufreq_lock);
		list_for_each_entry(cpufreq_policy_node,
			&cpufreq_policy_list.list,
			list) {
			freq_qos_update_request(&(cpufreq_min_request[i]), 0);
			i ++;
		}
		mutex_unlock(&cpufreq_lock);
	}
					...
					enum dbmdx_clocks dbmdx_clk)
{
	...
	clk = p->clocks[dbmdx_clk];
	rate = p->pdata->clock_rates[dbmdx_clk];
...
static int dbmdx_va_ve_alive_with_lock(struct dbmdx_private *p)
{
	int ret = 0;
	int ret2 = 0;
	enum dbmdx_chip	cur_active_chip;

	if (!p)
		return -EAGAIN;

	cur_active_chip = p->active_chip;
	...
	case DBMDX_PM_ACTIVE:
		...
		case DBMDX_PM_ACTIVE:
			...
			if (ret) {
...
#if IS_ENABLED(CONFIG_AMAZON_MINERVA_METRICS_LOG)
				log_counter_to_vitals_v2(ANDROID_LOG_INFO, VITALS_DSP_GROUP_ID,
					VITALS_DSP_COUNTER_SCHEMA_ID, "Kernel", "Kernel",
					"DBMD4_DSP_metrics_count", "DSP_Watchdog", 1, "count",
				...
				log_to_metrics(ANDROID_LOG_INFO, "voice_dsp",
					"voice_dsp:def:DSP_Watchdog=1;CT;1:NR");
#endif
				goto out;
				}
			break;
...
static void dbmdx_delayed_pm_work_hibernate_va_ve(struct work_struct *work)
{
	...
	if (!p) {
		dev_err(p->dev, "%s: NULL pointer p\n", __func__);
		return;
	}
	if (!(p->chip)) {
		dev_err(p->dev, "%s: NULL pointer p->chip\n", __func__);
		return;
	}
...
int dbmdx_set_usecase_mode(struct dbmdx_private *p, int mode)
{
	...
	cur_opmode = p->va_ve_flags.usecase_mode;
...
u8 *adfModel_PreLoad(struct firmware *wakewordModel)
{
	...
	if (!p) {
		dev_err(p->dev, "%s: NULL pointer dbmdx_data\n",
		__func__);
		return NULL;
	}
...
static int dbmdx_perform_recovery(struct dbmdx_private *p)
{
	...
	log_counter_to_vitals_v2(ANDROID_LOG_INFO, VITALS_DSP_GROUP_ID,
			VITALS_DSP_COUNTER_SCHEMA_ID, "Kernel", "Kernel",
			"DBMD4_DSP_metrics_count", "DSP_Reset", 1, "count", NULL,
					...
					bool to_start_buffering)
{
	...
	if (!p->va_ve_flags.ignore_vt_data) {
		...
		if (p->pdata->send_uevent_on_detection) {
			...
			log_counter_to_vitals_v2(ANDROID_LOG_INFO, VITALS_DSP_GROUP_ID,
				VITALS_DSP_COUNTER_SCHEMA_ID, "Kernel", "Kernel",
				"DBMD4_DSP_metrics_count", "DSP_IRQ", 1, "count", NULL,
				VITALS_NORMAL, NULL, NULL);
			...
			log_counter_to_vitals(ANDROID_LOG_INFO, "Kernel",
				"Kernel", "DBMD4_DSP_metrics_count", "DSP_IRQ",
				1, "count", NULL, VITALS_NORMAL);

			log_to_metrics(ANDROID_LOG_INFO, "voice_dsp",
				"voice_dsp:def:DSP_IRQ=1;CT;1:NR");
#endif
		}
	}
			...
			if (nr_samples >= 2*samples_chunk_size)
				...
				if (p->fgMetricLogPrint == false) {
					...
					if (p->dspStreamDuration >= MIN_SUSPEND_AUDIO_DURATION) {
						...
						log_timer_to_vitals_v2(ANDROID_LOG_INFO, VITALS_DSP_GROUP_ID,
							VITALS_DSP_TIMER_SCHEMA_ID, "Kernel", "Kernel",
							"DBMD4_DSP_metrics_time", "DSP_DATA_CATCH_UP_FINISH",
...
#elif IS_ENABLED(CONFIG_AMAZON_METRICS_LOG)
					if (p->dspStreamDuration >= MIN_SUSPEND_AUDIO_DURATION) {
						snprintf(dbmdx_metrics_log_buf, sizeof(dbmdx_metrics_log_buf),
							"voice_dsp:def:DSP_catchup_ms=%lld;TI;1:NR",
							ktime_to_ms(Current) - ktime_to_ms(p->StreamOpenTime));
						log_to_metrics(ANDROID_LOG_INFO, "voice_dsp",
							dbmdx_metrics_log_buf);
...
static void dbmdx_do_metrics_upload_work(struct work_struct *work)
{
...
#if IS_ENABLED(CONFIG_AMAZON_METRICS_LOG) || IS_ENABLED(CONFIG_AMAZON_MINERVA_METRICS_LOG)
	char dbmdx_metrics_log_buf[DBMDX_METRICS_LOG_MAX_SIZE] = {'\0'};
	...
	if (dbmdx_data->device_ready) {
		...
		if (freqTs) {
			...
			*/
#if IS_ENABLED(CONFIG_AMAZON_MINERVA_METRICS_LOG)
			ret = minerva_metrics_log(dbmdx_metrics_log_buf, DBMDX_METRICS_LOG_MAX_SIZE,
				"%s:%s:100:%s,%s,%s,FREQ_IDL_TS=%d;IN,FREQ_RUN_TS=%d;IN,"
				"FREQ_VAD_TS=%d;IN:us-east-1", METRICS_DSP_GROUP_ID,
				METRICS_DSP_FREQ_SCHEMA_ID, PREDEFINED_ESSENTIAL_KEY,
				PREDEFINED_DEVICE_ID_KEY, PREDEFINED_DEVICE_LANGUAGE_KEY,
				*freqTs / 1000, *(freqTs + 1) / 1000, *(freqTs + 2) / 1000);
			if (ret)
				pr_err("minerva_metrics_log: fails to upload dsp freq duration %d\n", ret);
#elif IS_ENABLED(CONFIG_AMAZON_METRICS_LOG)
			snprintf(metricsLogBuf, DSP_METRICS_LOG_MAX_SIZE,
...
static int dbmdx_platform_probe(struct platform_device *pdev)
{
	...
	/*we don`t want to get dummy regulator*/
	p->vregulator = devm_regulator_get_optional(p->dev, "dbmdx_regulator");
	...
	p->dbmdx_workq = alloc_workqueue("dbmdx-wq", WQ_HIGHPRI, 0);
	if (!p->dbmdx_workq) {
