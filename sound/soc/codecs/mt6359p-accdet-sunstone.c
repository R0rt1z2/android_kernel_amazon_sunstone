...
#include "../../../drivers/misc/was4780.h"
#endif
...
/* check sprintf function return value*/
#define SPRINTF_CHECK(x)             \
({ if (x < 0) \
	 pr_notice("sprintf failed, line:%d.\n", __LINE__); \
})
...
int accdet_read_audio_res(unsigned int res_value)
{
	...
	if (accdet == NULL) {
		pr_err("%s: accdet is invalid!", __func__);
		return -EINVAL;
	}
...
static void send_key_event(u32 keycode, u32 flag)
{
	...
	case DW_KEY:
		...
		snd_soc_jack_report(&accdet->jack, report,
				SND_JACK_BTN_1);
		pr_info("accdet KEY_VOLUMEDOWN %d\n", flag);
#if IS_ENABLED(CONFIG_AMAZON_METRICS_LOG) || IS_ENABLED(CONFIG_AMAZON_MINERVA_METRICS_LOG)
	...
	case MD_KEY:
		...
		snd_soc_jack_report(&accdet->jack, report,
				SND_JACK_BTN_0);
		pr_info("accdet KEY_PLAYPAUSE %d\n", flag);
#if IS_ENABLED(CONFIG_AMAZON_METRICS_LOG) || IS_ENABLED(CONFIG_AMAZON_MINERVA_METRICS_LOG)
	...
	case AS_KEY:
		...
		snd_soc_jack_report(&accdet->jack, report,
				SND_JACK_BTN_3);
		pr_info("accdet KEY_VOICECOMMAND %d\n", flag);
#if IS_ENABLED(CONFIG_AMAZON_METRICS_LOG) || IS_ENABLED(CONFIG_AMAZON_MINERVA_METRICS_LOG)
...
#if IS_ENABLED(CONFIG_WAS4780_AUDIO_SWITCH)
	/* diasble accdet comparator */
	accdet_clear_bit(ACCDET_CMP_PWM_EN_ADDR, 0);
#endif
	/* enable ACCDET unit */
...
#if IS_ENABLED(CONFIG_WAS4780_AUDIO_SWITCH)
static unsigned int accdet_switch_manual_work(void)
{
	u32 pre_accdet_adc = 0;
	u32 new_accdet_adc = 0;

	msleep(mic_voltage_delay);
	pre_accdet_adc = accdet_get_auxadc();
	if (pre_accdet_adc > switch_mode_voltage) {
		pr_info("[accdet] 4-pole CTIA headset. no need swap.");
		was4780_switch_event(accdet->typec_node, WAS_EARPIECE_TYPE, CTIA);
	} else {
		/* CTIA/OMTP changes */
		was4780_switch_event(accdet->typec_node, WAS_MIC_GND_SWAP, 0);
		new_accdet_adc = accdet_get_auxadc();
		if (new_accdet_adc > switch_mode_voltage) {
			was4780_switch_event(accdet->typec_node, WAS_EARPIECE_TYPE, OMTP);
			pr_info("[accdet] 4-pole OMTP headset : %u", pre_accdet_adc);
		} else {
			was4780_switch_event(accdet->typec_node, WAS_EARPIECE_TYPE, THREE_POLE);
			pr_info("[accdet] three-pole headset : %u", pre_accdet_adc);
		}
	}
	...
	accdet_set_debounce(accdet_state001, cust_pwm_deb->debounce1);
	accdet_set_debounce(accdet_state011, cust_pwm_deb->debounce3);
	/* enable accdet comparator */
	accdet_update_bit(ACCDET_CMP_PWM_EN_ADDR, 0);
	accdet_update_bit(ACCDET_SW_EN_ADDR, ACCDET_SW_EN_SFT);
...
static void eint_work_callback(struct work_struct *work)
{
	...
	} else if (!accdet->typec_node && HAS_CAP(accdet->data->caps, ACCDET_AP_GPIO_EINT))
...
static inline void check_cable_type(void)
{
	...
	cur_AB = accdet_read(ACCDET_MEM_IN_ADDR) >> ACCDET_STATE_MEM_IN_OFFSET;
		cur_AB = cur_AB & ACCDET_STATE_AB_MASK;
	pr_notice("accdet %s(), cur_status:%d current AB = %d\n", __func__,
		     accdet->accdet_status, cur_AB);

	accdet->button_status = 0;
	accdet->pre_status = accdet->accdet_status;
...
static void accdet_work_callback(struct work_struct *work)
{
	...
	pr_info("%s: success, pre_cable_type:%u ", __func__, pre_cable_type);
...
static int ext_typec_event_changed(struct notifier_block *nb,
				      unsigned long evt, void *ptr)
{
	pr_info("%s evt: %u", __func__, evt);
...
static int accdet_get_dts_data(void)
{
...
#if IS_ENABLED(CONFIG_WAS4780_AUDIO_SWITCH)
	/*typec-switch deive node*/
	accdet->typec_node = of_parse_phandle(node, "was4780_audio_switch", 0);
	if (IS_ERR_OR_NULL(accdet->typec_node)) {
		pr_err("%s failed to parse audio switch node, please check", __func__);
		return -1;
	} else {
		pr_info("%s typec_node name: %s", __func__, accdet->typec_node->name);
	}

	ret = of_property_read_u32(node, "mic_voltage_delay",
			 &mic_voltage_delay);
	if (ret < 0) {
		mic_voltage_delay = 20;
		pr_notice("%s: mic_voltage_delay still enabled just can`t read from dts\n",
			__func__);
	}
	...
	ret = of_property_read_u32(node, "switch-mode-vol",
			 &switch_mode_voltage);
	if (ret < 0) {
		switch_mode_voltage = 521;
		pr_notice("%s: switch mode voltage is set to 521 mv by default\n", __func__);
	}
#endif
...
static void accdet_init_once(void)
{
	...
	if (accdet_dts.eint_pol == IRQ_TYPE_LEVEL_HIGH) {
		/* set EINT IN inverse */
		accdet_update_bits(ACCDET_IRQ_ADDR, 4, 1, 1);
		/* set MTEST nomarl mode for EINT default low, use HIGH level T*/
		accdet_clear_bit(MT6359_AUDENC_ANA_CON19, 8);
		accdet_clear_bit(MT6359_AUDENC_ANA_CON19, 9);
		accdet_clear_bit(MT6359_AUDENC_ANA_CON19, 10);
		accdet_clear_bit(MT6359_AUDENC_ANA_CON19, 11);
		accdet_clear_bit(MT6359_AUDENC_ANA_CON19, 12);
		accdet_clear_bit(MT6359_AUDENC_ANA_CON19, 13);
		accdet_clear_bit(MT6359_AUDENC_ANA_CON19, 14);
		accdet_clear_bit(MT6359_AUDENC_ANA_CON19, 15);
	}
	pr_info("%s done!\n", __func__);
}
...
void accdet_late_init(unsigned long data)
{
	...
	if (atomic_cmpxchg(&accdet_first, 1, 0)) {
		...
		accdet_init_once();
#if IS_ENABLED(CONFIG_WAS4780_AUDIO_SWITCH)
		accdet->typec_nb.notifier_call = ext_typec_event_changed;
		accdet->typec_nb.priority = 0;
		ret = was4780_reg_notifier(&accdet->typec_nb, accdet->typec_node, NOTIFY_REGISTER_BIT_FOR_ACCDET);
		if (ret) {
			pr_err("%s failed to register notifier, ret:%d", __func__, ret);
			was4780_unreg_notifier(&accdet->typec_nb, accdet->typec_node, NOTIFY_REGISTER_BIT_FOR_ACCDET);
		}
#endif
	} else
...
static void delay_init_work_callback(struct work_struct *work)
{
	...
	if (atomic_cmpxchg(&accdet_first, 1, 0)) {
		...
		pr_info("%s init proccess has done\n", __func__);
	}
	pr_info("%s() done\n", __func__);
}
...
static int accdet_probe(struct platform_device *pdev)
{
	...
	if (!accdet->typec_node && HAS_CAP(accdet->data->caps, ACCDET_AP_GPIO_EINT)) {
