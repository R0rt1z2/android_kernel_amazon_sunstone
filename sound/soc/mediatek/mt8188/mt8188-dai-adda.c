						       ...
						       const char *name)
{
	...
	if (strstr(name, "aud_adc_hires"))
		dai_id = MT8188_AFE_IO_UL_SRC1;
	else if (strstr(name, "aud_dac_hires"))
		dai_id = MT8188_AFE_IO_DL_SRC;
	else
		return NULL;
...
static const struct snd_kcontrol_new mtk_dai_adda_o176_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I004 Switch", AFE_CONN176, 4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I006 Switch", AFE_CONN176, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I008 Switch", AFE_CONN176, 8, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I010 Switch", AFE_CONN176, 10, 1, 0),
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I168 Switch", AFE_CONN176_5, 8, 1, 0),
};
...
static const struct snd_kcontrol_new mtk_dai_adda_o177_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I005 Switch", AFE_CONN177, 5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I007 Switch", AFE_CONN177, 7, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I009 Switch", AFE_CONN177, 9, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I011 Switch", AFE_CONN177, 11, 1, 0),
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I169 Switch", AFE_CONN177_5, 9, 1, 0),
};
...
static int mt8188_adda_loopback_put(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(component);
	long enable = ucontrol->value.integer.value[0];
	unsigned int reg = 0;
	unsigned int mask = 0;
	unsigned int value;

	if (!strcmp(kcontrol->id.name, "ADDA_Loopback1_Switch")) {
		reg = AFE_ADDA_MTKAIF_CFG0;
		mask = (MTKAIF_IF_LOOPBACK1 |
			MTKAIF_TXIF_PROTOCOL2 |
			MTKAIF_TXIF_8TO5);
	} else if (!strcmp(kcontrol->id.name, "ADDA_Loopback3_Switch")) {
		reg = AFE_ADDA_UL_SRC_CON0;
		mask = UL_LOOPBACK_MODE_CTL;
	}

	value = (enable ? mask : 0);

	if (reg)
		regmap_update_bits(afe->regmap, reg, mask, value);

	return 0;
}

static int mt8188_adda_loopback_get(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(component);
...
static int mt8188_adda_dl_gain_put(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(component);
...
static int mt8188_adda_dl_gain_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(component);
...
static const struct snd_kcontrol_new mtk_dai_adda_controls[] = {
	SOC_SINGLE_BOOL_EXT("ADDA_Loopback1_Switch", 0,
			    mt8188_adda_loopback_get,
			    mt8188_adda_loopback_put),
	SOC_SINGLE_BOOL_EXT("ADDA_Loopback3_Switch", 0,
			    mt8188_adda_loopback_get,
			    mt8188_adda_loopback_put),
	SOC_SINGLE_EXT("ADDA_DL_Gain", SND_SOC_NOPM, 0, 65535, 0,
			    mt8188_adda_dl_gain_get,
			    mt8188_adda_dl_gain_put),
...
static int init_adda_priv_data(struct mtk_base_afe *afe)
{
	struct mt8188_afe_private *afe_priv = afe->platform_priv;
	struct mtk_dai_adda_priv *adda_priv;
	int adda_dai_list[] = { MT8188_AFE_IO_DL_SRC,
