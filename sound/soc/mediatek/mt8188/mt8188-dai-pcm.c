...
static const struct snd_soc_dapm_widget mtk_dai_pcm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER("O000", SND_SOC_NOPM, 0, 0,
		mtk_dai_pcm_o000_mix, ARRAY_SIZE(mtk_dai_pcm_o000_mix)),
	SND_SOC_DAPM_MIXER("O001", SND_SOC_NOPM, 0, 0,
		mtk_dai_pcm_o001_mix, ARRAY_SIZE(mtk_dai_pcm_o001_mix)),
...
static int pcm_tx_rx_lpbk_en_get(
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	value &= PCM_INTF_CON2_PCM_TX2RX_LPBK;
	value = !!value;
...
static int pcm_tx_rx_lpbk_en_put(
	struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
	...
	if (ucontrol->value.integer.value[0])
		val |= PCM_INTF_CON2_PCM_TX2RX_LPBK;

	regmap_update_bits(afe->regmap, PCM_INTF_CON2,
		PCM_INTF_CON2_PCM_TX2RX_LPBK, val);

	return 0;
}

static const struct snd_kcontrol_new mtk_dai_pcm_controls[] = {
	SOC_SINGLE_BOOL_EXT("pcm_tx_rx_lpbk_en", 0,
		pcm_tx_rx_lpbk_en_get,
		pcm_tx_rx_lpbk_en_put),
};
